...
class TestDiskCacheManagerImpl : public ::testing::Test {
 protected:
  ...
  virtual void SetUp() {
    ...
    diskCacheWrite_->Init(client_, wrapper_, "/mnt/test", 0, 1, cachedObjName);
...
TEST_F(TestDiskCacheManagerImpl, WriteClosure) {
  ...
  EXPECT_CALL(*diskCacheWrite_, WriteDiskFile(_, _, _, _)).WillOnce(Return(0));
...
TEST_F(TestDiskCacheManagerImpl, Write) {
  ...
  EXPECT_CALL(*diskCacheManager_, IsDiskUsedInited()).WillOnce(Return(true));
  ret = diskCacheManagerImpl_->Write(fileName, const_cast<char*>(buf.c_str()),
                                     10);
  ASSERT_EQ(-1, ret);

  EXPECT_CALL(*diskCacheManager_, IsDiskCacheFull()).WillOnce(Return(false));
  EXPECT_CALL(*diskCacheWrite_, WriteDiskFile(_, _, _, _)).WillOnce(Return(-1));
  EXPECT_CALL(*diskCacheManager_, IsDiskUsedInited()).WillOnce(Return(true));
  ret = diskCacheManagerImpl_->Write(fileName, const_cast<char*>(buf.c_str()),
                                     10);
  ASSERT_EQ(-1, ret);
...
TEST_F(TestDiskCacheManagerImpl, Read) {
  ...
  ret = diskCacheManagerImpl_->Read(fileName, const_cast<char*>(buf2.c_str()),
                                    10, length);
  ASSERT_EQ(-1, ret);
...
TEST_F(TestDiskCacheManagerImpl, WriteReadDirect) {
  ...
  EXPECT_CALL(*diskCacheManager_, IsDiskCacheFull()).WillOnce(Return(false));
  EXPECT_CALL(*diskCacheManager_, IsDiskUsedInited()).WillOnce(Return(true));
  EXPECT_CALL(*diskCacheManager_, WriteReadDirect(_, _, _)).WillOnce(Return(0));
  ret = diskCacheManagerImpl_->WriteReadDirect(
      fileName, const_cast<char*>(buf.c_str()), 10);
  ASSERT_EQ(0, ret);
}
