...
TEST_F(TestFuseS3Client, test_Init_with_KVCache) {
  ...
  auto testclient =
      std::make_shared<FuseS3Client>(mdsClient_, metaClient_, inodeManager_,
                                     dentryManager_, s3ClientAdaptor_, nullptr);
...
TEST_F(TestFuseS3Client, warmUp_inodeBadFd) {
  ...
  EXPECT_CALL(*inodeManager_, GetInode(_, _))
      .WillOnce(
          DoAll(SetArgReferee<1>(inodeWrapper), Return(CURVEFS_ERROR::BAD_FD)));
...
TEST_F(TestFuseS3Client, warmUp_Warmfile_error_GetDentry01) {
  ...
  EXPECT_CALL(*dentryManager_, GetDentry(_, _, _))
      .WillOnce(DoAll(SetArgPointee<2>(dentry), Return(CURVEFS_ERROR::BAD_FD)));
  EXPECT_CALL(*inodeManager_, GetInode(_, _))
      .WillOnce(
  ...
  EXPECT_CALL(*s3ClientAdaptor_, Read(_, _, _, _))
      .WillOnce(DoAll(SetArrayArgument<3>(tmpbuf, tmpbuf + len), Return(len)));
...
TEST_F(TestFuseS3Client, FuseOpWriteSmallSize) {
  ...
  size_t smallSize = 3;
  EXPECT_CALL(*s3ClientAdaptor_, Write(_, _, _, _)).WillOnce(Return(smallSize));
...
TEST_F(TestFuseS3Client, FuseOpFlush) {
  ...
  LOG(INFO) << "############ case3: test enable cto, but flush all cache fail";
...
TEST_F(TestFuseS3Client, FuseOpGetXattr_EnableSumInDir) {
  ...
  EXPECT_CALL(*dentryManager_, ListDentry(_, _, _, _, _))
      .WillOnce(DoAll(SetArgPointee<1>(dlist), Return(CURVEFS_ERROR::OK)))
      .WillOnce(DoAll(SetArgPointee<1>(emptyDlist), Return(CURVEFS_ERROR::OK)));
...
TEST_F(TestFuseS3Client, FuseOpUnlink_EnableSummary) {
  ...
  EXPECT_CALL(*dentryManager_, DeleteDentry(parent, name, FsFileType::TYPE_S3))
      .WillOnce(Return(CURVEFS_ERROR::OK));
...
TEST_F(TestFuseS3Client, FuseOpListXattr) {
  ...
  EXPECT_CALL(*inodeManager_, GetInodeAttr(ino, _))
      .WillOnce(
          DoAll(SetArgPointee<1>(inode), Return(CURVEFS_ERROR::INTERNAL)));
  CURVEFS_ERROR ret = client_->FuseOpListXattr(req, ino, buf, size, &realSize);
  ASSERT_EQ(CURVEFS_ERROR::INTERNAL, ret);
  ...
  auto expected = key.length() + 1 + strlen(XATTR_DIR_RFILES) + 1 +
                  strlen(XATTR_DIR_RSUBDIRS) + 1 + strlen(XATTR_DIR_RENTRIES) +
                  1 + strlen(XATTR_DIR_RFBYTES) + 1;
...
TEST_F(TestFuseS3Client, FuseOpWriteQosTest) {
  ...
  auto qosWriteTest = [&](int id, int len) {
    ...
    EXPECT_CALL(*s3ClientAdaptor_, Write(_, _, _, _)).WillOnce(Return(s3Size));

    client_->Add(false, size);

    CURVEFS_ERROR ret =
        client_->FuseOpWrite(req, ino, buf.c_str(), size, off, &fi, &fileOut);
    ASSERT_EQ(CURVEFS_ERROR::OK, ret);
