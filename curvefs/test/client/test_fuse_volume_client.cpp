...
class TestFuseVolumeClient : public ::testing::Test {
 protected:
  ...
  virtual void SetUp() {
    ...
    client_ = std::make_shared<FuseVolumeClient>(mdsClient_, metaClient_,
...
TEST_F(TestFuseVolumeClient, FuseOpInit_when_fs_exist) {
  ...
  EXPECT_CALL(*blockDeviceClient_, Open(_, _)).WillOnce(Return(true));
...
TEST_F(TestFuseVolumeClient, FuseOpLookup) {
  ...
  EntryOut entryOut;
  CURVEFS_ERROR ret =
      client_->FuseOpLookup(req, parent, name.c_str(), &entryOut);
  ASSERT_EQ(CURVEFS_ERROR::OK, ret);
}
...
TEST_F(TestFuseVolumeClient, FuseOpWrite) {
  ...
  for (auto ret :
       {CURVEFS_ERROR::OK, CURVEFS_ERROR::IO_ERROR, CURVEFS_ERROR::NO_SPACE}) {
    EXPECT_CALL(*volumeStorage_, Write(_, _, _, _, _)).WillOnce(Return(ret));
...
TEST_F(TestFuseVolumeClient, FuseOpRead) {
  ...
  for (auto ret :
       {CURVEFS_ERROR::OK, CURVEFS_ERROR::IO_ERROR, CURVEFS_ERROR::NO_SPACE}) {
    EXPECT_CALL(*volumeStorage_, Read(_, _, _, _)).WillOnce(Return(ret));
...
TEST_F(TestFuseVolumeClient, FuseOpMkDir) {
  ...
  EntryOut entryOut;
  CURVEFS_ERROR ret = client_->FuseOpMkDir(req, parent, name, mode, &entryOut);
  ASSERT_EQ(CURVEFS_ERROR::OK, ret);
}
...
TEST_F(TestFuseVolumeClient, FuseOpOpenAndFuseOpReadDir) {
  ...
  EXPECT_CALL(*dentryManager_, ListDentry(ino, _, listDentryLimit_, _, _))
      .WillOnce(DoAll(SetArgPointee<1>(dentryList), Return(CURVEFS_ERROR::OK)));
  EXPECT_CALL(*inodeManager_, BatchGetInodeAttrAsync(ino, _, _))
      .Times(1)
      .WillOnce(Return(CURVEFS_ERROR::OK));

  ret = client_->FuseOpReadDir(req, ino, size, off, &fi, &buffer, &rSize, true);
  ASSERT_EQ(CURVEFS_ERROR::OK, ret);
}
...
TEST_F(TestFuseVolumeClient, FuseOpOpenAndFuseOpReadDirFailed) {
  ...
  EXPECT_CALL(*dentryManager_, ListDentry(ino, _, listDentryLimit_, _, _))
      .WillOnce(
          DoAll(SetArgPointee<1>(dentryList), Return(CURVEFS_ERROR::INTERNAL)));

  ret =
      client_->FuseOpReadDir(req, ino, size, off, &fi, &buffer, &rSize, false);
  ASSERT_EQ(CURVEFS_ERROR::INTERNAL, ret);
}
...
TEST_F(TestFuseVolumeClient, FuseOpRenameOverwrite) {
  ...
  auto dstDentry = GenDentry(fsId, newparent, newname, txId, oldInodeId, FILE);
  dstDentry.set_type(FsFileType::TYPE_S3);
  EXPECT_CALL(*dentryManager_, GetDentry(parent, name, _))
      .WillOnce(DoAll(SetArgPointee<2>(srcDentry), Return(CURVEFS_ERROR::OK)));
  EXPECT_CALL(*dentryManager_, GetDentry(newparent, newname, _))
      .WillOnce(DoAll(SetArgPointee<2>(dstDentry), Return(CURVEFS_ERROR::OK)));
  ...
  EXPECT_CALL(*inodeManager_, GetInodeAttr(oldInodeId, _))
      .Times(1)
      .WillRepeatedly(DoAll(SetArgPointee<1>(attr), Return(CURVEFS_ERROR::OK)));
  ...
  EXPECT_CALL(*metaClient_, PrepareRenameTx(_))
      .WillOnce(Invoke([&](const std::vector<Dentry>& dentrys) {
        ...
        auto dstDentry = GenDentry(fsId, newparent, newname, txId + 1, inodeId,
                                   FILE | TX_PREPARE);
...
TEST_F(TestFuseVolumeClient, FuseOpRenameNameTooLong) {
  ...
  CURVEFS_ERROR ret = client_->FuseOpRename(req, parent, name1.c_str(),
                                            newparent, newname1.c_str(), flags);
  ASSERT_EQ(CURVEFS_ERROR::NAMETOOLONG, ret);
...
TEST_F(TestFuseVolumeClient, FuseOpRenameParallel) {
  ...
  EXPECT_CALL(*metaClient_, GetInodeAttr(_, 1, _))
      .Times(times)
      .WillRepeatedly(
          DoAll(SetArgPointee<2>(srcAttr), Return(MetaStatusCode::OK)));
  ...
  EXPECT_CALL(*metaClient_, GetInodeAttr(_, 10, _))
      .Times(times)
      .WillRepeatedly(
          DoAll(SetArgPointee<2>(oldAttr), Return(MetaStatusCode::OK)));
...
TEST_F(TestFuseVolumeClient, FuseOpGetAttrEnableCto) {
  ...
  ASSERT_EQ(CURVEFS_ERROR::OK, client_->FuseOpGetAttr(req, ino, &fi, &attrOut));
...
TEST_F(TestFuseVolumeClient, FuseOpSymlink) {
  ...
  EntryOut entryOut;
  CURVEFS_ERROR ret =
      client_->FuseOpSymlink(req, link, parent, name, &entryOut);
  ASSERT_EQ(CURVEFS_ERROR::OK, ret);
}
...
TEST_F(TestFuseVolumeClient, FuseOpLinkFailed) {
  ...
  EXPECT_CALL(*metaClient_, GetInodeAttr(_, _, _))
      .WillRepeatedly(
          DoAll(SetArgPointee<2>(attr), Return(MetaStatusCode::OK)));
...
TEST_F(TestFuseVolumeClient, FuseOpReadLink) {
  ...
  EXPECT_CALL(*inodeManager_, GetInodeAttr(ino, _))
      .WillOnce(DoAll(SetArgPointee<1>(inodeAttr), Return(CURVEFS_ERROR::OK)));
