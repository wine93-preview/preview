...
class MockDiskCacheManager : public DiskCacheManager {
 public:
  ...
  MOCK_METHOD2(Init, int(std::shared_ptr<S3Client> client,
                         const S3ClientAdaptorOption option));
  MOCK_METHOD0(IsDiskCacheFull, bool());
  MOCK_METHOD3(WriteReadDirect, int(const std::string fileName, const char* buf,
                                    uint64_t length));
  MOCK_METHOD0(IsDiskUsedInited, bool());
};
