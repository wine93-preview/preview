...
USING_FLAG(normal2unstable_io_error_num);
USING_FLAG(tick_duration_second);
USING_FLAG(unstable2down_second);
USING_FLAG(unstable2normal_io_succ_num);
...
TEST_F(DiskStateMachineTest, Normal2Unstable) {
  ...
  for (auto i = 0; i <= FLAGS_normal2unstable_io_error_num; i++) {
...
TEST_F(DiskStateMachineTest, Unstable2Normal) {
  ...
  {
    for (auto i = 0; i <= FLAGS_unstable2normal_io_succ_num; i++) {
