...
class TestTopologyManager : public ::testing::Test {
 ...
 protected:
  ...
  void PrepareAddPartition(FsIdType fsId, PoolIdType poolId, CopySetIdType csId,
                           ...
                           uint64_t idEnd, uint64_t txId = 0) {
...
TEST_F(TestTopologyManager, test_ListMetaServer_ByIdSuccess) {
  ...
  ASSERT_THAT(response.metaserverinfos(0).metaserverid(), AnyOf(csId1, csId2));
  ...
  ASSERT_THAT(response.metaserverinfos(1).metaserverid(), AnyOf(csId1, csId2));
...
TEST_F(TestTopologyManager, test_ListZoneServer_ByIdSuccess) {
  ...
  PrepareAddZone(zoneId);
  PrepareAddServer(serverId, "hostname1", "ip1", 0, "ip2", 0, zoneId, poolId);
  PrepareAddServer(serverId2, "hostname1", "ip3", 0, "ip4", 0, zoneId, poolId);
...
TEST_F(TestTopologyManager, test_CreatePartitionsAndGetMinPartition_Success) {
  PoolIdType poolId = 0x11;
  ...
  PartitionInfo pInfo;
  auto ret = serviceManager_->CreatePartitionsAndGetMinPartition(0x01, &pInfo);
  ASSERT_EQ(TopoStatusCode::TOPO_OK, ret);
...
TEST_F(TestTopologyManager,
  ...
  request.set_count(1);
  serviceManager_->CreatePartitions(&request, &response);
  ASSERT_EQ(TopoStatusCode::TOPO_CREATE_PARTITION_FAIL, response.statuscode());
}
...
TEST_F(TestTopologyManager,
  ...
  PrepareAddServer(0x32, "server2", "127.0.0.1", 0, "127.0.0.1", 0, 0x22, 0x11);
  PrepareAddServer(0x33, "server3", "127.0.0.1", 0, "127.0.0.1", 0, 0x23, 0x11);
  PrepareAddServer(0x34, "server4", "127.0.0.1", 0, "127.0.0.1", 0, 0x24, 0x11);
  PrepareAddMetaServer(0x41, "ms1", "token1", 0x31, "127.0.0.1", 7777, "ip2",
                       8888, OnlineState::OFFLINE);
  PrepareAddMetaServer(0x42, "ms2", "token2", 0x32, "127.0.0.1", 7778, "ip2",
                       8888, OnlineState::OFFLINE);
...
TEST_F(TestTopologyManager,
  ...
  PrepareAddServer(0x35, "server5", "127.0.0.1", 0, "127.0.0.1", 0, 0x25, 0x12);
  PrepareAddServer(0x36, "server6", "127.0.0.1", 0, "127.0.0.1", 0, 0x26, 0x12);
  PrepareAddServer(0x37, "server7", "127.0.0.1", 0, "127.0.0.1", 0, 0x27, 0x12);
...
TEST_F(TestTopologyManager, test_AllocOrGetMemcacheCluster_1_Success) {
  ...
  ASSERT_EQ(clusters[0], static_cast<MemcacheCluster>(response.cluster()));
}
