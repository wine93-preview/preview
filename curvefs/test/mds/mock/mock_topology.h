...
class MockStorage : public TopologyStorage {
 public:
  ...
  MOCK_METHOD2(LoadMemcacheCluster,
               bool(std::unordered_map<MemcacheClusterIdType, MemcacheCluster>*
                        memcacheclusterMap,
  ...
  MOCK_METHOD1(StorageMemcacheCluster, bool((const MemcacheCluster& cluster)));
...
class MockTopology : public TopologyImpl {
 public:
  ...
  MOCK_CONST_METHOD1(GetCopysetNumInMetaserver, uint32_t(MetaServerIdType id));
  ...
  MOCK_METHOD2(GetPartition, bool(PartitionIdType partitionId, Partition* out));
...
class MockTopologyManager : public TopologyManager {
 public:
  ...
  MOCK_METHOD2(GetZone,
               void(const GetZoneRequest* request, GetZoneResponse* response));
  ...
  MOCK_METHOD2(GetPool,
               void(const GetPoolRequest* request, GetPoolResponse* response));
  ...
  MOCK_METHOD2(RegistMemcacheCluster, void(const RegistMemcacheClusterRequest*,
