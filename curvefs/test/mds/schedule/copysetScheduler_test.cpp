...
TEST_F(TestCopysetSheduler, empty_pool_test) {
  ...
  EXPECT_CALL(*topoAdapter_, GetZoneInPool(poolid)).WillOnce(Return(zoneList));
...
TEST_F(TestCopysetSheduler, no_need_balance_test) {
  ...
  MetaServerInfo msInfo1(testCopySetInfo.peers[0], OnlineState::ONLINE, space);
  MetaServerInfo msInfo2(testCopySetInfo.peers[1], OnlineState::ONLINE, space);
  MetaServerInfo msInfo3(testCopySetInfo.peers[2], OnlineState::ONLINE, space);
...
TEST_F(TestCopysetSheduler, overload_get_zone_num_fail_test) {
  ...
  std::vector<MetaServerInfo> metaservers = {msInfo1, msInfo2, msInfo3, msInfo4,
                                             msInfo5};
  std::list<ZoneIdType> zoneList;
...
TEST_F(TestCopysetSheduler, normal_balanca_by_metaserver_usage_success_test) {
  ...
  MetaServerSpace space2(100, 70);
  MetaServerSpace space3(100, 30);
  MetaServerInfo msInfo1(testCopySetInfo.peers[0], OnlineState::ONLINE, space2);
  MetaServerInfo msInfo2(testCopySetInfo.peers[1], OnlineState::ONLINE, space);
  MetaServerInfo msInfo3(testCopySetInfo.peers[2], OnlineState::ONLINE, space);
  // peer4, peer5 has same zone with ms1
  PeerInfo peer4(4, 1, 1, "192.168.10.1", 9001);
  ...
  EXPECT_CALL(*topoAdapter_, CreateCopySetAtMetaServer(testCopySetInfo.id, 4))
      .WillOnce(Return(true));
...
TEST_F(TestCopysetSheduler, normal_balanca_by_copyset_num_success_test) {
  ...
  EXPECT_CALL(*topoAdapter_, CreateCopySetAtMetaServer(testCopySetInfo.id, 5))
      .WillOnce(Return(true));
