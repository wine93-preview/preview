...
class ScheduleMetricsTest : public testing::Test {
 public:
  void SetUp() {
    ...
    topo = std::make_shared<MockTopology>(idGenerator, tokenGenerator, storage);
  ...
  ::curvefs::mds::topology::Server GetServer(int id) {
    ...
    return ::curvefs::mds::topology::Server(id, hostName, "", 0, "", 0, id, 1);
  }
...
TEST_F(ScheduleMetricsTest, test_add_rm_addOp) {
  ...
  addCsInfo.SetCopySetMembers(std::set<MetaServerIdType>{1, 2});
  {
    ...
    ASSERT_EQ(
        std::to_string(addCsInfo.GetPoolId()),
        scheduleMetrics->operators[addOp.copysetID].GetValueByKey("poolId"));
    ASSERT_EQ(
...
TEST_F(ScheduleMetricsTest, test_add_rm_rmOp) {
...

  {
    ...
    std::string res =
        ...
        std::string("\"poolId\":\"1\",") + std::string("\"startEpoch\":\"1\"}");
...
TEST_F(ScheduleMetricsTest, test_add_rm_transferOp) {
...

  {
    ...
    ASSERT_EQ(std::to_string(transCsInfo.GetPoolId()),
...
TEST_F(ScheduleMetricsTest, test_add_rm_changeOp) {
...

  {
    ...
    ASSERT_EQ(
        std::to_string(changeCsInfo.GetPoolId()),
        scheduleMetrics->operators[changeOp.copysetID].GetValueByKey("poolId"));
...
TEST_F(ScheduleMetricsTest, test_abnormal) {
  ...
  EXPECT_CALL(*topo, GetMetaServer(3, _))
      .Times(2)
      .WillRepeatedly(DoAll(SetArgPointee<1>(GetMetaServer(3)), Return(true)));
