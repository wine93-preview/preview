...
::curvefs::mds::schedule::CopySetInfo GetCopySetInfoForTest() {
  ...
  return ::curvefs::mds::schedule::CopySetInfo(copySetKey, epoch, leader, peers,
                                               ConfigChangeInfo{});
}
...
void GetCopySetInMetaServersForTest(
  ...
  for (int i = 1; i <= 10; i++) {
    ...
    std::vector<PeerInfo> peers({peer1, peer2, peer3});
    ::curvefs::mds::schedule::CopySetInfo copyset(copySetKey, epoch, leader,
...
std::vector<::curvefs::mds::topology::Server> GetServerForTest() {
  ...
  for (int i = 1; i <= 4; i++) {
    std::string ip = "192.168.10." + std::to_string(i);
    ::curvefs::mds::topology::Server server(i, "server", ip, 0, "", 0, i, 1);
