...
TEST_F(TestRecoverSheduler, test_server_has_more_offline_metaserver) {
  ...
  MetaServerInfo csInfo1(testCopySetInfo.peers[0], OnlineState::OFFLINE, space);
  MetaServerInfo csInfo2(testCopySetInfo.peers[1], OnlineState::ONLINE, space);
  MetaServerInfo csInfo3(testCopySetInfo.peers[2], OnlineState::ONLINE, space);
...
TEST_F(TestRecoverSheduler, test_all_metaServer_online_offline) {
  ...
  EXPECT_CALL(*topoAdapter_, GetCopySetInfos())
      .WillRepeatedly(Return(std::vector<CopySetInfo>({testCopySetInfo})));
  MetaServerSpace space(100, 100);
  MetaServerInfo csInfo1(testCopySetInfo.peers[0], OnlineState::ONLINE, space);
  MetaServerInfo csInfo2(testCopySetInfo.peers[1], OnlineState::ONLINE, space);
  MetaServerInfo csInfo3(testCopySetInfo.peers[2], OnlineState::ONLINE, space);
...

  {
    ...
    EXPECT_CALL(*topoAdapter_, GetStandardZoneNumInPool(_)).WillOnce(Return(3));
