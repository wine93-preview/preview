  ...
  for (int i = 0; i < desc->field_count(); ++i) {
    ...
    switch (fieldDesc->type()) {
      ...
      case google::protobuf::FieldDescriptor::TYPE_MESSAGE: {
        auto* subMessage =
            refl->MutableMessage(message.get(), fieldDesc);  // NOLINT
        auto generatedSubMessage = GenerateAnDefaultInitializedMessage(
            fieldDesc->message_type()->full_name());  // NOLINT
        if (!generatedSubMessage || !generatedSubMessage->IsInitialized()) {
          return message;
        }
      ...
      case google::protobuf::FieldDescriptor::TYPE_ENUM: {
        ...
        const google::protobuf::EnumValueDescriptor* valueDesc = nullptr;
