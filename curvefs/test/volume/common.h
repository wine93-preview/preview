...
inline uint64_t TotalLength(const Extents& exts) {
  return std::accumulate(exts.begin(), exts.end(), 0ull,
                         [](uint64_t v, const Extent& e) { return v + e.len; });
}

inline bool ExtentsNotOverlap(Extents exts) {
  std::sort(exts.begin(), exts.end(), [](const Extent& e1, const Extent& e2) {
...
inline bool operator==(const AllocateHint& h1, const AllocateHint& h2) {
