...
class BlockDeviceClientTest : public ::testing::Test {
 protected:
  void SetUp() override {
    ...
    ON_CALL(*fileClient_, Init(_)).WillByDefault(Return(LIBCURVE_ERROR::OK));
    ASSERT_TRUE(client_->Init(options_));
  }

  void TearDown() override {}

  static ssize_t ReadCallback(int fd, char* buf, off_t offset, size_t length) {
...
TEST_F(BlockDeviceClientTest, TestUnInit) {
  EXPECT_CALL(*fileClient_, UnInit()).Times(1);
  client_->UnInit();
}
...
TEST_F(BlockDeviceClientTest, TestOpen) {
  ...
  EXPECT_CALL(*fileClient_, Open(_, _, _)).WillOnce(Return(-1));
  ASSERT_FALSE(client_->Open("/filename", "owner"));

  // CASE 2: open return fd (0)
  EXPECT_CALL(*fileClient_, Open(_, _, _)).WillOnce(Return(0));
  ASSERT_TRUE(client_->Open("/filename", "owner"));

  // CASE 3: open return fd (1)
  EXPECT_CALL(*fileClient_, Open(_, _, _)).WillOnce(Return(10));
  ASSERT_TRUE(client_->Open("/filename", "owner"));
}
...
TEST_F(BlockDeviceClientTest, TestClose) {
  ...
  EXPECT_CALL(*fileClient_, Close(10)).WillOnce(Return(LIBCURVE_ERROR::OK));
  ASSERT_TRUE(client_->Close());
}
...
TEST_F(BlockDeviceClientTest, TestStat) {
  ...
  // CASE 2: stat success
  EXPECT_CALL(*fileClient_, StatFile("/filename", userInfo, _))
      .WillOnce(Invoke([](const std::string& filename, const UserInfo& userinfo,
                          FileStatInfo* finfo) {
        finfo->length = 1000;
        finfo->fileStatus = 1;
...
struct FakeAioRequest {
  ...
  int operator()(int /*fd*/, CurveAioContext* aio,
...
TEST_F(BlockDeviceClientTest, TestReadBasic) {
  ...
  EXPECT_CALL(*fileClient_, AioRead(_, _, _)).Times(0);
...
TEST_F(BlockDeviceClientTest, TestReadWithUnAligned) {
  auto TEST_READ = [this](off_t offset, size_t length, off_t alignOffset,
    ...
    EXPECT_CALL(*fileClient_, AioRead(10, _, _))
        .WillOnce(
            Invoke([](int /*fd*/, CurveAioContext* ctx, UserDataType /*type*/) {
...
TEST_F(BlockDeviceClientTest, TestWriteBasic) {
  ...
  EXPECT_CALL(*fileClient_, AioWrite(_, _, _)).Times(0);
...
TEST_F(BlockDeviceClientTest, TestWriteWithUnAligned) {
  auto TEST_WRITE = [this](off_t offset, size_t length, off_t alignOffset,
                           size_t alignLength, AlignReads&& alignReads) {
  ...
  // Test Cases: write failed
  {
    ...
    EXPECT_CALL(*fileClient_, AioRead(10, _, _)).WillOnce(Return(-1));
    EXPECT_CALL(*fileClient_, AioWrite(_, _, _)).Times(0);
...
TEST_F(BlockDeviceClientTest, ReadvTest_AllSuccess) {
  ON_CALL(*fileClient_, Open(_, _, _)).WillByDefault(Return(1));
