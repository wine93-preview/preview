...
class S3CompactTest : public ::testing::Test {
 public:
  void SetUp() override {
    ...
    s3infoCache_ = std::make_shared<MockS3InfoCache>(cacheCapacity, mdsAddrs,
                                                     metaserverAddr);
...
TEST_F(S3CompactTest, test_CompactChunks) {
  ...
  auto mock_updateinode =
      [&](CopysetNode* copysetNode, const PartitionInfo& pinfo, uint64_t inode,
  ...
  EXPECT_CALL(*mockCopysetNodeWrapper, IsValid()).WillRepeatedly(Return(true));
