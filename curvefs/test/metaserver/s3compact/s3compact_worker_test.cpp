...
TEST_F(S3CompactWorkerTest, TestCancelCompact) {
  ...
  auto inodeManager =
      std::make_shared<InodeManager>(inodeStorage, nullptr, &filetype2InodeNum);
  ...
  EXPECT_CALL(*copysetNode, IsLeaderTerm()).WillRepeatedly(Invoke([&]() {
  ...
  std::this_thread::sleep_for(std::chrono::seconds(rand_r(&seed) % 10 + 5));
