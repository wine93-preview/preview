...
TEST_F(ConfEpochFileTest, LoadTest_OpenFailed) {
  ...
  EXPECT_NE(0, confFile_->Load(kEpochFilePath, &poolId_, &copysetId_, &epoch_));
}
...
TEST_F(ConfEpochFileTest, LoadTest_ReadFailed) {
  ...
  EXPECT_CALL(*mockfs_, Read(_, _, _, _)).WillOnce(Return(0));
...
TEST_F(ConfEpochFileTest, LoadTest_Success) {
  ...
  const char* data =
      "{\"poolId\": 123, \"copysetId\": 431, \"epoch\": 3, \"checksum\": "
      "1347065312}";  // NOLINT
  size_t datasize = strlen(data);
  EXPECT_CALL(*mockfs_, Read(_, _, _, _))
      .WillOnce(
  ...
  EXPECT_EQ(0, confFile_->Load(kEpochFilePath, &poolId_, &copysetId_, &epoch_));
}
...
TEST_F(ConfEpochFileTest, SaveTest_FsyncFailed) {
  ...
  EXPECT_CALL(*mockfs_, Fsync(_)).WillOnce(Return(-1));
