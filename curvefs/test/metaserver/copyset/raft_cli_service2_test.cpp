...
class RaftCliService2Test : public testing::Test {
 protected:
  ...
  static void StartOneCopysetGroup() {
    ...
    pid_t pid = StartOneFakeMetaserver("127.0.0.1", 29910,
    ...
    pid = StartOneFakeMetaserver("127.0.0.1", 29911, "local://" + copysetDir,
                                 kInitConf);
    CHECK_GE(pid, 0);
    servers_.emplace_back(pid, copysetDir);
    usleep(kElectionTimeoutMs * 1000);
    ...
    copysetDir = "./runlog/" + uuid_.GenerateUUID();
    pid = StartOneFakeMetaserver("127.0.0.1", 29912, "local://" + copysetDir,
                                 kInitConf);
    CHECK_GE(pid, 0);
    servers_.emplace_back(pid, copysetDir);
  ...
  static void UpdateConfigurations(
      const google::protobuf::RepeatedPtrField<curvefs::common::Peer>& peers) {
    conf_.reset();
