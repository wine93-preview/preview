...
TEST_F(TrashTest, RecycleCopysetTest) {
  ...
  {
    EXPECT_CALL(*mocklfs_, DirExists(_)).WillOnce(Return(false));

    EXPECT_CALL(*mocklfs_, Mkdir(_)).WillOnce(Return(-1));
  ...
  // dest path conflict
  {
    EXPECT_CALL(*mocklfs_, DirExists(_)).Times(2).WillRepeatedly(Return(true));
    EXPECT_FALSE(trash_->RecycleCopyset("/mnt/data/copysets/123456789"));
  }
  ...
  // rename failed
  {
    ...
    EXPECT_CALL(*mocklfs_, Rename(_, _, _)).WillOnce(Return(-1));
  ...
  // success
  {
    ...
    EXPECT_CALL(*mocklfs_, Rename(_, _, _)).WillOnce(Return(0));
...
TEST_F(TrashTest, DeleteExpiredCopysetsTest_TrashPathNotExist) {
  EXPECT_CALL(*mocklfs_, DirExists(_)).WillRepeatedly(Return(false));

  EXPECT_CALL(*mocklfs_, List(_, _)).Times(0);
...
TEST_F(TrashTest, DeleteExpiredCopysetsTest_ListDirFailed) {
  EXPECT_CALL(*mocklfs_, DirExists(_)).WillRepeatedly(Return(true));

  EXPECT_CALL(*mocklfs_, List(_, _)).WillRepeatedly(Return(-1));

  EXPECT_CALL(*mocklfs_, Open(_, _)).Times(0);
...
TEST_F(TrashTest, DeleteExpiredCopysetsTest_NotExpired) {
  ...
  EXPECT_CALL(*mocklfs_, Open(_, _)).WillRepeatedly(Return(0));
  EXPECT_CALL(*mocklfs_, Close(_)).WillRepeatedly(Return(0));
  ...
  EXPECT_CALL(*mocklfs_, Delete(_)).Times(0);
...
TEST_F(TrashTest, DeleteExpiredCopysetsTest_Expired) {
  ...
  EXPECT_CALL(*mocklfs_, Delete(_)).WillRepeatedly(Return(0));
