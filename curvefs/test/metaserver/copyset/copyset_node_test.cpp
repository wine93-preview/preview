...
class CopysetNodeTest : public testing::Test {
 protected:
  void SetUp() override {
    ...
    conf_.parse_from("127.0.0.1:29940:0,127.0.0.1:29941:0,127.0.0.1:29942:0");
    ...
    EXPECT_CALL(mockfs_, Mkdir(_)).WillRepeatedly(Return(0));
  }
...
TEST_F(CopysetNodeTest, LoadConEpochFailed_PoolIdIsNotIdentical) {
  ...
  const char* data =
      "{\"poolId\": 123, \"copysetId\": 1, \"epoch\": 3, \"checksum\": "
      "3480649501}";  // NOLINT

  size_t datasize = strlen(data);
  EXPECT_CALL(mockfs_, Read(_, _, _, _))
      .WillOnce(
          DoAll(SetArrayArgument<1>(data, data + datasize), Return(datasize)));
...
TEST_F(CopysetNodeTest, LoadConEpochFailed_CopysetIdIsNotIdentical) {
  ...
  EXPECT_CALL(mockfs_, Close(_)).Times(1);
  const char* data =
      "{\"poolId\": 1, \"copysetId\": 431, \"epoch\": 3, \"checksum\": "
      "2004834618}";  // NOLINT
...
TEST_F(CopysetNodeTest, LoadConEpoch_Success) {
  ...
  EXPECT_CALL(mockfs_, Close(_)).Times(1);
  const char* data =
      "{\"poolId\": 1, \"copysetId\": 1, \"epoch\": 3, \"checksum\": "
      "3896751047}";  // NOLINT
...
TEST_F(CopysetNodeTest, FetchLeaderStatusTest_LeaderIdIsEmpty) {
  ...
  EXPECT_CALL(*mockRaftNode, get_status(_)).WillOnce(SetArgPointee<0>(status));
  EXPECT_CALL(*mockRaftNode, shutdown(_)).Times(1);
  EXPECT_CALL(*mockRaftNode, join()).Times(1);
...
TEST_F(CopysetNodeTest, FetchLeaderStatusTest_CurrentNodeIsLeader) {
  ...
  EXPECT_THAT(status.leader_id.to_string(), leaderStatus.leader_id.to_string());
}
