...
TEST_F(HeartbeatTaskExecutorTest, EpochMismatch) {
  ...
  EXPECT_CALL(mockCopysetMgr_, PurgeCopysetNode(_, _)).Times(0);

  EXPECT_CALL(mockCopysetNode_, GetConfEpoch()).WillOnce(Return(100));

  EXPECT_CALL(mockCopysetNode_, TransferLeader(_)).Times(0);
  EXPECT_CALL(mockCopysetNode_, AddPeer(_, _)).Times(0);
...
TEST_F(HeartbeatTaskExecutorTest, TransferLeader_Success) {
  ...
  EXPECT_CALL(mockCopysetNode_, GetConfEpoch()).WillOnce(Return(1));
