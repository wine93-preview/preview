 ...
 protected:
  ...
  bool CompareInode(const Inode& first, const Inode& second) {
    return first.fsid() == second.fsid() && first.atime() == second.atime() &&
           ...
           first.type() == second.type() && first.mtime() == second.mtime() &&
...
TEST_F(InodeManagerTest, test1) {
  ...
  ASSERT_EQ(manager->CreateInode(6, param_, &inode5), MetaStatusCode::OK);
...
TEST_F(InodeManagerTest, GetOrModifyS3ChunkInfo) {
  ...
  // CASE 1: GetOrModifyS3ChunkInfo() success
  {
    ...
    rc = manager->GetOrModifyS3ChunkInfo(fsId, inodeId, map2add, map2del, true,
                                         &iterator);
    ASSERT_EQ(rc, MetaStatusCode::OK);
...
TEST_F(InodeManagerTest, testCreateManageInode) {
  ...
  Inode temp1;
  ASSERT_EQ(manager->GetInode(1, inode.inodeid(), &temp1), MetaStatusCode::OK);
