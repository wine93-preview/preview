...
class MetastoreTest : public ::testing::Test {
 protected:
  ...
  bool CompareInode(const Inode& first, const Inode& second) {
    ...
    return first.fsid() == second.fsid() && first.atime() == second.atime() &&
           first.atime_ns() == second.atime_ns() &&
           first.inodeid() == second.inodeid() &&
           first.length() == second.length() && first.uid() == second.uid() &&
           first.gid() == second.gid() && first.mode() == second.mode() &&
           first.type() == second.type() && firstMtime >= secondMtime &&
           firstCtime >= secondCtime && first.symlink() == second.symlink() &&
...
TEST_F(MetastoreTest, testBatchGetXAttr) {
  ...
  if (batchResponse.xattr(0).inodeid() == inodeId1) {
    ASSERT_EQ(batchResponse.xattr(0).xattrinfos().find(XATTR_DIR_FILES)->second,
              "1");
    ASSERT_EQ(
    ...
    ASSERT_EQ(
        ...
        "100");
    ASSERT_EQ(batchResponse.xattr(1).xattrinfos().find(XATTR_DIR_FILES)->second,
              "0");
    ASSERT_EQ(
...
TEST_F(MetastoreTest, GetOrModifyS3ChunkInfo) {
  ...
  // CASE 1: partition not found -> failed
  {
    ...
    MetaStatusCode rc =
        metastore.GetOrModifyS3ChunkInfo(&request, &response, nullptr);
  ...
  // CASE 2: GetOrModifyS3ChunkInfo success
  {
    ...
    for (size_t i = 0; i < chunkIndexs.size(); i++) {
      request.mutable_s3chunkinfoadd()->insert({chunkIndexs[i], lists2add[i]});
    }

    std::shared_ptr<Iterator> iterator;
    MetaStatusCode rc =
        metastore.GetOrModifyS3ChunkInfo(&request, &response, &iterator);
    ASSERT_EQ(rc, MetaStatusCode::OK);
    ASSERT_EQ(response.statuscode(), rc);
...
TEST_F(MetastoreTest, GetInodeWithPaddingS3Meta) {
  ...
  // step2: append s3chunkinfo within limit
  {
    ...
    request.set_returns3chunkinfomap(false);
    for (size_t i = 0; i < chunkIndexs.size(); i++) {
      request.mutable_s3chunkinfoadd()->insert({chunkIndexs[i], list2add[i]});
    }
