...
TEST_F(InodeStorageTest, testGetAttrNotFound) {
  ...
  InodeAttr attr;
  ASSERT_EQ(storage.GetAttr(Key4Inode(1, 2), &attr), MetaStatusCode::NOT_FOUND);
}
...
TEST_F(InodeStorageTest, TestGetVolumeExtentByOffset) {
  ...
  for (auto& store : {kvStorage_, memStore}) {
    ...
    std::vector<VolumeExtentSlice> slices;
    ASSERT_TRUE(PrepareGetAllVolumeExtentTest(&storage, fsId, inodeId, &slices))
        << ToString(store->Type());

    VolumeExtentSlice slice;
    auto st = storage.GetVolumeExtentByOffset(fsId, inodeId, slices[0].offset(),
                                              &slice);
