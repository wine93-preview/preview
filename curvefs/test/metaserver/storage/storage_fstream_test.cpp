...
void StorageFstreamTest::TestSave(std::shared_ptr<KVStorage> kvStorage,
  ...
  auto children = std::vector<std::shared_ptr<Iterator>>{
      std::make_shared<IteratorWrapper>(ENTRY_TYPE::PARTITION, 1,
                                        patitionIterator),
      std::make_shared<IteratorWrapper>(ENTRY_TYPE::INODE, 2,
                                        kvStorage->HGetAll(TableName(2))),
      std::make_shared<IteratorWrapper>(ENTRY_TYPE::DENTRY, 3,
                                        kvStorage->SGetAll(TableName(3))),
  };
  ...
  auto callback = [&](uint8_t version, ENTRY_TYPE type, uint32_t partitionId,
