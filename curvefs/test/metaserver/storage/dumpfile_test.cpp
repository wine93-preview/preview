...
class DumpFileTest : public ::testing::Test {
 protected:
  DumpFileTest() : dirname_(".dumpfile") {}
  ...
  bool InitSignals() { return dumpfile_->InitSignals() == DUMPFILE_ERROR::OK; }
...
TEST_F(DumpFileTest, TestLoadLargeValue) {
  ...
  auto hashIterator = std::make_shared<HashIterator>(&hash);
  auto checkLoading = [](std::shared_ptr<Iterator> iter, const int length,
                         const int npairs) {
    int count = 0;
    for (iter->SeekToFirst(); iter->Valid(); iter->Next()) {
      count++;
