...
void TestHSize(std::shared_ptr<KVStorage> kvStorage) {
  ...
  // CASE 5: big size
  for (int i = 0; i < 100; i++) {
    s = kvStorage->HSet(TableName(1), "key" + std::to_string(i),
                        Value("value"));
    ASSERT_TRUE(s.ok());
  }
  size = kvStorage->HSize(TableName(1));
...
void TestSSeek(std::shared_ptr<KVStorage> kvStorage) {
  ...
  std::map<std::string, std::string> pairs{
      {"1:1:/a:0", "100:0"},  {"1:1:/b:0", "101:0"},   {"1:1:/b:1", "101:0"},
      {"1:1:/b:2", "101:1"},  {"1:1:/c:0", "102"},     {"1:2:/a:0", "200:0"},
      {"1:3:/a:0", "300:0"},  {"1:3:/a/b:0", "301:0"}, {"2:1:/a:0", "1000:0"},
      {"3:1:/a:0", "1000:0"},
  };
...
void TestSSize(std::shared_ptr<KVStorage> kvStorage) {
  ...
  // CASE 5: big size
  for (int i = 0; i < 100; i++) {
    s = kvStorage->SSet(TableName(1), "key" + std::to_string(i),
                        Value("value"));
    ASSERT_TRUE(s.ok());
  }
  size = kvStorage->SSize(TableName(1));
