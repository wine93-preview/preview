...
TEST_F(MemoryStorageTest, HGetTest) {
  TestHGet(kvStorage_);
  TestHGet(kvStorage2_);
}
TEST_F(MemoryStorageTest, HSetTest) {
  TestHSet(kvStorage_);
  TestHSet(kvStorage2_);
}
TEST_F(MemoryStorageTest, HDelTest) {
  TestHDel(kvStorage_);
  TestHDel(kvStorage2_);
}
TEST_F(MemoryStorageTest, HGetAllTest) {
  TestHGetAll(kvStorage_);
  TestHGetAll(kvStorage2_);
}
TEST_F(MemoryStorageTest, HSizeTest) {
  TestHSize(kvStorage_);
  TestHSize(kvStorage2_);
}
TEST_F(MemoryStorageTest, HClearTest) {
  TestHClear(kvStorage_);
  TestHClear(kvStorage2_);
}

TEST_F(MemoryStorageTest, SGetTest) {
  TestSGet(kvStorage_);
  TestSGet(kvStorage2_);
}
TEST_F(MemoryStorageTest, SSetTest) {
  TestSSet(kvStorage_);
  TestSSet(kvStorage2_);
}
TEST_F(MemoryStorageTest, SDelTest) {
  TestSDel(kvStorage_);
  TestSDel(kvStorage2_);
}
TEST_F(MemoryStorageTest, SSeekTest) {
  TestSSeek(kvStorage_);
  TestSSeek(kvStorage2_);
}
TEST_F(MemoryStorageTest, SGetAllTest) {
  TestSGetAll(kvStorage_);
  TestSGetAll(kvStorage2_);
}
TEST_F(MemoryStorageTest, SSizeTest) {
  TestSSize(kvStorage_);
  TestSSize(kvStorage2_);
}
TEST_F(MemoryStorageTest, SClearTest) {
  TestSClear(kvStorage_);
  TestSClear(kvStorage2_);
}
TEST_F(MemoryStorageTest, MixOperatorTest) {
