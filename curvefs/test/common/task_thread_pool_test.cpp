  ...
  void AddSum(uint64_t key, uint64_t start, uint64_t end) {
    ...
    auto leftTask = [key, start, end, this]() {
      ...
      if (left <= end) {
        ...
        auto task = [this, key, left, end]() { AddSum(key, left, end); };
        AddSumTask(key, task);
      }
    };
    ...
    auto rightTask = [key, start, end, this]() {
      ...
      if (right <= end) {
        WriteLockGuard lock(sumMutex_);
        sum_.emplace(key, SumTask(start, end));
        auto task = [this, key, right, end]() { AddSum(key, right, end); };
        AddSumTask(key, task);
      }
    };
    ...
    if (iter.second) {
      LOG(INFO) << "add task:" << key << " start:" << start << " success!";
    }
  }
  ...
  void Unit() {
...

    {
      ...
      for (auto iter : sum_) {
        ...
        LOG(INFO) << "Sum Done:" << iter.first << " sum:" << iter.second.sum_
                  << " result:" << result;
        ...
        for (auto iter = sum_.begin(); iter != sum_.end();) {
          if (SumDone(iter->first)) {
            ...
            LOG(INFO) << "rm Sum Done:" << iter->first
                      << " sum:" << iter->second.sum_ << " result:" << result;
            ASSERT_EQ(iter->second.sum_, result);
