        ...
        if (isStreaming_) {
          for (auto const& i : inode2S3ChunkInfoList_) {
            std::cout << "fsId: " << i.first.fsid()
                      << " inodeId: " << i.first.inodeid() << std::endl;
            for (auto const& j : i.second.s3chunks()) {
              std::cout << "  s3ChunkInfo: " << j.DebugString() << std::endl;
            }
          }
        } else {
          std::cout << "response: " << response_->DebugString() << std::endl;
        }
        ret = true;
      }
    } else {
...
void InodeS3InfoMapTool::UpdateInode2S3ChunkInfoList_(
    const InodeBase& inode, const S3ChunkInfoList& list) {
  ...
  auto iter =
      std::find_if(inode2S3ChunkInfoList_.begin(), inode2S3ChunkInfoList_.end(),
