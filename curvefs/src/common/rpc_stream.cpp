...
StreamConnection::StreamConnection()
    : streamId_(-1), status_(StreamStatus::STREAM_OK) {}
...
brpc::StreamId StreamConnection::GetStreamId() { return streamId_; }

std::string StreamConnection::GetEOFMessage() { return kEOFMessage_; }

void StreamConnection::SetStatus(StreamStatus status) { status_ = status; }
...
std::shared_ptr<StreamConnection> StreamClient::Connect(
    brpc::Controller* cntl, const ReceiveCallback& callback,
...
std::shared_ptr<StreamConnection> StreamClient::GetConnection(
  ...
  if (brpc::StreamClose(id) != 0) {
    LOG(ERROR) << "stream (streamId=" << id << ") close failed in client-side";
  }
  return nullptr;
}
