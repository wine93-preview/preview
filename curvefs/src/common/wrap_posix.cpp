...
int PosixWrapper::close(int fd) { return ::close(fd); }

int PosixWrapper::remove(const char* pathname) { return ::remove(pathname); }
...
int PosixWrapper::renameat2(const char* oldpath, const char* newpath,
                            unsigned int flags) {
...
DIR* PosixWrapper::opendir(const char* name) { return ::opendir(name); }

struct dirent* PosixWrapper::readdir(DIR* dirp) { return ::readdir(dirp); }

int PosixWrapper::closedir(DIR* dirp) { return ::closedir(dirp); }
...
ssize_t PosixWrapper::pwrite(int fd, const void* buf, size_t count,
                             off_t offset) {
  return ::pwrite(fd, buf, count, offset);
}

int PosixWrapper::fstat(int fd, struct stat* buf) { return ::fstat(fd, buf); }
...
int PosixWrapper::fsync(int fd) { return ::fsync(fd); }

int PosixWrapper::fdatasync(int fd) { return ::fdatasync(fd); }

int PosixWrapper::statfs(const char* path, struct statfs* buf) {
  return ::statfs(path, buf);
}

int PosixWrapper::uname(struct utsname* buf) { return ::uname(buf); }
...
void* PosixWrapper::malloc(size_t size) { return ::malloc(size); }

void* PosixWrapper::memset(void* s, int c, size_t n) {
  return ::memset(s, c, n);
}

void PosixWrapper::free(void* s) { return ::free(s); }
