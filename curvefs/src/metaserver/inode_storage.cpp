...
MetaStatusCode InodeStorage::UpdateInodeS3MetaSize(Transaction txn,
  ...
  if (s.ok()) {
    ...
    LOG(ERROR) << "failed to get inode s3 meta size, status=" << s.ToString();
    return MetaStatusCode::STORAGE_INTERNAL_ERROR;
  }
  ...
  if (!s.ok()) {
    LOG(ERROR) << "failed to set inode s3 meta size, status=" << s.ToString();
    return MetaStatusCode::STORAGE_INTERNAL_ERROR;
  }
  return MetaStatusCode::OK;
}
...
MetaStatusCode InodeStorage::AddS3ChunkInfoList(
  ...
  Key4S3ChunkInfoList key(fsId, inodeId, chunkIndex, firstChunkId, lastChunkId,
                          size);
  std::string skey = conv_.SerializeToString(key);
  Status s;
  if (txn) {
