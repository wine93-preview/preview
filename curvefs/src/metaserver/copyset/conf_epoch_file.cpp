...
int ConfEpochFile::Load(const std::string& path, PoolId* poolID,
  ...
  if (!json2pb::JsonToProtoMessage(jsonStr, &confEpoch, opt, &err)) {
    LOG(ERROR) << "LoadConfEpoch failed to decode conf epoch, encoded data: "
  ...
  if (crc32c != confEpoch.checksum()) {
    LOG(ERROR) << "LoadConfEpoch crc checksum error, encoded data: " << jsonStr
               << ", expect crc: " << crc32c
               << ", encoded crc: " << confEpoch.checksum() << ", conf file: '"
               << path << "'";
    return -1;
  }
...
int ConfEpochFile::Save(const std::string& path, const PoolId poolID,
  ...
  if (fd < 0) {
    LOG(ERROR) << "Create epoch file '" << path << "' failed, err: " << errno
