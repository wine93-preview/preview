...
bool RecycleCleaner::DeleteNode(const Dentry& dentry) {
  ...
  if (ret != MetaStatusCode::OK) {
    LOG(WARNING) << "get parent inode fail, ret = " << MetaStatusCode_Name(ret)
                 << ", fsId = " << fsId << ", inodeId = " << parent;
    return false;
  }
  ...
  ret = metaClient_->UpdateInodeAttrWithOutNlink(fsId, parent, parentDirtyAttr);
  if (ret != MetaStatusCode::OK) {
  ...
  *dirtyAttr.mutable_parent() = inode.parent();
  ...
  if (ret != MetaStatusCode::OK) {
    LOG(WARNING) << "UpdateInodeAttr failed, ret = " << MetaStatusCode_Name(ret)
  ...
  while (true) {
    ...
    for (const auto& it : dentryList) {
      ...
      if (it.has_type()) {
        ...
        if (ret != MetaStatusCode::OK) {
          LOG(WARNING) << "DeleteDirRecursive get inode attr fail, ret = "
...
uint64_t RecycleCleaner::GetTxId() {
  ...
  if (FSStatusCode::OK !=
    ...
    LOG(WARNING) << "GetLatestTxId fail, use UINT64_MAX, fsId = " << GetFsId()
...
bool RecycleCleaner::ScanRecycle() {
  ...
  while (true) {
    ...
    auto ret = partition_->ListDentry(dentry, &tempDentrys, limit_, onlyDir);
    if (ret != MetaStatusCode::OK) {
    ...
    for (const auto& it : tempDentrys) {
      ...
      if (IsDirTimeOut(it.name())) {
        ...
        LOG(INFO) << "recycle dir is timeout, delete dir recursive"
                  ...
                  << ", partitionId = " << GetPartitionId();
        if (!DeleteDirRecursive(it)) {
          LOG(WARNING) << "ScanRecycle dir is timeout"
        ...
        LOG(INFO) << "recycle dir is not timeout, skip. dir = " << it.name()
                  << ", recycle time = " << GetRecycleTime()
                  << ", fsId = " << GetFsId()
                  << ", partitionId = " << GetPartitionId();
      }
