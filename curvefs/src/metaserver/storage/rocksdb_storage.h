...
class RocksDBStorage : public KVStorage, public StorageTransaction {
 ...
 private:
  ...
  static std::string ToInternalName(const std::string& name, bool ordered,
                                    bool start);

  std::string ToInternalKey(const std::string& name, const std::string& key,
                            bool ordered);

  std::string ToUserKey(const std::string& ikey);

  Status Get(const std::string& name, const std::string& key, ValueType* value,
             bool ordered);

  Status Set(const std::string& name, const std::string& key,
             const ValueType& value, bool ordered);

  Status Del(const std::string& name, const std::string& key, bool ordered);
...
class RocksDBStorageIterator : public Iterator {
 public:
  RocksDBStorageIterator(RocksDBStorage* storage, std::string prefix,
                         size_t size, int status, bool ordered)
      : storage_(storage),
  ...
  uint64_t Size() { return static_cast<uint64_t>(size_); }
  ...
  void DisablePrefixChecking() { prefixChecking_ = false; }
