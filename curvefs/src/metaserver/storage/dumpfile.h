 ...
 *   +--------------+-------+----------------+---------+-----+--------------+-------+----------------+---------+
 *   | key_1_length | key_1 | value_1_length | value_1 | ... | key_n_length |
 * key_n | value_n_length | value_n |
 *   +--------------+-------+----------------+---------+-----+--------------+-------+----------------+---------+
...
class DumpFile {
 ...
 private:
  ...
  DUMPFILE_ERROR SaveString(const std::string& str, off_t* offset,
                            uint32_t* checkSum);

  DUMPFILE_ERROR SaveEntry(const std::string& entry, off_t* offset,
                           uint32_t* checkSum);

  template <typename Int>
  DUMPFILE_ERROR LoadInt(Int* num, off_t* offset, uint32_t* checkSum);

  DUMPFILE_ERROR LoadString(std::string* str, off_t* offset, size_t length,
                            uint32_t* checkSum);

  DUMPFILE_ERROR LoadEntry(std::string* entry, off_t* offset,
                           uint32_t* checkSum);
