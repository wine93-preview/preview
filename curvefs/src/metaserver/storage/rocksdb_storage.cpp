...
//  RocksDBStorage
RocksDBStorage::RocksDBStorage() : InTransaction_(false) { InitDbOptions(); }

RocksDBStorage::RocksDBStorage(StorageOptions options)
    : inited_(false), options_(std::move(options)), InTransaction_(false) {
  InitDbOptions();
}
...
STORAGE_TYPE RocksDBStorage::Type() { return STORAGE_TYPE::ROCKSDB_STORAGE; }
...
bool RocksDBStorage::Close() {
  ...
  for (auto handle : handles_) {
    ...
    if (!s.ok()) {
      LOG(ERROR) << "Destroy column family failed, status = " << s.ToString();
      return false;
    }
  }
  ...
  s = txnDB_->Close();
  if (!s.ok()) {
    LOG(ERROR) << "Close rocksdb failed, status = " << s.ToString();
    return false;
  }
...
Status RocksDBStorage::Set(const std::string& name, const std::string& key,
...
std::shared_ptr<Iterator> RocksDBStorage::Seek(const std::string& name,
  ...
  return std::make_shared<RocksDBStorageIterator>(this, ikey, 0, status, true);
}
...
std::shared_ptr<Iterator> RocksDBStorage::GetAll(const std::string& name,
  ...
  return std::make_shared<RocksDBStorageIterator>(this, std::move(ikey), 0,
                                                  status, ordered);
}
...
StorageOptions RocksDBStorage::GetStorageOptions() const { return options_; }
