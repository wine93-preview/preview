...
#ifndef CURVEFS_SRC_MDS_SCHEDULE_SCHEDULEMETRICSTEMPLATE_H_
#define CURVEFS_SRC_MDS_SCHEDULE_SCHEDULEMETRICSTEMPLATE_H_
...
class ScheduleMetricsT {
 ...
 private:
   ...
   */
  void UpdateOperatorsMap(const Operator& op, std::string type, IdType target);
...
void ScheduleMetricsT<IdType, CopySetInfoT, CopySetConfT, TopologyT,
  ...
  // set copysetId
  operators[op.copysetID].Set("copySetId", std::to_string(op.copysetID.second));
  ...
  operators[op.copysetID].Set("opItem", topo_->GetHostNameAndPortById(target));
...
#endif  // CURVEFS_SRC_MDS_SCHEDULE_SCHEDULEMETRICSTEMPLATE_H_
