...
#ifndef CURVEFS_SRC_MDS_SCHEDULE_OPERATORCONTROLLERTEMPLATE_H_
#define CURVEFS_SRC_MDS_SCHEDULE_OPERATORCONTROLLERTEMPLATE_H_
...
class OperatorControllerT {
 public:
  ...
  using ScheduleMetrics = ScheduleMetricsT<IdType, CopySetInfoT, CopySetConfT,
                                           TopologyT, TopoCopySetInfoT>;
...
bool OperatorControllerT<
  ...
  if (operators_[originInfo.id].IsTimeout()) {
    LOG(WARNING) << "apply operator: " << operators_[originInfo.id].OpToString()
    ...
    switch (res) {
      ...
      case ApplyStatus::Finished:
        LOG(INFO) << "apply operator " << operators_[originInfo.id].OpToString()
...
#endif  // CURVEFS_SRC_MDS_SCHEDULE_OPERATORCONTROLLERTEMPLATE_H_
