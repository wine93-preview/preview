...
void TopologyMetricService::UpdateTopologyMetrics() {
  ...
  for (auto msId : metaservers) {
    ...
    if (topo_->GetMetaServer(msId, &ms)) {
      ...
      it->second->diskUsed.set_value(ms.GetMetaServerSpace().GetDiskUsed());
      ...
      it->second->memoryUsed.set_value(ms.GetMetaServerSpace().GetMemoryUsed());
  ...
  for (auto pid : pools) {
    ...
    for (auto pit = partitions.begin(); pit != partitions.end(); ++pit) {
      ...
      if (itFsId2FileType2InodeNum == fsId2FileType2InodeNum.end()) {
        fsId2FileType2InodeNum.emplace(fsId, std::move(fileType2InodeNum));
      } else {
        for (auto const& fileType2Inode : fileType2InodeNum) {
          auto itFileType2InodeNum =
              itFsId2FileType2InodeNum->second.find(fileType2Inode.first);
          if (itFileType2InodeNum == itFsId2FileType2InodeNum->second.end()) {
            itFsId2FileType2InodeNum->second.emplace(fileType2Inode);
          } else {
    ...
    for (const auto& cm : metaServerMetricInfo) {
      ...
      if (ix == gMetaServerMetrics.end()) {
        ...
        ix = gMetaServerMetrics.emplace(cm.first, std::move(cptr)).first;
      }
  ...
  for (const auto& fsId2InodeNumPair : fsId2InodeNum) {
    ...
    if (it == gFsMetrics.end()) {
      ...
      it = gFsMetrics.emplace(fsId2InodeNumPair.first, std::move(cptr)).first;
    }
  ...
  for (auto const& fsId2FileType2InodeNumPair : fsId2FileType2InodeNum) {
    ...
    if (it == gFsMetrics.end()) {
      ...
      it = gFsMetrics.emplace(fsId2FileType2InodeNumPair.first, std::move(cptr))
               .first;
    }
  ...
  for (auto it = gMetaServerMetrics.begin(); it != gMetaServerMetrics.end();) {
