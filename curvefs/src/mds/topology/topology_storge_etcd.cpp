...
bool TopologyStorageEtcd::LoadPool(
    std::unordered_map<PoolIdType, Pool>* poolMap, PoolIdType* maxPoolId) {
...
bool TopologyStorageEtcd::LoadZone(
    std::unordered_map<ZoneIdType, Zone>* zoneMap, ZoneIdType* maxZoneId) {
  std::vector<std::string> out;
  zoneMap->clear();
  *maxZoneId = 0;
...
bool TopologyStorageEtcd::LoadServer(
  ...
  *maxServerId = 0;
  ...
  for (uint32_t i = 0; i < out.size(); i++) {
    ...
    if (!ret.second) {
      LOG(ERROR) << "LoadSever: "
                 << "SererId duplicated, ServerId = " << id;
      return false;
    }
    if (*maxServerId < id) {
      *maxServerId = id;
  ...
  for (uint32_t i = 0; i < out.size(); i++) {
    ...
    if (!ret.second) {
      LOG(ERROR) << "LoadChunkSever: "
                 << "ChunkSererId duplicated, MetaServerId = " << id;
      return false;
    }
...
bool TopologyStorageEtcd::LoadCopySet(
  ...
  for (uint32_t i = 0; i < out.size(); i++) {
    ...
    auto ret = copySetMap->emplace(std::make_pair(pid, id), std::move(data));
    if (!ret.second) {
      LOG(ERROR) << "LoadCopySet: "
                 << "Id duplicated, poolId = " << pid << ", copySetId = " << id;
      return false;
    }
  ...
  for (uint32_t i = 0; i < out.size(); i++) {
    ...
    if (!ret.second) {
      LOG(ERROR) << "LoadPartition: "
                 << "PartitionId duplicated, PartitionId = " << id;
      return false;
    }
...
bool TopologyStorageEtcd::StoragePool(const Pool& data) {
  ...
  if (errCode != EtcdErrCode::EtcdOK) {
    LOG(ERROR) << "Put Pool into etcd err"
               << ", errcode = " << errCode << ", poolId = " << data.GetId();
    return false;
  }
  return true;
}
...
bool TopologyStorageEtcd::StorageZone(const Zone& data) {
  ...
  if (errCode != EtcdErrCode::EtcdOK) {
    LOG(ERROR) << "Put Zone into etcd err"
               << ", errcode = " << errCode << ", zoneId = " << data.GetId();
    return false;
  }
  return true;
}
...
bool TopologyStorageEtcd::StorageServer(const Server& data) {
  ...
  if (errCode != EtcdErrCode::EtcdOK) {
    LOG(ERROR) << "Put Server into etcd err"
               << ", errcode = " << errCode << ", serverId = " << data.GetId();
    return false;
  }
  return true;
}
...
bool TopologyStorageEtcd::StorageCopySet(const CopySetInfo& data) {
  ...
  if (errCode != EtcdErrCode::EtcdOK) {
    LOG(ERROR) << "Put Copyset into etcd err"
               << ", errcode = " << errCode << ", poolId = " << data.GetPoolId()
               << ", copysetId = " << data.GetId();
    return false;
  }
  return true;
...
bool TopologyStorageEtcd::DeleteMetaServer(MetaServerIdType id) {
  ...
  if (errCode != EtcdErrCode::EtcdOK) {
    LOG(ERROR) << "delete MetaServer from etcd err"
               << ", errcode = " << errCode << ", metaServerId = " << id;
    return false;
  }
  return true;
}
...
bool TopologyStorageEtcd::DeleteCopySet(CopySetKey key) {
  ...
  if (errCode != EtcdErrCode::EtcdOK) {
    LOG(ERROR) << "delete Copyset from etcd err"
               << ", errcode = " << errCode << ", poolId = " << key.first
...
bool TopologyStorageEtcd::DeletePartition(PartitionIdType id) {
  ...
  if (errCode != EtcdErrCode::EtcdOK) {
    LOG(ERROR) << "delete Partition from etcd err"
               << ", errcode = " << errCode << ", partitionId = " << id;
    return false;
  }
  return true;
}
...
bool TopologyStorageEtcd::UpdatePartitions(
  ...
  for (uint32_t i = 0; i < datas.size(); i++) {
    ...
    Operation op{OpType::OpPut, const_cast<char*>(keys[i].data()),
...
bool TopologyStorageEtcd::StorageClusterInfo(const ClusterInformation& info) {
  ...
  std::string value;
  if (codec_->EncodeClusterInfoData(info, &value) != true) {
...
bool TopologyStorageEtcd::StorageMemcacheCluster(const MemcacheCluster& data) {
  ...
  if (!ret) {
    LOG(ERROR) << "EncodeMemcacheCLusterData err, clusterId = " << data.GetId();
  } else {
    int errCode = client_->Put(key, value);
    if (errCode != EtcdErrCode::EtcdOK) {
