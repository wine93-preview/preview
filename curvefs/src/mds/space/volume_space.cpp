...
BlockGroup BuildBlockGroupFromClean(uint64_t offset, uint64_t size,
...
VolumeSpace::VolumeSpace(uint32_t fsId, Volume volume,
                         BlockGroupStorage* storage, FsStorage* fsStorage)
    : fsId_(fsId),
...
uint32_t VolumeSpace::AllocateFromAvailableGroups(
    uint32_t count, const std::string& owner, std::vector<BlockGroup>* groups) {
  uint32_t allocated = 0;
...
SpaceErrCode VolumeSpace::AcquireBlockGroupInternal(uint64_t blockGroupOffset,
  ...
  // block group and in this case, block group must not be emtpy
  {
    ...
    if (cleanGroups_.count(blockGroupOffset) != 0) {
      *group =
          BuildBlockGroupFromClean(blockGroupOffset, volume_.blockgroupsize(),
  ...
  for (auto& group : blockGroups) {
    ...
    if (it != allocatedGroups_.end()) {
      if (it->second.owner() != group.owner()) {
        LOG(WARNING) << "Owner is not identical, block group may "
