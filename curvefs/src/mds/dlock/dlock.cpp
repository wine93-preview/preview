...
DLock::DLock(const DLockOptions& options,
    ...
    : options_(options), etcdClient_(etcdClient) {}

LOCK_STATUS DLock::Lock(const std::string& name, const std::string& owner) {
  bool succ = false;
  ...
  if (!succ) {
    LOG(ERROR) << "DLock lock failed, name=" << name << ", owner=" << owner;
...
LOCK_STATUS DLock::UnLock(const std::string& name, const std::string& owner) {
  NameLockGuard lg(nameLock_, name);
  std::string oldOwner;
  OWNER_STATUS status = GetOwner(name, &oldOwner);
  if (status == OWNER_STATUS::EXPIRED || status == OWNER_STATUS::NOT_EXIST ||
      (status == OWNER_STATUS::OK && oldOwner == owner)) {
    return ClearOwner(name) ? LOCK_STATUS::OK : LOCK_STATUS::FAILED;
  }
  return LOCK_STATUS::FAILED;
}

bool DLock::CheckOwner(const std::string& name, const std::string& owner) {
  NameLockGuard lg(nameLock_, name);
  std::string oldOwner;
  OWNER_STATUS status = GetOwner(name, &oldOwner);
  LOG(INFO) << "DLock check owner, retCode=" << status << ", name=" << name
            << ", expect owner=" << owner << ", actual owner=" << oldOwner;
...
OWNER_STATUS DLock::GetOwner(const std::string& name, std::string* owner) {
...
bool DLock::SetOwner(const std::string& name, const std::string& owner,
                     uint64_t ttlMs) {
  std::string skey = EncodeKey(name);
  std::string svalue;
  DLockValue value;
...
bool DLock::ReplaceOwner(const std::string& name, const std::string& owner,
                         uint64_t ttlMs) {
  std::string oldOwner;
  OWNER_STATUS status = GetOwner(name, &oldOwner);
  LOG(INFO) << "DLock get owner, retCode=" << status << ", name=" << name
            << ", owner=" << oldOwner;
  if (status == OWNER_STATUS::NOT_EXIST || status == OWNER_STATUS::EXPIRED ||
      oldOwner == owner) {
