        ...
        if (len < blockSize) {  // just one block
            ...
            if (!firstBlockFull) {
                ...
                prefetchObjs->push_back(
                    std::make_pair(key, firstBlockSize));
            } else {
            ...
            if (!lastBlockFull) {
                ...
                prefetchObjs->push_back(
                    std::make_pair(key, lastBlockSize));
            } else {
...
void WarmupManagerS3Impl::PutObjectToCache(
    ...
    case curvefs::client::common::WarmupStorageType::kWarmupStorageTypeDisk:
        {
          // FIXME: let it more easy
          BlockKey key;
          Block block(context->buf, context->len);
          auto items = Split(context->key);
          assert(items.size() > 0);
          assert(key.ParseFilename(items.back()));
          auto blockCache = s3Adaptor_->GetBlockCache();
