...
CURVEFS_ERROR FuseVolumeClient::FuseOpCreate(fuse_req_t req, fuse_ino_t parent,
                                             ...
                                             struct fuse_file_info* fi,
                                             EntryOut* entryOut) {
  VLOG(3) << "FuseOpCreate, parent: " << parent << ", name: " << name
          << ", mode: " << mode;

  std::shared_ptr<InodeWrapper> inode;
  CURVEFS_ERROR ret =
      MakeNode(req, parent, name, mode, FsFileType::TYPE_FILE, 0, false, inode);
  if (ret != CURVEFS_ERROR::OK) {
    return ret;
  }
...
CURVEFS_ERROR FuseVolumeClient::FuseOpMkNod(fuse_req_t req, fuse_ino_t parent,
  ...
  std::shared_ptr<InodeWrapper> inode;
  CURVEFS_ERROR rc = MakeNode(req, parent, name, mode, FsFileType::TYPE_FILE,
                              rdev, false, inode);
...
CURVEFS_ERROR FuseVolumeClient::FuseOpFsync(fuse_req_t req, fuse_ino_t ino,
  ...
  if (datasync) {
    VLOG(3) << "FuseOpFsync end, ino: " << ino << ", datasync: " << datasync;
    return CURVEFS_ERROR::OK;
  }
