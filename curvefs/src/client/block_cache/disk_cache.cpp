...
#include "curvefs/src/client/block_cache/disk_cache.h"
...
using CacheValue = DiskCacheManager::CacheValue;

BlockReaderImpl::BlockReaderImpl(int fd, std::shared_ptr<LocalFileSystem> fs)
    : fd_(fd), fs_(fs) {}

BCACHE_ERROR BlockReaderImpl::ReadAt(off_t offset, size_t length, char* buffer) {
...
bool DiskCache::IsCached(const BlockKey& key) {
  ...
  if (rc == BCACHE_ERROR::OK) {
    return true;
  } else if (loader_->Loading() && fs_->FileExists(GetCachePath(key))) {
    return true;
  }
  return false;
}
