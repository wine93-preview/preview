...
std::string S3ClientImpl::S3Key(const std::string& key) {
...
BCACHE_ERROR S3ClientImpl::Range(const std::string& key, off_t offset,
  ...
  if (rc < 0) {
    if (!client_->ObjectExist(S3Key(key))) {
...
void S3ClientImpl::AsyncPut(const std::string& key, const std::string& filepath,
