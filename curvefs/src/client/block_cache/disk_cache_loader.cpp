...
DiskCacheLoader::DiskCacheLoader(std::shared_ptr<LocalFileSystem> fs,
    ...
    : thread_(), running_(false), fs_(fs), layout_(layout), manager_(manager) {}
...
void DiskCacheLoader::DoLoad(const std::string& root, LoadType type) {
  ...
  auto rc = fs_->Walk(root, [&](const std::string& prefix,
    ...
    if (type == LoadType::LOAD_STAGE) {
      ...
      manager_->Add(key, DiskCacheManager::CacheValue(info.size, info.atime));
    }
...
std::string DiskCacheLoader::StrType(LoadType type) {
  ...
  case LoadType::LOAD_CACHE:
    return "cache";
  default:
    CHECK(false) << "Invalid load type: " << type;
  }
}
