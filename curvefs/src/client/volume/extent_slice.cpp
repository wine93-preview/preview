...
void ExtentSlice::DivideForWrite(uint64_t offset, uint64_t len,
                                 const char* data,
  ...
  while (curOff < curEnd && lower != upper) {
    ...
    if (curOff < extStart) {
      ...
      part.data = datap;
      // write           |----|        |----|      |----|   |-----| |----|   //
      // NOLINT(whitespace/line_length) extents  |----|          |----| |------|
      // |-------|   |-----|      // NOLINT(whitespace/line_length)
      if (curOff >= extEnd) {
        // write           |----|        |----|
        // extents  |----|          |----|
        if (curOff == extEnd ||
            align_down(curOff, ExtentCache::option_.blockSize) == extEnd) {
          leftHintOffset = lower->second.pOffset + lower->second.len;
        }
  ...
  if (curOff < curEnd) {
    ...
    part.allocInfo.lOffset = alignedoffset;
    part.allocInfo.len = align_up(
        std::max(alignedend - alignedoffset, ExtentCache::option_.preAllocSize),
        ExtentCache::option_.blockSize);

    if (upper != extents_.end()) {
      part.allocInfo.len =
          std::min(upper->first - alignedoffset, part.allocInfo.len);
      assert(is_aligned(part.allocInfo.len, ExtentCache::option_.blockSize));
    }
...
void ExtentSlice::DivideForRead(uint64_t offset, uint64_t len, char* data,
    ...
    if (curOff < extStart) {
      ...
      if (curEnd <= extEnd) {
        ...
        if (lower->second.UnWritten) {
          ...
          reads->emplace_back(lower->second.pOffset, curEnd - curOff, datap);
          return;
        }
      } else {
        // read    |-------|
        ...
        if (lower->second.UnWritten) {
          ...
          reads->emplace_back(lower->second.pOffset, extEnd - curOff, datap);
        }
      ...
      if (curOff >= extEnd) {
        ...
        if (lower->second.UnWritten) {
          ...
          reads->emplace_back(lower->second.pOffset + (curOff - extStart),
                              curEnd - curOff, datap);
        }
...
bool ExtentSlice::MarkWritten(uint64_t offset, uint64_t len) {
  ...
  auto mergeable =
      [this,
       &prev](std::map<uint64_t, curvefs::client::PExtent>::iterator current) {
        return prev != extents_.end() && !prev->second.UnWritten &&
  ...
  while (curOff < curEnd && curr != upper) {
    if (nonoverlap(curOff, curEnd - curOff, curr->first, curr->second.len)) {
      prev = curr;
      ++curr;
      continue;
    }
