...
bool DiskCacheManager::IsCacheClean() { return cacheWrite_->IsCacheClean(); }
...
void DiskCacheManager::SetDiskInitUsedBytes() {
  ...
  if (result.empty()) {
    LOG_EVERY_N(WARNING, 100) << "get disk used size failed.";
    return;
  }
...
void DiskCacheManager::TrimCache() {
  ...
  std::string cacheReadFullDir, cacheWriteFullDir, cacheReadFile,
      cacheWriteFile, cacheKey;
  ...
  while (true) {
    ...
    while (!IsDiskCacheSafe()) {
      ...
      cacheReadFile =
          cacheReadFullDir + "/" +
          curvefs::common::s3util::GenPathByObjName(cacheKey, objectPrefix_);
      cacheWriteFile =
          cacheWriteFullDir + "/" +
          curvefs::common::s3util::GenPathByObjName(cacheKey, objectPrefix_);
      struct stat statFile;
      ...
      if (ret < 0) {
        LOG(ERROR) << "remove disk file error, file is: " << cacheKey
                   << "error is: " << errno;
        continue;
      }
...
void DiskCacheManager::InitMetrics(const std::string& fsName) {
  ...
  diskInitThread_ =
      curve::common::Thread(&DiskCacheManager::SetDiskInitUsedBytes, this);
}
