...
void DiskCacheWrite::Init(
    std::shared_ptr<S3Client> client,
    std::shared_ptr<PosixWrapper> posixWrapper, const std::string cacheDir,
    uint32_t objectPrefix, uint64_t asyncLoadPeriodMs,
...
int DiskCacheWrite::ReadFile(const std::string name, char** buf,
  ...
  *size = fileSize;
  ...
  if (readLen < 0) {
    LOG(ERROR) << "read file error, ret = " << readLen << ", errno = " << errno
               << ", file = " << name;
    posixWrapper_->free(buffer);
    posixWrapper_->close(fd);
    return -1;
...
int DiskCacheWrite::GetUploadFile(const std::string& inode,
  ...
  waitUpload_.remove_if([&](const std::string& filename) {
    bool inodeFile = curvefs::common::s3util::ValidNameOfInode(inode, filename,
                                                               objectPrefix_);
    if (inodeFile) {
...
int DiskCacheWrite::FileExist(const std::string& inode) {
  ...
  if (ret < 0) {
    LOG(ERROR) << "DiskCacheWrite, load all cached file fail ret = " << ret;
    return ret;
  }

  for (auto iter = cachedObj.begin(); iter != cachedObj.end(); iter++) {
    bool exist =
        curvefs::common::s3util::ValidNameOfInode(inode, *iter, objectPrefix_);
    if (exist) {
      return 1;
    }
  }
...
int DiskCacheWrite::UploadFileByInode(const std::string& inode) {
  ...
  do {
    ...
    VLOG(3) << "UploadFileByInode, inode: " << inode << ", task num: " << num;
...
int DiskCacheWrite::UploadAllCacheWriteFile() {
  ...
  for (auto iter = uploadObjs.begin(); iter != uploadObjs.end(); iter++) {
    ...
    if (doRet < 0 || buffer == nullptr) {
      if (buffer != nullptr) posixWrapper_->free(buffer);
...
int DiskCacheWrite::WriteDiskFile(const std::string fileName, const char* buf,
  ...
  VLOG(6) << "WriteDiskFile start. name = " << fileName << ", force = " << force
          << ", length = " << length;
  std::string fileFullPath;
  int fd, ret;
