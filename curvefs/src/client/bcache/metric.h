...
class DiskCacheMetric {
 public:
    DiskCacheMetric::DiskCacheMetric(DiskCacheOption option) {
        std::string prefix = StrFormat("block_cache.disk_caches[%d]",
                                       option.index);
        metric_ = Metric(prefix);
        ...
        metric_.statue.set_value("DOWN");
    }

    virtual ~DiskCacheMetric() = default;

    void SetStatus(const std::string& value) {
        metric_.status.set_value(value);
    }

    void AddStageSkips() {
        metirc_.stageSkips << 1;
    }
 ...
 private:
    struct Metric {
        Metric(const std::string& prefix) :
            ...
            freeSpaceRatio(prefix, "free_space_ratio"),
        ...
        bvar::PassiveStatus<int64_t> used;
