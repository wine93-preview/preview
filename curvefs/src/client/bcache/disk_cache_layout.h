...
class DiskCacheLayout {
 public:
    explicit DiskCacheLayout(const std::string& rootDir)
        : rootDir_(rootDir) {}
    ...
    std::string GetStageDir() const  {
        return StrFormat("%s/stage", rootDir_);
    }

    std::string GetCacheDir() const  {
        return StrFormat("%s/cache", rootDir_);
    }

    std::string GetProbeDir() const {
        return StrFormat("%s/probe", rootDir_);
    }

    std::string GetMetaPath() const {
        return StrFormat("%s/.meta", rootDir_);
    }
