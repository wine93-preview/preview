...
class DiskCacheManager {
    struct CacheItem {
        CacheItem(CacheKey key, CacheValue value)
            : key(key), value(value) {}
 ...
 public:
    ...
    struct CacheValue {
        CacheValue(size_t size, TimeSpec atime)
 ...
 public:
    DiskCacheManager(uint64_t capacity,
 ...
 private:
    ...
    void DeleteBlocks(const CacheItems& toDel);
 ...
 private:
    ...
    std::unordered_map<CacheKey, CacheValue> caches_;
};
