...
bool MetaCache::GetTarget(uint32_t fsID, uint64_t inodeID,
  ...
  if (!GetCopysetIDwithInodeID(inodeID, &target->groupID, &target->partitionID,
                               &target->txId)) {
    // list infos from mds
...
bool MetaCache::CreatePartitions(int currentNum,
  ...
  WriteLockGuard wl4PartitionMap(rwlock4Partitions_);
  WriteLockGuard wl4CopysetMap(rwlock4copysetInfoMap_);
  DoAddOrResetPartitionAndCopyset(*newPartitions, std::move(copysetMap), false);
...
bool MetaCache::DoListOrCreatePartitions(
  ...
  if (list) {
    ...
    if (!mdsClient_->CreatePartition(fsID_, metacacheopt_.createPartitionOnce,
                                     partitionInfos)) {
  ...
  // get copyset infos
  bool ok = false;
  for (auto iter = lpool2Copyset.begin(); iter != lpool2Copyset.end(); ++iter) {
    // get copyset info
    std::vector<CopysetInfo<MetaserverID>> cpinfoVec;
    if (!mdsClient_->GetMetaServerListInCopysets(iter->first, iter->second,
                                                 &cpinfoVec)) {
      LOG(ERROR) << "get metaserver list of copyset in {poolid:" << iter->first
                 << "} fail";
      ok = false;
      break;
    }
...
bool MetaCache::GetPartitionIdByInodeId(uint32_t fsID, uint64_t inodeID,
                                        PartitionID* pid) {
  if (!TryGetPartitionIdByInodeId(partitionInfos_, &rwlock4Partitions_, inodeID,
                                  pid)) {
    // list form mds
    if (!ListPartitions(fsID)) {
