...
int Ext4FileSystemImpl::Read(int fd, char* buf, uint64_t offset, int length) {
  ...
  while (remainLength > 0) {
    int ret =
        posixWrapper_->pread(fd, buf + relativeOffset, remainLength, offset);
    ...
    if (ret < 0) {
      ...
      LOG(ERROR) << "pread failed, fd: " << fd << ", size: " << remainLength
                 << ", offset: " << offset << ", error: " << strerror(errno);
      return -errno;
    }
    remainLength -= ret;
    offset += ret;
...
int Ext4FileSystemImpl::Write(int fd, const char* buf, uint64_t offset,
  ...
  while (remainLength > 0) {
    int ret =
        posixWrapper_->pwrite(fd, buf + relativeOffset, remainLength, offset);
    if (ret < 0) {
      if (errno == EINTR && retryTimes < MAX_RETYR_TIME) {
        ++retryTimes;
        continue;
      }
      LOG(ERROR) << "pwrite failed, fd: " << fd << ", size: " << remainLength
                 << ", offset: " << offset << ", error: " << strerror(errno);
      return -errno;
    }
    remainLength -= ret;
