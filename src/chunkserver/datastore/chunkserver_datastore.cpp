  ...
  for (size_t i = 0; i < files.size(); ++i) {
    ...
    if (info.type == FileNameOperator::FileType::CHUNK) {
      ...
      string chunkFilePath =
          baseDir_ + "/" + FileNameOperator::GenerateChunkFileName(info.id);
...
CSErrorCode CSDataStore::DeleteSnapshotChunkOrCorrectSn(
  ...
  if (chunkFile != nullptr) {
    CSErrorCode errorCode =
        chunkFile->DeleteSnapshotOrCorrectSn(correctedSn);  // NOLINT
    if (errorCode != CSErrorCode::Success) {
      LOG(WARNING) << "Delete snapshot chunk or correct sn failed."
                   << "ChunkID = " << id << ", correctedSn = " << correctedSn;
      return errorCode;
    }
  }
  return CSErrorCode::Success;
...
CSErrorCode CSDataStore::ReadChunk(ChunkID id, SequenceNum sn, char* buf,
...
CSErrorCode CSDataStore::ReadSnapshotChunk(ChunkID id, SequenceNum sn,
...
CSErrorCode CSDataStore::CreateChunkFile(const ChunkOptions& options,
  ...
  auto tempChunkFile =
      std::make_shared<CSChunkFile>(lfs_, chunkFilePool_, options);
  CSErrorCode errorCode = tempChunkFile->Open(true);
  if (errorCode != CSErrorCode::Success) {
    LOG(WARNING) << "Create chunk file failed."
                 << "ChunkID = " << options.id << ", ErrorCode = " << errorCode;
    return errorCode;
  }
...
CSErrorCode CSDataStore::WriteChunk(ChunkID id, SequenceNum sn,
                                    const butil::IOBuf& buf, off_t offset,
...
CSErrorCode CSDataStore::CreateCloneChunk(ChunkID id, SequenceNum sn,
                                          SequenceNum correctedSn,
                                          ChunkSizeType size,
                                          const string& location) {
  // Check the validity of the parameters
  if (size != chunkSize_ || sn == kInvalidSeq || location.empty()) {
    LOG(ERROR) << "Invalid arguments."
               << "ChunkID = " << id << ", sn = " << sn
               << ", correctedSn = " << correctedSn << ", size = " << size
               << ", location = " << location;
    return CSErrorCode::InvalidArgError;
  }
  auto chunkFile = metaCache_.Get(id);
  ...
  if (info.location.compare(location) != 0 || info.curSn != sn ||
...
CSErrorCode CSDataStore::PasteChunk(ChunkID id, const char* buf, off_t offset,
                                    size_t length) {
  auto chunkFile = metaCache_.Get(id);
...
CSErrorCode CSDataStore::GetChunkInfo(ChunkID id, CSChunkInfo* chunkInfo) {
  auto chunkFile = metaCache_.Get(id);
  if (chunkFile == nullptr) {
...
CSErrorCode CSDataStore::GetChunkHash(ChunkID id, off_t offset, size_t length,
                                      std::string* hash) {
  auto chunkFile = metaCache_.Get(id);
  if (chunkFile == nullptr) {
...
CSErrorCode CSDataStore::loadChunkFile(ChunkID id) {
  ...
  if (metaCache_.Get(id) == nullptr) {
    ...
    if (errorCode != CSErrorCode::Success) return errorCode;
    metaCache_.Set(id, chunkFilePtr);
  }
  return CSErrorCode::Success;
}

ChunkMap CSDataStore::GetChunkMap() { return metaCache_.GetMap(); }
