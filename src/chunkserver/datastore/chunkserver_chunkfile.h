...
class CSChunkFile {
 public:
   ...
   * @return: return error code
   */
  CSErrorCode Write(SequenceNum sn, const butil::IOBuf& buf, off_t offset,
   ...
   * @return: return error code
   */
  CSErrorCode ReadSpecifiedChunk(SequenceNum sn, char* buf, off_t offset,
                                 size_t length);
  /**
   * Delete chunk files.
   * Normally there is no concurrency, mutually exclusive with other
   ...
   * @return: error code
   */
  CSErrorCode GetHash(off_t offset, size_t length, std::string* hash);
  /**
   * Get chunkFileMetaPage
   * @return: metapage
   */
  virtual const ChunkFileMetaPage& GetChunkFileMetaPage() { return metaPage_; }
 ...
 private:
  ...
  inline string path() {
    return baseDir_ + "/" + FileNameOperator::GenerateChunkFileName(chunkId_);
  }
  ...
  inline int SyncData() { return lfs_->Sync(fd_); }
  ...
  inline bool CheckOffsetAndLength(off_t offset, size_t len, size_t align) {
    ...
    return common::is_aligned(offset, align) && common::is_aligned(len, align);
  }
