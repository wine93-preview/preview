...
struct CSChunkInfo {
  ...
  bool operator==(const CSChunkInfo& rhs) const {
    if (chunkId != rhs.chunkId || pageSize != rhs.pageSize ||
        chunkSize != rhs.chunkSize || curSn != rhs.curSn ||
        snapSn != rhs.snapSn || correctedSn != rhs.correctedSn ||
        isClone != rhs.isClone || location != rhs.location) {
      return false;
    }
    // If the bitmap is not nullptr, compare whether the contents are equal
    if (bitmap != nullptr && rhs.bitmap != nullptr) {
      if (*bitmap != *rhs.bitmap) return false;
    } else {
  ...
  bool operator!=(const CSChunkInfo& rhs) const { return !(*this == rhs); }
};
