...
class FilePoolHelper {
 public:
  ...
  static int PersistEnCodeMetaInfo(std::shared_ptr<LocalFileSystem> fsptr,
                                   uint32_t fileSize, uint32_t metaPageSize,
   ...
   * @return: success 0, otherwise -1
   */
  static int DecodeMetaInfoFromMetaFile(std::shared_ptr<LocalFileSystem> fsptr,
...
class CURVE_CACHELINE_ALIGNMENT FilePool {
 public:
   ...
   */
  virtual FilePoolOptions GetFilePoolOpt() { return poolOpt_; }

  /**
   * @brief: Return the suffix of clean chunk
   */
  static std::string GetCleanChunkSuffix() { return kCleanChunkSuffix_; }
