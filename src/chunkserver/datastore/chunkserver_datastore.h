...
struct DataStoreStatus {
  ...
  DataStoreStatus() : chunkFileCount(0), snapshotCount(0), cloneChunkCount(0) {}
};
...
class CSDataStore {
 public:
   ...
   * @return: return error code
   */
  virtual CSErrorCode DeleteSnapshotChunkOrCorrectSn(ChunkID id,
   ...
   * @return: return error code
   */
  virtual CSErrorCode ReadChunk(ChunkID id, SequenceNum sn, char* buf,
   ...
   * @return: return error code
   */
  virtual CSErrorCode ReadChunkMetaPage(ChunkID id, SequenceNum sn, char* buf);
   ...
   * @return: return error code
   */
  virtual CSErrorCode ReadSnapshotChunk(ChunkID id, SequenceNum sn, char* buf,
                                        off_t offset, size_t length);
  /**
   * Write data
   ...
   * @return: return error code
   */
  virtual CSErrorCode WriteChunk(ChunkID id, SequenceNum sn,
  ...
  virtual CSErrorCode WriteChunk(ChunkID id, SequenceNum sn, const char* buf,
                                 off_t offset, size_t length, uint32_t* cost,
                                 const std::string& cloneSourceLocation = "") {
    butil::IOBuf data;
    data.append_user_data(const_cast<char*>(buf), length, TrivialDeleter);

    return WriteChunk(id, sn, data, offset, length, cost, cloneSourceLocation);
  }
   ...
   * @return: return error code
   */
  virtual CSErrorCode CreateCloneChunk(ChunkID id, SequenceNum sn,
                                       SequenceNum correctedSn,
                                       ChunkSizeType size,
   ...
   * @return: return error code
   */
  virtual CSErrorCode PasteChunk(ChunkID id, const char* buf, off_t offset,
                                 size_t length);
  /**
   ...
   */
  virtual CSErrorCode GetChunkInfo(ChunkID id, CSChunkInfo* chunkInfo);
   ...
   * @return: return error code
   */
  virtual CSErrorCode GetChunkHash(ChunkID id, off_t offset, size_t length,
                                   std::string* hash);
  /**
