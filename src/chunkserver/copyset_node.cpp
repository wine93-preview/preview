...
int CopysetNode::Init(const CopysetNodeOptions& options) {
  ...
  dataStore_ = std::make_shared<CSDataStore>(options.localFileSystem,
                                             options.chunkFilePool, dsOptions);
  ...
  if (!enableOdsyncWhenOpenChunkFile_) {
    ...
    dataStore_->SetCacheLimits(options.syncChunkLimit, options.syncThreshold);
    LOG(INFO) << "init sync thread success limit = " << options.syncChunkLimit
   ...
   * 初始化copyset性能metrics
   */
  int ret = ChunkServerMetric::GetInstance()->CreateCopysetMetric(logicPoolId_,
                                                                  copysetId_);
  if (ret != 0) {
...
void CopysetNode::on_apply(::braft::Iterator& iter) {
  for (; iter.valid(); iter.next()) {
    ...
    if (nullptr != closure) {
       ...
       * context进行apply
       */
      ChunkClosure* chunkClosure = dynamic_cast<ChunkClosure*>(iter.done());
      CHECK(nullptr != chunkClosure) << "ChunkClosure dynamic cast failed";
      ...
      butil::IOBuf data;
      auto opReq = ChunkOpRequest::Decode(log, &request, &data, iter.index(),
                                          GetLeaderId());
...
void CopysetNode::save_snapshot_background(::braft::SnapshotWriter* writer,
   ...
   */
  std::string filePathTemp = writer->get_path() + "/" + kCurveConfEpochFilename;
...
int CopysetNode::on_snapshot_load(::braft::SnapshotReader* reader) {
  ...
  if (fs_->DirExists(snapshotChunkDataDir)) {
    ...
    bool ret = nodeOptions_.snapshot_file_system_adaptor->get()->delete_file(
  ...
  if (fs_->FileExists(filePath)) {
    if (0 != LoadConfEpoch(filePath)) {
      LOG(ERROR) << "load conf.epoch failed. "
                 << "path:" << filePath << ", Copyset: " << GroupIdString();
      return -1;
    }
  }
...
void CopysetNode::on_start_following(const ::braft::LeaderChangeContext& ctx) {
  LOG(INFO) << "Copyset: " << GroupIdString()
            << ", peer id: " << peerId_.to_string() << "start following" << ctx;
}
...
int CopysetNode::LoadConfEpoch(const std::string& filePath) {
  ...
  int ret =
      epochFile_->Load(filePath, &loadLogicPoolID, &loadCopysetID, &loadEpoch);
  if (0 == ret) {
...
butil::Status CopysetNode::TransferLeader(const Peer& peer) {
  ...
  if (rc != 0) {
    status = butil::Status(rc,
                           ...
                           "%s to peer %s, error: %s",
                           GroupIdString().c_str(), peerId.to_string().c_str(),
                           berror(rc));
  ...
  for (ChunkID chunk : chunkIds) {
    copysetSyncPool_->Enqueue([=]() {
      ...
      if (r != CSErrorCode::Success) {
        LOG(FATAL) << "Sync Chunk failed in Copyset: " << GroupIdString()
                   << ", chunkid: " << chunk << " data store return: " << r;
      }
    });
  }
}
