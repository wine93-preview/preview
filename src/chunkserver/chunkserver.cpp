...
int ChunkServer::Run(int argc, char** argv) {
  ...
  LOG_IF(FATAL,
         !conf.GetUInt32Value("global.min_io_alignment", &FLAGS_minIoAlignment))
  ...
  LOG_IF(FATAL,
         !conf.GetBoolValue("fs.enable_renameat2", &lfsOption.enableRenameat2));
  ...
  if (raftLogProtocol == kProtocalCurve) {
    ...
    if (!useChunkFilePoolAsWalPool) {
      ...
      LOG_IF(FATAL,
             !conf.GetUInt32Value("walfilepool.use_chunk_file_pool_reserve",
  ...
  // 从本地获取meta
  std::string metaPath =
      UriParser::GetPathFromUri(registerOptions.chunkserverMetaUri).c_str();

  auto epochMap = std::make_shared<EpochMap>();
  if (fs->FileExists(metaPath)) {
    LOG_IF(FATAL,
           GetChunkServerMetaFromLocal(registerOptions.chunserverStoreUri,
                                       registerOptions.chunkserverMetaUri,
                                       registerOptions.fs, &localMetadata) != 0)
        << "Failed to GetChunkServerMetaFromLocal.";
    LOG_IF(FATAL,
           registerMDS.RegisterToMDS(&localMetadata, &metadata, epochMap) != 0)
        << "Failed to register to MDS.";
  } else {
    // 如果本地获取不到，向mds注册
    LOG(INFO) << "meta file " << metaPath << " do not exist, register to mds";
    LOG_IF(FATAL, registerMDS.RegisterToMDS(nullptr, &metadata, epochMap) != 0)
        << "Failed to register to MDS.";
  }
   ...
   */
  int checkCycles;
  LOG_IF(FATAL, !conf.GetIntValue("chunkserver.snapshot_throttle_check_cycles",
                                  &checkCycles));
  ...
  int ret = server.AddService(&copysetService, brpc::SERVER_DOESNT_OWN_SERVICE);
  CHECK(0 == ret) << "Fail to add CopysetService";

  // inflight throttle
  int maxInflight;
  LOG_IF(FATAL,
         !conf.GetIntValue("chunkserver.max_inflight_requests", &maxInflight));
  ...
  ret = server.AddService(&kCurveFileService, brpc::SERVER_DOESNT_OWN_SERVICE);
  CHECK(0 == ret) << "Fail to add CurveFileService";

  // chunkserver service
  ChunkServerServiceImpl chunkserverService(copysetNodeManager_);
  ret = server.AddService(&chunkserverService, brpc::SERVER_DOESNT_OWN_SERVICE);
  CHECK(0 == ret) << "Fail to add ChunkServerService";

  // scan copyset service
  ScanServiceImpl scanCopysetService(&scanManager_);
  ret = server.AddService(&scanCopysetService, brpc::SERVER_DOESNT_OWN_SERVICE);
  CHECK(0 == ret) << "Fail to add ScanCopysetService";

  // 启动rpc service
  LOG(INFO) << "Internal server is going to serve on: " << copysetNodeOptions.ip
  ...
  LOG_IF(ERROR, scanManager_.Fini() != 0) << "Failed to shutdown scan manager.";
...
void ChunkServer::InitChunkFilePoolOptions(
  ...
  LOG_IF(FATAL, !conf->GetBoolValue("chunkfilepool.enable_get_chunk_from_pool",
  ...
  if (chunkFilePoolOptions->getFileFromPool == false) {
    ...
    std::string metaUri;
    LOG_IF(FATAL, !conf->GetStringValue("chunkfilepool.meta_path", &metaUri));
    ::memcpy(chunkFilePoolOptions->metaPath, metaUri.c_str(), metaUri.size());
...
void ChunkServer::InitWalFilePoolOptions(common::Configuration* conf,
  ...
  LOG_IF(FATAL, !conf->GetBoolValue("walfilepool.enable_get_segment_from_pool",
                                    &walPoolOptions->getFileFromPool));

  if (walPoolOptions->getFileFromPool == false) {
    std::string filePoolUri;
    LOG_IF(FATAL,
           !conf->GetStringValue("walfilepool.file_pool_dir", &filePoolUri));
...
void ChunkServer::InitCopysetNodeOptions(
  ...
  LOG_IF(FATAL,
         !conf->GetUInt32Value("copyset.check_loadmargin_interval_ms",
  ...
  if (!copysetNodeOptions->enableOdsyncWhenOpenChunkFile) {
    ...
    LOG_IF(FATAL,
           !conf->GetUInt32Value("copyset.check_syncing_interval_ms",
...
void ChunkServer::InitCopyerOptions(common::Configuration* conf,
  ...
  LOG_IF(FATAL,
         !conf->GetStringValue("curve.config_path", &copyerOptions->curveConf));
  LOG_IF(FATAL,
...
void ChunkServer::InitHeartbeatOptions(common::Configuration* conf,
  ...
  LOG_IF(FATAL, !conf->GetUInt32Value("global.port", &heartbeatOptions->port));
...
void ChunkServer::InitRegisterOptions(common::Configuration* conf,
  ...
  LOG_IF(FATAL,
         !conf->GetIntValue("global.port", &registerOptions->chunkserverPort));
  LOG_IF(FATAL, !conf->GetStringValue("chunkserver.stor_uri",
...
void ChunkServer::LoadConfigFromCmdline(common::Configuration* conf) {
  ...
  if (GetCommandLineFlagInfo("chunkServerIp", &info) && !info.is_default) {
    ...
    LOG(FATAL) << "chunkServerIp must be set when run chunkserver in command.";
  }
  ...
  if (GetCommandLineFlagInfo("chunkServerMetaUri", &info) && !info.is_default) {
  ...
  if (GetCommandLineFlagInfo("walFilePoolDir", &info) && !info.is_default) {
    ...
    LOG(FATAL) << "walFilePoolDir must be set when run chunkserver in command.";
  }

  if (GetCommandLineFlagInfo("walFilePoolMetaPath", &info) &&
      !info.is_default) {
    conf->SetStringValue("walfilepool.meta_path", FLAGS_walFilePoolMetaPath);
  } else {
    LOG(FATAL)
  ...
  if (GetCommandLineFlagInfo("enableWalfilepool", &info) && !info.is_default) {
  ...
  if (GetCommandLineFlagInfo("copysetLoadConcurrency", &info) &&
      !info.is_default) {
    conf->SetIntValue("copyset.load_concurrency", FLAGS_copysetLoadConcurrency);
  }
