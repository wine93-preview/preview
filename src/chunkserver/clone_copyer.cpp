...
std::ostream& operator<<(std::ostream& out, const AsyncDownloadContext& rhs) {
  out << "{ location: " << rhs.location << ", offset: " << rhs.offset
      << ", size: " << rhs.size << " }";
  return out;
}
...
void OriginCopyer::DeleteExpiredCurveCache(void* arg) {
  ...
  if (taskCopyer->curveOpenTime_.size() > 0) {
    ...
    taskCopyer->timerId_ = taskCopyer->timer_.schedule(&DeleteExpiredCurveCache,
                                                       arg, nextTimespec);
  } else {
...
OriginCopyer::OriginCopyer() : curveClient_(nullptr), s3Client_(nullptr) {}
...
void OriginCopyer::DownloadAsync(DownloadClosure* done) {
  ...
  if (type == OriginType::CurveOrigin) {
    ...
    DownloadFromCurve(fileName, chunkOffset + context->offset, context->size,
                      context->buf, done);
    doneGuard.release();
...
void OriginCopyer::DownloadFromCurve(const string& fileName, off_t off,
  ...
  int fd = 0;
  {
    ...
    if (iter != fdMap_.end()) {
      fd = iter->second;
      auto fdIter =
          std::find_if(curveOpenTime_.cbegin(), curveOpenTime_.cend(),
      ...
      if (fd < 0) {
        LOG(ERROR) << "Open curve file failed."
                   << "file name: " << fileName << " ,return code: " << fd;
        done->SetFailed();
        return;
      }
      fdMap_[fileName] = fd;
      ...
      curveOpenTime_.emplace_back(fd, fileName, now.tv_sec);
      if (timerId_ == bthread::TimerThread::INVALID_TASK_ID) {
        timespec nextTimespec = butil::seconds_from_now(curveFileTimeoutSec_);
        timerId_ =
            timer_.schedule(&DeleteExpiredCurveCache, this, nextTimespec);
      }
    }
  }
