...
int CopysetNodeManager::Init(const CopysetNodeOptions& copysetNodeOptions) {
  ...
  CopysetNode::copysetSyncPool_ = std::make_shared<common::TaskThreadPool<>>();
...
int CopysetNodeManager::ReloadCopysets() {
  ...
  for (; it != items.end(); ++it) {
    ...
    if (copysetLoader_ == nullptr) {
      ...
      copysetLoader_->Enqueue(std::bind(&CopysetNodeManager::LoadCopyset, this,
                                        poolId, copysetId, true));
    }
  }
  ...
  while (retryTimes < copysetNodeOptions_.checkRetryTimes) {
    ...
    if (mayInstallSnapshot) {
      LOG(WARNING) << "Copyset " << ToGroupIdString(logicPoolId, copysetId)
                   ...
                   << "stop checking. "
                   << "fist log index on leader: " << leaderStaus.first_index
                   << ", last log index on current node: " << status.last_index;
      return false;
    }

    // 判断当前副本已经apply的日志是否接近已经committed的日志
    int64_t margin = leaderStaus.committed_index - status.known_applied_index;
    bool catchupLeader = margin < (int64_t)copysetNodeOptions_.finishLoadMargin;
    if (catchupLeader) {
      LOG(INFO) << "Load copyset " << ToGroupIdString(logicPoolId, copysetId)
                << " finished, "
                << "leader CommittedIndex: " << leaderStaus.committed_index
...
bool CopysetNodeManager::CreateCopysetNode(const LogicPoolID& logicPoolId,
  ...
  if (copysetNodeMap_.end() == copysetNodeMap_.find(groupId)) {
    copysetNode = std::make_shared<CopysetNode>(logicPoolId, copysetId, conf);
    ...
    copysetNodeMap_.insert(
        std::pair<GroupId, std::shared_ptr<CopysetNode>>(groupId, copysetNode));
...
int CopysetNodeManager::AddService(brpc::Server* server,
  ...
  do {
    ...
    ret =
        server->AddService(new BRaftCliServiceImpl, brpc::SERVER_OWNS_SERVICE);
    ...
    // add other services
    ret =
        server->AddService(new BRaftCliServiceImpl2, brpc::SERVER_OWNS_SERVICE);
    ...
    ret =
        server->AddService(new ChunkServiceImpl(chunkServiceOptions, epochMap),
                           brpc::SERVER_OWNS_SERVICE);
...
bool CopysetNodeManager::DeleteCopysetNode(const LogicPoolID& logicPoolId,
...

  {
    ...
    if (copysetNodeMap_.end() != it) {
      ...
      ret = true;
      LOG(INFO) << "Delete copyset " << ToGroupIdString(logicPoolId, copysetId)
                << " success.";
    }
  }
...
bool CopysetNodeManager::PurgeCopysetNodeData(const LogicPoolID& logicPoolId,
...

  {
    ...
    if (copysetNodeMap_.end() != it) {
      ...
      LOG(INFO) << "Move copyset" << ToGroupIdString(logicPoolId, copysetId)
                << "to trash success.";
      copysetNodeMap_.erase(it);
      ret = true;
    }
