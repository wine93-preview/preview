...
int CurveSegment::create() {
  if (!_is_open) {
    CHECK(false) << "Create on a closed segment at first_index=" << _first_index
                 << " in " << _path;
    return -1;
  }
  ...
  if (_fd >= 0) {
    ...
    LOG(ERROR) << "Open path: " << path << " fail, error: " << strerror(errno);
    return -1;
  }
...
int CurveSegment::load(braft::ConfigurationManager* configuration_manager) {
  ...
  if (_is_open) {
    ...
    butil::string_appendf(&path, "/" CURVE_SEGMENT_CLOSED_PATTERN, _first_index,
                          _last_index.load());
  }
...
int CurveSegment::_load_entry(off_t offset, EntryHeader* head,
  ...
  if (!verify_checksum(tmp.checksum_type, p, kEntryHeaderSize - 4,
                       header_checksum)) {
  ...
  if (data != NULL) {
    if (buf.length() < kEntryHeaderSize + data_real_len) {
      const size_t to_read = kEntryHeaderSize + data_real_len - buf.length();
      const ssize_t n =
          braft::file_pread(&buf, _fd, offset + buf.length(), to_read);
      if (n != (ssize_t)to_read) {
        return n < 0 ? -1 : 1;
      }
    ...
    if (!verify_checksum(tmp.checksum_type, buf, tmp.data_checksum)) {
      LOG(ERROR) << "Found corrupted data at offset="
                 << offset + kEntryHeaderSize << " header=" << tmp
                 << " path: " << _path;
      return -1;
    }
    data->swap(buf);
...
int CurveSegment::append(const braft::LogEntry* entry) {
  ...
  switch (entry->type) {
    ...
    case braft::ENTRY_TYPE_CONFIGURATION: {
      ...
      if (!status.ok()) {
        LOG(ERROR) << "Fail to serialize ConfigurationPBMeta, path: " << _path;
        return -1;
      }
    } break;
    default:
      LOG(FATAL) << "unknow entry type: " << entry->type << ", path: " << _path;
      return -1;
  }
  ...
  if (to_write % FLAGS_walAlignSize != 0) {
    zero_bytes_num =
        (to_write / FLAGS_walAlignSize + 1) * FLAGS_walAlignSize - to_write;
  }
  ...
  packer.pack32(get_checksum(_checksum_type, write_buf, kEntryHeaderSize - 4));
  if (FLAGS_enableWalDirectWrite) {
  ...
  do {
    ...
    switch (header.type) {
      ...
      case braft::ENTRY_TYPE_CONFIGURATION: {
        butil::Status status = parse_configuration_meta(data, entry);
        if (!status.ok()) {
          LOG(WARNING) << "Fail to parse ConfigurationPBMeta, path: " << _path;
          ok = false;
          break;
        }
      } break;
...
int CurveSegment::close(bool will_sync) {
  ...
  if (ret == 0) {
    ...
    LOG_IF(INFO, rc == 0) << "Renamed `" << old_path << "' to `" << new_path
                          << '\'';
...
int CurveSegment::sync(bool will_sync) {
  if (_last_index > _first_index) {
    // CHECK(_is_open);
    if (!FLAGS_enableWalDirectWrite && braft::FLAGS_raft_sync && will_sync) {
...
int CurveSegment::truncate(const int64_t last_index_kept) {
  ...
  if (!_is_open) {
    ...
    LOG_IF(INFO, ret == 0) << "Renamed `" << old_path << "' to `" << new_path
                           << '\'';
