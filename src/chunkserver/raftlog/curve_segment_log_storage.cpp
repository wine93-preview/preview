...
int CurveSegmentLogStorage::list_segments(bool is_empty) {
  ...
  while (dir_reader.Next()) {
    ...
    if (match == 2) {
      ...
      BraftSegment* segment =
          new BraftSegment(_path, first_index, last_index, _checksum_type);
    ...
    if (match == 1) {
      BRAFT_VLOG << "restore open segment, path: " << _path
                 << " first_index: " << first_index;
      if (!_open_segment) {
        _open_segment = new BraftSegment(_path, first_index, _checksum_type);
        continue;
      } else {
    ...
    if (match == 2) {
      LOG(INFO) << "restore closed segment, path: " << _path
                << " first_index: " << first_index
                << " last_index: " << last_index;
      CurveSegment* segment = new CurveSegment(_path, first_index, last_index,
                                               _checksum_type, _walFilePool);
      _segments[first_index] = segment;
      continue;
    }

    match = sscanf(dir_reader.name(), CURVE_SEGMENT_OPEN_PATTERN, &first_index);
    if (match == 1) {
      BRAFT_VLOG << "restore open segment, path: " << _path
                 << " first_index: " << first_index;
      if (!_open_segment) {
        _open_segment =
            new CurveSegment(_path, first_index, _checksum_type, _walFilePool);
        continue;
      } else {
        LOG(WARNING) << "open segment conflict, path: " << _path
                     << " first_index: " << first_index;
...
int CurveSegmentLogStorage::append_entries(
    const std::vector<braft::LogEntry*>& entries, braft::IOMetric* metric) {
  (void)metric;
  if (entries.empty()) {
    return 0;
  }
...
int CurveSegmentLogStorage::reset(const int64_t next_log_index) {
  ...
  for (SegmentMap::const_iterator it = _segments.begin(); it != _segments.end();
       ++it) {
    popped.push_back(it->second);
  }
  _segments.clear();
...
braft::LogStorage* CurveSegmentLogStorage::new_instance(
  ...
  LogStorageOptions options =
      StoreOptForCurveSegmentLogStorage(LogStorageOptions());

  CHECK(nullptr != options.walFilePool) << "wal file pool is null";

  CurveSegmentLogStorage* logStorage =
      new CurveSegmentLogStorage(uri, true, options.walFilePool);
...
LogStorageStatus CurveSegmentLogStorage::GetStatus() {
  uint32_t count =
      (uint32_t)(_segments.size()) + (nullptr != _open_segment ? 1 : 0);
