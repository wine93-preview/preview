...
int CurveSnapshotAttachMetaTable::add_attach_file(const std::string& filename,
  ...
  LOG_IF(WARNING, !ret.second)
      << "attach file=" << filename << " already exists in snapshot";
...
int CurveSnapshotAttachMetaTable::get_attach_file_meta(
    const std::string& filename, braft::LocalFileMeta* file_meta) const {
...
int CurveSnapshotFileReader::read_file(butil::IOBuf* out,
                                       ...
                                       bool read_partly, size_t* read_count,
                                       bool* is_eof) const {
  ...
  return LocalDirReader::read_file_with_meta(out, filename, &file_meta, offset,
                                             new_max_count, read_count, is_eof);
}
