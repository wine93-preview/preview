...
void RegisterCurveSnapshotStorageOrDie() {
  ...
  braft::snapshot_storage_extension()->RegisterOrDie("curve", &snapshotStorage);
}
...
int CurveSnapshotStorage::init() {
  ...
  if (!_fs->create_directory(_path, &e,
                             braft::FLAGS_raft_create_parent_directories)) {
...
braft::SnapshotWriter* CurveSnapshotStorage::create() { return create(true); }
...
braft::SnapshotCopier* CurveSnapshotStorage::start_to_copy_from(
  ...
  CurveSnapshotCopier* copier = new CurveSnapshotCopier(
      this, _filter_before_copy_remote, _fs.get(), _snapshot_throttle.get());
...
int CurveSnapshotStorage::close(braft::SnapshotWriter* writer_base,
  ...
  CurveSnapshotWriter* writer = dynamic_cast<CurveSnapshotWriter*>(writer_base);
...
braft::SnapshotReader* CurveSnapshotStorage::open() {
  ...
  if (_last_snapshot_index != 0) {
    ...
    butil::string_appendf(&snapshot_path, "/" BRAFT_SNAPSHOT_PATTERN,
                          last_snapshot_index);
    CurveSnapshotReader* reader = new CurveSnapshotReader(
