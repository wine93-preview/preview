...
void BRaftCliServiceImpl::add_peer(RpcController* controller,
  ...
  butil::Status st =
      get_node(&node, logicPoolId, copysetId, request->leader_id());
  if (!st.ok()) {
    cntl->SetFailed(st.error_code(), "%s", st.error_cstr());
    return;
  }
...
void BRaftCliServiceImpl::remove_peer(RpcController* controller,
  ...
  LOG(WARNING) << "Receive RemovePeerRequest to " << node->node_id() << " from "
               << cntl->remote_side() << ", removing " << request->peer_id();
...
butil::Status BRaftCliServiceImpl::get_node(
    scoped_refptr<braft::NodeImpl>* node,  // NOLINT
...
void BRaftCliServiceImpl::transfer_leader(RpcController* controller,
