...
class RaftNode {
 public:
  ...
  virtual braft::NodeId node_id() { return node_->node_id(); }

  virtual PeerId leader_id() { return node_->leader_id(); }

  virtual bool is_leader() { return node_->is_leader(); }

  virtual int init(const braft::NodeOptions& options) {
    return node_->init(options);
  }

  virtual void shutdown(braft::Closure* done) { node_->shutdown(done); }

  virtual void join() { node_->join(); }

  virtual void apply(const braft::Task& task) { node_->apply(task); }
  ...
  virtual void snapshot(braft::Closure* done) { node_->snapshot(done); }
  ...
  virtual void get_status(NodeStatus* status) { node_->get_status(status); }
