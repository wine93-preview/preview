...
void DownloadClosure::Run() {
  ...
  csMetric->OnResponse(request->logicpoolid(), request->copysetid(),
                       CSIOMetricType::DOWNLOAD, downloadCtx_->size, latencyUs,
                       isFailed_);
  ...
  if (isFailed_) {
    ...
    cloneCore_->SetResponse(readRequest_,
  ...
  if (CHUNK_OP_TYPE::CHUNK_OP_RECOVER == request->optype()) {
    ...
    cloneCore_->PasteCloneData(readRequest_, &copyData, downloadCtx_->offset,
                               downloadCtx_->size, doneGuard.release());
...
int CloneCore::CloneReadByLocalInfo(
  ...
  if (offset % pageSize != 0 || length % pageSize != 0) {
    LOG(ERROR) << "Invalid offset or length: "
               << " logic pool id: " << request->logicpoolid()
               << " copyset id: " << request->copysetid()
               << " chunkid: " << request->chunkid() << " offset: " << offset
               << " length: " << length << " page size: " << pageSize;
    SetResponse(readRequest, CHUNK_OP_STATUS::CHUNK_OP_STATUS_INVALID_REQUEST);
    return -1;
  }
  ...
  bool needClone =
      chunkInfo.isClone &&
      (chunkInfo.bitmap->NextClearBit(beginIndex, endIndex) != Bitmap::NO_POS);
  if (needClone) {
...
int CloneCore::HandleReadRequest(std::shared_ptr<ReadChunkRequest> readRequest,
  ...
  switch (errorCode) {
    case CSErrorCode::Success:
      return CloneReadByLocalInfo(readRequest, chunkInfo, doneGuard.release());
...
int CloneCore::ReadChunk(std::shared_ptr<ReadChunkRequest> readRequest) {
  ...
  if (CSErrorCode::Success != errorCode) {
    SetResponse(readRequest, CHUNK_OP_STATUS::CHUNK_OP_STATUS_FAILURE_UNKNOWN);
...
int CloneCore::SetReadChunkResponse(
  ...
  // TODO(yyk) fix it
  bool expect =
      errorCode == CSErrorCode::Success ||
      (errorCode == CSErrorCode::ChunkNotExistError && existCloneInfo(request));
  if (!expect) {
    LOG(ERROR) << "get chunkinfo failed: "
               << " logic pool id: " << request->logicpoolid()
               << " copyset id: " << request->copysetid()
  ...
  for (auto& range : copiedRanges) {
    ...
    if (CSErrorCode::Success != errorCode) {
      LOG(ERROR) << "read chunk failed: "
                 ...
                 << " copyset id: " << request->copysetid()
                 << " chunkid: " << request->chunkid()
                 << " read offset: " << readOff << " read length: " << readSize
                 << " error code: " << errorCode;
      return -1;
    }
  }
...
void CloneCore::PasteCloneData(std::shared_ptr<ReadChunkRequest> readRequest,
  ...
  req = std::make_shared<PasteChunkInternalRequest>(
      readRequest->node_, pasteRequest, pasteResponse, cloneData, pasteClosure);
  req->Process();
}
