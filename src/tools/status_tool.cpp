...
bool StatusTool::SupportCommand(const std::string& command) {
  return (command == kSpaceCmd || command == kStatusCmd ||
          command == kChunkserverListCmd || command == kChunkserverStatusCmd ||
          command == kMdsStatusCmd || command == kEtcdStatusCmd ||
          command == kClientStatusCmd || command == kClientListCmd ||
          command == kSnapshotCloneStatusCmd || command == kClusterStatusCmd ||
          command == kServerListCmd || command == kLogicalPoolList ||
...
int StatusTool::SpaceCmd() {
  ...
  if (spaceInfo.totalChunkSize != 0) {
    physicalUsedRatio =
        static_cast<double>(spaceInfo.usedChunkSize) / spaceInfo.totalChunkSize;
  }
  ...
  if (spaceInfo.totalCapacity != 0) {
    logicalUsedRatio =
        static_cast<double>(spaceInfo.allocatedSize) / spaceInfo.totalCapacity;
    logicalLeftRatio =
        static_cast<double>(spaceInfo.totalCapacity - spaceInfo.allocatedSize) /
  ...
  std::cout << "logical: total = " << spaceInfo.totalCapacity / mds::kGB << "GB"
            << ", used = " << spaceInfo.allocatedSize / mds::kGB << "GB"
            << "(" << logicalUsedRatio * 100
            << "%, can be recycled = " << spaceInfo.recycleAllocSize / mds::kGB
            << "GB(" << canBeRecycledRatio * 100 << "%))"
            << ", left = "
            << (spaceInfo.totalCapacity - spaceInfo.allocatedSize) / mds::kGB
            << "GB(" << logicalLeftRatio * 100 << "%)"
            << ", created file size = " << spaceInfo.currentFileSize / mds::kGB
  ...
  for (const auto& i : spaceInfo.lpoolspaceinfo) {
    std::cout << "logicalPool: name = " << i.second.poolName
              ...
              << ", left = "
              << (i.second.totalCapacity - i.second.allocatedSize) / mds::kGB
              << "GB" << std::endl;
  }
  return 0;
}
...
int StatusTool::ChunkServerListCmd() {
  ...
  for (auto& chunkserver : chunkservers) {
    ...
    if (ret != 0) {
      std::cout << "GetCopySetsInChunkServer fail, chunkserver id = " << csId;
      return -1;
    }
    ...
    std::cout << "chunkServerID = " << csId
              ...
              << ", rwStatus = " << ChunkServerStatus_Name(chunkserver.status())
              << ", diskState = " << DiskState_Name(chunkserver.diskstatus())
              << ", onlineState = "
...
int StatusTool::LogicalPoolListCmd() {
  ...
  for (auto& lgPool : lgPools) {
    ...
    std::cout << "id = " << lgPool.logicalpoolid()
              ...
              << ", physicalPoolID = " << lgPool.physicalpoolid() << ", type = "
              ...
              << "GB"
              << "(" << recycleRatio * 100 << "%))"
              << ", left space = " << (totalSize - usedSize) / curve::mds::kGB
...
bool StatusTool::CheckServiceHealthy(const ServiceName& name) {
  ...
  switch (name) {
    ...
    case ServiceName::kEtcd: {
      int res = etcdClient_->GetEtcdClusterStatus(&leaderVec, &onlineStatus);
      if (res != 0) {
...
int StatusTool::PrintClientStatus() {
  ...
  for (const auto& item : versionMap) {
    ...
    for (const auto& item2 : item.second) {
      ...
      std::cout << "version-" << item2.first << ": " << item2.second.size();
      first = false;
    }
    std::cout << std::endl;
    if (FLAGS_detail) {
...
int CheckUseWalPool(
  ...
  if (!poolChunkservers.empty()) {
    ...
    MetricRet res =
        metricClient->GetConfValueFromMetric(csAddr, metricName, &metricValue);
    ...
    *useWalPool = kProtocalCurve == raftLogProtocol ? true : false;
    ...
    // check whether use chunkfilepool as walpool from chunkserver conf metric
    // // NOLINT
...
int PrintChunkserverOnlineStatus(
  ...
  if (offlineCs.size() > 0) {
    ...
    int res = mdsClient->QueryChunkServerRecoverStatus(offlineCs, &statusMap);
    if (res != 0) {
...
int GetChunkserverLeftSize(
  ...
  for (const auto& poolChunkserver : poolChunkservers) {
    ...
    for (const auto& chunkserver : poolChunkserver.second) {
      ...
      if (res != MetricRet::kOK) {
        std::cout << "Get left chunk size of chunkserver " << csAddr << " fail!"
                  << std::endl;
        ret = -1;
        continue;
      }
      ...
      if (useWalPool && !useChunkFilePoolAsWalPool) {
        ...
        res = metricClient->GetMetricUint(csAddr, metricName, &walSegmentNum);
        if (res != MetricRet::kOK) {
    ...
    poolWalSegmentLeftSize->emplace(poolChunkserver.first, walSegmentLeftSize);
  }
  return ret;
}
...
int StatusTool::PrintChunkserverStatus(bool checkLeftSize) {
  ...
  std::string version;
  std::vector<std::string> failedList;
  int res = versionTool_->GetAndCheckChunkServerVersion(&version, &failedList);
  int ret = 0;
  if (res != 0) {
...
int StatusTool::GetSpaceInfo(SpaceInfo* spaceInfo) {
  ...
  for (const auto& lgPool : lgPools) {
    ...
    spaceInfo->lpoolspaceinfo.insert(std::pair<uint32_t, LogicalpoolSpaceInfo>(
