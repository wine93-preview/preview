...
bool NameSpaceTool::SupportCommand(const std::string& command) {
  return (command == kGetCmd || command == kListCmd || command == kSegInfoCmd ||
          command == kDeleteCmd || command == kCreateCmd ||
          command == kExtendCmd || command == kCleanRecycleCmd ||
          command == kChunkLocatitonCmd || command == kUpdateThrottle);
}
...
int NameSpaceTool::RunCommand(const std::string& cmd) {
  ...
  if (cmd == kGetCmd) {
    ...
    return core_->CreateFile(name, FLAGS_fileLength * mds::kGB, normalFile,
    ...
    return core_->UpdateFileThrottle(fileName, FLAGS_throttleType, FLAGS_limit,
                                     FLAGS_burst, FLAGS_burstLength);
  } else {
...
void NameSpaceTool::PrintHelp(const std::string& cmd) {
  ...
  if (cmd == kGetCmd || cmd == kListCmd) {
    ...
    std::cout << "If -fileName is specified, delete the files in recyclebin "
                 "that the original directory is fileName"
              << std::endl;  // NOLINT
    std::cout
    ...
    std::cout << "curve_ops_tool " << cmd
              << " -fileName=/test -userName=test -password=123 -fileLength=20 "
                 "[-stripeUnit=32768] [-stripeCount=32]  "
                 "[-mdsAddr=127.0.0.1:6666] [-confPath=/etc/curve/tools.conf]"
              << std::endl;  // NOLINT
    std::cout << "curve_ops_tool " << cmd
              << " -dirName=/dir -userName=test -password=123 "
                 "[-mdsAddr=127.0.0.1:6666] [-confPath=/etc/curve/tools.conf]"
              << std::endl;  // NOLINT
    std::cout << "The first example can create a volume and the second create "
                 "a directory."
              << std::endl;  // NOLINT
  } else if (cmd == kExtendCmd) {
    std::cout << "curve_ops_tool " << cmd
    ...
    std::cout
        << "curve_ops_tool " << cmd
        << " -fileName=/test -userName=test -password=123 -forcedelete=true  "
           "[-mdsAddr=127.0.0.1:6666] [-confPath=/etc/curve/tools.conf]"
        << std::endl;  // NOLINT
  } else if (cmd == kChunkLocatitonCmd) {
    std::cout << "curve_ops_tool " << cmd
...
int NameSpaceTool::PrintChunkLocation(const std::string& fileName,
  ...
  int res = core_->GetChunkServerListInCopySet(logicPoolId, copysetId, &csLocs);
  if (res != 0) {
  ...
  for (uint64_t i = 0; i < csLocs.size(); ++i) {
    ...
    std::cout << location.hostip() << ":" << std::to_string(location.port());
  }
  std::cout << "}" << std::endl;
  return 0;
}
