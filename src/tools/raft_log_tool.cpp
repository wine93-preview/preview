...
int SegmentParser::Init(const std::string& fileName) {
  ...
  if (fd_ < 0) {
    std::cout << "Fail to open " << fileName << ", " << berror() << std::endl;
    return -1;
  }
...
bool SegmentParser::GetNextEntryHeader(EntryHeader* head) {
  ...
  if (n != (ssize_t)ENTRY_HEADER_SIZE) {
    if (n < 0) {
      std::cout << "read header from file, fd: " << fd_ << ", offset: " << off_
                << ", " << berror() << std::endl;
    } else {
      std::cout << "Read size not match, header size: " << ENTRY_HEADER_SIZE
                << ", read size: " << n << std::endl;
    }
    return false;
  }
