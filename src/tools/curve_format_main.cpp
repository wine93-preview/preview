...
int AllocateFiles(AllocateStruct* allocatestruct) {
  ...
  while (count < allocatestruct->chunknum) {
    ...
    std::string tmpchunkfilepath =
        FLAGS_filePoolDir + "/" + filename + allocatestruct->cleanChunkSuffix;
...
int main(int argc, char** argv) {
  ...
  if (FLAGS_allocateByPercent) {
    preAllocateChunkNum =
        preAllocateSize / (FLAGS_fileSize + FLAGS_metaPagSize);
  } else {
  ...
  int ret = curve::chunkserver::FilePoolHelper::PersistEnCodeMetaInfo(
      fsptr, FLAGS_fileSize, FLAGS_metaPagSize, FLAGS_filePoolDir,
  ...
  ret = curve::chunkserver::FilePoolHelper::DecodeMetaInfoFromMetaFile(
      fsptr, FLAGS_filePoolMetaPath, 4096, &chunksize, &metapagesize,
      &chunkfilePath);
  if (ret == -1) {
  ...
  do {
    ...
    if (strcmp(chunkfilePath.c_str(), FLAGS_filePoolDir.c_str()) != 0) {
