...
class MDSClient {
 public:
   ...
   *  @return 成功返回0，失败返回-1
   */
  virtual int CreateFile(const std::string& fileName, uint64_t length = 0,
   ...
   *  @return 成功返回0，失败返回-1
   */
  virtual int ListPhysicalPoolsInCluster(std::vector<PhysicalPoolInfo>* pools);
  ...
  virtual const std::map<std::string, std::string>& GetDummyServerMap() const {
