...
int NameSpaceToolCore::GetChunkServerListInCopySet(
  ...
  return client_->GetChunkServerListInCopySet(logicalPoolId, copysetId, csLocs);
}
...
int NameSpaceToolCore::CreateFile(const std::string& fileName, uint64_t length,
                                  bool normalFile, uint64_t stripeUnit,
                                  uint64_t stripeCount) {
  return client_->CreateFile(fileName, length, normalFile, stripeUnit,
                             stripeCount);
}
...
int NameSpaceToolCore::UpdateFileThrottle(const std::string& fileName,
  ...
  if (!curve::mds::ThrottleType_Parse(throttleType, &type)) {
    std::cout << "Parse throttle type '" << throttleType
              << "' failed, only support "
...
int NameSpaceToolCore::QueryChunkCopyset(
  ...
  PageFileSegment segment;
  GetSegmentRes segRes = client_->GetSegmentInfo(fileName, segOffset, &segment);
