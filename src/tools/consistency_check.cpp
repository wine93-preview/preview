...
std::ostream& operator<<(std::ostream& os, const CopySet& copyset) {
  os << "logicPoolId = " << copyset.first << ",copysetId = " << copyset.second;
  return os;
}
...
std::ostream& operator<<(std::ostream& os, const CsAddrsType& csAddrs) {
  ...
  for (uint32_t i = 0; i < csAddrs.size(); ++i) {
    std::string ip;
    uint32_t port;
    if (curve::common::NetCommon::SplitAddrToIpPort(csAddrs[i], &ip, &port)) {
...
int ConsistencyCheck::CheckFileConsistency(const std::string& fileName,
  ...
  if (res != 0) {
    std::cout << "FetchFileCopyset of " << fileName << " fail!" << std::endl;
    return -1;
  }
...
void ConsistencyCheck::PrintHelp(const std::string& cmd) {
  ...
  std::cout
      << "curve_ops_tool check-consistency -filename=/test [-check_hash=false]"
      << std::endl;  // NOLINT
}
...
int ConsistencyCheck::GetCopysetStatusResponse(
  ...
  if (res != 0) {
    std::cout << "GetCopysetStatus from " << csAddr << " fail!" << std::endl;
    return -1;
  }
  return 0;
}
