...
CheckResult CopysetCheckCore::CheckOneCopyset(const PoolIdType& logicalPoolId,
  ...
  for (const auto& csl : chunkserverLocation) {
    ...
    if (copysetInfos.empty()) {
      std::cout << "copyset not found on chunkserver " << csAddr << std::endl;
...
int CopysetCheckCore::CheckIfChunkServerInCopysets(
  ...
  for (const auto& info : csServerInfos) {
    ...
    for (const auto& csLoc : info.cslocs()) {
      std::string addr = csLoc.hostip() + ":" + std::to_string(csLoc.port());
      if (addr == csAddr) {
        (*result)[groupId] = true;
        break;
      }
...
int CopysetCheckCore::CheckCopysetsWithMds() {
  ...
  for (const auto& copyset : copysetsInMds) {
    std::string gId = ToGroupId(copyset.logicalpoolid(), copyset.copysetid());
  ...
  iter = std::set_difference(copysets_[kTotal].begin(), copysets_[kTotal].end(),
...
int CopysetCheckCore::QueryChunkServer(const std::string& chunkserverAddr,
  ...
  auto csClient = (csClient_ == nullptr) ? std::make_shared<ChunkServerClient>()
                                         : csClient_;
  ...
  for (uint32_t i = 0; i < peers.size() - 1; ++i) {
    ...
    for (auto info : repInfos) {
      ...
      if (info.substr(0, pos) == kNextIndex) {
        res = curve::common::StringToUll(info.substr(pos + 1), &nextIndex);
        if (!res) {
...
void CopysetCheckCore::UpdatePeerNotOnlineCopysets(const std::string& csAddr) {
  ...
  if (res != 0) {
    std::cout << "GetCopySetsInChunkServer " << csAddr << " fail!" << std::endl;
    return;
