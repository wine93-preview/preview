...
#define CHECK_FLAG(flagname)                                              \
  do {                                                                    \
    if ((FLAGS_##flagname).empty()) {                                     \
      std::cout << __FUNCTION__ << " requires --" #flagname << std::endl; \
      return -1;                                                          \
    }                                                                     \
  } while (0);
...
int CurveCli::ResetPeer() {
  ...
  if (!st.ok()) {
    std::cout << "Reset peer of copyset "
              << "(" << FLAGS_logicalPoolId << ", " << FLAGS_copysetId << ")"
              << " to " << newConf << " fail, requestPeer: " << requestPeerId
              << ", detail: " << st << std::endl;
    return -1;
  }
  std::cout << "Reset peer of copyset "
...
int CurveCli::DoSnapshot(uint32_t lgPoolId, uint32_t copysetId,
  ...
  if (!st.ok()) {
    std::cout << "Do snapshot of copyset "
              << "(" << FLAGS_logicalPoolId << ", " << FLAGS_copysetId << ")"
              << " fail, requestPeer: " << peer.address() << ", detail: " << st
              << std::endl;
    return -1;
  }
  return 0;
...
void CurveCli::PrintHelp(const std::string& cmd) {
  ...
  if (cmd == kResetPeerCmd) {
    std::cout
        << "curve_ops_tool " << cmd
        << " -logicalPoolId=1 -copysetId=10001 -peer=127.0.0.1:8080:0 "  // NOLINT
