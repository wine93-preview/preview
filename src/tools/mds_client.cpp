...
int MDSClient::CreateFile(const std::string& fileName, uint64_t length,
  ...
  std::cout << "CreateFile fail with errCode: " << response.statuscode()
            << std::endl;
  return -1;
}
...
int MDSClient::GetChunkServerListInCopySet(
  ...
  int res =
      GetChunkServerListInCopySets(logicalPoolId, {copysetId}, &csServerInfos);
  if (res != 0) {
...
int MDSClient::GetChunkServerListInCopySets(
  ...
  if (SendRpcToMds(&request, &response, &stub, fp) != 0) {
    std::cout << "GetChunkServerListInCopySets from all mds fail!" << std::endl;
    return -1;
  }
...
int MDSClient::ListLogicalPoolsInCluster(std::vector<LogicalPoolInfo>* pools) {
  ...
  for (const auto& phyPool : phyPools) {
    ...
    ret = ListLogicalPoolsInPhysicalPool(phyPool.physicalpoolid(), &lgPools);
    if (ret != 0) {
      std::cout << "ListLogicalPoolsInPhysicalPool " << phyPool.physicalpoolid()
                << " fail" << std::endl;
      return -1;
    }
...
int MDSClient::ListChunkServersInCluster(
  ...
  for (const auto& server : servers) {
    ...
    if (ListChunkServersOnServer(server.serverid(), &chunkserverList) != 0) {
      std::cout << "ListChunkServersOnServer fail!" << std::endl;
      return -1;
    }
...
int MDSClient::GetMetric(const std::string& metricName, std::string* value) {
  ...
  std::cout << "GetMetric " << metricName << " from all mds fail!" << std::endl;
  return -1;
}
...
int MDSClient::UpdateFileThrottleParams(
  ...
  std::cout << "UpdateFileThrottleParams fail with errCode: "
            << curve::mds::StatusCode_Name(response.statuscode()) << std::endl;
  return -1;
}
...
int MDSClient::SendRpcToMds(Request* request, Response* response, T* obp,
  ...
  while (changeTimeLeft >= 0) {
    ...
    if (needRetry && cntl.ErrorCode() != brpc::ERPCTIMEDOUT) {
      std::cout << "Send RPC to mds fail, error content: " << cntl.ErrorText()
                << std::endl;
      return -1;
    }
    changeTimeLeft--;
...
void MDSClient::FillUserInfo(T* request) {
  ...
  if (userName_ == rootUserName && !password_.empty()) {
    std::string str2sig = Authenticator::GetString2Signature(date, userName_);
    std::string sig = Authenticator::CalcString2Signature(str2sig, password_);
