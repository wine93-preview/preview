...
class CloneCore {
 public:
  ...
  virtual int CloneOrRecoverPre(const UUID& source, const std::string& user,
                                ...
                                CloneTaskType taskType, CloneInfo* info) = 0;
...
class CloneCoreImpl : public CloneCore {
 ...
 public:
  ...
  int CloneOrRecoverPre(const UUID& source, const std::string& user,
                        const std::string& destination, bool lazyFlag,
                        CloneTaskType taskType, CloneInfo* info) override;
 ...
 private:
   ...
   * @return 错误码
   */
  int CreateOrUpdateCloneMeta(std::shared_ptr<CloneTaskInfo> task, FInfo* fInfo,
   ...
   * @return 错误码
   */
  int CreateCloneFile(std::shared_ptr<CloneTaskInfo> task, const FInfo& fInfo);
   ...
   * @return 错误码
   */
  int CreateCloneChunk(std::shared_ptr<CloneTaskInfo> task, const FInfo& fInfo,
                       CloneSegmentMap* segInfos);
   ...
   * @return 错误码
   */
  int CompleteCloneMeta(std::shared_ptr<CloneTaskInfo> task, const FInfo& fInfo,
   ...
   * @return 错误码
   */
  int RenameCloneFile(std::shared_ptr<CloneTaskInfo> task, const FInfo& fInfo);
   ...
   * @return 错误码
   */
  int CompleteCloneFile(std::shared_ptr<CloneTaskInfo> task, const FInfo& fInfo,
                        const CloneSegmentMap& segInfos);
