...
class TaskCloneInfo {
 public:
  ...
  void LoadFromJsonObj(const Json::Value& jsonObj) {
    ...
    info.SetTaskType(static_cast<CloneTaskType>(jsonObj["TaskType"].asInt()));
    ...
    info.SetFileType(static_cast<CloneFileType>(jsonObj["FileType"].asInt()));
    SetCloneInfo(info);
  }
...
class CloneServiceManagerBackendImpl : public CloneServiceManagerBackend {
 public:
  explicit CloneServiceManagerBackendImpl(std::shared_ptr<CloneCore> cloneCore)
...
class CloneServiceManager {
 public:
   ...
   * @return 错误码
   */
  virtual int CleanCloneTask(const std::string& user, const TaskIdType& taskId);
 ...
 private:
   ...
   * @return 错误码
   */
  int BuildAndPushCloneOrRecoverLazyTask(CloneInfo cloneInfo,
                                         std::shared_ptr<CloneClosure> closure);
