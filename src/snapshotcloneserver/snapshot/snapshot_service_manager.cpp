...
int SnapshotServiceManager::CreateSnapshot(const std::string& file,
  ...
  *uuid = snapInfo.GetUuid();
  ...
  std::shared_ptr<SnapshotCreateTask> task =
      std::make_shared<SnapshotCreateTask>(snapInfo.GetUuid(), taskInfo, core_);
...
int SnapshotServiceManager::DeleteSnapshot(const UUID& uuid,
  ...
  std::shared_ptr<SnapshotDeleteTask> task =
      std::make_shared<SnapshotDeleteTask>(snapInfo.GetUuid(), taskInfo, core_);
  ret = taskMgr_->PushTask(task);
  if (ret < 0) {
    LOG(ERROR) << "Push Task error, "
               << " ret = " << ret;
    ...
    if (snap.GetUser() == user) {
      ...
      switch (st) {
        ...
        case Status::pending: {
          ...
          std::shared_ptr<SnapshotTask> task = taskMgr_->GetTask(uuid);
          if (task != nullptr) {
            info->emplace_back(snap, task->GetTaskInfo()->GetProgress());
          } else {
            // 刚刚完成
  ...
  for (auto& snap : list) {
    ...
    switch (st) {
      case Status::pending: {
        ...
        taskInfo->UpdateMetric();
        std::shared_ptr<SnapshotCreateTask> task =
            std::make_shared<SnapshotCreateTask>(snap.GetUuid(), taskInfo,
                                                 core_);
        ret = taskMgr_->PushTask(task);
        if (ret < 0) {
          LOG(ERROR) << "RecoverSnapshotTask push task error, ret = " << ret
                     << ", uuid = " << snap.GetUuid();
          return ret;
        }
        break;
      ...
      case Status::errorDeleting: {
        ...
        taskInfo->UpdateMetric();
        std::shared_ptr<SnapshotDeleteTask> task =
            std::make_shared<SnapshotDeleteTask>(snap.GetUuid(), taskInfo,
                                                 core_);
        ret = taskMgr_->PushTask(task);
        if (ret < 0) {
          LOG(ERROR) << "RecoverSnapshotTask push task error, ret = " << ret
