...
*/
int S3SnapshotDataStore::DataChunkTranferInit(
...
int S3SnapshotDataStore::DataChunkTranferAddPart(
  ...
  Aws::S3::Model::CompletedPart cp = s3Adapter4Data_->UploadOnePart(
...
int S3SnapshotDataStore::DataChunkTranferComplete(
    const ChunkDataName& name, std::shared_ptr<TransferTask> task) {
  std::string key = name.ToDataChunkKey();
  const Aws::String aws_key(key.c_str(), key.size());
  const Aws::String uploadId(task->uploadId_.c_str(), task->uploadId_.size());
  ...
  for (auto& v : task->GetPartInfo()) {
    ...
    cp_v.push_back(
        Aws::S3::Model::CompletedPart().WithETag(str).WithPartNumber(v.first));
  }
  return s3Adapter4Data_->CompleteMultiUpload(aws_key, uploadId, cp_v);
}

int S3SnapshotDataStore::DataChunkTranferAbort(
    const ChunkDataName& name, std::shared_ptr<TransferTask> task) {
  std::string key = name.ToDataChunkKey();
  const Aws::String aws_key(key.c_str(), key.size());
  const Aws::String uploadId(task->uploadId_.c_str(), task->uploadId_.size());
