...
void SnapshotCoreImpl::HandleCreateSnapshotTask(
  ...
  if (task->IsCanceled()) {
    return CancelAfterTransferSnapshotData(task, indexData, fileSnapshotMap);
  }
...
int SnapshotCoreImpl::CreateSnapshotOnCurvefs(
  ...
  std::this_thread::sleep_for(std::chrono::microseconds(mdsSessionTimeUs_ * 2));
...
int SnapshotCoreImpl::DeleteSnapshotOnCurvefs(const SnapshotInfo& info) {
  ...
  do {
    ...
    ret = client_->CheckSnapShotStatus(info.GetFileName(), info.GetUser(),
                                       seqNum, &status);
    LOG(INFO) << "Doing CheckSnapShotStatus, fileName = " << info.GetFileName()
              << ", user = " << info.GetUser() << ", seqNum = " << seqNum
...
int SnapshotCoreImpl::BuildChunkIndexData(
  ...
  for (uint64_t i = 0; i < fileLength / segmentSize; i++) {
    ...
    SegmentInfo segInfo;
    int ret = client_->GetSnapshotSegmentInfo(fileName, user, seqNum, offset,
                                              &segInfo);

    if (LIBCURVE_ERROR::OK == ret) {
      segInfos->emplace(i, segInfo);
      for (std::vector<uint64_t>::size_type j = 0; j < segInfo.chunkvec.size();
           j++) {
        ...
        if (chunkInfo.chunkSn.size() == 2) {
          uint64_t seq = std::min(chunkInfo.chunkSn[0], chunkInfo.chunkSn[1]);
      ...
      LOG(ERROR) << "GetSnapshotSegmentInfo error,"
                 ...
                 << ", offset = " << offset << ", uuid = " << task->GetUuid();
      return kErrCodeInternalError;
    }
  }
...
int SnapshotCoreImpl::BuildSegmentInfo(
  ...
  for (uint64_t i = 0; i < fileLength / segmentSize; i++) {
    ...
    SegmentInfo segInfo;
    ret =
        client_->GetSnapshotSegmentInfo(fileName, user, seq, offset, &segInfo);
    ...
    if (LIBCURVE_ERROR::OK == ret) {
      ...
      LOG(ERROR) << "GetSnapshotSegmentInfo error,"
                 << " ret = " << ret << ", fileName = " << fileName
                 << ", user = " << user << ", seq = " << seq
                 << ", offset = " << offset << ", uuid = " << info.GetUuid();
      return kErrCodeInternalError;
    }
  }
  return kErrCodeSuccess;
...
int SnapshotCoreImpl::TransferSnapshotData(
  ...
  double progressPerData = static_cast<double>(totalProgress) / transferDataNum;
  uint32_t index = 0;
  ...
  for (auto& chunkIndex : chunkIndexVec) {
    ...
    if (it == segInfos.end()) {
      LOG(ERROR) << "TransferSnapshotData has encounter an interanl error"
                 ...
                 << " chunkIndex = " << chunkIndex << ", segNum = " << segNum
                 << ", uuid = " << task->GetUuid();
      return kErrCodeInternalError;
    }
  ...
  for (auto& chunkIndex : chunkIndexVec) {
    ...
    if (it != segInfos.end()) {
      ...
      if (!filter(chunkDataName)) {
        auto taskInfo = std::make_shared<TransferSnapshotDataChunkTaskInfo>(
            chunkDataName, chunkSize, cidInfo, chunkSplitSize_,
            clientAsyncMethodRetryTimeSec_, clientAsyncMethodRetryIntervalMs_,
            readChunkSnapshotConcurrency_);
        UUID taskId = UUIDGenerator().GenerateUUID();
        auto task = new TransferSnapshotDataChunkTask(taskId, taskInfo, client_,
                                                      dataStore_);
    ...
    task->SetProgress(static_cast<uint32_t>(kProgressTransferSnapshotDataStart +
...
void SnapshotCoreImpl::HandleDeleteSnapshotTask(
  ...
  if (dataStore_->ChunkIndexDataExist(name)) {
    ...
    uint32_t totalProgress =
        kDelProgressDeleteChunkDataComplete - kDelProgressDeleteChunkDataStart;
    uint32_t chunkDataNum = chunkIndexVec.size();
    double progressPerData = static_cast<double>(totalProgress) / chunkDataNum;
    uint32_t index = 0;
    ...
    for (auto& chunkIndex : chunkIndexVec) {
      ...
      task->SetProgress(static_cast<uint32_t>(kDelProgressDeleteChunkDataStart +
                                              index * progressPerData));
      task->UpdateMetric();
      index++;
    }
...
int SnapshotCoreImpl::BuildSnapshotMap(const std::string& fileName,
  ...
  for (auto& snap : snapInfos) {
    if (snap.GetSeqNum() != seqNum && snap.GetSeqNum() != kUnInitializeSeqNum) {
      ChunkIndexDataName name(snap.GetFileName(), snap.GetSeqNum());
      ChunkIndexData indexData;
      ret = dataStore_->GetChunkIndexData(name, &indexData);
      if (ret < 0) {
        LOG(ERROR) << "GetChunkIndexData error, "
                   << " ret = " << ret << ", fileName = " << snap.GetFileName()
