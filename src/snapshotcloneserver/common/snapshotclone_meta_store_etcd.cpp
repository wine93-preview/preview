...
int SnapshotCloneMetaStoreEtcd::GetSnapshotInfo(const UUID& uuid,
                                                SnapshotInfo* info) {
...
int SnapshotCloneMetaStoreEtcd::AddCloneInfo(const CloneInfo& info) {
  ...
  if (errCode != EtcdErrCode::EtcdOK) {
    LOG(ERROR) << "Put cloneInfo into etcd err"
               << ", errcode = " << errCode << ", cloneInfo : " << info;
    return -1;
  }
...
int SnapshotCloneMetaStoreEtcd::GetCloneInfo(const std::string& uuid,
                                             CloneInfo* info) {
