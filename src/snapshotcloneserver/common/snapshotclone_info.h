...
class CloneInfo {
 public:
  ...
  CloneInfo(const TaskIdType& taskId, const std::string& user,
            ...
            const std::string& destination, CloneFileType fileType, bool isLazy)
      : taskId_(taskId),
        user_(user),
        type_(type),
        source_(source),
  ...
  CloneInfo(const TaskIdType& taskId, const std::string& user,
            ...
            uint64_t destinationId, uint64_t time, CloneFileType fileType,
            bool isLazy, CloneStep nextStep, CloneStatus status)
      : taskId_(taskId),
        user_(user),
        type_(type),
        source_(source),
...
class SnapshotInfo {
 public:
  ...
  SnapshotInfo(UUID uuid, const std::string& user, const std::string& fileName,
      ...
      : uuid_(uuid),
        ...
        status_(Status::pending) {}
  SnapshotInfo(UUID uuid, const std::string& user, const std::string& fileName,
               const std::string& desc, uint64_t seqnum, uint32_t chunksize,
               uint64_t segmentsize, uint64_t filelength, uint64_t stripeUnit,
               uint64_t stripeCount, uint64_t time, Status status)
      : uuid_(uuid),
        user_(user),
        fileName_(fileName),
        snapshotName_(desc),
