...
class RetryHelper {
 public:
  RetryHelper(const RetryMethod& retryMethod, const RetryCondition& condition) {
...
class CurveFsClient {
 public:
   ...
   * @return 错误码
   */
  virtual int GetSnapshot(const std::string& filename, const std::string& user,
  ...
  virtual int GetSnapshotSegmentInfo(const std::string& filename,
                                     ...
                                     uint64_t offset, SegmentInfo* segInfo) = 0;
   ...
   * @return 错误码
   */
  virtual int GetFileInfo(const std::string& filename, const std::string& user,
                          FInfo* fileInfo) = 0;
...
class CurveFsClientImpl : public CurveFsClient {
 public:
  ...
  int CheckSnapShotStatus(std::string filename, std::string user, uint64_t seq,
  ...
  int CreateCloneFile(const std::string& source, const std::string& filename,
                      ...
                      uint64_t stripeCount, FInfo* fileInfo) override;
  ...
  int GetOrAllocateSegmentInfo(bool allocate, uint64_t offset, FInfo* fileInfo,
