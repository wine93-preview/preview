...
int CurveFsClientImpl::CreateSnapshot(const std::string& filename,
  ...
  RetryMethod method = [this, &filename, &userInfo, seq]() {
    return snapClient_->CreateSnapShot(filename, userInfo, seq);
  };
...
int CurveFsClientImpl::DeleteSnapshot(const std::string& filename,
  ...
  RetryCondition condition = [](int ret) {
    return ret != LIBCURVE_ERROR::OK && ret != -LIBCURVE_ERROR::NOTEXIST &&
...
int CurveFsClientImpl::GetSnapshot(const std::string& filename,
                                   const std::string& user, uint64_t seq,
                                   FInfo* snapInfo) {
  UserInfo userInfo = GetUserInfo(user);
  RetryMethod method = [this, &filename, &userInfo, seq, snapInfo]() {
    return snapClient_->GetSnapShot(filename, userInfo, seq, snapInfo);
  };
  RetryCondition condition = [](int ret) { return ret != LIBCURVE_ERROR::OK; };
  RetryHelper retryHelper(method, condition);
  return retryHelper.RetryTimeSecAndReturn(clientMethodRetryTimeSec_,
                                           clientMethodRetryIntervalMs_);
}
...
int CurveFsClientImpl::GetSnapshotSegmentInfo(const std::string& filename,
  ...
  RetryMethod method = [this, &filename, &userInfo, seq, offset, segInfo]() {
    return snapClient_->GetSnapshotSegmentInfo(filename, userInfo, seq, offset,
                                               segInfo);
  };
  RetryCondition condition = [](int ret) {
    return ret != LIBCURVE_ERROR::OK && ret != -LIBCURVE_ERROR::NOT_ALLOCATE;
  };
  RetryHelper retryHelper(method, condition);
  return retryHelper.RetryTimeSecAndReturn(clientMethodRetryTimeSec_,
                                           clientMethodRetryIntervalMs_);
...
int CurveFsClientImpl::ReadChunkSnapshot(ChunkIDInfo cidinfo, uint64_t seq,
                                         uint64_t offset, uint64_t len,
                                         char* buf, SnapCloneClosure* scc) {
  RetryMethod method = [this, &cidinfo, seq, offset, len, buf, scc]() {
    return snapClient_->ReadChunkSnapshot(cidinfo, seq, offset, len, buf, scc);
  };
  RetryCondition condition = [](int ret) { return ret < 0; };
  RetryHelper retryHelper(method, condition);
  return retryHelper.RetryTimeSecAndReturn(clientMethodRetryTimeSec_,
                                           clientMethodRetryIntervalMs_);
}
...
int CurveFsClientImpl::CheckSnapShotStatus(std::string filename,
  ...
  RetryMethod method = [this, &filename, &userInfo, seq, filestatus]() {
    return snapClient_->CheckSnapShotStatus(filename, userInfo, seq,
                                            filestatus);
  };
  RetryCondition condition = [](int ret) {
    return ret != LIBCURVE_ERROR::OK && ret != -LIBCURVE_ERROR::NOTEXIST &&
...
int CurveFsClientImpl::GetChunkInfo(const ChunkIDInfo& cidinfo,
...
int CurveFsClientImpl::CreateCloneFile(const std::string& source,
  ...
  UserInfo userInfo = GetUserInfo(user);
  RetryMethod method = [this, &source, &filename, userInfo, size, sn, chunkSize,
                        stripeUnit, stripeCount, fileInfo]() {
    return snapClient_->CreateCloneFile(source, filename, userInfo, size, sn,
                                        chunkSize, stripeUnit, stripeCount,
                                        fileInfo);
  };
  RetryCondition condition = [](int ret) {
    return ret != LIBCURVE_ERROR::OK && ret != -LIBCURVE_ERROR::EXISTS;
  };
  RetryHelper retryHelper(method, condition);
  return retryHelper.RetryTimeSecAndReturn(clientMethodRetryTimeSec_,
                                           clientMethodRetryIntervalMs_);
...
int CurveFsClientImpl::CreateCloneChunk(const std::string& location,
                                        const ChunkIDInfo& chunkidinfo,
                                        uint64_t sn, uint64_t csn,
                                        uint64_t chunkSize,
                                        SnapCloneClosure* scc) {
  RetryMethod method = [this, &location, &chunkidinfo, sn, csn, chunkSize,
                        scc]() {
    return snapClient_->CreateCloneChunk(location, chunkidinfo, sn, csn,
                                         chunkSize, scc);
  };
  RetryCondition condition = [](int ret) { return ret < 0; };
  RetryHelper retryHelper(method, condition);
...
int CurveFsClientImpl::RecoverChunk(const ChunkIDInfo& chunkidinfo,
                                    uint64_t offset, uint64_t len,
                                    SnapCloneClosure* scc) {
...
int CurveFsClientImpl::CompleteCloneMeta(const std::string& filename,
                                         const std::string& user) {
  UserInfo userInfo = GetUserInfo(user);
...
int CurveFsClientImpl::CompleteCloneFile(const std::string& filename,
                                         const std::string& user) {
  UserInfo userInfo = GetUserInfo(user);
  RetryMethod method = [this, &filename, userInfo]() {
...
int CurveFsClientImpl::SetCloneFileStatus(const std::string& filename,
...
int CurveFsClientImpl::GetFileInfo(const std::string& filename,
                                   const std::string& user, FInfo* fileInfo) {
  UserInfo userInfo = GetUserInfo(user);
  RetryMethod method = [this, &filename, &userInfo, fileInfo]() {
    return snapClient_->GetFileInfo(filename, userInfo, fileInfo);
  };
  RetryCondition condition = [](int ret) {
    return ret != LIBCURVE_ERROR::OK && ret != -LIBCURVE_ERROR::NOTEXIST &&
...
int CurveFsClientImpl::GetOrAllocateSegmentInfo(bool allocate, uint64_t offset,
                                                FInfo* fileInfo,
                                                const std::string& user,
                                                SegmentInfo* segInfo) {
  fileInfo->userinfo = GetUserInfo(user);
  RetryMethod method = [this, &allocate, offset, fileInfo, segInfo]() {
    return snapClient_->GetOrAllocateSegmentInfo(allocate, offset, fileInfo,
                                                 segInfo);
  };
  RetryCondition condition = [](int ret) {
    return ret != LIBCURVE_ERROR::OK && ret != -LIBCURVE_ERROR::NOT_ALLOCATE;
...
int CurveFsClientImpl::RenameCloneFile(const std::string& user,
                                       uint64_t originId,
                                       uint64_t destinationId,
                                       const std::string& origin,
                                       const std::string& destination) {
  UserInfo userInfo = GetUserInfo(user);
  RetryMethod method = [this, &userInfo, originId, destinationId, origin,
                        destination]() {
    return snapClient_->RenameCloneFile(userInfo, originId, destinationId,
                                        origin, destination);
  };
  RetryCondition condition = [](int ret) {
    return ret != LIBCURVE_ERROR::OK && ret != -LIBCURVE_ERROR::NOTEXIST &&
...
int CurveFsClientImpl::DeleteFile(const std::string& fileName,
                                  const std::string& user, uint64_t fileId) {
  UserInfo userInfo = GetUserInfo(user);
  RetryMethod method = [this, fileName, userInfo, fileId]() {
    return snapClient_->DeleteFile(fileName, userInfo, fileId);
  };
  RetryCondition condition = [](int ret) {
    return ret != LIBCURVE_ERROR::OK && ret != -LIBCURVE_ERROR::NOTEXIST &&
           ret != -LIBCURVE_ERROR::NOT_SUPPORT &&
...
int CurveFsClientImpl::ChangeOwner(const std::string& filename,
  ...
  RetryCondition condition = [](int ret) {
    return ret != LIBCURVE_ERROR::OK && ret != LIBCURVE_ERROR::NOT_SUPPORT &&
           ret != LIBCURVE_ERROR::FILE_OCCUPIED;
  };
  RetryHelper retryHelper(method, condition);
  return retryHelper.RetryTimeSecAndReturn(clientMethodRetryTimeSec_,
