...
void SnapshotInfoMetric::Update(SnapshotTaskInfo* taskInfo) {
  ...
  metric.Set("Status", std::to_string(static_cast<int>(snapInfo.GetStatus())));
...
void CloneMetric::UpdateBeforeTaskBegin(const CloneTaskType& taskType) {
...
void CloneMetric::UpdateAfterTaskFinish(const CloneTaskType& taskType,
                                        const CloneStatus& status) {
  if (CloneStatus::done == status || CloneStatus::metaInstalled == status) {
    if (CloneTaskType::kClone == taskType) {
      cloneDoing << -1;
      cloneSucceed << 1;
    } else {
...
void CloneMetric::UpdateFlattenTaskBegin() { flattenDoing << 1; }

void CloneMetric::UpdateAfterFlattenTaskFinish(const CloneStatus& status) {
...
void CloneInfoMetric::Update(CloneTaskInfo* taskInfo) {
  ...
  metric.Set("CloneTaskType", (cloneInfo.GetTaskType() == CloneTaskType::kClone)
                                  ? "Clone"
                                  : "Recover");
  ...
  metric.Set("FileType", (cloneInfo.GetFileType() == CloneFileType::kFile)
                             ? "File"
                             : "Snapshot");
  metric.Set("Lazy", (cloneInfo.GetIsLazy()) ? "True" : "False");
  metric.Set("Step", std::to_string(static_cast<int>(cloneInfo.GetNextStep())));
  metric.Set("Status", std::to_string(static_cast<int>(cloneInfo.GetStatus())));
