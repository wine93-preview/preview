...
void InitClientOption(std::shared_ptr<Configuration> conf,
  ...
  conf->GetValueFatalIfFail("mds.rootPassword", &clientOption->mdsRootPassword);
...
void InitSnapshotCloneServerOptions(std::shared_ptr<Configuration> conf,
  ...
  conf->GetValueFatalIfFail("server.cloneTempDir", &serverOption->cloneTempDir);
...
void SnapShotCloneServer::StartDummy() {
  ...
  LOG(INFO) << "snapshotCloneServer version: " << curve::common::CurveVersion();
...
bool SnapShotCloneServer::Init() {
  ...
  metaStore_ = std::make_shared<SnapshotCloneMetaStoreEtcd>(etcdClient_, codec);
  ...
  if (snapshotServiceManager_->Init(snapshotCloneServerOptions_.serverOption) <
      0) {
  ...
  cloneTaskMgr_ = std::make_shared<CloneTaskManager>(cloneCore_, cloneMetric_);
  ...
  service_ = std::make_shared<SnapshotCloneServiceImpl>(snapshotServiceManager_,
