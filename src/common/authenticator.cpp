...
std::string Authenticator::GetString2Signature(uint64_t date,
  ...
  std::string ret;
  return ret.append(std::to_string(date)).append(":").append(owner);
}

int Authenticator::HMacSha256(const void* key, int key_size, const void* data,
  ...
  if (key == NULL) {
    ...
    if (HMAC(EVP_sha256(), key, key_size, (const unsigned char*)data, data_size,
