...
double LeakyBucket::Bucket::Add(double tokens) {
  ...
  VLOG(9) << ": avg is: " << avg << ", level is: " << level
          << ", burst is: " << burst << ", burstLevel is: " << burstLevel
          << ", token is: " << tokens;
  double available = 0;
  levelInitial = false;
  ...
  if (burst > 0) {
    ...
    // 1. bucket-level is limited by bucket capacity which is calculate by
    // [burst * burstSeconds]  // NOLINT
    // 2. bucket-burst-level is limited by burst limit which is equal to [burst]
    // // NOLINT
    available = std::max(std::min(capacity - level, burst - burstLevel), 0.0);
...
void LeakyBucket::Bucket::Leak() {
  ...
  VLOG(9) << "leak is: " << leak << ", level is: " << level;
  if (burst > 0) {
    BucketInitial(true);
    if (burstLevelInitial) return;

    leak = static_cast<double>(burst) * FLAGS_bucketLeakIntervalMs /
           TimeUtility::MilliSecondsPerSecond;
    burstLevel = std::max(burstLevel - leak, 0.0);
    VLOG(9) << "leak is: " << leak << ", burstLevel is: " << burstLevel;
  }
}
