...
void Coordinator::Run() {
  for (auto& v : schedulerController_) {
    runSchedulerThreads_[v.first] =
        common::Thread(&Coordinator::RunScheduler, this, v.second, v.first);
  }
}
...
ChunkServerIdType Coordinator::CopySetHeartbeat(
  ...
  if (!topo_->CopySetFromTopoToSchedule(originInfo, &info)) {
    LOG(ERROR) << "coordinator cannot convert copyset("
               << originInfo.GetLogicalPoolId() << "," << originInfo.GetId()
...
int Coordinator::RapidLeaderSchedule(PoolIdType lpid) {
  auto rapidLeaderScheduler =
      std::make_shared<RapidLeaderScheduler>(conf_, topo_, opController_, lpid);
...
bool Coordinator::BuildCopySetConf(const CopySetConf& res,
  ...
  ChunkServerInfo chunkServer;
  if (!topo_->GetChunkServerInfo(res.configChangeItem, &chunkServer)) {
    LOG(ERROR) << "coordinator can not get chunkServer " << res.configChangeItem
               << " from topology";
    return false;
  }
  ...
  if (res.oldOne != ::curve::mds::topology::UNINTIALIZE_ID) {
    if (!topo_->GetChunkServerInfo(res.oldOne, &chunkServer)) {
      LOG(ERROR) << "coordinator can not get chunkServer " << res.oldOne
                 << " from topology";
      return false;
    }
  ...
  for (auto peer : res.peers) {
    ...
    replica->set_address(
        ::curve::mds::topology::BuildPeerId(peer.ip, peer.port, 0));
  }

  return true;
}

bool Coordinator::ChunkserverGoingToAdd(ChunkServerIdType csId,
                                        CopySetKey key) {
  Operator op;
  ...
  if (res != nullptr && csId == res->GetTargetPeer()) {
    LOG(INFO) << "chunkserver " << csId << " is target of pending operator "
              << op.OpToString();
    return true;
  }
