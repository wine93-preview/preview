...
int ScanScheduler::Schedule() {
  ...
  for (const auto& lpid : logicPoolIds) {
    ...
    if (!duringScanTime || !lpool.ScanEnable()) {
      ...
      SelectCopysetsForScan(copysetInfos, &copysets2start, &copysets2cancel);
    }

    count += GenScanOperator(copysets2start, ConfigChangeType::START_SCAN_PEER);
    count +=
        GenScanOperator(copysets2cancel, ConfigChangeType::CANCEL_SCAN_PEER);
  }

  LOG(INFO) << "ScanScheduelr generate " << count << " operators at this round";
  return 1;
}
...
void ScanScheduler::SelectCopysetsForScan(const CopySetInfos& copysetInfos,
  ...
  for (const auto& copysetInfo : copysetInfos) {
    if (StartOrReadyToScan(copysetInfo)) {
      ...
      LOG(INFO) << "Copyset is on scaning: " << copysetInfo.CopySetInfoStr();
    } else {
...
int ScanScheduler::GenScanOperator(const CopySetInfos& copysetInfos,
  ...
  for (auto& copysetInfo : copysetInfos) {
    ...
    LOG(INFO) << "Generate operator " << op.OpToString() << " for "
              << copysetInfo.CopySetInfoStr() << (succ ? " success" : " fail");
  }
