...
int RecoverScheduler::Schedule() {
  ...
  for (auto copysetInfo : topo_->GetCopySetInfos()) {
    ...
    if (copysetInfo.HasCandidate()) {
      LOG(WARNING) << copysetInfo.CopySetInfoStr() << " already has candidate: "
    ...
    if (static_cast<int>(offlinelists.size()) > deadBound) {
      LOG(ERROR) << "recoverSchdeuler find " << copysetInfo.CopySetInfoStr()
    ...
    for (auto it = offlinelists.begin(); it != offlinelists.end();) {
      if (excludes.count(*it) > 0) {
        LOG(ERROR) << "can not recover offline chunkserver " << *it << " on "
                   << copysetInfo.CopySetInfoStr()
                   << ", because it's server has more than "
                   << chunkserverFailureTolerance_ << " offline chunkservers";
...
void RecoverScheduler::CalculateExcludesChunkServer(
  ...
  for (auto item : unhealthyStateCS) {
    if (static_cast<int>(item.second.size()) < chunkserverFailureTolerance_) {
      continue;
    }
