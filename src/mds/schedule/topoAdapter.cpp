...
namespace mds {
namespace schedule {
PeerInfo::PeerInfo(ChunkServerIdType id, ZoneIdType zoneId, ServerIdType sid,
...
CopySetConf::CopySetConf(const CopySetKey& key, EpochType epoch,
                         ...
                         ConfigChangeType type, ChunkServerIdType item,
...
ChunkServerInfo::ChunkServerInfo(
    const PeerInfo& info, OnlineState state, DiskState diskState,
    ChunkServerStatus status, uint32_t leaderCount, uint64_t capacity,
    uint64_t used, const ChunkServerStatisticInfo& statisticInfo) {
  this->info = info;
  this->state = state;
  this->status = status;
...
bool ChunkServerInfo::IsOnline() const { return state == OnlineState::ONLINE; }
...
std::vector<ChunkServerInfo> TopoAdapterImpl::GetChunkServersInLogicalPool(
    PoolIdType lid) {
  std::vector<ChunkServerInfo> infos;
  auto ids = topo_->GetChunkServerInLogicalPool(
      lid, [](const ChunkServer& chunkserver) {
...
bool TopoAdapterImpl::GetPeerInfo(ChunkServerIdType id, PeerInfo* peerInfo) {
  ...
  if ((canGetChunkServer = topo_->GetChunkServer(id, &cs)) &&
    ...
    *peerInfo = PeerInfo(cs.GetId(), server.GetZoneId(), server.GetId(),
                         cs.GetHostIp(), cs.GetPort());
  } else {
    LOG(ERROR) << "topoAdapter can not find chunkServer(" << id
               << ", res:" << canGetChunkServer
...
bool TopoAdapterImpl::ChunkServerFromTopoToSchedule(
  ...
  if (topo_->GetServer(origin.GetServerId(), &server)) {
    ...
    LOG(ERROR) << "can not get server:" << origin.GetId()
               << ", ip:" << origin.GetHostIp() << ", port:" << origin.GetPort()
               << " from topology";
...
void TopoAdapterImpl::GetChunkServerScatterMap(
  ...
  for (auto key : copySetsInCS) {
    ...
    if (!topo_->GetCopySet(key, &copySetInfo)) {
      LOG(WARNING) << "topoAdapter find can not get copySet (" << key.first
                   << "," << key.second << ")"
