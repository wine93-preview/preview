...
void ScheduleServiceImpl::RapidLeaderSchedule(
    ...
    RapidLeaderScheduleResponse* response, google::protobuf::Closure* done) {
  ...
  LOG(INFO) << "Received request[log_id=" << cntl->log_id() << "] from "
            ...
            << ". [RapidLeaderScheduleRequest] " << request->DebugString();
  ...
  if (errCode == kScheduleErrCodeSuccess) {
    LOG(INFO) << "Send response[log_id=" << cntl->log_id() << "] from "
              ...
              << ". [RapidLeaderScheduleResponse] " << response->DebugString();
  } else {
