...
int ReplicaScheduler::Schedule() {
  ...
  for (auto info : topo_->GetCopySetInfos()) {
    ...
    if (info.HasCandidate()) {
      LOG(WARNING) << info.CopySetInfoStr() << " has candidate "
                   << info.candidatePeerInfo.id << " but operator lost";
      continue;
    }
    ...
    if (copysetReplicaNum == standardReplicaNum) {
      ...
      LOG(ERROR) << "replicaScheduler find " << info.CopySetInfoStr()
                 ...
                 << " smaller than standardReplicaNum:" << standardReplicaNum;
      ...
      if (!opController_->AddOperator(op)) {
        ...
        LOG(WARNING) << "replicaScheduler create " << info.CopySetInfoStr()
                     << ") on chunkServer: " << csId << " error";
        opController_->RemoveOperator(info.id);
        continue;
      }
      LOG(INFO) << "replicaScheduler create " << info.CopySetInfoStr()
                << ") on chunkServer: " << csId
                << " success and generate operator: " << op.OpToString();
      oneRoundGenOp += 1;
    } else {
      ...
      LOG(WARNING) << "replicaScheduler find " << info.CopySetInfoStr()
                   << " replicaNum:" << copysetReplicaNum
                   << " larger than standardReplicaNum:" << standardReplicaNum;

      ChunkServerIdType csId = SelectRedundantReplicaToRemove(info);
      if (csId == UNINTIALIZE_ID) {
      ...
      if (opController_->AddOperator(op)) {
        LOG(INFO) << "replicaScheduler generate operator " << op.OpToString()
                  << " on " << info.CopySetInfoStr();
        oneRoundGenOp += 1;
      }
    }
  }
  LOG(INFO) << "replicaScheduelr generate " << oneRoundGenOp
            << " at this round";
  return 1;
}

int64_t ReplicaScheduler::GetRunningInterval() { return this->runInterval_; }
}  // namespace schedule
}  // namespace mds
}  // namespace curve
