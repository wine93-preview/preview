 ...
 *          2, 3, 4, 5, 6, 7...} to replace copy 1, and generate Operator
 * (copyset-m, +n, -1) //NOLINT
 *
 * Steps:
 ...
 *        some definitions:
 *            - the definition of scatter-width_map is
 * std::map<chunkserverIdType, int> //NOLINT
 *            - scatter-width here means len(scatter-width_map)
 *            - scatter-width_map of chunkserver-a:
 *              key: id of chunkserver that the other copies of copyset on
 * chunkserver-a located //NOLINT value: the number of copyset that own by both
 * the chunkserver 'key' and chunkserver-a //NOLINT example: chunkserver-a has
 * copyset-1{a, 1, 2}, copyset-2{a, 2, 3}, and now we want to migrate the
 * replica on chunkserver 1 to chunkserver n, which is operator (copyset-1, +n,
 * -1) ① for chunkserver n(mark as target, to which the replica migrate), value
 * of {2,3} in scatter-width_map will increase 1 //NOLINT ② for chunkserver
 * 1(mark as source, from which the replica migrate out), value of {2,3} in
 * scatter-width_map will decrease 1 //NOLINT ③ for chunkserver 2 and 3 (mark as
 * other), value of {n} will increase 1, and value of {1} will decrease 1
 * //NOLINT
...
ChunkServerIdType Scheduler::SelectBestPlacementChunkServer(
  ...
  if (chunkServers.size() <= copySetInfo.peers.size()) {
    LOG(ERROR) << "logicalPool " << copySetInfo.id.first << " has "
               << chunkServers.size()
               << " chunkservers, "
  ...
  for (auto& cs : chunkServers) {
    ...
    if (!cs.IsHealthy()) {
      ...
      LOG(WARNING) << "topoAdapter find chunkServer " << cs.info.id
                   ...
                   << ", onlineState: " << cs.state << ", capacity： "
                   << cs.diskCapacity << ", used: " << cs.diskUsed;
      continue;
    }
    ...
    if (minScatterWidth <= 0) {
      LOG(WARNING) << "minScatterWith in logical pool " << copySetInfo.id.first
                   << " is not initialized";
      return UNINTIALIZE_ID;
    }
    if (SchedulerHelper::InvovledReplicasSatisfyScatterWidthAfterMigration(
            copySetInfo, source, target, oldPeer, topo_, minScatterWidth,
            scatterWidthRangePerent_, &affected)) {
      LOG(INFO) << "SelectBestPlacementChunkServer select " << target << " for "
                << copySetInfo.CopySetInfoStr() << " to replace " << oldPeer
  ...
  LOG(INFO) << "SelectBestPlacementChunkServer select "
            << candidates[candidates.size() - 1].first << " for "
            << copySetInfo.CopySetInfoStr() << " to replace " << oldPeer
...
ChunkServerIdType Scheduler::SelectRedundantReplicaToRemove(
  ...
  if (standardReplicaNum <= 0) {
    LOG(ERROR) << "topoAdapter get standard replicaNum " << standardReplicaNum
               << " in logicalPool,"
  ...
  if (static_cast<int>(copySetInfo.peers.size()) <= standardReplicaNum) {
    LOG(ERROR) << "topoAdapter cannot select redundent replica for "
               ...
               << copySetInfo.peers.size() << " not bigger than standard num "
               << standardReplicaNum;
    return UNINTIALIZE_ID;
  }
  ...
  int standardZoneNum =
      topo_->GetStandardZoneNumInLogicalPool(copySetInfo.id.first);
  if (standardZoneNum <= 0) {
    LOG(ERROR) << "topoAdapter get standard zoneNum " << standardZoneNum
               << " in logicalPool " << copySetInfo.id.first
  ...
  for (auto& peer : copySetInfo.peers) {
    ...
    if (item == zoneList.end()) {
      zoneList.emplace(std::make_pair(
          peer.zoneId, std::vector<ChunkServerIdType>({peer.id})));
    } else {
  ...
  if (static_cast<int>(zoneList.size()) < standardZoneNum) {
    LOG(ERROR) << "topoAdapter find " << copySetInfo.CopySetInfoStr()
               << " replicas distribute in " << zoneList.size()
               << " zones, less than standard zoneNum " << standardZoneNum
               << ", please check";
    return UNINTIALIZE_ID;
  }
  ...
  // 2.1 equal
  // to satisfy the restriction of zone condition, should choose from
  // chunkserver under the zone //NOLINT for example, if the replica are
  // A(zone1) B(zone2) C(zone3) D(zone3) E(zone2) //NOLINT the one to remove
  // should be selected from BCDE 2.2 greater any chunkserver to remove will
  // satisfy the requirement, thus we first consider the one not online, then
  // the one is pendding , then consider the scatter-width for example, if the
  // replicas are: A(zone1) B(zone2) C(zone3) D(zone4) E(zone4) we can remove
  // anyone of it
  ...
  for (auto cs : candidateChunkServer) {
    ...
    if (!topo_->GetChunkServerInfo(cs, &csInfo)) {
      LOG(ERROR) << "scheduler cannot get chunkserver " << cs
                 << " which is a replica of " << copySetInfo.CopySetInfoStr();
      return UNINTIALIZE_ID;
    }

    // chunkserver is offline
    if (csInfo.IsOffline()) {
      LOG(WARNING) << "scheduler choose to remove offline chunkServer " << cs
                   << " from " << copySetInfo.CopySetInfoStr();
      return cs;
    }

    // chunkserver is pendding
    if (csInfo.IsPendding()) {
      LOG(WARNING) << "scheduler choose to remove pendding chunkServer " << cs
                   << " from " << copySetInfo.CopySetInfoStr();
      return cs;
    }
  }

  // if all chunkserver are online and not pendding, select according to the
  // impact on scatter-width //NOLINT first rank chunkser by the number of
  // copyset on it
  ...
  for (auto it = desc.begin(); it != desc.end(); it++) {
    ...
    int minScatterWidth = GetMinScatterWidth(copySetInfo.id.first);
    if (minScatterWidth <= 0) {
      LOG(ERROR) << "minScatterWith in logical pool " << copySetInfo.id.first
                 << " is not initialized";
      return UNINTIALIZE_ID;
    }
    if (SchedulerHelper::InvovledReplicasSatisfyScatterWidthAfterMigration(
            copySetInfo, source, target, ignore, topo_, minScatterWidth,
            scatterWidthRangePerent_, &affected)) {
      return source;
    } else {
      candidates.emplace_back(
