...
class Scheduler {
 public:
   ...
   */
  Scheduler(const ScheduleOption& opt, const std::shared_ptr<TopoAdapter>& topo,
...
class LeaderScheduler : public Scheduler {
 public:
  LeaderScheduler(const ScheduleOption& opt,
                  const std::shared_ptr<TopoAdapter>& topo,
                  const std::shared_ptr<OperatorController>& opController)
      : Scheduler(opt, topo, opController) {
...
class RecoverScheduler : public Scheduler {
 ...
 private:
  ...
  int64_t runInterval_;
  // the threshold of the failing chunkserver that the server will not be
  // recovered //NOLINT
...
class ReplicaScheduler : public Scheduler {
 public:
  ReplicaScheduler(const ScheduleOption& opt,
                   const std::shared_ptr<TopoAdapter>& topo,
                   const std::shared_ptr<OperatorController>& opController)
      : Scheduler(opt, topo, opController) {
...
class RapidLeaderScheduler : public Scheduler {
 public:
  RapidLeaderScheduler(const ScheduleOption& opt,
                       const std::shared_ptr<TopoAdapter>& topo,
                       const std::shared_ptr<OperatorController>& opController,
                       PoolIdType lpid)
 ...
 private:
  ...
  /**
   * @brief find the chunkserver that has the minimum number of leaders in peers
   * of a copyset //NOLINT
   ...
   * @param[in] stat The info of the distribution of leaders
   * @return true if the target node is able to be the new leader, false if not
   * //NOLINT
   */
  bool PossibleTargetPeerConfirm(ChunkServerIdType origLeader,
                                 ChunkServerIdType targetLeader,
                                 const LeaderStatInLogicalPool& stat);
   ...
   */
  bool GenerateLeaderChangeOperatorForCopySet(const CopySetInfo& info,
                                              ChunkServerIdType targetLeader);
...
class ScanScheduler : public Scheduler {
 ...
 public:
  ScanScheduler(const ScheduleOption& opt,
                const std::shared_ptr<TopoAdapter>& topo,
                const std::shared_ptr<OperatorController>& opController)
      : Scheduler(opt, topo, opController) {
 ...
 private:
   ...
   */
  void SelectCopysetsToStartScan(CopySetInfos* copysetInfos, int count,
                                 Selected* selected,
   ...
   */
  void SelectCopysetsToCancelScan(CopySetInfos* copysetInfos, int count,
