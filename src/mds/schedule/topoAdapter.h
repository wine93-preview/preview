...
struct PeerInfo {
 public:
  ...
  bool operator==(const PeerInfo& other) const {
    return id == other.id && zoneId == other.zoneId &&
           serverId == other.serverId && ip == other.ip && port == other.port;
  }
};
...
struct CopySetInfo {
  ...
  CopySetInfo(CopySetKey id, EpochType epoch, ChunkServerIdType leader,
              const std::vector<PeerInfo>& peers, const ConfigChangeInfo& info,
              const CopysetStatistics& statistics)
      : logicalPoolWork(false) {
...
struct ChunkServerInfo {
 public:
  ...
  ChunkServerInfo(const PeerInfo& info, OnlineState state, DiskState diskState,
                  ChunkServerStatus status, uint32_t leaderCount,
...
class TopoAdapter {
 public:
   ...
   * @return logical pool
   */
  virtual bool GetLogicalPool(PoolIdType id,
   ...
   */
  virtual bool GetChunkServerInfo(ChunkServerIdType id,
                                  ChunkServerInfo* info) = 0;
   ...
   * @return false if failed, true if succeeded
   */
  virtual bool CreateCopySetAtChunkServer(CopySetKey id,
                                          ChunkServerIdType csID) = 0;
  ...
  /**
   * @brief ChunkServerFromTopoToSchedule Transfer chunkserver info format from
   * //NOLINT topology module to schedule module
   *
   ...
   *       and so on
   */
  virtual void GetChunkServerScatterMap(
      const ChunkServerIdType& cs, std::map<ChunkServerIdType, int>* out) = 0;
};
...
class TopoAdapterImpl : public TopoAdapter {
 public:
  ...
  bool GetLogicalPool(PoolIdType id,
  ...
  bool GetChunkServerInfo(ChunkServerIdType id, ChunkServerInfo* info) override;
  ...
  bool CreateCopySetAtChunkServer(CopySetKey id,
                                  ChunkServerIdType csID) override;
