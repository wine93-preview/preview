 ...
 *     (newValue-oldValue<=-1)
 */
bool SchedulerHelper::SatisfyScatterWidth(bool target, int oldValue,
                                          int newValue, int minScatterWidth,
...
bool SchedulerHelper::SatisfyZoneAndScatterWidthLimit(
    ...
    ChunkServerIdType source, const CopySetInfo& candidate, int minScatterWidth,
    float scatterWidthRangePerent) {
  ...
  // determine whether the scatter-width of
  // source(-other)/target(+other)/other(-source, +target) satisfy the
  // requirement //NOLINT
  int affected = 0;
  if (SchedulerHelper::InvovledReplicasSatisfyScatterWidthAfterMigration(
          candidate, source, target, UNINTIALIZE_ID, topo, minScatterWidth,
          scatterWidthRangePerent, &affected)) {
    return true;
  }
  return false;
}
...
void SchedulerHelper::SortDistribute(
  ...
  for (auto item : distribute) {
    ...
    desc->emplace_back(std::pair<ChunkServerIdType, std::vector<CopySetInfo>>(
...
bool SchedulerHelper::InvovledReplicasSatisfyScatterWidthAfterMigration(
    ...
    const std::shared_ptr<TopoAdapter>& topo, int minScatterWidth,
    float scatterWidthRangePerent, int* affected) {
  ...
  SchedulerHelper::CalculateAffectOfMigration(copySetInfo, source, target, topo,
                                              &out);
  ...
  *affected = 0;
  for (auto iter = out.begin(); iter != out.end(); iter++) {
    ...
    if (!SchedulerHelper::SatisfyScatterWidth(
            iter->first == target, beforeMigrationScatterWidth,
            afterMigrationScatterWidth, minScatterWidth,
            scatterWidthRangePerent)) {
      allSatisfy = false;
    }

    // calculate the sum of the change of scatter-width
    *affected += afterMigrationScatterWidth - beforeMigrationScatterWidth;
  }
...
void SchedulerHelper::FilterCopySetDistributions(
