...
int ChunkServerClient::DeleteChunkSnapshotOrCorrectSn(
    ChunkServerIdType leaderId, LogicalPoolID logicalPoolId,
    CopysetID copysetId, ChunkID chunkId, uint64_t correctedSn) {
  ...
  do {
    ...
    LOG(INFO) << "Send DeleteChunkSnapshotOrCorrectSn[log_id=" << cntl.log_id()
              << "] from " << cntl.local_side() << " to " << cntl.remote_side()
              << ". [ChunkRequest] " << request.DebugString();
    if (cntl.Failed()) {
    ...
    switch (response.status()) {
      ...
      case CHUNK_OP_STATUS::CHUNK_OP_STATUS_CHUNK_NOTEXIST: {
        LOG(INFO) << "Received DeleteChunkSnapshotOrCorrectSn[log_id="
                  << cntl.log_id() << "] from " << cntl.remote_side() << " to "
                  << cntl.local_side() << ". [ChunkResponse] "
...
int ChunkServerClient::DeleteChunk(ChunkServerIdType leaderId,
  ...
  do {
    ...
    LOG(INFO) << "Send DeleteChunk[log_id=" << cntl.log_id() << "] from "
  ...
  if (cntl.Failed()) {
    ...
    switch (response.status()) {
      case CHUNK_OP_STATUS::CHUNK_OP_STATUS_SUCCESS:
      case CHUNK_OP_STATUS::CHUNK_OP_STATUS_CHUNK_NOTEXIST: {
        LOG(INFO) << "Received DeleteChunk[log_id=" << cntl.log_id()
      ...
      default: {
        LOG(ERROR) << "Received DeleteChunk error, [log_id=" << cntl.log_id()
                   << "] from " << cntl.remote_side() << " to "
                   << cntl.local_side() << ". [ChunkResponse] "
                   << response.DebugString();
        return kCsClientReturnFail;
...
int ChunkServerClient::GetLeader(ChunkServerIdType csId,
  ...
  do {
    ...
    LOG(INFO) << "Send GetLeader[log_id=" << cntl.log_id() << "] from "
              << cntl.local_side() << " to " << cntl.remote_side()
              << ". [GetLeaderRequest] " << request.DebugString();
    if (cntl.Failed()) {
  ...
  if (cntl.Failed()) {
    ...
    LOG(INFO) << "Received GetLeaderResponse[log_id=" << cntl.log_id()
              << "] from " << cntl.remote_side() << " to " << cntl.local_side()
              << ". [GetLeaderResponse] " << response.DebugString();
    ...
    if (SplitPeerId(leaderPeer, &leaderIp, &leaderPort)) {
      *leader = topology_->FindChunkServerNotRetired(leaderIp, leaderPort);
