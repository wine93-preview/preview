...
int CopysetClient::DeleteChunkSnapshotOrCorrectSn(LogicalPoolID logicalPoolId,
  ...
  CopySetInfo copyset;
  if (true !=
      topo_->GetCopySet(CopySetKey(logicalPoolId, copysetId), &copyset)) {
    LOG(ERROR) << "GetCopySet fail.";
    return kMdsFail;
  }

  ChunkServerIdType leaderId = copyset.GetLeader();
  ...
  while ((retry < updateLeaderRetryTimes_) &&
         ((UNINTIALIZE_ID == leaderId) || (kCsClientCSOffline == ret) ||
          (kRpcFail == ret) || (kCsClientNotLeader == ret))) {
...
int CopysetClient::DeleteChunk(LogicalPoolID logicalPoolId, CopysetID copysetId,
  ...
  if (leaderId != UNINTIALIZE_ID) {
    ret = chunkserverClient_->DeleteChunk(leaderId, logicalPoolId, copysetId,
                                          chunkId, sn);
    if (kMdsSuccess == ret) {
      return ret;
    }
...
int CopysetClient::UpdateLeader(CopySetInfo* copyset) {
  ...
  for (auto& csId : copyset->GetCopySetMembers()) {
    ret =
        chunkserverClient_->GetLeader(csId, logicalPoolId, copysetId, &leader);
    if (kMdsSuccess == ret) {
      success = true;
      break;
    }
