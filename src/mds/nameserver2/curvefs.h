...
class CurveFS {
 public:
   ...
   */
  StatusCode CreateFile(const std::string& fileName, const std::string& owner,
                        FileType filetype, uint64_t length, uint64_t stripeUnit,
   ...
   */
  StatusCode GetFileInfo(const std::string& filename, FileInfo* inode) const;
  ...
  StatusCode RecoverFile(const std::string& originFileName,
                         const std::string& recycleFileName, uint64_t fileId);
   ...
   *  @return StatusCode::kOK if succeeded
   */
  // TODO(hzsunjianliang): Add inode parameters of the source file for checking
  // //NOLINT
  StatusCode RenameFile(const std::string& sourceFileName,
                        const std::string& destFileName, uint64_t sourceFileId,
                        uint64_t destFileId);
  ...
  StatusCode ExtendFile(const std::string& filename, uint64_t newSize);
   ...
   *  @return StatusCode::kOK if succeeded
   */
  StatusCode GetOrAllocateSegment(const std::string& filename, offset_t offset,
   ...
   */
  FileInfo GetRootFileInfo(void) const { return rootFileInfo_; }
   ...
   *  @return StatusCode::kOK if succeeded
   */
  StatusCode DeleteFileSnapShotFile(
   ...
   *  @param[out] progress: Additional deletion progress when the status is
   * kFileDeleting //NOLINT
   *  @return StatusCode::kOK if succeeded
   */
   ...
   *  @return StatusCode::kOK if succeeded
   */
  StatusCode GetSnapShotFileInfo(const std::string& fileName, FileSeqType seq,
   ...
   *  @return StatusCode::kOK if succeeded
   */
  StatusCode GetSnapShotFileSegment(const std::string& filename,
   ...
   *  @return StatusCode::kOK if succeeded
   */
  StatusCode OpenFile(const std::string& fileName, const std::string& clientIP,
  ...
  StatusCode RefreshSession(const std::string& filename,
                            const std::string& sessionid, const uint64_t date,
  ...
  /**
   * @brief Clone a file. Clone file can only be created by the root user
   * currently //NOLINT
   * @param filename
  ...
  StatusCode CreateCloneFile(const std::string& filename,
                             ...
                             uint64_t length, FileSeqType seq,
                             ChunkSizeType chunksize, uint64_t stripeUnit,
                             uint64_t stripeCount, FileInfo* fileInfo,
   ...
   *
   */
  StatusCode SetCloneFileStatus(const std::string& filename, uint64_t fileID,
   ...
   *  @return StatusCode::kOK if succeeded, StatusCode::kOwnerAuthFail if failed
   * //NOLINT
   */
   ...
   */
  StatusCode CheckEpoch(const std::string& filename, uint64_t epoch);
   ...
   *  @return StatusCode::kOK if succeeded, StatusCode::KInternalError if failed
   * //NOLINT
   */
   ...
   * @return StatusCode::kOK if succeeded, StatusCode::kFileNotExists if failed
   * //NOLINT
   */
 ...
 private:
  ...
  StatusCode WalkPath(const std::string& fileName, FileInfo* fileInfo,
  ...
  std::string GetRootOwner() { return rootAuthOptions_.rootOwner; }
   ...
   *  @return true if legal, false if not
   */
  bool CheckSignature(const std::string& owner, const std::string& signature,
                      uint64_t date);
  ...
  StatusCode CheckPathOwnerInternal(const std::string& filename,
                                    const std::string& owner,
                                    const std::string& signature,
                                    std::string* lastEntry, uint64_t* parentID);
   ...
   *  @return StatusCode::kOK if succeeded
   */
  StatusCode GetFileSize(const std::string& fileName, const FileInfo& fileInfo,
                         uint64_t* fileSize);
  ...
  StatusCode CheckHasCloneRely(const std::string& filename,
                               const std::string& owner, bool* isHasCloneRely);
...
class ChunkServerRegistInfoBuilderImpl : public ChunkServerRegistInfoBuilder {
 public:
  explicit ChunkServerRegistInfoBuilderImpl(CurveFS* cfs) : cfs_(cfs) {}
