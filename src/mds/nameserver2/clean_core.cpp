...
StatusCode CleanCore::CleanSnapShotFile(const FileInfo& fileInfo,
  ...
  for (uint32_t i = 0; i < segmentNum; i++) {
    ...
    StoreStatus storeRet =
        storage_->GetSegment(fileInfo.parentid(), i * segmentSize, &segment);
    ...
    int chunkNum = segment.chunks_size();
...
StatusCode CleanCore::CleanFile(const FileInfo& commonFile,
  ...
  for (int i = 0; i != segmentNum; i++) {
    ...
    int64_t revision;
    storeRet =
        storage_->DeleteSegment(commonFile.id(), i * segmentSize, &revision);
...
StatusCode CleanCore::CleanDiscardSegment(
  ...
  if (ret != 0) {
    LOG(ERROR) << "CleanDiscardSegment failed, DeleteChunk Error, ret = " << ret
               << ", filename = " << fileInfo.filename()
               << ", inodeid = " << fileInfo.id()
               << ", segment offset = " << segment.startoffset();
    progress->SetStatus(TaskStatus::FAILED);
  ...
  if (storeRet != StoreStatus::OK) {
    LOG(ERROR) << "CleanDiscardSegment failed, filename = "
               << fileInfo.filename() << ", offset = " << segment.startoffset();
    progress->SetStatus(TaskStatus::FAILED);
    return StatusCode::KInternalError;
  }

  allocStatistic_->DeAllocSpace(segment.logicalpoolid(), segment.segmentsize(),
                                revision);
  progress->SetProgress(100);
  progress->SetStatus(TaskStatus::SUCCESS);

  timer.stop();
  LOG(INFO) << "CleanDiscardSegment success, filename = " << fileInfo.filename()
            << ", inodeid = " << fileInfo.id()
