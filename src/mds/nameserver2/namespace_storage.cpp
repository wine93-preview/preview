...
StoreStatus NameServerStorageImp::PutFile(const FileInfo& fileInfo) {
  std::string storeKey;
  if (GetStoreKey(fileInfo.filetype(), fileInfo.parentid(), fileInfo.filename(),
  ...
  if (errCode != EtcdErrCode::EtcdOK) {
    LOG(ERROR) << "put file: [" << fileInfo.filename() << "] err: " << errCode;
  } else {
    // update to cache
...
StoreStatus NameServerStorageImp::DeleteSnapshotFile(
    InodeID id, const std::string& filename) {
  std::string storeKey;
...
StoreStatus NameServerStorageImp::RenameFile(const FileInfo& oldFInfo,
  ...
  if (res != StoreStatus::OK) {
    LOG(ERROR) << "get store key failed, filename = " << oldFInfo.filename();
    return StoreStatus::InternalError;
  }
  ...
  if (res != StoreStatus::OK) {
    LOG(ERROR) << "get store key failed, filename = " << newFInfo.filename();
    return StoreStatus::InternalError;
  }
...
StoreStatus NameServerStorageImp::ReplaceFileAndRecycleOldFile(
  ...
  if (!NameSpaceStorageCodec::EncodeFileInfo(recycleFInfo,
                                             &encodeRecycleFInfo)) {
    LOG(ERROR) << "encode recycle file: " << recycleFInfo.filename() << " err";
    return StoreStatus::InternalError;
  }
...
StoreStatus NameServerStorageImp::MoveFileToRecycle(
  ...
  Operation op1{OpType::OpDelete, const_cast<char*>(originFileInfoKey.c_str()),
...
StoreStatus NameServerStorageImp::ListFile(InodeID startid, InodeID endid,
  ...
  auto res = GetStoreKey(FileType::INODE_PAGEFILE, startid, "", &startStoreKey);
  if (res != StoreStatus::OK) {
...
StoreStatus NameServerStorageImp::ListSegment(
    InodeID id, std::vector<PageFileSegment>* segments) {
  std::string startStoreKey =
...
StoreStatus NameServerStorageImp::ListSnapshotFile(
    InodeID startid, InodeID endid, std::vector<FileInfo>* files) {
  std::string startStoreKey;
  ...
  std::string endStoreKey;
  res = GetStoreKey(FileType::INODE_SNAPSHOT_PAGEFILE, endid, "", &endStoreKey);
  if (res != StoreStatus::OK) {
    LOG(ERROR) << "get store key failed, id = " << endid;
    return StoreStatus::InternalError;
  }
...
StoreStatus NameServerStorageImp::ListFileInternal(
    const std::string& startStoreKey, const std::string& endStoreKey,
    std::vector<FileInfo>* files) {
  std::vector<std::string> out;
  int errCode = client_->List(startStoreKey, endStoreKey, &out);
  ...
  for (size_t i = 0; i < out.size(); i++) {
    FileInfo fileInfo;
    bool decodeOK = NameSpaceStorageCodec::DecodeFileInfo(out[i], &fileInfo);
    if (decodeOK) {
...
StoreStatus NameServerStorageImp::PutSegment(InodeID id, uint64_t off,
                                             ...
                                             int64_t* revision) {
  std::string storeKey = NameSpaceStorageCodec::EncodeSegmentStoreKey(id, off);
  ...
  if (errCode != EtcdErrCode::EtcdOK) {
    LOG(ERROR) << "put segment of logicalPoolId:" << segment->logicalpoolid()
               << "err:" << errCode;
  } else {
...
StoreStatus NameServerStorageImp::ListDiscardSegment(
  ...
  int err = client_->List(DISCARDSEGMENTKEYPREFIX, DISCARDSEGMENTKEYEND, &out);
...
StoreStatus NameServerStorageImp::DiscardSegment(
  ...
  if (errCode != EtcdErrCode::EtcdOK) {
    LOG(ERROR) << "Discard segment failed, filename: " << fileInfo.filename()
               << ", inodeid = " << inodeId << ", offset: " << offset
               << ", errCode: " << errCode;
  } else {
