...
bool AllocStatistic::GetAllocByLogicalPool(PoolIdType lid, int64_t* alloc) {
  ...
  if (true == currentValueAvalible_.load()) {
    ...
    if (existSegmentAllocValues_.find(lid) != existSegmentAllocValues_.end()) {
...
void AllocStatistic::AllocSpace(PoolIdType lid, int64_t changeSize,
                                int64_t revision) {
  // segmentAlloc_ value is not available, changeSize needs to be updated to
  // existSegmentAllocValues_ //NOLINT
...
void AllocStatistic::DeAllocSpace(PoolIdType lid, int64_t changeSize,
                                  int64_t revision) {
  if (false == currentValueAvalible_.load()) {
...
void AllocStatistic::CalculateSegmentAlloc() {
  ...
  int res;
  do {
    res = AllocStatisticHelper::CalculateSegmentAlloc(curRevision_, client_,
                                                      &segmentAlloc_);
  ...
  if (res == 0) {
    ...
    do {
      ...
      if (errCode != EtcdErrCode::EtcdOK) {
        ...
        std::this_thread::sleep_for(std::chrono::milliseconds(retryInterMs_));
      }
...
void AllocStatistic::PeriodicPersist() {
  ...
  while (
      sleeper_.wait_for(std::chrono::milliseconds(periodicPersistInterMs_))) {
    ...
    for (auto& item : curPersist) {
      ...
      int errCode =
          client_->Put(NameSpaceStorageCodec::EncodeSegmentAllocKey(item.first),
                       NameSpaceStorageCodec::EncodeSegmentAllocValue(
                           item.first, item.second));

      if (EtcdErrCode::EtcdOK != errCode) {
        LOG(INFO) << "periodic persist logicalPool" << item.first
                  << " size: " << item.second
