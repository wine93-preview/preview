...
namespace curve {
namespace mds {
bool ChunkSegmentAllocatorImpl::AllocateChunkSegment(
    FileType type, SegmentSizeType segmentSize, ChunkSizeType chunkSize,
  ...
  if (!topologyChunkAllocator_->AllocateChunkRoundRobinInSingleLogicalPool(
  ...
  for (size_t i = 0; i < copysets.size(); i++) {
    if (copysets[i].logicalPoolId != logicalpoolId) {
      LOG(ERROR) << "Allocate Copysets id not same, copysets[" << i
