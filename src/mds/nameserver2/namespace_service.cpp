...
void NameSpaceService::CreateFile(
  ...
  if (!isPathValid(request->filename())) {
    ...
    LOG(ERROR) << "logid = " << cntl->log_id()
               ...
               << request->filename() << ", filetype = " << request->filetype()
...
void NameSpaceService::DeleteFile(
    ::google::protobuf::RpcController* controller,
...
void NameSpaceService::RecoverFile(
  ...
  retCode = kCurveFS.CheckRecycleFileOwner(recoverFileName, request->owner(),
                                           signature, request->date());
  if (retCode != StatusCode::kOK) {
    response->set_statuscode(retCode);
    if (google::ERROR != GetMdsLogLevel(retCode)) {
  ...
  retCode = kCurveFS.CheckDestinationOwner(
      recoverFileInfo.originalfullpathname(), request->owner(), signature,
      request->date());
  if (retCode != StatusCode::kOK) {
    response->set_statuscode(retCode);
    if (google::ERROR != GetMdsLogLevel(retCode)) {
  ...
  retCode = kCurveFS.RecoverFile(request->filename(), recoverFileName, fileId);
  if (retCode != StatusCode::kOK) {
    response->set_statuscode(retCode);
    if (google::ERROR != GetMdsLogLevel(retCode)) {
      LOG(WARNING) << "logid = " << cntl->log_id()
...
void NameSpaceService::GetFileInfo(
  ...
  retCode =
      kCurveFS.GetFileInfo(request->filename(), response->mutable_fileinfo());
  if (retCode != StatusCode::kOK) {
    response->set_statuscode(retCode);
    if (google::ERROR != GetMdsLogLevel(retCode)) {
      LOG(WARNING) << "logid = " << cntl->log_id()
...
void NameSpaceService::GetOrAllocateSegment(
  ...
  LOG(INFO) << "logid = " << cntl->log_id()
            << ", GetOrAllocateSegment request, filename = "
            << request->filename() << ", offset = " << request->offset()
            << ", allocateTag = " << request->allocateifnotexist();
  ...
  retCode = kCurveFS.GetOrAllocateSegment(
      request->filename(), request->offset(), request->allocateifnotexist(),
...
void NameSpaceService::DeAllocateSegment(
  ...
  if (retCode != StatusCode::kOK) {
    ...
    if (google::ERROR != GetMdsLogLevel(retCode)) {
      LOG(WARNING) << "logid = " << cntl->log_id()
                   ...
                   << request->ShortDebugString() << ", retCode = " << retCode;
    } else {
      LOG(ERROR) << "logid = " << cntl->log_id()
                 << ", DeAllocateSegment CheckFileOwner fail, "
                 << request->ShortDebugString() << ", retCode = " << retCode;
  ...
  retCode = kCurveFS.DeAllocateSegment(request->filename(), request->offset());
  ...
  if (retCode != StatusCode::kOK) {
    ...
    if (google::ERROR != GetMdsLogLevel(retCode)) {
      ...
      LOG(ERROR) << "logid = " << cntl->log_id() << ", DeAllocateSegment fail, "
                 << request->ShortDebugString() << ", statusCode = " << retCode
                 << ", StatusCode_Name = " << StatusCode_Name(retCode)
                 << ", cost " << timer.m_elapsed(0.0) << " ms";
    }
  } else {
...
void NameSpaceService::RenameFile(
    ::google::protobuf::RpcController* controller,
  ...
  if (retCode != StatusCode::kOK) {
    ...
    if (google::ERROR != GetMdsLogLevel(retCode)) {
      LOG(WARNING) << "logid = " << cntl->log_id()
                   << ", CheckFileOwner fail, filename = "
                   << request->oldfilename() << ", owner = " << request->owner()
                   << ", statusCode = " << retCode;
    } else {
      LOG(ERROR) << "logid = " << cntl->log_id()
                 << ", CheckFileOwner fail, filename = "
  ...
  retCode = kCurveFS.CheckDestinationOwner(
      request->newfilename(), request->owner(), signature, request->date());
  if (retCode != StatusCode::kOK) {
    response->set_statuscode(retCode);
    if (google::ERROR != GetMdsLogLevel(retCode)) {
      LOG(WARNING) << "logid = " << cntl->log_id()
                   << ", CheckDestinationOwner fail, filename = "
                   << request->newfilename() << ", owner = " << request->owner()
                   << ", statusCode = " << retCode;
    } else {
      LOG(ERROR) << "logid = " << cntl->log_id()
                 << ", CheckDestinationOwner fail, filename = "
  ...
  retCode = kCurveFS.RenameFile(request->oldfilename(), request->newfilename(),
                                oldFileId, newFileId);
  if (retCode != StatusCode::kOK) {
    response->set_statuscode(retCode);
    if (google::ERROR != GetMdsLogLevel(retCode)) {
      LOG(WARNING) << "logid = " << cntl->log_id()
                   << ", RenameFile fail, oldfilename = "
                   << request->oldfilename()
                   << ", newfilename = " << request->newfilename()
                   << ", statusCode = " << retCode
                   << ", StatusCode_Name = " << StatusCode_Name(retCode)
    ...
    LOG(INFO) << "logid = " << cntl->log_id()
              ...
              << ", newFileName = " << request->newfilename() << ", cost "
              << expiredTime.ExpiredMs() << " ms";
  }
  return;
}

void NameSpaceService::ExtendFile(
    ::google::protobuf::RpcController* controller,
  ...
  if (!isPathValid(request->filename())) {
    ...
    LOG(ERROR) << "logid = " << cntl->log_id()
               ...
               << request->filename() << ", newsize = " << request->newsize();
    return;
  }
  ...
  retCode = kCurveFS.ExtendFile(request->filename(), request->newsize());
  if (retCode != StatusCode::kOK) {
    response->set_statuscode(retCode);
    if (google::ERROR != GetMdsLogLevel(retCode)) {
      LOG(WARNING) << "logid = " << cntl->log_id()
...
void NameSpaceService::ChangeOwner(
  ...
  if (!isPathValid(request->filename())) {
    ...
    LOG(ERROR) << "logid = " << cntl->log_id()
               ...
               << request->filename() << ", newOwner = " << request->newowner();
    return;
  }
...
void NameSpaceService::ListDir(::google::protobuf::RpcController* controller,
  ...
  if (retCode != StatusCode::kOK) {
    ...
    LOG(INFO) << "logid = " << cntl->log_id()
              << ", ListDir ok, filename = " << request->filename() << ", cost "
              << expiredTime.ExpiredMs() << " ms";
  }
  return;
}
...
void NameSpaceService::IncreaseFileEpoch(
  ...
  LOG(INFO) << "logid = " << cntl->log_id()
            << ", IncreaseFileEpoch request, filename = "
            << request->filename();
  ...
  if (retCode != StatusCode::kOK) {
    ...
    response->set_statuscode(StatusCode::kOK);
    LOG(INFO) << "logid = " << cntl->log_id()
              << ", IncreaseFileEpoch ok, filename = " << request->filename()
              << ", cost " << expiredTime.ExpiredMs() << " ms";
  }
  return;
}
...
void NameSpaceService::CreateSnapShot(
  ...
  LOG(INFO) << "logid = " << cntl->log_id()
            << ", CreateSnapShot request, filename = " << request->filename();
...
void NameSpaceService::ListSnapShot(
  ...
  LOG(INFO) << "logid = " << cntl->log_id()
            << ", ListSnapShot request, filename = " << request->filename();
  ...
  retCode = kCurveFS.ListSnapShotFile(request->filename(), &snapShotFiles);
  ...
  if (retCode == StatusCode::kOK) {
    ...
    for (int i = 0; i != size; i++) {
      ...
      auto iter = std::find_if(
          snapShotFiles.begin(), snapShotFiles.end(),
...
void NameSpaceService::DeleteSnapShot(
  ...
  LOG(INFO) << "logid = " << cntl->log_id()
            << ", DeleteSnapShot request, filename = " << request->filename()
            << ", seq = " << request->seq();
  ...
  retCode = kCurveFS.DeleteFileSnapShotFile(request->filename(), request->seq(),
                                            nullptr);
  ...
  if (retCode != StatusCode::kOK) {
    ...
    if (google::ERROR != GetMdsLogLevel(retCode)) {
      ...
      LOG(ERROR) << "logid = " << cntl->log_id()
                 ...
                 << ", seq = " << request->seq() << ", statusCode = " << retCode
                 << ", StatusCode_Name = " << StatusCode_Name(retCode)
                 << ", cost " << expiredTime.ExpiredMs() << " ms";
    }
    return;
...
void NameSpaceService::CheckSnapShotStatus(
  ...
  uint32_t progress;
  retCode = kCurveFS.CheckSnapShotFileStatus(
      request->filename(), request->seq(), &fileStatus, &progress);
  if (retCode != StatusCode::kOK) {
    response->set_statuscode(retCode);
    if (google::ERROR != GetMdsLogLevel(retCode)) {
      LOG(WARNING) << "logid = " << cntl->log_id()
...
void NameSpaceService::GetSnapShotFileSegment(
  ...
  if (!request->has_seqnum()) {
    response->set_statuscode(StatusCode::kParaError);
    LOG(ERROR) << "logid = " << cntl->log_id()
               << ", GetSnapShotFileSegment, filename = " << request->filename()
               << ", seqnum not found";
    return;
  }
  ...
  retCode = kCurveFS.GetSnapShotFileSegment(
      request->filename(), request->seqnum(), request->offset(),
      response->mutable_pagefilesegment());
  if (retCode != StatusCode::kOK) {
    response->set_statuscode(retCode);
    if (google::ERROR != GetMdsLogLevel(retCode)) {
...
void NameSpaceService::OpenFile(::google::protobuf::RpcController* controller,
  ...
  retCode = kCurveFS.OpenFile(request->filename(), clientIP, protoSession,
                              fileInfo, cloneSourceSegment);
  if (retCode != StatusCode::kOK) {
    response->set_statuscode(retCode);
    if (google::ERROR != GetMdsLogLevel(retCode)) {
      LOG(WARNING) << "logid = " << cntl->log_id()
                   << ", OpenFile fail, filename = " << request->filename()
                   << ", clientip = " << clientIP
                   << ", clientport = " << clientPort
                   << ", statusCode = " << retCode
                   << ", StatusCode_Name = " << StatusCode_Name(retCode)
...
void NameSpaceService::RefreshSession(
  ...
  retCode = kCurveFS.RefreshSession(
      request->filename(), request->sessionid(), request->date(),
      request->signature(),
      request->has_clientip() ? request->clientip() : clientIP,
      request->has_clientport() ? request->clientport() : kInvalidPort,
      clientVersion, fileInfo);
  if (retCode != StatusCode::kOK) {
    response->set_statuscode(retCode);
    response->set_sessionid(request->sessionid());
    if (google::ERROR != GetMdsLogLevel(retCode)) {
      LOG(WARNING) << "logid = " << cntl->log_id()
                   << ", RefreshSession fail, filename = "
                   << request->filename()
                   << ", sessionid = " << request->sessionid()
                   << ", date = " << request->date()
                   << ", signature = " << request->signature()
...
void NameSpaceService::CreateCloneFile(
  ...
  LOG(INFO) << "logid = " << cntl->log_id()
            ...
            << ", filetype = " << request->filetype()
            << ", filelength = " << request->filelength()
            << ", seq = " << request->seq() << ", owner = " << request->owner()
  ...
  // check authority
  StatusCode ret = kCurveFS.CheckPathOwner(
      request->filename(), request->owner(), signature, request->date());
  ...
  // create clone file
  ret = kCurveFS.CreateCloneFile(request->filename(), request->owner(),
                                 request->filetype(), request->filelength(),
                                 request->seq(), request->chunksize(),
                                 request->stripeunit(), request->stripecount(),
                                 response->mutable_fileinfo(),
                                 request->clonesource(), request->filelength());
  response->set_statuscode(ret);
  if (ret != StatusCode::kOK) {
    if (google::ERROR != GetMdsLogLevel(ret)) {
      LOG(WARNING) << "logid = " << cntl->log_id()
                   << ", CreateCloneFile fail, filename = "
                   << request->filename() << ", statusCode = " << ret
      ...
      LOG(ERROR) << "logid = " << cntl->log_id()
                 ...
                 << ", statusCode = " << ret
                 << ", StatusCode_Name = " << StatusCode_Name(ret) << ", cost "
                 << expiredTime.ExpiredMs() << " ms";
    }
  } else {
    LOG(INFO) << "logid = " << cntl->log_id()
...
void NameSpaceService::SetCloneFileStatus(
  ...
  LOG(INFO) << "logid = " << cntl->log_id()
            << ", SetCloneFileStatus request, filename = "
            << request->filename()
  ...
  ret = kCurveFS.SetCloneFileStatus(request->filename(), fileID,
                                    request->filestatus());
  response->set_statuscode(ret);
  if (ret != StatusCode::kOK) {
    if (google::ERROR != GetMdsLogLevel(ret)) {
...
void NameSpaceService::ListClient(
  ...
  bool listAllClient = request->has_listallclient() && request->listallclient();
  ...
  if (retCode != StatusCode::kOK) {
    ...
    LOG(INFO) << "logid = " << cntl->log_id() << ", ListClient ok, "
              << ", return " << response->clientinfos_size() << " client infos"
              << ", cost " << expiredTime.ExpiredMs() << " ms";
  }
}
...
void NameSpaceService::FindFileMountPoint(
  ...
  retCode = kCurveFS.FindFileMountPoint(request->filename(), &infos);
  ...
  if (retCode != StatusCode::kOK) {
    ...
    LOG(INFO) << "logid = " << cntl->log_id()
              << ", FindFileMountPoint ok, fileName = " << request->filename()
              << ", cost " << expiredTime.ExpiredMs() << " ms";
  }
  return;
}
...
void NameSpaceService::ListVolumesOnCopysets(
  ...
  if (retCode != StatusCode::kOK) {
    response->set_statuscode(retCode);
    LOG(ERROR) << "logid = " << cntl->log_id()
               << ", ListAllVolumesOnCopysets fail, statusCode = " << retCode
               << ", StatusCode_Name = " << StatusCode_Name(retCode);
    return;
  }
  response->set_statuscode(StatusCode::kOK);
