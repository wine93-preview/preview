...
using Cache = ::curve::common::LRUCacheInterface<std::string, std::string>;
...
class NameServerStorage {
 public:
   ...
   * @return StoreStatus: error code
   */
  virtual StoreStatus GetFile(InodeID id, const std::string& filename,
                              FileInfo* fileInfo) = 0;
   ...
   * @return StoreStatus: error code
   */
  virtual StoreStatus DeleteFile(InodeID id, const std::string& filename) = 0;
   ...
   * @return StoreStaus: error code
   */
  virtual StoreStatus MoveFileToRecycle(const FileInfo& originFileInfo,
   ...
   * @return StoreStatus: error code
   */
  virtual StoreStatus ListFile(InodeID startid, InodeID endid,
   ...
   * @return StoreStatus: error code
   */
  virtual StoreStatus ListSnapshotFile(InodeID startid, InodeID endid,
                                       std::vector<FileInfo>* files) = 0;
   ...
   * @return StoreStatus: error code
   */
  virtual StoreStatus GetSegment(InodeID id, uint64_t off,
   ...
   * @return StoreStatus: error code
   */
  virtual StoreStatus PutSegment(InodeID id, uint64_t off,
   ...
   * @return StoreStatus: error code
   */
  virtual StoreStatus DeleteSegment(InodeID id, uint64_t off,
                                    int64_t* revision) = 0;
...
class NameServerStorageImp : public NameServerStorage {
 public:
  explicit NameServerStorageImp(std::shared_ptr<KVStorageClient> client,
  ...
  StoreStatus CleanDiscardSegment(uint64_t segmentSize, const std::string& key,
                                  int64_t* revision) override;
 ...
 private:
  ...
  StoreStatus GetStoreKey(FileType filetype, InodeID id,
                          const std::string& filename, std::string* storekey);
