...
bool CurveFS::InitRecycleBinDir() {
  ...
  StoreStatus ret =
      storage_->GetFile(ROOTINODEID, RECYCLEBINDIR, &recyclebinFileInfo);
  if (ret == StoreStatus::OK) {
    ...
    if (ret2 != StoreStatus::OK) {
      LOG(ERROR) << "RecycleBin dir create error, Path = " << RECYCLEBINDIR;
      return false;
    }
...
void CurveFS::Run() { fileRecordManager_->Start(); }
...
StatusCode CurveFS::WalkPath(const std::string& fileName, FileInfo* fileInfo,
...
StatusCode CurveFS::CreateFile(const std::string& fileName,
                               const std::string& owner, FileType filetype,
                               uint64_t length, uint64_t stripeUnit,
  ...
  if (allRemianingSpace < length) {
    LOG(ERROR) << "CreateFile file length > LeftSize, fileName = " << fileName
               << ", length = " << length;
    return StatusCode::kFileLengthNotSupported;
  }
...
StatusCode CurveFS::GetRecoverFileInfo(const std::string& originFileName,
  ...
  if (retCode != StatusCode::kOK) {
    ...
    for (auto iter = fileInfoList.begin(); iter != fileInfoList.end(); ++iter) {
...
StatusCode CurveFS::GetDirAllocSize(const std::string& fileName,
  ...
  for (const auto& file : files) {
    ...
    if (GetAllocatedSize(fullPathName, file, &size) != 0) {
      std::cout << "Get allocated size of " << fullPathName << " fail!"
                << std::endl;
      continue;
    }
    *allocSize += size;
...
StatusCode CurveFS::GetFileSize(const std::string& fileName, uint64_t* size) {
  ...
  *size = 0;
  ...
  if (fileInfo.filetype() != curve::mds::FileType::INODE_DIRECTORY &&
      fileInfo.filetype() != curve::mds::FileType::INODE_PAGEFILE) {
    LOG(ERROR) << "GetFileSize not support file type : " << fileInfo.filetype()
               << ", fileName = " << fileName;
    return StatusCode::kNotSupported;
  }
...
StatusCode CurveFS::GetFileSize(const std::string& fileName,
                                const FileInfo& fileInfo, uint64_t* fileSize) {
  ...
  switch (fileInfo.filetype()) {
    ...
    default: {
      LOG(ERROR) << "Get file size of type "
                 << FileType_Name(fileInfo.filetype()) << " not supported";
      return StatusCode::kNotSupported;
    }
  }
...
StatusCode CurveFS::isDirectoryEmpty(const FileInfo& fileInfo, bool* result) {
  ...
  std::vector<FileInfo> fileInfoList;
  auto storeStatus =
      storage_->ListFile(fileInfo.id(), fileInfo.id() + 1, &fileInfoList);
...
StatusCode CurveFS::IsSnapshotAllowed(const std::string& fileName) {
  ...
  bool exist =
      fileRecordManager_->GetMinimumFileClientVersion(fileName, &clientVersion);
  if (!exist) {
    return StatusCode::kOK;
  }
...
StatusCode CurveFS::DeleteFile(const std::string& filename, uint64_t fileId,
  ...
  if (fileInfo.filetype() == FileType::INODE_DIRECTORY) {
    ...
    if (ret1 != StatusCode::kOK) {
      LOG(ERROR) << "check is directory empty fail, filename = " << filename
                 << ", ret = " << ret1;
      return ret1;
    }
    if (!isEmpty) {
    ...
    auto ret = storage_->DeleteFile(fileInfo.parentid(), fileInfo.filename());
    if (ret != StoreStatus::OK) {
    ...
    if (ret == StatusCode::kDeleteFileBeingCloned) {
      bool isHasCloneRely;
      StatusCode ret1 =
          CheckHasCloneRely(filename, fileInfo.owner(), &isHasCloneRely);
      if (ret1 != StatusCode::kOK) {
    ...
    if (deleteForce == false) {
      ...
      StoreStatus ret1 = storage_->MoveFileToRecycle(fileInfo, recycleFileInfo);
      ...
      if (fileInfo.parentid() != RECYCLEBININODEID) {
        LOG(WARNING) << "force delete file not in recyclebin"
      ...
      if (!cleanManager_->SubmitDeleteCommonFileJob(fileInfo)) {
        LOG(ERROR) << "fileName = " << filename << ", inode = " << fileInfo.id()
                   << ", submit delete file job fail.";
        return StatusCode::KInternalError;
      }

      LOG(INFO) << "delete file task submitted, file is pagefile"
                << ", inode = " << fileInfo.id() << ", filename = " << filename;
      return StatusCode::kOK;
    }
  } else {
...
StatusCode CurveFS::RecoverFile(const std::string& originFileName,
  ...
  if (ret != StatusCode::kOK) {
    LOG(WARNING) << "WalkPath failed, the middle path of " << originFileName
                 << " is not exist";
    return ret;
  }
  ...
  if (ret != StatusCode::kOK) {
    LOG(INFO) << "get recycle file error, recycleFilename = " << recycleFileName
              << ", errCode = " << ret;
    return ret;
  }
  ...
  switch (recycleFileInfo.filestatus()) {
    ...
    case FileStatus::kFileCloneMetaInstalled:
      LOG(ERROR) << "recover fail, can not recover file "
                 << "cloneMetaInstalled, recycleFileName = " << recycleFileName;
...
StatusCode CurveFS::IncreaseFileEpoch(
  ...
  if (ret != StatusCode::kOK) {
    ...
    std::vector<ChunkServerIdType> chunkserverlist =
        topology_->GetChunkServerInCluster([](const ChunkServer& cs) {
...
StatusCode CurveFS::ReadDir(const std::string& dirname,
  ...
  if (storage_->ListFile(fileInfo.id(), fileInfo.id() + 1, files) !=
      StoreStatus::OK) {
    return StatusCode::kStorageError;
  }
  return StatusCode::kOK;
...
StatusCode CurveFS::CheckFileCanChange(const std::string& fileName,
  ...
  if (fileRecordManager_->FindFileMountPoint(fileName, &endPoints) &&
      !endPoints.empty()) {
    LOG(WARNING) << fileName << " has " << endPoints.size() << " mount points";
...
StatusCode CurveFS::RenameFile(const std::string& sourceFileName,
  ...
  if (ret != StatusCode::kOK) {
    LOG(ERROR) << "rename fail, can not rename file"
               << ", sourceFileName = " << sourceFileName << ", ret = " << ret;
    return ret;
  }
  ...
  if (ret3 == StatusCode::kOK) {
    ...
    if (existDestFileInfo.filetype() != FileType::INODE_PAGEFILE) {
      LOG(ERROR) << "rename sourceFileName = " << sourceFileName
                 ...
                 << sourceFileInfo.filetype()
                 << ", new fileType = " << existDestFileInfo.filetype();
      return StatusCode::kFileExists;
    }
    ...
    if (ret != StatusCode::kOK) {
      LOG(ERROR) << "cannot rename file"
                 << ", destFileName = " << destFileName << ", ret = " << ret;
      return ret;
    }
    ...
    auto ret1 = storage_->ReplaceFileAndRecycleOldFile(
        sourceFileInfo, destFileInfo, existDestFileInfo, recycleFileInfo);
    if (ret1 != StoreStatus::OK) {
      LOG(ERROR) << "storage_ ReplaceFileAndRecycleOldFile error"
                 << ", sourceFileName = " << sourceFileName
                 << ", destFileName = " << destFileName << ", ret = " << ret1;
...
StatusCode CurveFS::ExtendFile(const std::string& filename,
  ...
  if (newLength < fileInfo.length()) {
    LOG(INFO) << "newLength = " << newLength << ", smaller than file length "
              << fileInfo.length();
    ...
    if (fileInfo.has_throttleparams() &&
      ...
      fileInfo.set_allocated_throttleparams(
          new FileThrottleParams(GenerateDefaultThrottleParams(newLength)));
    }
  ...
  if (storeRet == StoreStatus::OK) {
    ...
    if (allocateIfNoExist == false) {
      ...
      auto ifok = chunkSegAllocator_->AllocateChunkSegment(
          fileInfo.filetype(), fileInfo.segmentsize(), fileInfo.chunksize(),
          offset, segment);
      if (ifok == false) {
      ...
      if (storage_->PutSegment(fileInfo.id(), offset, segment, &revision) !=
          StoreStatus::OK) {
        LOG(ERROR) << "PutSegment fail, fileInfo.id() = " << fileInfo.id()
                   << ", offset = " << offset;
        return StatusCode::kStorageError;
      }
      ...
      LOG(INFO) << "alloc segment success, fileInfo.id() = " << fileInfo.id()
                << ", offset = " << offset;
      return StatusCode::kOK;
    }
  } else {
...
StatusCode CurveFS::DeAllocateSegment(const std::string& fileName,
  ...
  if (storeRet != StoreStatus::OK) {
    LOG(WARNING) << "Storage CleanSegment return error, filename = " << fileName
                 << ", offset = " << offset << ", error = " << storeRet;
    return StatusCode::kStorageError;
  }
...
StatusCode CurveFS::CreateSnapShotFile(const std::string& fileName,
  ...
  if (snapShotFiles.size() != 0) {
    LOG(INFO) << fileName
              << " exist snapshotfile, num = " << snapShotFiles.size()
...
StatusCode CurveFS::ListSnapShotFile(
    const std::string& fileName,
  ...
  // list snapshot files
  auto storeStatus = storage_->ListSnapshotFile(
      fileInfo.id(), fileInfo.id() + 1, snapshotFileInfos);
  if (storeStatus == StoreStatus::KeyNotExist ||
...
StatusCode CurveFS::DeleteFileSnapShotFile(
    const std::string& fileName, FileSeqType seq,
  ...
  if (!cleanManager_->SubmitDeleteSnapShotFileJob(snapShotFileInfo, entity)) {
    LOG(ERROR) << "fileName = " << fileName << ", seq = " << seq
...
StatusCode CurveFS::CheckSnapShotFileStatus(const std::string& fileName,
  ...
  StatusCode ret = GetSnapShotFileInfo(fileName, seq, &snapShotFileInfo);
  if (ret != StatusCode::kOK) {
    LOG(WARNING) << "GetSnapShotFileInfo file fail, fileName = " << fileName
                 << ", seq = " << seq << ", ret = " << ret;
    return ret;
  }
  ...
  *status = snapShotFileInfo.filestatus();
  if (snapShotFileInfo.filestatus() == FileStatus::kFileDeleting) {
    ...
    if (task == nullptr) {
      ...
      StatusCode ret2 = GetSnapShotFileInfo(fileName, seq, &snapShotFileInfo);
      ...
      if (StatusCode::kSnapshotFileNotExists == ret2) {
        ...
        *progress = 0;
        LOG(ERROR) << "snapshot file delete fail, fileName = " << fileName
                   << ", seq = " << seq;
...
StatusCode CurveFS::GetSnapShotFileSegment(const std::string& fileName,
                                           FileSeqType seq, offset_t offset,
                                           PageFileSegment* segment) {
  assert(segment != nullptr);
  ...
  if (offset % snapShotFileInfo.segmentsize() != 0) {
    LOG(WARNING) << "offset not align with segment, fileName = " << fileName
                 << ", seq = " << seq;
    return StatusCode::kParaError;
  }

  if (offset + snapShotFileInfo.segmentsize() > snapShotFileInfo.length()) {
    LOG(WARNING) << "bigger than file length, fileName = " << fileName
                 << ", seq = " << seq;
    return StatusCode::kParaError;
  }
  ...
  if (ret1 != StatusCode::kOK) {
    LOG(ERROR) << "get origin file error, fileName = " << fileName
               << ", errCode = " << ret1;
    return ret1;
  }
  ...
  if (offset + fileInfo.segmentsize() > fileInfo.length()) {
    LOG(WARNING) << "bigger than origin file length, fileName = " << fileName
                 << ", offset = " << offset
                 << ", file segmentsize = " << fileInfo.segmentsize()
  ...
  if (storeRet == StoreStatus::OK) {
    ...
    LOG(INFO) << "get segment fail, kSegmentNotAllocated, fileName = "
              << fileName << ", fileInfo.id() = " << fileInfo.id()
              << ", offset = " << offset;
    return StatusCode::kSegmentNotAllocated;
  } else {
...
StatusCode CurveFS::CloseFile(const std::string& fileName,
  ...
  if (ret == StatusCode::kFileNotExists) {
    LOG(WARNING) << "CloseFile file not exist, fileName = " << fileName
                 << ", sessionID = " << sessionID << ", errCode = " << ret
                 << ", errName = " << StatusCode_Name(ret);
    return ret;
  } else if (ret != StatusCode::kOK) {
...
StatusCode CurveFS::RefreshSession(
    const std::string& fileName, const std::string& sessionid,
    const uint64_t date, const std::string& signature,
    const std::string& clientIP, uint32_t clientPort,
    const std::string& clientVersion, FileInfo* fileInfo) {
  // check the existence of the file
  StatusCode ret;
  ret = GetFileInfo(fileName, fileInfo);
  if (ret == StatusCode::kFileNotExists) {
    ...
    LOG(ERROR) << "RefreshSession get file info error, fileName = " << fileName
               << ", sessionid = " << sessionid << ", date = " << date
               << ", signature = " << signature << ", clientIP = " << clientIP
               << ", clientPort = " << clientPort << ", errCode = " << ret
               << ", errName = " << StatusCode_Name(ret);
    return ret;
  }
...
StatusCode CurveFS::CreateCloneFile(
    const std::string& fileName, const std::string& owner, FileType filetype,
    uint64_t length, FileSeqType seq, ChunkSizeType chunksize,
    uint64_t stripeUnit, uint64_t stripeCount, FileInfo* retFileInfo,
    const std::string& cloneSource, uint64_t cloneLength) {
  // check basic params
  if (filetype != FileType::INODE_PAGEFILE) {
...
StatusCode CurveFS::SetCloneFileStatus(const std::string& filename,
                                       uint64_t fileID, FileStatus fileStatus) {
  std::string lastEntry;
  FileInfo parentFileInfo;
  auto ret = WalkPath(filename, &parentFileInfo, &lastEntry);
  if (ret != StatusCode::kOK) {
    ...
    switch (fileInfo.filestatus()) {
      ...
      case kFileCloneMetaInstalled:
        ...
        break;
      case kFileCloned:
        if (fileStatus == kFileCloned || fileStatus == kFileBeingCloned) {
          // noop
        } else {
          return kCloneStatusNotMatch;
        }
        break;
      case kFileCreated:
        if (fileStatus != kFileCreated && fileStatus != kFileBeingCloned) {
          return kCloneStatusNotMatch;
        }
        break;
      case kFileBeingCloned:
        if (fileStatus != kFileBeingCloned && fileStatus != kFileCreated &&
  ...
  for (uint32_t i = 0; i < paths.size() - 1; i++) {
    ...
    if (ret == StoreStatus::OK) {
      ...
      if (fileInfo.owner() != owner) {
        LOG(ERROR) << fileInfo.filename() << " auth fail, owner = " << owner;
        return StatusCode::kOwnerAuthFail;
      }
    } else if (ret == StoreStatus::KeyNotExist) {
...
StatusCode CurveFS::CheckDestinationOwner(const std::string& filename,
  ...
  if (owner == GetRootOwner()) {
    ret = CheckSignature(owner, signature, date) ? StatusCode::kOK
  ...
  ret =
      CheckPathOwnerInternal(filename, owner, signature, &lastEntry, &parentID);
...
StatusCode CurveFS::CheckPathOwner(const std::string& filename,
  ...
  std::string lastEntry;
  uint64_t parentID;
  return CheckPathOwnerInternal(filename, owner, signature, &lastEntry,
                                &parentID);
}
...
StatusCode CurveFS::CheckEpoch(const std::string& filename, uint64_t epoch) {
  FileInfo fileInfo;
  auto ret = GetFileInfo(filename, &fileInfo);
  if (ret != StatusCode::kOK) {
    LOG(INFO) << "get source file error, errCode = " << ret;
  ...
  if (fileInfo.has_epoch() && fileInfo.epoch() > epoch) {
    LOG(ERROR) << "Check Epoch Failed, fileInfo.epoch: " << fileInfo.epoch()
               << ", epoch: " << epoch;
...
StatusCode CurveFS::CheckHasCloneRely(const std::string& filename,
  ...
  StatusCode ret = snapshotCloneClient_->GetCloneRefStatus(
      filename, owner, &refStatus, &fileCheckList);
  if (ret != StatusCode::kOK) {
    LOG(ERROR) << "delete file, check file clone ref fail,"
               << "filename = " << filename << ", ret = " << ret;
    return ret;
  ...
  if (refStatus == CloneRefStatus::kHasRef) {
    ...
    for (int i = 0; i < recordNum; i++) {
      FileInfo destFileInfo;
      StatusCode ret2 = GetFileInfo(fileCheckList[i].filename, &destFileInfo);
...
StatusCode CurveFS::ListCloneSourceFileSegments(
  ...
  if (ret != StatusCode::kOK) {
    LOG(ERROR) << "OpenFile failed, Get clone source file info failed, ret = "
               << StatusCode_Name(ret)
  ...
  if (segments.empty()) {
    ...
    for (const auto& segment : segments) {
      cloneSourceSegment->add_allocatedsegmentoffset(segment.startoffset());
    }
  }
...
StatusCode CurveFS::ListAllFiles(uint64_t inodeId,
  ...
  for (const auto& file : tempFiles) {
    if (file.filetype() == FileType::INODE_PAGEFILE) {
      ...
      if (ret == StatusCode::kOK) {
        files->insert(files->end(), tempFiles2.begin(), tempFiles2.end());
      } else {
...
StatusCode CurveFS::BuildEpochMap(
...
uint64_t CurveFS::GetDefaultChunkSize() { return defaultChunkSize_; }

uint64_t CurveFS::GetDefaultSegmentSize() { return defaultSegmentSize_; }

uint64_t CurveFS::GetMinFileLength() { return minFileLength_; }

uint64_t CurveFS::GetMaxFileLength() { return maxFileLength_; }

StatusCode CheckStripeParam(uint64_t segmentSize, uint64_t chunkSize,
  ...
  if (stripeCount > (segmentSize / chunkSize)) {
    LOG(WARNING) << "stripe count is too large, maximum is "
                 ...
                 << ", current stripe count: " << stripeCount;
    return StatusCode::kParaError;
  }
...
FileThrottleParams CurveFS::GenerateDefaultThrottleParams(
  ...
  uint64_t iopsLimit =
      common::Clamp<uint64_t>(length / kGB * throttleOption_.iopsPerGB,
                              throttleOption_.iopsMin, throttleOption_.iopsMax);
  iopsTotal.set_type(ThrottleType::IOPS_TOTAL);
  iopsTotal.set_limit(iopsLimit);

  ThrottleParams bpsTotal;
  uint64_t bpsLimit =
      common::Clamp<uint64_t>(length / kGB * throttleOption_.bpsPerGB,
                              throttleOption_.bpsMin, throttleOption_.bpsMax);
...
int ChunkServerRegistInfoBuilderImpl::BuildEpochMap(
    ::google::protobuf::Map<::google::protobuf::uint64,
                            ::google::protobuf::uint64>* epochMap) {
  if (cfs_ == nullptr) {
    return -1;
...
bvar::PassiveStatus<uint64_t> g_open_file_num_bvar(
    CURVE_MDS_CURVEFS_METRIC_PREFIX, "open_file_num", GetOpenFileNum,
    &kCurveFS);
}  // namespace mds
}  // namespace curve
