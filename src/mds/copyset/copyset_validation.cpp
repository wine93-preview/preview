...
bool CopysetValidation::Validate(const std::vector<Copyset>& copysets) const {
  ...
  if ((option_.scatterWidthVariance != 0) ||
    ...
    variance = StatisticsTools::CalcVariance(scatterWidthVec, average);
  }
  // check variance
  if (option_.scatterWidthVariance != 0 &&
      variance > option_.scatterWidthVariance) {
    LOG(WARNING) << "Validate copyset failed in scatterWidthVariance = "
                 << option_.scatterWidthVariance << ", actual = " << variance;
    return false;
  }

  // check standard deviation
  if (option_.scatterWidthStandardDevation != 0) {
    double standardDevation = StatisticsTools::CalcStandardDevation(variance);
  ...
  if ((option_.scatterWidthRange != 0) ||
    ...
    range = StatisticsTools::CalcRange(scatterWidthVec, &minValue, &maxValue);
  }

  // check range
  if ((option_.scatterWidthRange != 0) && (range > option_.scatterWidthRange)) {
    LOG(WARNING) << "Validate copyset failed in "
                 << "scatterWidthRange = " << option_.scatterWidthRange
                 << ", actual = " << range;
    return false;
  }
  ...
  if (option_.scatterWidthFloatingPercentage != 0) {
    ...
    if ((minPercent > option_.scatterWidthFloatingPercentage) ||
      ...
      LOG(WARNING) << "Validate copyset failed in "
                   ...
                   << ", maxValue = " << maxValue << ", average = " << average;
      return false;
    }
  }
  return true;
...
bool CopysetValidation::ValidateScatterWidth(
    uint32_t scatterWidth, uint32_t* scatterWidthOut,
...
void CopysetValidation::CalcScatterWidth(
...
double StatisticsTools::CalcStandardDevation(const std::vector<double>& values,
                                             double average) {
