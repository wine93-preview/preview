...
bool CopysetManager::GenCopyset(const ClusterInfo& cluster, int numCopysets,
  ...
  if (numCopysets != 0) {
    if (GenCopyset(cluster, numCopysets, out)) {
      if (validator_->ValidateScatterWidth(targetScatterWidth, scatterWidth,
                                           *out)) {
        return true;
      } else {
    ...
    policy_->GetMinCopySetFromScatterWidth(numChunkServers, targetScatterWidth,
                                           constrait_.replicaNum, &numCopysets);
...
bool CopysetManager::GenCopyset(const ClusterInfo& cluster, int numCopysets,
  ...
  while (retry < option_.copysetRetryTimes) {
    ...
    LOG(WARNING) << "Validate copyset metric failed, retry = " << retry;
    retry++;
  }
