...
void MDS::InitMdsOptions(std::shared_ptr<Configuration> conf) {
  ...
  conf_->GetValueFatalIfFail("mds.segment.alloc.retryInterMs",
                             &options_.retryInterTimes);
  conf_->GetValueFatalIfFail("mds.segment.alloc.periodic.persistInterMs",
  ...
  conf_->GetValueFatalIfFail("mds.dummy.listen.port",
                             &options_.dummyListenPort);

  conf_->GetValueFatalIfFail("mds.filelock.bucketNum",
                             &options_.mdsFilelockBucketNum);
}
...
void MDS::StartCompaginLeader() {
  ...
  int etcdTimeout;
  conf_->GetValueFatalIfFail("mds.etcd.operation.timeoutMs", &etcdTimeout);
  int etcdRetryTimes;
...
void MDS::Init() {
  ...
  fileLockManager_ = new FileLockManager(options_.mdsFilelockBucketNum);
  inited_ = true;
}
...
void MDS::InitEtcdConf(EtcdConf* etcdConf) {
  ...
  conf_->GetValueFatalIfFail("mds.etcd.dailtimeoutMs", &etcdConf->DialTimeout);
}
...
void MDS::InitEtcdClient(const EtcdConf& etcdConf, int etcdTimeout,
                         int retryTimes) {
  ...
  LOG_IF(FATAL,
         res != EtcdErrCode::EtcdOK && res != EtcdErrCode::EtcdKeyNotExist)
...
void MDS::InitMdsLeaderElectionOption(LeaderElectionOptions* electionOp) {
  conf_->GetValueFatalIfFail("mds.listen.addr", &electionOp->leaderUniqueName);
...
void MDS::InitTopologyOption(TopologyOption* topologyOption) {
  ...
  conf_->GetValueFatalIfFail("mds.topology.CreateCopysetRpcTimeoutMs",
                             &topologyOption->CreateCopysetRpcTimeoutMs);
  conf_->GetValueFatalIfFail("mds.topology.CreateCopysetRpcRetryTimes",
                             &topologyOption->CreateCopysetRpcRetryTimes);
  conf_->GetValueFatalIfFail("mds.topology.CreateCopysetRpcRetrySleepTimeMs",
  ...
  conf_->GetValueFatalIfFail("mds.topology.PoolUsagePercentLimit",
  ...
  conf_->GetValueFatalIfFail("mds.topology.enableLogicalPoolStatus",
...
void MDS::InitTopologyStat() {
  ...
  LOG_IF(FATAL, topologyStat_->Init() < 0) << "init topologyStat fail.";
  LOG(INFO) << "init topologyStat success.";
}

void MDS::InitTopologyMetricService(const TopologyOption& option) {
  topologyMetricService_ = std::make_shared<TopologyMetricService>(
      topology_, topologyStat_, segmentAllocStatistic_);
...
void MDS::InitTopologyServiceManager(const TopologyOption& option) {
  ...
  topologyServiceManager_ = std::make_shared<TopologyServiceManager>(
      topology_, topologyStat_, topologyChunkAllocator_, copysetManager,
      registerInfoBuilder);
...
void MDS::InitCopysetOption(CopysetOption* copysetOption) {
  ...
  conf_->GetValueFatalIfFail("mds.copyset.scatterWidthStandardDevation",
  ...
  conf_->GetValueFatalIfFail("mds.copyset.scatterWidthFloatingPercentage",
...
void MDS::InitTopologyChunkAllocator(const TopologyOption& option) {
  ...
  topologyChunkAllocator_ = std::make_shared<TopologyChunkAllocatorImpl>(
      topology_, segmentAllocStatistic_, topologyStat_, chunkFilePoolAllocHelp_,
      option);
...
void MDS::InitNameServerStorage(int mdsCacheCount) {
  ...
  auto cache = std::make_shared<LRUCache>(
      mdsCacheCount,
  ...
  nameServerStorage_ =
      std::make_shared<NameServerStorageImp>(etcdClient_, cache);
...
void MDS::InitCurveFS(const CurveFSOption& curveFSOptions) {
  ...
  auto chunkSegmentAllocate = std::make_shared<ChunkSegmentAllocatorImpl>(
  ...
  LOG_IF(FATAL, !kCurveFS.Init(nameServerStorage_, inodeIdGenerator,
                               ...
                               fileRecordManager, segmentAllocStatistic_,
                               curveFSOptions, topology_, snapshotCloneClient_))
...
void MDS::InitFileRecordOptions(FileRecordOptions* fileRecordOptions) {
  conf_->GetValueFatalIfFail("mds.file.expiredTimeUs",
                             &fileRecordOptions->fileRecordExpiredTimeUs);
  conf_->GetValueFatalIfFail("mds.file.scanIntevalTimeUs",
                             &fileRecordOptions->scanIntervalTimeUs);
}

void MDS::InitAuthOptions(RootAuthOption* authOptions) {
  conf_->GetValueFatalIfFail("mds.auth.rootUserName", &authOptions->rootOwner);
  conf_->GetValueFatalIfFail("mds.auth.rootPassword",
...
void MDS::InitCurveFSOptions(CurveFSOption* curveFSOptions) {
  conf_->GetValueFatalIfFail("mds.curvefs.defaultChunkSize",
                             &curveFSOptions->defaultChunkSize);
  conf_->GetValueFatalIfFail("mds.curvefs.defaultSegmentSize",
                             &curveFSOptions->defaultSegmentSize);
  conf_->GetValueFatalIfFail("mds.curvefs.minFileLength",
                             &curveFSOptions->minFileLength);
  conf_->GetValueFatalIfFail("mds.curvefs.maxFileLength",
                             &curveFSOptions->maxFileLength);
...
void MDS::InitDLockOption(std::shared_ptr<DLockOpts> dlockOpts) {
  conf_->GetValueFatalIfFail("mds.etcd.retry.times", &dlockOpts->retryTimes);
  conf_->GetValueFatalIfFail("mds.etcd.dlock.timeoutMs",
                             &dlockOpts->ctx_timeoutMS);
  conf_->GetValueFatalIfFail("mds.etcd.dlock.ttlSec", &dlockOpts->ttlSec);
}
...
void MDS::InitCleanManager() {
  ...
  auto copysetClient = std::make_shared<CopysetClient>(
      topology_, chunkServerClientOption, channelPool);

  auto cleanCore = std::make_shared<CleanCore>(
      nameServerStorage_, copysetClient, segmentAllocStatistic_);

  // init dlock options
  auto dlockOpts = std::make_shared<DLockOpts>();
  InitDLockOption(dlockOpts);

  cleanManager_ = std::make_shared<CleanManager>(cleanCore, taskManager,
                                                 nameServerStorage_);
