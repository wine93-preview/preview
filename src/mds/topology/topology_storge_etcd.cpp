  ...
  for (size_t i = 0; i < out.size(); i++) {
    ...
    if (!ret.second) {
      LOG(ERROR) << "LoadLogicalPool: "
                 << "LogicalPoolId duplicated, logicalPoolId = " << id;
      return false;
    }
...
bool TopologyStorageEtcd::LoadPhysicalPool(
  ...
  *maxPhysicalPoolId = 0;
  int errCode = client_->List(PHYSICALPOOLKEYPREFIX, PHYSICALPOOLKEYEND, &out);
  if (errCode == EtcdErrCode::EtcdKeyNotExist) {
    return true;
  }
  if (errCode != EtcdErrCode::EtcdOK) {
  ...
  for (size_t i = 0; i < out.size(); i++) {
    ...
    if (!ret.second) {
      LOG(ERROR) << "LoadPhysicalPool: "
                 << "PhysicalPoolId duplicated, PhysicalPoolId = " << id;
      return false;
    }
  ...
  for (size_t i = 0; i < out.size(); i++) {
    ...
    if (!ret.second) {
      LOG(ERROR) << "LoadChunkSever: "
                 << "ChunkSererId duplicated, ChunkServerId = " << id;
      return false;
    }
...
bool TopologyStorageEtcd::LoadCopySet(
  ...
  for (size_t i = 0; i < out.size(); i++) {
    ...
    auto ret = copySetMap->emplace(std::make_pair(lpid, id), std::move(data));
    if (!ret.second) {
      LOG(ERROR) << "LoadCopySet: "
                 << "Id duplicated, logicalPoolId = " << lpid
                 << ", copySetId = " << id;
      return false;
    }
...
bool TopologyStorageEtcd::DeleteLogicalPool(PoolIdType id) {
  ...
  if (errCode != EtcdErrCode::EtcdOK) {
    LOG(ERROR) << "delete LogicalPool from etcd err"
               << ", errcode = " << errCode << ", logicalPoolId = " << id;
    return false;
  }
  return true;
}
...
bool TopologyStorageEtcd::DeletePhysicalPool(PoolIdType id) {
  ...
  if (errCode != EtcdErrCode::EtcdOK) {
    LOG(ERROR) << "delete PhysicalPool from etcd err"
               << ", errcode = " << errCode << ", physicalPoolId = " << id;
    return false;
  }
  return true;
}
...
bool TopologyStorageEtcd::DeleteChunkServer(ChunkServerIdType id) {
  ...
  if (errCode != EtcdErrCode::EtcdOK) {
    LOG(ERROR) << "delete ChunkServer from etcd err"
               << ", errcode = " << errCode << ", chunkServerId = " << id;
    return false;
  }
  return true;
}
...
bool TopologyStorageEtcd::DeleteCopySet(CopySetKey key) {
  ...
  if (errCode != EtcdErrCode::EtcdOK) {
    LOG(ERROR) << "delete Copyset from etcd err"
               << ", errcode = " << errCode << ", logicalPoolId = " << key.first
