...
class LogicalPool {
 ...
 public:
  ...
  static bool TransUserPolicyFromJsonStr(const std::string& jsonStr,
 ...
 public:
  LogicalPool()
      : id_(UNINTIALIZE_ID),
        ...
        avaliable_(false),
        scanEnable_(true) {}
  LogicalPool(PoolIdType id, const std::string& name, PoolIdType phyPoolId,
              LogicalPoolType type, const RedundanceAndPlaceMentPolicy& rap,
              const UserPolicy& policy, uint64_t createTime, bool avaliable,
              bool scanEnable)
      : id_(id),
        name_(name),
  ...
  PoolIdType GetPhysicalPoolId() const { return physicalPoolId_; }

  LogicalPoolType GetLogicalPoolType() const { return type_; }
  ...
  void SetUserPolicy(const UserPolicy& policy) { policy_ = policy; }

  bool SetUserPolicyByJson(const std::string& jsonStr);

  UserPolicy GetUserPolicy() const { return policy_; }
  ...
  uint32_t GetScatterWidth() const { return initialScatterWidth_; }

  void SetStatus(AllocateStatus status) { status_ = status; }

  AllocateStatus GetStatus() const { return status_; }

  void SetLogicalPoolAvaliableFlag(bool avaliable) { avaliable_ = avaliable; }

  bool GetLogicalPoolAvaliableFlag() const { return avaliable_; }

  bool SerializeToString(std::string* value) const;

  bool ParseFromString(const std::string& value);

  void SetScanEnable(bool scanEnable) { scanEnable_ = scanEnable; }

  bool ScanEnable() const { return scanEnable_; }
...
class PhysicalPool {
 public:
  PhysicalPool()
      : id_(UNINTIALIZE_ID), name_(""), desc_(""), diskCapacity_(0) {}
  PhysicalPool(PoolIdType id, const std::string& name, const std::string& desc)
      : id_(id), name_(name), desc_(desc), diskCapacity_(0) {}
  ...
  void SetDiskCapacity(uint64_t diskCapacity) { diskCapacity_ = diskCapacity; }
 ...
 public:
  Zone()
      : id_(UNINTIALIZE_ID),
        ...
        physicalPoolId_(UNINTIALIZE_ID),
        desc_("") {}
  Zone(PoolIdType id, const std::string& name, PoolIdType physicalPoolId,
       const std::string& desc)
      : id_(id), name_(name), physicalPoolId_(physicalPoolId), desc_(desc) {}
...
class Server {
 public:
  ...
  Server(ServerIdType id, const std::string& hostName,
         ...
         ZoneIdType zoneId, PoolIdType physicalPoolId, const std::string& desc)
      : id_(id),
        hostName_(hostName),
  ...
  void AddChunkServer(ChunkServerIdType id) { chunkserverList_.push_back(id); }
  void RemoveChunkServer(ChunkServerIdType id) { chunkserverList_.remove(id); }
...
class ChunkServerState {
 public:
  ChunkServerState() : diskState_(DISKNORMAL), diskCapacity_(0), diskUsed_(0) {}

  void SetDiskState(DiskState state) { diskState_ = state; }
  DiskState GetDiskState() const { return diskState_; }
...
class ChunkServer {
 public:
  ...
  ChunkServer(ChunkServerIdType id, const std::string& token,
              const std::string& diskType, ServerIdType serverId,
              const std::string& hostIp, uint32_t port,
              const std::string& diskPath, ChunkServerStatus status = READWRITE,
              OnlineState onlineState = OnlineState::OFFLINE,
              const std::string& externalHostIp = "", std::string version = "")
      : id_(id),
        token_(token),
        diskType_(diskType),
        serverId_(serverId),
  ...
  ChunkServerIdType GetId() const { return id_; }
  std::string GetToken() const { return token_; }

  void SetToken(std::string token) { token_ = token; }

  std::string GetDiskType() const { return diskType_; }
  void SetServerId(ServerIdType id) { serverId_ = id; }
  ServerIdType GetServerId() const { return serverId_; }
  void SetHostIp(const std::string& ip) { internalHostIp_ = ip; }
  std::string GetHostIp() const { return internalHostIp_; }
  void SetExternalHostIp(const std::string& ip) { externalHostIp_ = ip; }
  std::string GetExternalHostIp() const { return externalHostIp_; }
  ...
  void SetStatus(ChunkServerStatus status) { status_ = status; }
  ChunkServerStatus GetStatus() const { return status_; }

  void SetOnlineState(OnlineState state) { onlineState_ = state; }
  OnlineState GetOnlineState() const { return onlineState_; }

  void SetChunkServerState(const ChunkServerState& state) { state_ = state; }
  ChunkServerState GetChunkServerState() const { return state_; }
...
class CopySetInfo {
 public:
  ...
  CopySetInfo(PoolIdType logicalPoolId, CopySetIdType id)
  ...
  PoolIdType GetLogicalPoolId() const { return logicalPoolId_; }
  ...
  ChunkServerIdType GetLeader() const { return leader_; }

  void SetLeader(ChunkServerIdType leader) { leader_ = leader; }

  CopySetKey GetCopySetKey() const {
    return CopySetKey(logicalPoolId_, copySetId_);
  }

  std::set<ChunkServerIdType> GetCopySetMembers() const { return peers_; }
  ...
  bool GetScaning() const { return scaning_; }
  ...
  LastScanSecType GetLastScanSec() const { return lastScanSec_; }

  void SetLastScanConsistent(bool lastScanConsistent) {
    lastScanConsistent_ = lastScanConsistent;
  }

  bool GetLastScanConsistent() const { return lastScanConsistent_; }
 ...
 * @return peerId
 */
inline std::string BuildPeerId(const std::string& ip, uint32_t port,
                               uint32_t idx = 0) {
