...
struct ChunkServerRegistInfoBuilder {
  ...
  virtual int BuildEpochMap(
      ::google::protobuf::Map< ::google::protobuf::uint64,
                               ::google::protobuf::uint64>* epochMap) = 0;
};
...
class TopologyServiceManager {
 public:
  ...
  virtual void Init(const TopologyOption& option) { option_ = option; }
  ...
  virtual void CreateZone(const ZoneRequest* request, ZoneResponse* response);

  virtual void DeleteZone(const ZoneRequest* request, ZoneResponse* response);

  virtual void GetZone(const ZoneRequest* request, ZoneResponse* response);
  ...
  virtual void GetCopySetsInCluster(const GetCopySetsInClusterRequest* request,
                                    GetCopySetsInClusterResponse* response);

  virtual void GetCopyset(const GetCopysetRequest* request,
                          GetCopysetResponse* response);

  virtual void GetClusterInfo(const GetClusterInfoRequest* request,
                              GetClusterInfoResponse* response);

  virtual void SetCopysetsAvailFlag(const SetCopysetsAvailFlagRequest* request,
                                    SetCopysetsAvailFlagResponse* response);

  virtual void ListUnAvailCopySets(const ListUnAvailCopySetsRequest* request,
 ...
 private:
   ...
   * @return error code
   */
  int CreateCopysetForLogicalPool(const LogicalPool& lPool,
   ...
   * @return error code
   */
  int GenCopysetForPageFilePool(const LogicalPool& lPool,
                                uint32_t* scatterWidth,
                                std::vector<CopySetInfo>* copysetInfos);
   ...
   * @return error code
   */
  int RemoveErrLogicalPoolAndCopyset(
   ...
   */
  void ConvertCopyset(const CopySetInfo& in, ::curve::common::CopysetInfo* out);
