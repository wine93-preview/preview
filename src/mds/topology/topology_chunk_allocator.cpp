...
bool TopologyChunkAllocatorImpl::ChooseSingleLogicalPool(
  ...
  if (ChoosePoolPolicy::kWeight == policy_) {
    return AllocateChunkPolicy::ChooseSingleLogicalPoolByWeight(poolWeightMap,
                                                                poolOut);
  } else {
...
void TopologyChunkAllocatorImpl::GetRemainingSpaceInLogicalPool(
  ...
  for (auto pid : logicalPools) {
    ...
    if (chunkFilePoolAllocHelp_->GetUseChunkFilepool()) {
      topoStat_->GetChunkPoolSize(lPool.GetPhysicalPoolId(), &diskCapacity);
      available = available * chunkFilePoolAllocHelp_->GetAvailable() / 100;
    ...
    uint64_t diskRemainning =
        (static_cast<int64_t>(diskCapacity) > alloc) ? diskCapacity - alloc : 0;
