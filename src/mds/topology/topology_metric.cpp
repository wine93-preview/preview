...
void TopologyMetricService::UpdateTopologyMetrics() {
  ...
  std::vector<ChunkServerIdType> chunkservers =
      topo_->GetChunkServerInCluster([](const ChunkServer& cs) {
  ...
  for (auto csId : chunkservers) {
    ...
    if (it == gChunkServerMetrics.end()) {
      ChunkServerMetricPtr cptr(new ChunkServerMetric(csId));
      it = gChunkServerMetrics.emplace(csId, std::move(cptr)).first;
    }
    ChunkServer cs;
    if (topo_->GetChunkServer(csId, &cs)) {
      it->second->diskCapacity.set_value(
          cs.GetChunkServerState().GetDiskCapacity());
      it->second->diskUsed.set_value(cs.GetChunkServerState().GetDiskUsed());
    }
    ChunkServerStat csStat;
    if (topoStat_->GetChunkServerStat(csId, &csStat)) {
      it->second->leaderCount.set_value(csStat.leaderCount);
      it->second->copysetCount.set_value(csStat.copysetCount);
      it->second->readRate.set_value(csStat.readRate);
      it->second->writeRate.set_value(csStat.writeRate);
      it->second->readIOPS.set_value(csStat.readIOPS);
      it->second->writeIOPS.set_value(csStat.writeIOPS);
      it->second->chunkSizeUsedBytes.set_value(csStat.chunkSizeUsedBytes);
      it->second->chunkSizeLeftBytes.set_value(csStat.chunkSizeLeftBytes);
      it->second->chunkSizeTrashedBytes.set_value(csStat.chunkSizeTrashedBytes);
      it->second->chunkSizeTotalBytes.set_value(csStat.chunkSizeUsedBytes +
  ...
  for (auto pid : lPools) {
    ...
    std::map<ChunkServerIdType, ChunkServerMetricInfo> chunkServerMetricInfo;
    ...
    if (it == gLogicalPoolMetrics.end()) {
      ...
      it = gLogicalPoolMetrics.emplace(pid, std::move(lptr)).first;
    }

    auto servers = topo_->GetServerInLogicalPool(pid);
    it->second->serverNum.set_value(servers.size());
    it->second->chunkServerNum.set_value(chunkServerMetricInfo.size());
    ...
    it->second->scatterWidthAvg.set_value(poolMetricInfo.scatterWidthAvg);
    ...
    it->second->scatterWidthRange.set_value(poolMetricInfo.scatterWidthRange);
    it->second->scatterWidthMin.set_value(poolMetricInfo.scatterWidthMin);
    it->second->scatterWidthMax.set_value(poolMetricInfo.scatterWidthMax);
    it->second->copysetNumAvg.set_value(poolMetricInfo.copysetNumAvg);
    it->second->copysetNumVariance.set_value(poolMetricInfo.copysetNumVariance);
    it->second->copysetNumStandardDeviation.set_value(
        poolMetricInfo.copysetNumStandardDeviation);
    it->second->copysetNumRange.set_value(poolMetricInfo.copysetNumRange);
    it->second->copysetNumMin.set_value(poolMetricInfo.copysetNumMin);
    it->second->copysetNumMax.set_value(poolMetricInfo.copysetNumMax);
    it->second->leaderNumAvg.set_value(poolMetricInfo.leaderNumAvg);
    it->second->leaderNumVariance.set_value(poolMetricInfo.leaderNumVariance);
    it->second->leaderNumStandardDeviation.set_value(
        poolMetricInfo.leaderNumStandardDeviation);
    it->second->leaderNumRange.set_value(poolMetricInfo.leaderNumRange);
    it->second->leaderNumMin.set_value(poolMetricInfo.leaderNumMin);
    it->second->leaderNumMax.set_value(poolMetricInfo.leaderNumMax);
    ...
    for (auto cm : chunkServerMetricInfo) {
      ...
      if (ix == gChunkServerMetrics.end()) {
        ChunkServerMetricPtr cptr(new ChunkServerMetric(cm.first));
        ix = gChunkServerMetrics.emplace(cm.first, std::move(cptr)).first;
      }
      ...
      totalChunkSizeUsedBytes += ix->second->chunkSizeUsedBytes.get_value();
      totalChunkSizeLeftBytes += ix->second->chunkSizeLeftBytes.get_value();
      totalChunkSizeTrashedBytes +=
          ix->second->chunkSizeTrashedBytes.get_value();
      totalChunkSizeBytes += ix->second->chunkSizeTotalBytes.get_value();
    }
    ...
    if (pool.GetReplicaNum() != 0) {
      it->second->logicalCapacity.set_value(totalChunkSizeBytes /
                                            pool.GetReplicaNum());
    }

    uint64_t readRate = 0, writeRate = 0, readIOPS = 0, writeIOPS = 0;
    for (auto iterCsStat : copysetStatsCluster) {
      ...
      DVLOG(6) << "copyset Metrics, csid is: " << iterCsStat.copysetId
               << ", write iops: " << iterCsStat.writeIOPS
               << ", write bps: " << iterCsStat.writeRate
               << ", read bps: " << iterCsStat.readRate
               << ", read iops: " << iterCsStat.readIOPS;
    }
    ...
    DVLOG(6) << "pool metrics, pid is: " << pid << ", write iops: " << writeIOPS
             << ", write bps: " << writeRate << ", read bps: " << readRate
             << ", read iops: " << readIOPS;
  }
  ...
  for (auto iy = gLogicalPoolMetrics.begin();
    ...
    if (std::find(lPools.begin(), lPools.end(), iy->first) == lPools.end()) {
  ...
  // cluster metric
  uint64_t clusterReadIOPS = 0, clusterWriteIOPS = 0, clusterReadRate = 0,
           clusterWriteRate = 0, copysetNum = 0;
  ...
  DVLOG(6) << "cluster metrics, logical pool num: "
           ...
           << ", cluster readiops: " << gClusterMetrics->readIOPS.get_value()
           << ", cluster writeiops: " << gClusterMetrics->writeIOPS.get_value()
           << ", cluster readbps: " << gClusterMetrics->readRate.get_value()
           << ", cluster writebps: " << gClusterMetrics->writeRate.get_value();
  return;
}
...
void TopologyMetricService::CalcChunkServerMetrics(
  ...
  for (auto& pair : *csMetricInfoMap) {
    ...
    for (auto& cs : copysets) {
      std::set<ChunkServerIdType> csMbs = cs.GetCopySetMembers();
...
void TopologyMetricService::CalcLogicalPoolMetrics(
  ...
  poolMetricInfo->scatterWidthVariance = StatisticsTools::CalcVariance(
      scatterWidthVec, poolMetricInfo->scatterWidthAvg);
  poolMetricInfo->scatterWidthStandardDeviation =
      StatisticsTools::CalcStandardDevation(
          poolMetricInfo->scatterWidthVariance);
  poolMetricInfo->scatterWidthRange = StatisticsTools::CalcRange(
      scatterWidthVec, &poolMetricInfo->scatterWidthMin,
      &poolMetricInfo->scatterWidthMax);

  poolMetricInfo->copysetNumAvg = StatisticsTools::CalcAverage(copysetNumVec);
  poolMetricInfo->copysetNumVariance = StatisticsTools::CalcVariance(
      copysetNumVec, poolMetricInfo->copysetNumAvg);
  poolMetricInfo->copysetNumStandardDeviation =
      StatisticsTools::CalcStandardDevation(poolMetricInfo->copysetNumVariance);
  poolMetricInfo->copysetNumRange =
      StatisticsTools::CalcRange(copysetNumVec, &poolMetricInfo->copysetNumMin,
                                 &poolMetricInfo->copysetNumMax);

  poolMetricInfo->leaderNumAvg = StatisticsTools::CalcAverage(leaderNumVec);
  poolMetricInfo->leaderNumVariance =
      StatisticsTools::CalcVariance(leaderNumVec, poolMetricInfo->leaderNumAvg);
  poolMetricInfo->leaderNumStandardDeviation =
      StatisticsTools::CalcStandardDevation(poolMetricInfo->leaderNumVariance);
  poolMetricInfo->leaderNumRange =
      StatisticsTools::CalcRange(leaderNumVec, &poolMetricInfo->leaderNumMin,
