...
void TopologyServiceManager::RegistChunkServer(
  ...
  if (chunkServerId == static_cast<ChunkServerIdType>(UNINTIALIZE_ID)) {
  ...
  ChunkServer chunkserver(chunkServerId, token, request->disktype(), serverId,
                          request->hostip(), request->port(),
                          request->diskpath(), READWRITE, ONLINE, externalIp);
...
void TopologyServiceManager::ListChunkServer(
    const ListChunkServerRequest* request, ListChunkServerResponse* response) {
  Server server;
  if (request->has_ip()) {
    uint32_t port = 0;
    if (request->has_port()) {
  ...
  for (ChunkServerIdType id : chunkserverList) {
    ...
    if (topology_->GetChunkServer(id, &cs)) {
      ...
      LOG(ERROR) << "Topology has counter an internal error: "
                 ...
                 << "[msg:] chunkserver not found, id = " << id;
      response->set_statuscode(kTopoErrCodeInternalError);
      return;
    }
  }
...
void TopologyServiceManager::GetChunkServer(
  ...
  if (request->has_chunkserverid()) {
    ...
    if (!topology_->GetChunkServerNotRetired(request->hostip(), request->port(),
                                             &cs)) {
      response->set_statuscode(kTopoErrCodeChunkServerNotFound);
      return;
    }
...
void TopologyServiceManager::SetChunkServer(
  ...
  bool find = topology_->GetChunkServer(request->chunkserverid(), &chunkserver);
...
void TopologyServiceManager::RegistServer(const ServerRegistRequest* request,
  ...
  if ((!request->has_hostname()) && (!request->has_internalip()) &&
      (!request->has_externalip()) && (!request->has_desc())) {
    response->set_statuscode(kTopoErrCodeInvalidParam);
    return;
  }
...
void TopologyServiceManager::GetServer(const GetServerRequest* request,
  ...
  if (!topology_->GetPhysicalPool(zone.GetPhysicalPoolId(), &pPool)) {
    LOG(ERROR) << "Topology has counter an internal error: "
               ...
               << zone.GetId()
               << " physicalPoolId = " << zone.GetPhysicalPoolId();
    response->set_statuscode(kTopoErrCodeInternalError);
    return;
  }
...
void TopologyServiceManager::ListZoneServer(
  ...
  if (request->has_zoneid()) {
    ...
    if (!topology_->GetZone(request->zonename(), request->physicalpoolname(),
                            &zone)) {
      response->set_statuscode(kTopoErrCodeZoneNotFound);
      return;
    }
...
void TopologyServiceManager::CreateZone(const ZoneRequest* request,
  ...
  if ((request->has_zonename()) && (request->has_physicalpoolname()) &&
      (request->has_desc())) {
    PhysicalPool pPool;
    if (!topology_->GetPhysicalPool(request->physicalpoolname(), &pPool)) {
      response->set_statuscode(kTopoErrCodePhysicalPoolNotFound);
    ...
    Zone zone(zid, request->zonename(), pPool.GetId(), request->desc());
...
void TopologyServiceManager::CreatePhysicalPool(
    const PhysicalPoolRequest* request, PhysicalPoolResponse* response) {
  if ((request->has_physicalpoolname()) && (request->has_desc())) {
    ...
    PhysicalPool pool(pid, request->physicalpoolname(), request->desc());
...
int TopologyServiceManager::CreateCopysetForLogicalPool(
    const LogicalPool& lPool, uint32_t* scatterWidth,
    std::vector<CopySetInfo>* copysetInfos) {
  switch (lPool.GetLogicalPoolType()) {
    case LogicalPoolType::PAGEFILE: {
      int errcode =
          GenCopysetForPageFilePool(lPool, scatterWidth, copysetInfos);
      if (kTopoErrCodeSuccess != errcode) {
...
int TopologyServiceManager::GenCopysetForPageFilePool(
    ...
    std::vector<CopySetInfo>* copysetInfos) {
  ClusterInfo cluster;
  std::list<ChunkServerIdType> csList = topology_->GetChunkServerInLogicalPool(
      lPool.GetId(), [](const ChunkServer& cs) {
        return cs.GetStatus() != ChunkServerStatus::RETIRED;
      });
  ...
  if (copysetManager_->Init(constrait)) {
    if (!copysetManager_->GenCopyset(cluster, rap.pageFileRAP.copysetNum,
                                     scatterWidth, &copysets)) {
      LOG(ERROR) << "GenCopysetForPageFilePool failed"
                 << ", Cluster size = " << cluster.GetClusterSize()
                 << ", copysetNum = " << rap.pageFileRAP.copysetNum
    ...
    LOG(ERROR) << "GenCopysetForPageFilePool invalid param :"
               << " zoneNum = " << rap.pageFileRAP.zoneNum
               << " replicaNum = " << rap.pageFileRAP.replicaNum
               << ", logicalPoolid = " << lPool.GetId();
    return kTopoErrCodeInvalidParam;
  }

  for (const Copyset& cs : copysets) {
    CopySetIdType copysetId = topology_->AllocateCopySetId(logicalPoolId);
    if (copysetId == static_cast<PoolIdType>(UNINTIALIZE_ID)) {
...
bool TopologyServiceManager::CreateCopysetNodeOnChunkServer(
    ChunkServerIdType id, const std::vector<CopySetInfo>& copysetInfos) {
  ChunkServer chunkServer;
  ...
  for (auto& cs : copysetInfos) {
    ::curve::chunkserver::Copyset* copyset = copysetRequest.add_copysets();
    ...
    for (ChunkServerIdType id : cs.GetCopySetMembers()) {
      ...
      std::string address =
          BuildPeerId(chunkserverInfo.GetHostIp(), chunkserverInfo.GetPort());
  ...
  do {
    ...
    stub.CreateCopysetNode2(&cntl, &copysetRequest, &copysetResponse, nullptr);
    LOG(INFO) << "Send CopysetRequest[log_id=" << cntl.log_id() << "] from "
    ...
    if (cntl.Failed()) {
      ...
      std::this_thread::sleep_for(
          std::chrono::milliseconds(option_.CreateCopysetRpcRetrySleepTimeMs));
    }
    retry++;
  } while (cntl.Failed() && retry < option_.CreateCopysetRpcRetryTimes);

  if (cntl.Failed()) {
    LOG(ERROR) << "Send CopysetRequest failed, retry times exceed, "
               << "cntl.errorText = " << cntl.ErrorText() << std::endl;
    return false;
  } else {
    if ((copysetResponse.status() !=
      ...
      LOG(ERROR) << "Received CopysetResponse[log_id=" << cntl.log_id()
                 << "] from " << cntl.remote_side() << " to "
                 << cntl.local_side() << ". [CopysetResponse] "
                 << copysetResponse.DebugString();
      return false;
    } else {
      LOG(INFO) << "Received CopysetResponse[log_id=" << cntl.log_id()
                << "] from " << cntl.remote_side() << " to "
                << cntl.local_side() << ". [CopysetResponse] "
                << copysetResponse.DebugString();
    }
...
int TopologyServiceManager::RemoveErrLogicalPoolAndCopyset(
    const LogicalPool& pool, const std::vector<CopySetInfo>* copysetInfos) {
  int errcode = kTopoErrCodeSuccess;
...
void TopologyServiceManager::CreateLogicalPool(
  ...
  auto lgPools = topology_->GetLogicalPoolInPhysicalPool(
      pPool.GetId(), [&](const LogicalPool& pool) {
        return pool.GetName() == request->logicalpoolname();
      });
  if (!lgPools.empty()) {
    LOG(WARNING) << "Logical pool " << request->logicalpoolname() << " exist";
    response->set_statuscode(kTopoErrCodeLogicalPoolExist);
    return;
  }

  PoolIdType lPoolId = topology_->AllocateLogicalPoolId();
  if (lPoolId == static_cast<PoolIdType>(UNINTIALIZE_ID)) {
    response->set_statuscode(kTopoErrCodeAllocateIdFail);
    return;
  }
  ...
  if (!LogicalPool::TransRedundanceAndPlaceMentPolicyFromJsonStr(
          request->redundanceandplacementpolicy(), request->type(), &rap)) {
  ...
  if (!LogicalPool::TransUserPolicyFromJsonStr(request->userpolicy(),
                                               request->type(), &userPolicy)) {
    LOG(ERROR) << "[TopologyServiceManager::CreateLogicalPool]:"
  ...
  if (kTopoErrCodeSuccess == errcode) {
    std::vector<CopySetInfo> copysetInfos;
    errcode = CreateCopysetForLogicalPool(lPool, &scatterWidth, &copysetInfos);
    if (kTopoErrCodeSuccess != errcode) {
      if (kTopoErrCodeSuccess ==
          RemoveErrLogicalPoolAndCopyset(lPool, &copysetInfos)) {
        response->set_statuscode(errcode);
      } else {
        LOG(ERROR) << "Topology has counter an internal error: "
...
void TopologyServiceManager::GetLogicalPool(
    const GetLogicalPoolRequest* request, GetLogicalPoolResponse* response) {
  LogicalPool lPool;
  if (request->has_logicalpoolid()) {
    ...
    if (!topology_->GetLogicalPool(request->logicalpoolname(),
                                   request->physicalpoolname(), &lPool)) {
      response->set_statuscode(kTopoErrCodeLogicalPoolNotFound);
      return;
    }
  } else {
...
void TopologyServiceManager::ListLogicalPool(
    const ListLogicalPoolRequest* request, ListLogicalPoolResponse* response) {
  PhysicalPool pPool;
  if (request->has_physicalpoolid()) {
    if (!topology_->GetPhysicalPool(request->physicalpoolid(), &pPool)) {
      response->set_statuscode(kTopoErrCodePhysicalPoolNotFound);
  ...
  for (PoolIdType id : lPoolIdList) {
    ...
    if (topology_->GetLogicalPool(id, &lPool)) {
      ...
      LOG(ERROR) << "Topology has counter an internal error: "
                 ...
                 << "[msg:] LogicalPool not found, id = " << id;
      response->set_statuscode(kTopoErrCodeInternalError);
      return;
    }
  }
...
void TopologyServiceManager::SetLogicalPool(
    const SetLogicalPoolRequest* request, SetLogicalPoolResponse* response) {
      ...
      for (ChunkServerIdType csId : csInfo.GetCopySetMembers()) {
        ...
        if (topology_->GetChunkServer(csId, &cs)) {
          ...
          LOG(ERROR) << "Topology has counter an internal error: "
                     ...
                     << "[msg:] ChunkServer not found, id = " << csId;
          response->set_statuscode(kTopoErrCodeInternalError);
          return;
        }
      }
...
void TopologyServiceManager::GetCopySetsInCluster(
  ...
  for (const auto& lpid : logicalPoolIds) {
    auto copysetInfos = topology_->GetCopySetInfosInLogicalPool(lpid, filter);
...
void TopologyServiceManager::GetClusterInfo(
    const GetClusterInfoRequest* request, GetClusterInfoResponse* response) {
  (void)request;
...
void TopologyServiceManager::ListUnAvailCopySets(
  ...
  (void)request;
  std::vector<CopySetKey> copysets = topology_->GetCopySetsInCluster();
  for (const CopySetKey& copyset : copysets) {
    CopySetInfo csInfo;
