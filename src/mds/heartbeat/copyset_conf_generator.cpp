...
bool CopysetConfGenerator::FollowerGenCopysetConf(
  ...
  // to what non-leader copy report,                //NOLINT copy(ies) can be
  // deleted according to the configuration of MDS
  ...
  if (recordCopySetInfo.GetEpoch() >= reportCopySetInfo.GetEpoch()) {
    ...
    // the initial config of a copyset is ABC, in which A is the leader, and
    // epoch = 8                                      //NOLINT
    // 1. MDS gereration an operator ABC+D and dispatch to A
    // 2. A installed snapshot to D successfully, and start copying journal
    // 3. When D is copying the journal, old config of the copyset is applied
    // (e.g. ABE, epoch = 5)                         //NOLINT
    // 4. MDS restart now and operator is lost
    // 5. After the MDS has restarted, it first receive heartbeat from
    // follower-id (ABE, epoch = 5).                        //NOLINT
    //    In normal cases (without delay cleaning), this copyset on D will be
    //    deleted since its epoch number is lower,      //NOLINT but it will be
    //    a problem in our case since D is already a member of the copyset, and
    //    the deletion of D means      //NOLINT the elimination of copies.
    // here's how delay cleaning works:
    //    In one way leader will report the existence of D, which promise that D
    //    will not be deleted by mistake. In another //NOLINT way, the
    //    configuration that D report will be up-to-date since the journal
    //    replay has completed                    //NOLINT
    ...
    // judge whether the reporting chunkserver is in the copyset it reported,
    // and whether this                             //NOLINT chunkserver is a
    // candidate or going to be added into the copyset
    ...
    // if the chunkserver doesn't belong to any of the copyset, MDS will
    // dispatch the configuration                        //NOLINT
    ...
    if (recordCopySetInfo.HasCandidate()) {
      std::string candidate =
          BuildPeerByChunkserverId(recordCopySetInfo.GetCandidate());
      if (candidate.empty()) {
        return false;
      }
...
std::string CopysetConfGenerator::BuildPeerByChunkserverId(
  ...
  return ::curve::mds::topology::BuildPeerId(chunkServer.GetHostIp(),
                                             chunkServer.GetPort(), 0);
}
}  // namespace heartbeat
}  // namespace mds
}  // namespace curve
