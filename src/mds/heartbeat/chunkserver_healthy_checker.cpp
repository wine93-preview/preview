...
bool ChunkserverHealthyChecker::ChunkServerStateNeedUpdate(
  ...
  steady_clock::duration timePass = steady_clock::now() - info.lastReceivedTime;

  bool shouldOnline = (timePass < milliseconds(option_.heartbeatMissTimeOutMs));
  if (shouldOnline) {
...
void ChunkserverHealthyChecker::UpdateLastReceivedHeartbeatTime(
  ...
  if (heartbeatInfos_.find(csId) == heartbeatInfos_.end()) {
    heartbeatInfos_.emplace(csId,
                            HeartbeatInfo(csId, time, OnlineState::UNSTABLE));
    return;
  }
  heartbeatInfos_[csId].lastReceivedTime = time;
}

bool ChunkserverHealthyChecker::GetHeartBeatInfo(ChunkServerIdType id,
                                                 HeartbeatInfo* info) {
...
bool ChunkserverHealthyChecker::TrySetChunkServerRetiredIfNeed(
  ...
  if (!topo_->GetChunkServer(info.csId, &cs)) {
    LOG(ERROR) << "heartbeatManager can not get chunkserver " << info.csId
               << " from topo";
    return false;
  }
  ...
  if (cs.GetStatus() == ChunkServerStatus::RETIRED) {
    LOG(INFO) << "chunkserver " << info.csId << " is already in retired state";
    return true;
  }
  ...
  int updateErrCode =
      topo_->UpdateChunkServerRwState(ChunkServerStatus::RETIRED, info.csId);
