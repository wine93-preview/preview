...
HeartbeatManager::HeartbeatManager(HeartbeatOption option,
    ...
    : topology_(topology), topologyStat_(topologyStat) {
  healthyChecker_ =
...
void HeartbeatManager::Run() {
  if (isStop_.exchange(false)) {
    backEndThread_ = Thread(&HeartbeatManager::ChunkServerHealthyChecker, this);
  }
}
...
void HeartbeatManager::UpdateChunkServerDiskStatus(
  ...
  int ret =
      topology_->UpdateChunkServerDiskStatus(state, request.chunkserverid());
  if (ret != curve::mds::topology::kTopoErrCodeSuccess) {
    LOG(ERROR) << "heartbeat UpdateDiskStatus get an error, ret =" << ret;
  }
}
...
void HeartbeatManager::UpdateChunkServerStatistics(
  ...
  if (request.has_stats()) {
    ...
    for (int i = 0; i < request.copysetinfos_size(); i++) {
      ...
      std::string leaderPeer = request.copysetinfos(i).leaderpeer().address();
      std::string leaderIp;
      uint32_t leaderPort;
      if (SplitPeerId(leaderPeer, &leaderIp, &leaderPort)) {
        cstat.leader =
            topology_->FindChunkServerNotRetired(leaderIp, leaderPort);
        if (UNINTIALIZE_ID == cstat.leader) {
          LOG(INFO) << "hearbeat receive from chunkserver(id:"
                    << request.chunkserverid() << ",ip:" << request.ip()
                    << ",port:" << request.port() << "), in which copyset("
                    << cstat.logicalPoolId << "," << cstat.copysetId
                    << ") dose not have leader.";
        }
      } else {
...
void HeartbeatManager::ChunkServerHeartbeat(
  ...
  if (request.has_starttime()) {
    topology_->UpdateChunkServerStartUpTime(request.starttime(),
  ...
  for (auto& value : request.copysetinfos()) {
    ...
    if (!FromHeartbeatCopySetInfoToTopologyOne(value, &reportCopySetInfo)) {
      LOG(ERROR) << "heartbeatManager receive copyset(" << value.logicalpoolid()
                 << "," << value.copysetid()
...
HeartbeatStatusCode HeartbeatManager::CheckRequest(
  ...
  if (chunkServer.GetStatus() == ChunkServerStatus::RETIRED) {
    LOG(ERROR) << "heartbeatManager receive heartbeat from"
               << "retired chunkserver: " << chunkServer.GetId() << ", reject.";
  ...
  if (request.ip() != chunkServer.GetHostIp() ||
    ...
    LOG(ERROR) << "heartbeatManager receive heartbeat from chunkServer: "
               ...
               << chunkServer.GetHostIp()
               << ", record port:" << chunkServer.GetPort();
...
bool HeartbeatManager::FromHeartbeatCopySetInfoToTopologyOne(
  ...
  if (leader == UNINTIALIZE_ID) {
    LOG(ERROR) << "leader not found, logicalpoolid: " << info.logicalpoolid()
...
void HeartbeatManager::UpdateChunkServerVersion(
  ...
  if (request.has_version()) {
    int ret = topology_->UpdateChunkServerVersion(request.version(),
                                                  request.chunkserverid());
    if (ret != 0) {
      LOG(ERROR) << "heartbeat UpdateChunkServerVersion failed, chunkServerId: "
                 << request.chunkserverid()
                 << ", version: " << request.version() << ", error is: " << ret;
    }
  }
}
