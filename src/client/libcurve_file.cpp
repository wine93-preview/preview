...
class LoggerGuard {
 private:
  ...
  explicit LoggerGuard(const std::string& confpath) { InitInternal(confpath); }
...
int FileClient::IncreaseEpoch(const std::string& filename,
  ...
  if (mdsClient_ != nullptr) {
    ret = mdsClient_->IncreaseEpoch(filename, userinfo, &fi, &fEpoch, &csLocs);
    LOG_IF(ERROR, ret != LIBCURVE_ERROR::OK)
        << "IncreaseEpoch failed, filename: " << filename << ", ret: " << ret;
  } else {
...
int FileClient::AioWrite(int fd, CurveAioContext* aioctx,
  ...
  if (CheckAligned(aioctx->offset, aioctx->length) == false) {
    LOG(ERROR) << "AioWrite request not aligned, length = " << aioctx->length
               << ", offset = " << aioctx->offset << ", fd = " << fd;
    return -LIBCURVE_ERROR::NOT_ALIGNED;
  }
...
int FileClient::Extend(const std::string& filename, const UserInfo_t& userinfo,
  ...
  if (mdsClient_ != nullptr) {
    ...
    LOG_IF(ERROR, ret != LIBCURVE_ERROR::OK)
        << "Extend failed, filename: " << filename << ", NewSize: " << newsize
        << ", ret: " << ret;
  } else {
    LOG(ERROR) << "global mds client not inited!";
    return -LIBCURVE_ERROR::FAILED;
...
int FileClient::Unlink(const std::string& filename, const UserInfo_t& userinfo,
  ...
  if (mdsClient_ != nullptr) {
    ...
    LOG_IF(ERROR, ret != LIBCURVE_ERROR::OK)
        << "Unlink failed, filename: " << filename << ", force: " << deleteforce
        << ", ret: " << ret;
  } else {
    LOG(ERROR) << "global mds client not inited!";
    return -LIBCURVE_ERROR::FAILED;
...
int FileClient::Listdir(const std::string& dirpath, const UserInfo_t& userinfo,
  ...
  if (mdsClient_ != nullptr) {
    ...
    LOG_IF(ERROR, ret != LIBCURVE_ERROR::OK && ret != LIBCURVE_ERROR::NOTEXIST)
  ...
  if (mdsClient_ != nullptr) {
    ...
    if (ret != LIBCURVE_ERROR::OK) {
      if (ret == LIBCURVE_ERROR::EXISTS) {
        ...
        LOG_IF(ERROR, ret != LIBCURVE_ERROR::OK)
            << "Create directory failed, dir: " << dirpath << ", ret: " << ret;
      }
    }
  } else {
    LOG(ERROR) << "global mds client not inited!";
...
int FileClient::Close(int fd) {
  ...
  if (ret == LIBCURVE_ERROR::OK || ret == -LIBCURVE_ERROR::SESSION_NOT_EXIST) {
    std::string filename =
...
int AioWrite(int fd, CurveAioContext* aioctx) {
  ...
  DVLOG(9) << "offset: " << aioctx->offset << " length: " << aioctx->length
           << " op: " << aioctx->op << " buf: " << *(unsigned int*)aioctx->buf;
...
int Listdir(DirInfo_t* dirinfo) {
  ...
  for (uint64_t i = 0; i < dirinfo->dirSize; i++) {
    ...
    memcpy(dirinfo->fileStat[i].filename, fileStat[i].filename, NAME_MAX_SIZE);
  }
