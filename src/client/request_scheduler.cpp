...
void RequestScheduler::ProcessAligned(RequestContext* ctx) {
  ...
  switch (ctx->optype_) {
    case OpType::READ:
      ...
      client_.ReadChunk(ctx->idinfo_, ctx->seq_, ctx->offset_, ctx->rawlength_,
                        ctx->appliedindex_, ctx->sourceInfo_, guard.release());
      break;
    case OpType::WRITE:
      ctx->done_->GetInflightRPCToken();
      client_.WriteChunk(ctx->idinfo_, ctx->fileId_, ctx->epoch_, ctx->seq_,
                         ctx->writeData_, ctx->offset_, ctx->rawlength_,
    ...
    case OpType::DELETE_SNAP:
      client_.DeleteChunkSnapshotOrCorrectSn(ctx->idinfo_, ctx->correctedSeq_,
                                             guard.release());
      break;
