...
int RequestSender::ReadChunk(const ChunkIDInfo& idinfo, uint64_t sn,
...
int RequestSender::WriteChunk(const ChunkIDInfo& idinfo, uint64_t fileId,
...
int RequestSender::ReadChunkSnapshot(const ChunkIDInfo& idinfo, uint64_t sn,
...
int RequestSender::CreateCloneChunk(const ChunkIDInfo& idinfo,
                                    ...
                                    uint64_t correntSn, uint64_t sn,
                                    uint64_t chunkSize) {
  brpc::ClosureGuard doneGuard(done);
  brpc::Controller* cntl = new brpc::Controller();
  ChunkResponse* response = new ChunkResponse();
  ...
  ChunkRequest request;
  request.set_optype(curve::chunkserver::CHUNK_OP_TYPE::CHUNK_OP_CREATE_CLONE);
  request.set_logicpoolid(idinfo.lpid_);
  request.set_copysetid(idinfo.cpid_);
  request.set_chunkid(idinfo.cid_);
...
int RequestSender::RecoverChunk(const ChunkIDInfo& idinfo, ClientClosure* done,
