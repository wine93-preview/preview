...
class MetaCache {
 public:
   ...
   */
  virtual void UpdateAppliedIndex(LogicPoolID logicPoolId, CopysetID copysetId,
   ...
   */
  virtual void AddCopysetIDInfo(ChunkServerID csid, const CopysetIDInfo& cpid);
  ...
  void SetLatestFileStatus(FileStatus status) { fileInfo_.filestatus = status; }
   ...
   */
  virtual bool IsLeaderMayChange(LogicPoolID logicpoolId, CopysetID copysetId);
