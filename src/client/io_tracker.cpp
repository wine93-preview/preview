...
void IOTracker::DoDiscard(MDSClient* mdsClient, const FInfo* fileInfo,
  ...
  if (ret == 0 && !discardSegments_.empty()) {
    for (auto index : discardSegments_) {
      ...
      bool ret = taskManager->ScheduleTask(index, mc_, mdsClient, abstime);
      std::ostringstream taskinfo;
      taskinfo << "filename = " << fileInfo->fullPathName
               << ", segment index = " << index
               << ", segment offset = " << index * GiB;
      if (!ret) {
        LOG(ERROR) << "Schedule discard task failed, " << taskinfo.str();
      } else {
  ...
  if (errcode_ == LIBCURVE_ERROR::OK) {
    ...
    if (OpType::READ == type_ || OpType::READ_SNAP == type_) {
      ...
      switch (userDataType_) {
        ...
        case UserDataType::IOBuffer: {
          butil::IOBuf* userData = reinterpret_cast<butil::IOBuf*>(data_);
          *userData = readData;
    ...
    if (type_ == OpType::READ || type_ == OpType::WRITE) {
      ...
      if (OpType::CREATE_CLONE == type_ && LIBCURVE_ERROR::EXISTS == errcode_) {
