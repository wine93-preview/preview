...
bool FileInstance::Initialize(const std::string& filename,
  ...
  do {
    ...
    if (!iomanager4file_.Initialize(filename, fileopt_.ioOpt,
                                    mdsclient_.get())) {
      LOG(ERROR) << "Init io context manager failed, filename = " << filename;
      break;
    }

    iomanager4file_.UpdateFileInfo(finfo_);

    leaseExecutor_.reset(new (std::nothrow)
                             LeaseExecutor(fileopt_.leaseOpt, finfo_.userinfo,
                                           mdsclient_.get(), &iomanager4file_));
    if (CURVE_UNLIKELY(leaseExecutor_ == nullptr)) {
      LOG(ERROR) << "Allocate LeaseExecutor failed, filename = " << filename;
      break;
    }
...
int FileInstance::Open(const std::string& filename, const UserInfo& userinfo,
  ...
  FileEpoch_t fEpoch;
  ret =
      mdsclient_->OpenFile(filename, finfo_.userinfo, &finfo_, &fEpoch, &lease);
...
FileInstance* FileInstance::NewInitedFileInstance(
  ...
  if (!ret) {
    LOG(ERROR) << "FileInstance initialize failed"
               << ", filename = " << filename << ", owner = " << userInfo.owner
               << ", readonly = " << readonly;
    delete instance;
    return nullptr;
  }
