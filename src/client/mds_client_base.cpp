...
void MDSClientBase::IncreaseEpoch(const std::string& filename,
  ...
  LOG(INFO) << "IncreaseEpoch, filename: " << filename
            << ", user: " << userinfo.owner << ", log id = " << cntl->log_id();
...
void MDSClientBase::GetSnapshotSegmentInfo(
    const std::string& filename, const UserInfo_t& userinfo, uint64_t seq,
    uint64_t offset, GetOrAllocateSegmentResponse* response,
    brpc::Controller* cntl, brpc::Channel* channel) {
...
void MDSClientBase::GetServerList(
    ...
    GetChunkServerListInCopySetsResponse* response, brpc::Controller* cntl,
    brpc::Channel* channel) {
...
void MDSClientBase::SetCloneFileStatus(const std::string& filename,
  ...
  LOG(INFO) << "SetCloneFileStatus: filename = " << filename
            ...
            << ", fileID = " << fileID << ", log id = " << cntl->log_id();

  curve::mds::CurveFSService_Stub stub(channel);
  stub.SetCloneFileStatus(cntl, &request, response, NULL);
}

void MDSClientBase::GetOrAllocateSegment(bool allocate, uint64_t offset,
                                         const FInfo_t* fi,
...
void MDSClientBase::DeAllocateSegment(const FInfo* fileInfo,
  ...
  LOG(INFO) << "DeAllocateSegment: filename = " << fileInfo->fullPathName
            << ", offset = " << segmentOffset << ", logid = " << cntl->log_id();
...
void MDSClientBase::Extend(const std::string& filename,
                           ...
                           ExtendFileResponse* response, brpc::Controller* cntl,
                           brpc::Channel* channel) {
...
void MDSClientBase::DeleteFile(const std::string& filename,
                               ...
                               uint64_t fileid, DeleteFileResponse* response,
                               brpc::Controller* cntl, brpc::Channel* channel) {
...
void MDSClientBase::ChangeOwner(const std::string& filename,
  ...
  LOG(INFO) << "ChangeOwner: filename = " << filename
            ...
            << ", new owner = " << newOwner << ", log id = " << cntl->log_id();
...
void MDSClientBase::GetChunkServerInfo(const std::string& ip, uint16_t port,
...
std::string MDSClientBase::CalcSignature(const UserInfo& userinfo,
  ...
  if (IsRootUserAndHasPassword(userinfo)) {
    std::string str2sig =
        Authenticator::GetString2Signature(date, userinfo.owner);
    std::string sig =
        Authenticator::CalcString2Signature(str2sig, userinfo.password);
    return sig;
  }
