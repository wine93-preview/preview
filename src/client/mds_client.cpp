...
LIBCURVE_ERROR MDSClient::OpenFile(const std::string& filename,
  ...
  auto task = RPCTaskDefine {
    ...
    LOG_IF(WARNING, retcode != LIBCURVE_ERROR::OK)
        << "OpenFile: filename = " << filename << ", owner = " << userinfo.owner
        << ", errocde = " << retcode
    ...
    if (flag) {
      ...
      if (protoFileInfo.has_clonesource() &&
        ...
        ServiceHelper::ProtoCloneSourceInfo2Local(response, &fi->sourceInfo);
      }
    } else {
  ...
  return ReturnError(rpcExcutor_.DoRPCTask(task, metaServerOpt_.mdsMaxRetryMS));
}
...
LIBCURVE_ERROR MDSClient::CreateFile(const std::string& filename,
  ...
  auto task = RPCTaskDefine {
    ...
    MDSClientBase::CreateFile(filename, userinfo, size, normalFile, stripeUnit,
                              stripeCount, &response, cntl, channel);
    ...
    LOG_IF(WARNING, retcode != LIBCURVE_ERROR::OK)
        ...
        << ", owner = " << userinfo.owner << ", is nomalfile: " << normalFile
        << ", errocde = " << retcode
        << ", error msg = " << StatusCode_Name(stcode)
        << ", log id = " << cntl->log_id();
    return retcode;
...
LIBCURVE_ERROR MDSClient::CloseFile(const std::string& filename,
  ...
  auto task = RPCTaskDefine {
    ...
    if (cntl->Failed()) {
      ...
      LOG(WARNING) << "close file failed, errcorde = " << cntl->ErrorCode()
                   << ", error content:" << cntl->ErrorText()
                   << ", log id = " << cntl->log_id();
      return -cntl->ErrorCode();
    }
...
LIBCURVE_ERROR MDSClient::GetFileInfo(const std::string& filename,
  ...
  auto task = RPCTaskDefine {
    ...
    MDSStatusCode2LibcurveError(stcode, &retcode);
    LOG_IF(WARNING, retcode != LIBCURVE_ERROR::OK)
        << "GetFileInfo: filename = " << filename << ", owner = " << uinfo.owner
        << ", errocde = " << retcode
        << ", error msg = " << StatusCode_Name(stcode)
        << ", log id = " << cntl->log_id();
    return retcode;
...
LIBCURVE_ERROR MDSClient::IncreaseEpoch(
  ...
  auto task = RPCTaskDefine {
    ...
    MDSClientBase::IncreaseEpoch(filename, userinfo, &response, cntl, channel);
...
LIBCURVE_ERROR MDSClient::CreateSnapShot(const std::string& filename,
  ...
  auto task = RPCTaskDefine {
    ...
    MDSClientBase::CreateSnapShot(filename, userinfo, &response, cntl, channel);
...
LIBCURVE_ERROR MDSClient::ListSnapShot(const std::string& filename,
  ...
  auto task = RPCTaskDefine {
    ...
    if (cntl->Failed()) {
      LOG(WARNING) << "list snap file failed, errcorde = " << cntl->ErrorCode()
                   << ", error content:" << cntl->ErrorText()
                   << ", log id = " << cntl->log_id();
      return -cntl->ErrorCode();
    }
  ...
  auto task = RPCTaskDefine {
    ...
    switch (stcode) {
      ...
      case StatusCode::kOK:
        if (response.has_fileinfo()) {
          FileEpoch_t fEpoch;
          ServiceHelper::ProtoFileInfo2Local(response.fileinfo(), &resp->finfo,
                                             &fEpoch);
...
LIBCURVE_ERROR MDSClient::CheckSnapShotStatus(const std::string& filename,
  ...
  auto task = RPCTaskDefine {
    ...
    MDSClientBase::CheckSnapShotStatus(filename, userinfo, seq, &response, cntl,
                                       channel);

    if (cntl->Failed()) {
      LOG(WARNING) << "check snap file failed, errcorde = " << cntl->ErrorCode()
                   << ", error content:" << cntl->ErrorText();
      return -cntl->ErrorCode();
    }
...
MDSClient::GetServerList(const LogicPoolID& logicalpooid,
  ...
  auto task = RPCTaskDefine {
    ...
    MDSClientBase::GetServerList(logicalpooid, copysetidvec, &response, cntl,
                                 channel);
    if (cntl->Failed()) {
...
LIBCURVE_ERROR MDSClient::CreateCloneFile(
  ...
  auto task = RPCTaskDefine {
    ...
    LOG_IF(WARNING, retcode != LIBCURVE_ERROR::OK)
        ...
        << ", destination = " << destination << ", owner = " << userinfo.owner
        << ", seqnum = " << sn << ", size = " << size
        << ", chunksize = " << chunksize << ", errocde = " << retcode
        << ", error msg = " << StatusCode_Name(stcode)
        << ", log id = " << cntl->log_id();
...
LIBCURVE_ERROR MDSClient::SetCloneFileStatus(const std::string& filename,
  ...
  auto task = RPCTaskDefine {
    ...
    MDSClientBase::SetCloneFileStatus(filename, filestatus, userinfo, fileID,
                                      &response, cntl, channel);
    if (cntl->Failed()) {
...
LIBCURVE_ERROR MDSClient::GetOrAllocateSegment(bool allocate, uint64_t offset,
  ...
  auto task = RPCTaskDefine {
    ...
    MDSClientBase::GetOrAllocateSegment(allocate, offset, fi, fEpoch, &response,
                                        cntl, channel);
    if (cntl->Failed()) {
...
LIBCURVE_ERROR MDSClient::DeAllocateSegment(const FInfo* fileInfo,
  ...
  auto task = RPCTaskDefine {
    ...
    if (cntl->Failed()) {
      ...
      LOG(WARNING) << "DeAllocateSegment failed, error = " << cntl->ErrorText()
...
LIBCURVE_ERROR MDSClient::Extend(const std::string& filename,
  ...
  auto task = RPCTaskDefine {
    ...
    MDSStatusCode2LibcurveError(stcode, &retcode);
    LOG_IF(WARNING, retcode != LIBCURVE_ERROR::OK)
        << "Extend: filename = " << filename << ", owner = " << userinfo.owner
        << ", newsize = " << newsize << ", errocde = " << retcode
        << ", error msg = " << StatusCode_Name(stcode)
        << ", log id = " << cntl->log_id();
    return retcode;
  };
...
LIBCURVE_ERROR MDSClient::ChangeOwner(const std::string& filename,
  ...
  auto task = RPCTaskDefine {
    ...
    MDSClientBase::ChangeOwner(filename, newOwner, userinfo, &response, cntl,
                               channel);
    if (cntl->Failed()) {
...
LIBCURVE_ERROR MDSClient::Listdir(const std::string& dirpath,
  ...
  auto task = RPCTaskDefine {
    ...
    if (cntl->Failed()) {
      ...
      LOG(WARNING) << "Listdir invoke failed, errcorde = " << cntl->ErrorCode()
                   << ", error content:" << cntl->ErrorText()
                   << ", log id = " << cntl->log_id();
      return -cntl->ErrorCode();
    }
    ...
    MDSStatusCode2LibcurveError(stcode, &retcode);
    LOG_IF(WARNING, retcode != LIBCURVE_ERROR::OK)
        << "Listdir: filename = " << dirpath << ", owner = " << userinfo.owner
        << ", errocde = " << retcode
        << ", error msg = " << StatusCode_Name(stcode)
        << ", log id = " << cntl->log_id();
    ...
    if (retcode == LIBCURVE_ERROR::OK) {
      ...
      for (int i = 0; i < fileinfoNum; i++) {
        ...
        memcpy(filestat.filename, finfo.filename().c_str(), NAME_MAX_SIZE);
...
LIBCURVE_ERROR MDSClient::GetChunkServerInfo(
  ...
  if (!valid) {
    LOG(ERROR) << "chunkserver address " << csAddr.ToString() << " invalid!";
    return LIBCURVE_ERROR::FAILED;
  }
...
MDSClient::ListChunkServerInServer(const std::string& serverIp,
  ...
  auto task = RPCTaskDefine {
    ...
    MDSClientBase::ListChunkServerInServer(serverIp, &response, cntl, channel);

    if (cntl->Failed()) {
      LOG(WARNING) << "ListChunkServerInServer failed, " << cntl->ErrorText()
                   << ", log id = " << cntl->log_id();
      return -cntl->ErrorCode();
    }
    ...
    LOG_IF(WARNING, statusCode != 0)
        ...
        << ", chunkserver ip = " << serverIp << ", log id = " << cntl->log_id();

    if (statusCode == 0) {
      csIds->reserve(response.chunkserverinfos_size());
      for (int i = 0; i < response.chunkserverinfos_size(); ++i) {
        csIds->emplace_back(response.chunkserverinfos(i).chunkserverid());
      }
