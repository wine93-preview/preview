...
void ServiceHelper::ProtoFileInfo2Local(const curve::mds::FileInfo& finfo,
  ...
  if (finfo.has_throttleparams()) {
    fi->throttleParams = ProtoFileThrottleParamsToLocal(finfo.throttleparams());
  }
 ...
 public:
  ...
  int Wait(ChunkServerID* leaderId, PeerAddr* leaderAddr) {
    ...
    LOG(INFO) << "GetLeader returned, logicpool id = " << logicPooldId_
              << ", copyset id = " << copysetId_ << ", proxy id = " << proxyId_
              << ", leader " << leader_.DebugString();
  ...
  void StartGetLeader(const std::unordered_set<std::string>& peerAddresses,
...

    {
      ...
      for (const auto& ipPort : peerAddresses) {
        ...
        if (ret != 0) {
          LOG(WARNING) << "GetLeader init channel to " << ipPort << " failed, "
                       << "logicpool id = " << logicPoolId
                       << "copyset id = " << copysetId;
          continue;
        }

        GetLeaderClosure* done =
            new GetLeaderClosure(logicPooldId_, copysetId_, shared_from_this());
    ...
    for (size_t i = 0; i < channels_.size(); ++i) {
      ...
      stub.GetLeader(&(closures_[i]->cntl), &request, &(closures_[i]->response),
                     closures_[i]);
    }
  }
