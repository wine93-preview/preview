...
struct FileMetric {
  ...
  explicit FileMetric(const std::string& name)
      : filename(name),
        ...
        discardSizeRecorder(
            prefix, filename + "_discard_request_size_recoder"),  // NOLINT
...
class MetricHelper {
 public:
   ...
   */
  static void IncremUserQPSCount(FileMetric* fm, uint64_t length, OpType type) {
    if (fm != nullptr) {
      switch (type) {
        case OpType::READ:
   ...
   * @param: read为当前操作是读操作还是写操作
   */
  static void IncremRPCQPSCount(FileMetric* fm, uint64_t length, OpType type) {
    if (fm != nullptr) {
      switch (type) {
        case OpType::READ:
