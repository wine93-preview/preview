...
void ClientClosure::PreProcessBeforeRetry(int rpcstatus, int cntlstatus) {
  ...
  if (cntlstatus == brpc::ERPCTIMEDOUT || cntlstatus == ETIMEDOUT) {
    ...
    bool leaderMayChange =
        metaCache_->IsLeaderMayChange(chunkIdInfo_.lpid_, chunkIdInfo_.cpid_);
  ...
  LOG(WARNING)
      ...
      << *reqCtx_ << ", cntl status = " << cntlstatus << ", response status = "
...
uint64_t ClientClosure::OverLoadBackOff(uint64_t currentRetryTimes) {
  ...
  nextsleeptime = std::min(
      nextsleeptime, failReqOpt_.chunkserverMaxRetrySleepIntervalUS);  // NOLINT
  nextsleeptime = std::max(nextsleeptime,
                           failReqOpt_.chunkserverOPRetryIntervalUS);  // NOLINT
    ...
    switch (status_) {
      ...
      case CHUNK_OP_STATUS::CHUNK_OP_STATUS_BACKWARD:
        if (reqCtx_->optype_ == OpType::WRITE) {
          ...
          LOG(ERROR) << OpTypeToString(reqCtx_->optype_) << " return backward, "
...
void ClientClosure::OnRpcFailed() {
  ...
  if (reqCtx_->optype_ == OpType::WRITE) {
    metaCache_->UpdateAppliedIndex(chunkIdInfo_.lpid_, chunkIdInfo_.cpid_, 0);
  }
...
void ClientClosure::ProcessUnstableState() {
  UnstableState state = metaCache_->GetUnstableHelper().GetCurrentUnstableState(
    ...
    case UnstableState::ServerUnstable: {
      ...
      if (ret != 0) {
        LOG(WARNING) << "Set server(" << ip << ") unstable failed, "
                     << "now set chunkserver(" << chunkserverID_
                     << ") unstable";
...
void ClientClosure::OnRetry() {
  ...
  if (reqDone_->GetRetriedTimes() >= failReqOpt_.chunkserverOPMaxRetry) {
    reqDone_->SetFailed(status_);
    LOG(ERROR) << OpTypeToString(reqCtx_->optype_) << " retried times exceeds"
               << ", IO id = " << reqDone_->GetIOTracker()->GetID()
  ...
  if (!reqDone_->IsSuspendRPC() &&
      reqDone_->GetRetriedTimes() >=
...
void ReadChunkClosure::SendRetryRequest() {
  client_->ReadChunk(reqCtx_->idinfo_, reqCtx_->seq_, reqCtx_->offset_,
                     reqCtx_->rawlength_, reqCtx_->appliedindex_,
                     reqCtx_->sourceInfo_, done_);
}
...
void ReadChunkClosure::OnSuccess() {
  ...
  metaCache_->UpdateAppliedIndex(reqCtx_->idinfo_.lpid_, reqCtx_->idinfo_.cpid_,
                                 response_->appliedindex());
}
...
void ReadChunkSnapClosure::SendRetryRequest() {
  client_->ReadChunkSnapshot(reqCtx_->idinfo_, reqCtx_->seq_, reqCtx_->offset_,
...
void CreateCloneChunkClosure::SendRetryRequest() {
  client_->CreateCloneChunk(reqCtx_->idinfo_, reqCtx_->location_, reqCtx_->seq_,
                            reqCtx_->correctedSeq_, reqCtx_->chunksize_, done_);
}

void RecoverChunkClosure::SendRetryRequest() {
  client_->RecoverChunk(reqCtx_->idinfo_, reqCtx_->offset_, reqCtx_->rawlength_,
                        done_);
}
