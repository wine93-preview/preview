...
class MDSClient : public MDSClientBase,
 ...
 public:
   ...
   * @param: newsize新的size
   */
  LIBCURVE_ERROR Extend(const std::string& filename, const UserInfo_t& userinfo,
                        uint64_t newsize);
  /**
   * 删除文件
   * @param: userinfo是用户信息
  ...
  LIBCURVE_ERROR RenameFile(const UserInfo_t& userinfo,
                            ...
                            const std::string& destination,
                            uint64_t originId = 0, uint64_t destinationId = 0);
   ...
   */
  LIBCURVE_ERROR Listdir(const std::string& dirpath, const UserInfo_t& userinfo,
