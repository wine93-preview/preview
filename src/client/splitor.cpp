...
int Splitor::SingleChunkIO2ChunkRequests(
  ...
  while (leftLength > 0) {
    ...
    padding.aligned =
        true;  // TODO(wuhanqing): add test case for normal file  // NOLINT
    uint64_t requestLength = std::min(leftLength, maxSplitSizeBytes);

    if (metaCache->IsCloneFile()) {
      requestLength =
          ProcessUnalignedRequests(currentOffset, requestLength, &padding);
    }
    ...
    DVLOG(9) << "request split"
             << ", off = " << currentOffset << ", len = " << requestLength
             << ", seqnum = " << seq << ", chunkid = " << idinfo.cid_
...
bool Splitor::AssignInternal(IOTracker* iotracker, MetaCache* metaCache,
                             ...
                             MDSClient* mdsclient, const FInfo_t* fileInfo,
                             const FileEpoch_t* fEpoch, ChunkIndex chunkidx) {
  lldiv_t res = std::div(
  ...
  if (iotracker->Optype() == OpType::DISCARD) {
    return MarkDiscardBitmap(iotracker, fileSegment, segmentIndex, startOffset,
                             len);
  }
  ...
  if (NeedGetOrAllocateSegment(errCode, iotracker->Optype(), chunkIdInfo,
                               metaCache)) {
    bool isAllocateSegment = iotracker->Optype() == OpType::READ ? false : true;
  ...
  if (errCode == MetaCacheErrorType::OK) {
    ...
    if (iotracker->Optype() == OpType::READ) {
      appliedindex_ =
          metaCache->GetAppliedIndex(chunkIdInfo.lpid_, chunkIdInfo.cpid_);
    }
    ...
    for (auto& ctx : templist) {
      ...
      ctx->sourceInfo_ = CalcRequestSourceInfo(iotracker, metaCache, chunkidx);
    }
...
bool Splitor::GetOrAllocateSegment(bool allocateIfNotExist, uint64_t offset,
  ...
  errCode =
      mdsClient->GetServerList(segmentInfo.lpcpIDInfo.lpid,
                               segmentInfo.lpcpIDInfo.cpidVec, &copysetInfos);
  ...
  for (const auto& copysetInfo : copysetInfos) {
    metaCache->UpdateCopysetInfo(segmentInfo.lpcpIDInfo.lpid, copysetInfo.cpid_,
                                 copysetInfo);
  }
...
int Splitor::SplitForNormal(IOTracker* iotracker, MetaCache* metaCache,
  ...
  while (currentChunkIndex <= endChunkIndex) {
    const uint64_t currentChunkEndOffset = chunksize * (currentChunkIndex + 1);
    uint64_t requestLength = std::min(currentChunkEndOffset, endRequestOffest) -
                             currentRequestOffset;

    DVLOG(9) << "request split"
             << ", off = " << currentChunkOffset << ", len = " << requestLength
    ...
    if (!AssignInternal(iotracker, metaCache, targetlist, data,
                        currentChunkOffset, requestLength, mdsclient, fileInfo,
                        fEpoch, currentChunkIndex)) {
...
int Splitor::SplitForStripe(IOTracker* iotracker, MetaCache* metaCache,
  ...
  while (left > 0) {
    ...
    if (!AssignInternal(iotracker, metaCache, targetlist, data, curChunkOffset,
                        requestLength, mdsclient, fileInfo, fEpoch,
                        curChunkIndex)) {
      LOG(ERROR) << "request split failed"
                 << ", off = " << curChunkOffset << ", len = " << requestLength
                 << ", seqnum = " << fileInfo->seqnum
                 << ", chunksize = " << chunksize
...
uint64_t Splitor::ProcessUnalignedRequests(const off_t currentOffset,
  ...
  if (static_cast<uint64_t>(currentOffset) == alignedStartOffset &&
    ...
    if (static_cast<uint64_t>(currentOffset) == alignedStartOffset) {
      ...
      padding->offset =
          common::align_down(currentOffset, iosplitopt_.alignment.cloneVolume);
      padding->length = iosplitopt_.alignment.cloneVolume;
    } else {
      if (alignedEndOffset > alignedStartOffset) {
        ...
        padding->aligned = false;
        padding->type = RequestContext::Padding::Left;
        padding->offset = common::align_down(currentOffset,
