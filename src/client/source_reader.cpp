...
SourceReader::~SourceReader() { Stop(); }
...
void SourceReader::SetReadHandlers(
  ...
  for (const auto& handler : handlers) {
    readHandlers_.emplace(
        std::piecewise_construct, std::forward_as_tuple(handler.first),
        std::forward_as_tuple(handler.second.file_, handler.second.lastUsedSec_,
  ...
  while (sleeper_->wait_for(sleepSec)) {
    ...
    while (iter != readHandlers_.end()) {
      ...
      if (::time(nullptr) - lastUsedSec > fdTimeout) {
        LOG(INFO) << iter->first << " timedout, last used: " << lastUsedSec;
