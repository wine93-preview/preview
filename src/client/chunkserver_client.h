...
class ChunkServerClientClosure : public Closure {
 public:
  ...
  void SetErrCode(int ret) { err_ = ret; }

  int GetErrorCode() { return err_; }
...
class ChunkServerClient {
 public:
  ...
  int UpdateFileEpoch(const CopysetPeerInfo<ChunkServerID>& cs, uint64_t fileId,
                      uint64_t epoch, ChunkServerClientClosure* done);
