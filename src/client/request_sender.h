...
class RequestSender {
 public:
  ...
  int ReadChunk(const ChunkIDInfo& idinfo, uint64_t sn, off_t offset,
                size_t length, uint64_t appliedindex,
                const RequestSourceInfo& sourceInfo, ClientClosure* done);
  ...
  int DeleteChunkSnapshotOrCorrectSn(const ChunkIDInfo& idinfo,
                                     uint64_t correctedSn, ClientClosure* done);
   ...
   * @return 0成功，-1失败
   */
  int ResetSender(ChunkServerID chunkServerId, butil::EndPoint serverEndPoint);
