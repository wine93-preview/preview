...
int MetaCache::UpdateLeaderInternal(LogicPoolID logicPoolId,
  ...
  GetLeaderInfo getLeaderInfo(logicPoolId, copysetId, toupdateCopyset->csinfos_,
  ...
  if (ret == -1 && !leaderaddr.IsEmpty()) {
    ...
    UpdateCopysetInfoIfMatchCurrentLeader(logicPoolId, copysetId, leaderaddr);
...
int MetaCache::UpdateCopysetInfoFromMDS(LogicPoolID logicPoolId,
  ...
  int ret = mdsclient_->GetServerList(logicPoolId, {copysetId}, &copysetInfos);
...
void MetaCache::UpdateCopysetInfoIfMatchCurrentLeader(
  ...
  bool needUpdate =
      (!copysetInfos.empty()) && (copysetInfos[0].HasPeerInCopyset(leaderAddr));
  if (needUpdate) {
...
void MetaCache::UpdateAppliedIndex(LogicPoolID logicPoolId, CopysetID copysetId,
...
void MetaCache::UpdateChunkserverCopysetInfo(
  ...
  if (previouscpinfo != lpcsid2CopsetInfoMap_.end()) {
    ...
    for (auto iter : cpinfo.csinfos_) {
      auto it = std::find(changedID.begin(), changedID.end(), iter.peerID);
      if (it != changedID.end()) {
        changedID.erase(it);
      } else {
