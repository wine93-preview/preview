...
class SnapshotCloneServerTest : public ::testing::Test {
 public:
  ...
  bool JudgeSnapTaskFailCleanEnvAndCheck(const std::string& user,
    ...
    FInfo fInfo;
    int ret =
        server_->GetCurveFsClient()->GetSnapshot(file, user, seqNum, &fInfo);
  ...
  bool PrepreTestSnapshotIfNotExist() {
    if (testSnapId_.empty()) {
      bool ret =
          PrepreTestSnapshot(testUser1, testFile1, "testSnap", &testSnapId_);
      return ret;
    }
    return true;
  }
...
TEST_F(SnapshotCloneServerTest, TestCreateSnapshotFailOnCurvefs) {
  ...
  SnapshotInfo snapInfo;
  ASSERT_TRUE(JudgeSnapTaskFailCleanTaskAndCheck(user, file, uuid, &snapInfo));
}
...
TEST_F(SnapshotCloneServerTest, TestLazyCloneSnapFailOnCreateCloneFile) {
  ...
  fiu_enable(
      "test/integration/snapshotcloneserver/FakeCurveFsClient.CreateCloneFile",  // NOLINT
      1, NULL, 0);
...
TEST_F(SnapshotCloneServerTest, TestLazyCloneSnapFailOnRenameCloneFile) {
  ...
  fiu_enable(
      "test/integration/snapshotcloneserver/FakeCurveFsClient.RenameCloneFile",  // NOLINT
      1, NULL, 0);
...
TEST_F(SnapshotCloneServerTest, TestLazyCloneSnapFailOnCreateCloneChunk) {
  ...
  fiu_enable(
      "test/integration/snapshotcloneserver/FakeCurveFsClient.CreateCloneChunk",  // NOLINT
      1, NULL, 0);
...
TEST_F(SnapshotCloneServerTest, TestLazyCloneImageFailOnCreateCloneFile) {
  ...
  int ret = CloneOrRecover("Clone", testUser1, testFile1, "/user1/clone1", true,
                           &uuid1);
  ASSERT_EQ(kErrCodeInternalError, ret);
...
TEST_F(SnapshotCloneServerTest, TestNotLazyRecoverSnapFailOnRecoverChunk) {
  ...
  int ret = CloneOrRecover("Recover", testUser1, testSnapId_, testFile1, false,
                           &uuid1);
  ASSERT_EQ(0, ret);
