...
int FakeCurveFsClient::CreateCloneFile(const std::string& source,
  ...
  fiu_return_on(
      "test/integration/snapshotcloneserver/FakeCurveFsClient.CreateCloneFile",
      -LIBCURVE_ERROR::FAILED);  // NOLINT
...
int FakeCurveFsClient::CreateCloneChunk(const std::string& location,
  ...
  scc->Run();
  fiu_return_on(
      "test/integration/snapshotcloneserver/FakeCurveFsClient.CreateCloneChunk",
      -LIBCURVE_ERROR::FAILED);  // NOLINT
  return LIBCURVE_ERROR::OK;
}
...
int FakeCurveFsClient::RenameCloneFile(const std::string& user,
  ...
  fiu_return_on(
      "test/integration/snapshotcloneserver/FakeCurveFsClient.RenameCloneFile",
      -LIBCURVE_ERROR::FAILED);  // NOLINT
  ...
  for (auto& f : fileMap_) {
    if (2 == f.second.parentid) {
      LOG(INFO) << "Clone dir has file, fileinfo is :"
                << " id = " << f.second.id << ", owner = " << f.second.owner
                << ", filename = " << f.second.filename
                << ", fullPathName = " << f.second.fullPathName
                << ", filestatus = " << static_cast<int>(f.second.filestatus);
      return true;
    }
  }
  return false;
