...
int SendRequest(const std::string& url, Json::Value* jsonObj) {
  ...
  if (channel_.Init(url.c_str(), "", &option) != 0) {
    LOG(ERROR) << "Fail to init channel" << std::endl;
    return -1;
  }
...
int MakeSnapshot(const std::string& user, const std::string& fileName,
                 const std::string& snapName, std::string* uuidOut) {
  std::string url = std::string("http://") + kSnapshotCloneServerIpPort + "/" +
                    kServiceName + "?" + kActionStr + "=" +
                    kCreateSnapshotAction + "&" + kVersionStr + "=1&" +
                    kUserStr + "=" + user + "&" + kFileStr + "=" + fileName +
                    "&" + kNameStr + "=" + snapName;
...
int CancelSnapshot(const std::string& user, const std::string& fileName,
                   const std::string& uuid) {
  std::string url = std::string("http://") + kSnapshotCloneServerIpPort + "/" +
                    kServiceName + "?" + kActionStr + "=" +
                    kCancelSnapshotAction + "&" + kVersionStr + "=1&" +
                    kUserStr + "=" + user + "&" + kFileStr + "=" + fileName +
                    "&" + kUUIDStr + "=" + uuid;
...
int GetSnapshotInfo(const std::string& user, const std::string& fileName,
                    const std::string& uuid, FileSnapshotInfo* info) {
  std::string url = std::string("http://") + kSnapshotCloneServerIpPort + "/" +
                    kServiceName + "?" + kActionStr + "=" +
                    kGetFileSnapshotInfoAction + "&" + kVersionStr + "=1&" +
                    kUserStr + "=" + user + "&" + kFileStr + "=" + fileName +
                    "&" + kUUIDStr + "=" + uuid;
...
int ListFileSnapshotInfo(const std::string& user, const std::string& fileName,
  ...
  std::string url = std::string("http://") + kSnapshotCloneServerIpPort + "/" +
                    ...
                    kGetFileSnapshotInfoAction + "&" + kVersionStr + "=1&" +
                    kUserStr + "=" + user + "&" + kFileStr + "=" + fileName +
                    "&" + kLimitStr + "=" + std::to_string(limit) + kOffsetStr +
...
int DeleteSnapshot(const std::string& user, const std::string& fileName,
                   const std::string& uuid) {
  std::string url = std::string("http://") + kSnapshotCloneServerIpPort + "/" +
                    kServiceName + "?" + kActionStr + "=" +
                    kDeleteSnapshotAction + "&" + kVersionStr + "=1&" +
                    kUserStr + "=" + user + "&" + kFileStr + "=" + fileName +
                    "&" + kUUIDStr + "=" + uuid;
...
int CloneOrRecover(const std::string& action, const std::string& user,
                   const std::string& src, const std::string& dst, bool lazy,
                   std::string* uuidOut) {
  std::string url = std::string("http://") + kSnapshotCloneServerIpPort + "/" +
                    kServiceName + "?" + kActionStr + "=" + action + "&" +
                    kVersionStr + "=1&" + kUserStr + "=" + user + "&" +
                    kSourceStr + "=" + src + "&" + kDestinationStr + "=" + dst +
                    "&" + kLazyStr + "=" + (lazy ? "1" : "0");
...
int Flatten(const std::string& user, const std::string& uuid) {
  std::string url = std::string("http://") + kSnapshotCloneServerIpPort + "/" +
...
int GetCloneTaskInfo(const std::string& user, const std::string& uuid,
                     TaskCloneInfo* info) {
  std::string url = std::string("http://") + kSnapshotCloneServerIpPort + "/" +
                    kServiceName + "?" + kActionStr + "=" +
                    kGetCloneTasksAction + "&" + kVersionStr + "=1&" +
                    kUserStr + "=" + user + "&" + kUUIDStr + "=" + uuid;
...
int ListCloneTaskInfo(const std::string& user, int limit, int offset,
                      std::vector<TaskCloneInfo>* infoVec) {
  std::string url =
      std::string("http://") + kSnapshotCloneServerIpPort + "/" + kServiceName +
      "?" + kActionStr + "=" + kGetCloneTasksAction + "&" + kVersionStr +
      "=1&" + kUserStr + "=" + user + "&" + kLimitStr + "=" +
      std::to_string(limit) + kOffsetStr + "=" + std::to_string(offset);
  LOG(INFO) << "GetCloneTasks, url = " << url;
...
int CleanCloneTask(const std::string& user, const std::string& uuid) {
  std::string url = std::string("http://") + kSnapshotCloneServerIpPort + "/" +
                    kServiceName + "?" + kActionStr + "=" +
                    kCleanCloneTaskAction + "&" + kVersionStr + "=1&" +
                    kUserStr + "=" + user + "&" + kUUIDStr + "=" + uuid;
...
bool CheckSnapshotSuccess(const std::string& user, const std::string& file,
                          const std::string& uuid) {
  bool success = false;
  for (int i = 0; i < 600; i++) {
    FileSnapshotInfo info1;
    int retCode = GetSnapshotInfo(user, file, uuid, &info1);
    if (retCode != 0) {
...
int DeleteAndCheckSnapshotSuccess(const std::string& user,
...
bool CheckCloneOrRecoverSuccess(const std::string& user,
                                const std::string& uuid, bool isClone) {
  bool success = false;
  for (int i = 0; i < 600; i++) {
    TaskCloneInfo info1;
    int retCode = GetCloneTaskInfo(user, uuid, &info1);
    if (retCode != 0) {
      break;
    }
    CloneStatus st;
...
bool WaitMetaInstalledSuccess(const std::string& user, const std::string& uuid,
                              bool isClone) {
  bool success = false;
  for (int i = 0; i < 600; i++) {
    TaskCloneInfo info1;
    ...
    if (info1.GetCloneInfo().GetStatus() == st) {
      ...
      continue;
    } else if (info1.GetCloneInfo().GetStatus() == CloneStatus::metaInstalled) {
      success = true;
      break;
    } else {
      LOG(ERROR) << "Clone Fail On status = "
