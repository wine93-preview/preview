...
int FakeSnapshotCloneMetaStore::AddSnapshot(const SnapshotInfo& info) {
  fiu_return_on(
      ...
      "FakeSnapshotCloneMetaStore.AddSnapshot",
      -1);  // NOLINT
...
int FakeSnapshotCloneMetaStore::DeleteSnapshot(const UUID& uuid) {
  fiu_return_on(
      "test/integration/snapshotcloneserver/"
      "FakeSnapshotCloneMetaStore.DeleteSnapshot",
      -1);  // NOLINT
  std::lock_guard<std::mutex> guard(snapInfos_mutex);
...
int FakeSnapshotCloneMetaStore::UpdateSnapshot(const SnapshotInfo& info) {
  fiu_return_on(
      "test/integration/snapshotcloneserver/"
      "FakeSnapshotCloneMetaStore.UpdateSnapshot",
      -1);  // NOLINT
  std::lock_guard<std::mutex> guard(snapInfos_mutex);
...
int FakeSnapshotCloneMetaStore::GetSnapshotInfo(const UUID& uuid,
                                                SnapshotInfo* info) {
  std::lock_guard<std::mutex> guard(snapInfos_mutex);
  auto search = snapInfos_.find(uuid);
  if (search != snapInfos_.end()) {
...
int FakeSnapshotCloneMetaStore::GetSnapshotList(const std::string& name,
  ...
  std::lock_guard<std::mutex> guard(snapInfos_mutex);
  for (auto it = snapInfos_.begin(); it != snapInfos_.end(); it++) {
...
int FakeSnapshotCloneMetaStore::UpdateCloneInfo(const CloneInfo& info) {
  fiu_return_on(
      "test/integration/snapshotcloneserver/"
      "FakeSnapshotCloneMetaStore.UpdateCloneInfo",
      -1);  // NOLINT
  curve::common::WriteLockGuard guard(cloneInfos_lock_);
...
int FakeSnapshotCloneMetaStore::GetCloneInfo(const std::string& taskID,
                                             CloneInfo* info) {
...
int FakeSnapshotCloneMetaStore::GetCloneInfoList(std::vector<CloneInfo>* v) {
  curve::common::ReadLockGuard guard(cloneInfos_lock_);
