 ...
 public:
  ...
  static bool WriteFile(const std::string& fileName, const std::string& user,
    ...
    *fdOut = fileClient_->Open(fileName, userinfo);
    ...
    for (uint64_t i = 0; i < testFile1AllocSegmentNum; i++) {
      ret = fileClient_->Write(*fdOut, dataSample.c_str(), i * segmentSize,
                               dataSample.size());
      if (ret < 0) {
  ...
  int PrepareCreateCloneMeta(FInfo* fInfoOut, const std::string& newFileName,
    ...
    for (int i = 0; i < testFile1AllocSegmentNum; i++) {
      ...
      int ret = snapClient_->GetOrAllocateSegmentInfo(true, i * segmentSize,
                                                      fInfoOut, &segInfoOut);
    ...
    if (IsRecover) {
      ...
      for (int i = 0; i < testFile1AllocSegmentNum; i++) {
        for (uint64_t j = 0; j < segmentSize / chunkSize; j++) {
          std::string location = LocationOperator::GenerateCurveLocation(
          ...
          LOG(INFO) << "CreateCloneChunk, location = " << location
                    ...
                    << ", chunkId = " << cidInfo.cid_ << ", seqNum = " << 1
                    << ", csn = " << 0;
          int ret = snapClient_->CreateCloneChunk(location, cidInfo,
                                                  1,  // 克隆使用初始版本号1
                                                  0,  // 克隆使用0
                                                  chunkSize, cb);
          if (ret != LIBCURVE_ERROR::OK) {
      ...
      for (int i = 0; i < testFile1AllocSegmentNum; i++) {
        ...
        for (uint64_t k = 0; k < chunkSize / chunkSplitSize; k++) {
          ...
          LOG(INFO) << "RecoverChunk"
                    ...
                    << ", offset = " << offset;
          int ret =
              snapClient_->RecoverChunk(cidInfo, offset, chunkSplitSize, cb);
          if (ret != LIBCURVE_ERROR::OK) {
            return ret;
          }
        }
...
TEST_F(SnapshotCloneServerTest, TestRecoverSnapshotWhenNotCreateSnapOnCurvefs) {
  ...
  SnapshotInfo snapInfo(uuid1, testUser1_, testFile1_, "snapxxx", 0, chunkSize,
                        segmentSize, testFile1Length, 0, 0, 0, Status::pending);
...
TEST_F(SnapshotCloneServerTest,
  ...
  SnapshotInfo snapInfo(uuid1, testUser1_, testFile1_, "snapxxx", seq,
                        chunkSize, segmentSize, testFile1Length, 0, 0, 0,
                        Status::pending);
  cluster_->metaStore_->AddSnapshot(snapInfo);
...
TEST_F(SnapshotCloneServerTest, TestRecoverCloneHasNotCreateCloneFile) {
  ...
  CloneInfo cloneInfo(uuid1, testUser1_, CloneTaskType::kClone, testFile1_,
                      dstFile, 0, 0, 0, CloneFileType::kFile, false,
...
TEST_F(SnapshotCloneServerTest, TestRecoverCloneHasNotCreateCloneMeta) {
  ...
  CloneInfo cloneInfo(uuid1, testUser1_, CloneTaskType::kClone, testFile1_,
                      ...
                      CloneFileType::kFile, false, CloneStep::kCreateCloneMeta,
                      CloneStatus::cloning);
...
TEST_F(SnapshotCloneServerTest, TestRecoverCloneHasNotCreateCloneChunk) {
  ...
  CloneInfo cloneInfo(uuid1, testUser1_, CloneTaskType::kClone, testFile1_,
                      dstFile, fInfoOut.id, fInfoOut.id, 0,
                      CloneFileType::kFile, false, CloneStep::kCreateCloneChunk,
                      CloneStatus::cloning);
...
TEST_F(SnapshotCloneServerTest, TestRecoverCloneHasNotRenameCloneFile) {
  ...
  CloneInfo cloneInfo(uuid1, testUser1_, CloneTaskType::kClone, testFile1_,
                      dstFile, fInfoOut.id, fInfoOut.id, 0,
                      CloneFileType::kFile, false, CloneStep::kRenameCloneFile,
                      CloneStatus::cloning);
...
TEST_F(SnapshotCloneServerTest,
  ...
  ASSERT_EQ(LIBCURVE_ERROR::OK, PrepareRenameCloneFile(fInfoOut.id, fInfoOut.id,
                                                       fileName, dstFile));
...
TEST_F(SnapshotCloneServerTest, TestRecoverCloneLazyHasNotCreateCloneFile) {
  ...
  CloneInfo cloneInfo(uuid1, testUser1_, CloneTaskType::kRecover, snapId,
                      testFile1_, 0, 0, 0, CloneFileType::kSnapshot, true,
...
TEST_F(SnapshotCloneServerTest, TestRecoverCloneLazyHasNotRecoverChunk) {
  ...
  CloneInfo cloneInfo(uuid1, testUser1_, CloneTaskType::kRecover, snapId,
                      testFile1_, fInfoOut.id, testFd1_, 0,
                      CloneFileType::kSnapshot, true, CloneStep::kRecoverChunk,
                      CloneStatus::recovering);
