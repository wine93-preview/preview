...
class SnapshotCloneServerTest : public ::testing::Test {
 public:
  static void SetUpTestCase() {
    ...
    LOG(INFO) << "etcd 1 started on " << kEtcdPeerIpPort << ", pid = " << pid;
    ASSERT_GT(pid, 0);
    ...
    cluster_->PrepareConfig<S3ConfigGenerator>(kS3ConfigPath, s3ConfigOptions);
    ...
    cluster_->PrepareConfig<SnapClientConfigGenerator>(kSnapClientConfigPath,
  ...
  static bool WriteFile(const std::string& fileName, const std::string& user,
    ...
    for (uint64_t i = 0; i < totalChunk / chunkGap; i++) {
      ret = fileClient_->Write(testfd1_, dataSample.c_str(),
    ...
    if (retCode == 0) {
      if (info1.GetSnapshotInfo().GetStatus() == Status::pending ||
        ...
        if (!isCancel) {
          ...
          retCode = CancelSnapshot(testUser1_, testFile1_, "notExistUUId");
...
TEST_F(SnapshotCloneServerTest, TestSnapLazyRecover) {
  ...
  // 预期2:  返回用户认证失败
  ret = CloneOrRecover("Recover", testUser2_, snapId, testFile1_, true, &uuid1);
  ASSERT_EQ(kErrCodeInvalidUser, ret);

  // 操作3：testUser1_ Recover快照snap1，fileName=testFile1_
  // 预期3  返回恢复成功
  ret = CloneOrRecover("Recover", testUser1_, snapId, testFile1_, true, &uuid1);
  ASSERT_EQ(0, ret);
...
TEST_F(SnapshotCloneServerTest, TestSnapNotLazyRecover) {
  ...
  // 预期2:  返回用户认证失败
  ret =
      CloneOrRecover("Recover", testUser2_, snapId, testFile1_, false, &uuid1);
  ASSERT_EQ(kErrCodeInvalidUser, ret);

  // 操作3：testUser1_ Recover快照snap1，fileName=testFile1_
  // 预期3  返回恢复成功
  ret =
      CloneOrRecover("Recover", testUser1_, snapId, testFile1_, false, &uuid1);
  ASSERT_EQ(0, ret);
...
TEST_F(SnapshotCloneServerTest, TestImageLazyClone) {
  ...
  ret = CloneOrRecover("Clone", testUser1_, testFile1_, dstFile, true, &uuid2);
  ASSERT_EQ(0, ret);
...
TEST_F(SnapshotCloneServerTest, TestImageNotLazyClone) {
  ...
  ret = CloneOrRecover("Clone", testUser1_, testFile1_, dstFile, false, &uuid1);
  ASSERT_EQ(0, ret);
...
TEST_F(SnapshotCloneServerTest, TestSnapAndCloneWhenSnapHasError) {
  ...
  // 预期3：返回快照存在异常
  ret = CloneOrRecover("Recover", testUser1_, snapId, testFile4_, true, &uuid2);
  ASSERT_EQ(kErrCodeInvalidSnapshot, ret);

  // 操作4：非lazy 从 快照snap1 recover
  // 预期4：返回快照存在异常
  ret =
      CloneOrRecover("Recover", testUser1_, snapId, testFile4_, false, &uuid2);
  ASSERT_EQ(kErrCodeInvalidSnapshot, ret);
...
TEST_F(SnapshotCloneServerTest, TestCloneHasSameDest) {
  ...
  ret = CloneOrRecover("Clone", testUser1_, testFile1_, dstFile, true, &uuid3);
  ASSERT_EQ(0, ret);
  ...
  // 预期4  返回克隆成功
  ret = CloneOrRecover("Clone", testUser1_, testFile2_, dstFile, true, &uuid4);
  ASSERT_EQ(0, ret);
  ...
  ret = CloneOrRecover("Clone", testUser1_, testFile1_, dstFile, true, &uuid5);
  ASSERT_EQ(0, ret);
  ...
  // 预期6  返回克隆成功
  ret = CloneOrRecover("Clone", testUser1_, testFile2_, dstFile, false, &uuid6);
  ASSERT_EQ(0, ret);
  ...
  // fileName=CloneHasSameDest3
  // 预期7  返回克隆成功
  ret = CloneOrRecover("Clone", testUser1_, testFile2_, dstFile, true, &uuid7);
  ASSERT_EQ(0, ret);
...
TEST_F(SnapshotCloneServerTest, TestDeleteLazyCloneDestThenDeleteSrc) {
  ...
  ret = CloneOrRecover("Clone", testUser1_, testFile5_, dstFile2, true, &uuid2);
  ASSERT_EQ(0, ret);
