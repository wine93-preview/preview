...
class RaftConfigChangeTest : public testing::Test {
 protected:
  virtual void SetUp() {
    ...
    cg1.SetKV("copyset.election_timeout_ms", std::to_string(electionTimeoutMs));
    cg1.SetKV("copyset.snapshot_interval_s", std::to_string(snapshotIntervalS));
    cg1.SetKV("chunkserver.common.logDir", kRaftConfigChangeTestLogDir);
    cg1.SetKV("mds.listen.addr", kFakeMdsAddr);
    cg2.SetKV("copyset.election_timeout_ms", std::to_string(electionTimeoutMs));
    cg2.SetKV("copyset.snapshot_interval_s", std::to_string(snapshotIntervalS));
    cg2.SetKV("chunkserver.common.logDir", kRaftConfigChangeTestLogDir);
    cg2.SetKV("mds.listen.addr", kFakeMdsAddr);
    cg3.SetKV("copyset.election_timeout_ms", std::to_string(electionTimeoutMs));
    cg3.SetKV("copyset.snapshot_interval_s", std::to_string(snapshotIntervalS));
    cg3.SetKV("chunkserver.common.logDir", kRaftConfigChangeTestLogDir);
    cg3.SetKV("mds.listen.addr", kFakeMdsAddr);
    cg4.SetKV("copyset.election_timeout_ms", std::to_string(electionTimeoutMs));
    cg4.SetKV("copyset.snapshot_interval_s", std::to_string(snapshotIntervalS));
    cg4.SetKV("chunkserver.common.logDir", kRaftConfigChangeTestLogDir);
    cg4.SetKV("mds.listen.addr", kFakeMdsAddr);
    cg5.SetKV("copyset.election_timeout_ms", std::to_string(electionTimeoutMs));
    cg5.SetKV("copyset.snapshot_interval_s", std::to_string(snapshotIntervalS));
...
TEST_F(RaftConfigChangeTest, ThreeNodeRemoveShutdownPeer) {
  ...
  ASSERT_EQ(
      0, cluster.StartPeer(shutdownPeer, PeerCluster::PeerToId(shutdownPeer)));
  ASSERT_EQ(0, cluster.WaitLeader(&leaderPeer));
  ...
  // read之前写入的数据验证
  ReadVerify(leaderPeer, logicPoolId, copysetId, chunkId, length, ch - 1, loop);
...
TEST_F(RaftConfigChangeTest, ThreeNodeRemoveLeader) {
  ...
  butil::Status st3 = AddPeer(logicPoolId, copysetId, conf, oldLeader, options);
  ASSERT_TRUE(st3.ok());
...
TEST_F(RaftConfigChangeTest, ThreeNodeShutdownPeerThenTransferLeaderTo) {
  ...
  for (int i = 0; i < kMaxLoop; ++i) {
    ...
    LOG(INFO) << i + 1 << " th transfer leader to " << shutdownPeer.address()
              << " failed";
    ::sleep(1);
  }
...
TEST_F(RaftConfigChangeTest, ThreeNodeHangPeerThenTransferLeaderTo) {
  ...
  for (int i = 0; i < kMaxLoop; ++i) {
    status = TransferLeader(logicPoolId, copysetId, conf, hangPeer, options);
    if (0 == status.error_code()) {
      cluster.WaitLeader(&leaderPeer);
...
TEST_F(RaftConfigChangeTest,
  ...
  // 删除此peer的log，然后重启
  ::system(
      PeerCluster::RemoveCopysetLogDirCmd(shutdownPeer, logicPoolId, copysetId)
          .c_str());
  std::shared_ptr<LocalFileSystem> fs(
      LocalFsFactory::CreateFs(FileSystemType::EXT4, ""));
 ...
 * 1. {A、B、C} 3个节点正常启动
 * 2. 挂了follower
 * 3. 重启恢复follower（需要额外确保通过snapshot恢复），恢复过程中leader
 * hang一会
 */
TEST_F(
    RaftConfigChangeTest,
 ...
 * 1. {A、B、C} 3个节点正常启动
 * 2. 挂了follower，
 * 3. 重启恢复follower（需要额外确保通过snapshot恢复），恢复过程中follower
 * hang了
 * 4. 一段时间后恢复
 */
TEST_F(RaftConfigChangeTest,
