...
int PeerCluster::StartPeer(const Peer& peer, int id, const bool empty) {
  ...
  if (it != peersMap_.end()) {
    LOG(ERROR) << "StartPeer failed. since " << peer.address() << " is exist";
    return -1;
  }
  ...
  if (0 == ret) {
    LOG(INFO) << "create copyset " << ToGroupIdString(logicPoolID_, copysetID_)
...
int PeerCluster::WaitLeader(Peer* leaderPeer) {
  ...
  for (int i = 0; i < kMaxLoop; ++i) {
    ...
    if (status.ok()) {
      ...
      LOG(INFO) << "Wait leader success, leader is: " << leaderPeer->address();
      std::string leaderAddr = leaderPeer->address();
      int ret = ConfirmLeader(logicPoolID_, copysetID_, leaderAddr, leaderPeer);
      if (ret == 0) {
        return ret;
      }
    } else {
...
const std::string PeerCluster::CopysetDirWithProtocol(const Peer& peer) {
  ...
  butil::string_printf(&copysetdir, "local://./%s-%d-%d",
                       butil::ip2str(peerId.addr.ip).c_str(), peerId.addr.port,
                       0);
  return copysetdir;
}
...
int PeerCluster::CreateCopyset(LogicPoolID logicPoolID, CopysetID copysetID,
  ...
  for (int i = 0; i < 5; ++i) {
    ...
    if (cntl.Failed()) {
      LOG(ERROR) << "failed create copsyet, " << cntl.ErrorText() << std::endl;
      ::usleep(1000 * 1000);
      continue;
    }
    ...
    if (response.status() == COPYSET_OP_STATUS::COPYSET_OP_STATUS_SUCCESS ||
      ...
      LOG(INFO) << "create copyset " << ToGroupIdString(logicPoolID, copysetID)
                << " success.";
      return 0;
    }
...
void TransferLeaderAssertSuccess(PeerCluster* cluster, const Peer& targetLeader,
  ...
  butil::Status status;
  for (int i = 0; i < kMaxLoop; ++i) {
    status = TransferLeader(cluster->GetLogicPoolId(), cluster->GetCopysetId(),
