...
TEST_F(ClusterBasicTest, DISABLED_test_start_stop_module1) {
  ...
  // 创建chunkserver
  pid = curveCluster_->StartSingleChunkServerInBackground(1, chunkserverConf1);
  LOG(INFO) << "chunkserver 1 started in background, pid = " << pid;
  ASSERT_GT(pid, 0);
  pid = curveCluster_->StartSingleChunkServerInBackground(2, chunkserverConf2);
  LOG(INFO) << "chunkserver 2 started in background, pid = " << pid;
  ASSERT_GT(pid, 0);
  pid = curveCluster_->StartSingleChunkServerInBackground(3, chunkserverConf3);
...
TEST_F(ClusterBasicTest, test_multi_mds_and_etcd) {
  ...
  pid = curveCluster_->StartSingleMDS(2, "127.0.0.1:2311", 2314, copy2, false);
  ...
  pid = curveCluster_->StartSingleMDS(3, "127.0.0.1:2312", 2315, copy3, false);
