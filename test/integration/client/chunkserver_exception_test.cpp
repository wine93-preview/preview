...
class CSModuleException : public ::testing::Test {
 public:
  void SetUp() {
    ...
    cluster->PrepareConfig<curve::ClientConfigGenerator>(confPath, clientConf);
    ...
    pid = cluster->StartSingleMDS(1, "127.0.0.1:22122", 22128, mdsConf, true);
    LOG(INFO) << "mds 1 started on 127.0.0.1:22122, pid = " << pid;
    ASSERT_GT(pid, 0);
    std::this_thread::sleep_for(std::chrono::seconds(2));
    pid = cluster->StartSingleMDS(2, "127.0.0.1:22123", 22129, mdsConf, false);
    LOG(INFO) << "mds 2 started on 127.0.0.1:22123, pid = " << pid;
    ASSERT_GT(pid, 0);
    std::this_thread::sleep_for(std::chrono::seconds(2));
    pid = cluster->StartSingleMDS(3, "127.0.0.1:22124", 22130, mdsConf, false);
  ...
  void CreateOpenFileBackend() {
    ...
    auto func = [&]() {
      for (int i = 0; i < 20; i++) {
        ...
        int ret = curve::test::FileCommonOperation::Open(filename, "curve");
...
TEST_F(CSModuleException, ChunkserverException) {
  ...
  pid = cluster->StartSingleChunkServer(1, "127.0.0.1:22125", chunkserverConf4);
  LOG(INFO) << "chunkserver 1 started on 127.0.0.1:22125, pid = " << pid;
  ASSERT_GT(pid, 0);
  ...
  pid = cluster->StartSingleChunkServer(2, "127.0.0.1:22126", chunkserverConf5);
  LOG(INFO) << "chunkserver 2 started on 127.0.0.1:22126, pid = " << pid;
  ASSERT_GT(pid, 0);
  ...
  pid = cluster->StartSingleChunkServer(3, "127.0.0.1:22127", chunkserverConf6);
  LOG(INFO) << "chunkserver 3 started on 127.0.0.1:22127, pid = " << pid;
  ASSERT_GT(pid, 0);
