...
class MDSModuleException : public ::testing::Test {
 public:
  void SetUp() {
    ...
    pid = cluster->StartSingleMDS(0, "127.0.0.1:22222", 22240, mdsConf, true);
    LOG(INFO) << "mds 0 started on 127.0.0.1:22222, pid = " << pid;
    ASSERT_GT(pid, 0);
    std::this_thread::sleep_for(std::chrono::seconds(2));
    pid = cluster->StartSingleMDS(1, "127.0.0.1:22223", 22241, mdsConf, false);
    LOG(INFO) << "mds 1 started on 127.0.0.1:22223, pid = " << pid;
    ASSERT_GT(pid, 0);
    std::this_thread::sleep_for(std::chrono::seconds(2));
    pid = cluster->StartSingleMDS(2, "127.0.0.1:22224", 22242, mdsConf, false);
...
TEST_F(MDSModuleException, MDSExceptionTest) {
  ...
  pid = -1;
  while (pid < 0) {
    pid = cluster->StartSingleMDS(1, "127.0.0.1:22223", 22229, mdsConf, true);
    LOG(INFO) << "mds 1 started on 127.0.0.1:22223, pid = " << pid;
    std::this_thread::sleep_for(std::chrono::seconds(3));
  }
...
TEST_F(MDSModuleException, StripeMDSExceptionTest) {
  ...
  // 1. 创建一个条带的卷
  int stripefd =
      curve::test::FileCommonOperation::Open("/test2", "curve", 1024 * 1024, 8);
