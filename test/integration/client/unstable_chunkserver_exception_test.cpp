 ...
 protected:
  static void SetUpTestCase() {
    ...
    pid_t pid = cluster->StartSingleEtcd(
        ...
        std::vector<std::string>{" --name module_exception_curve_unstable_cs"});
  ...
  static void StartAllChunkserver() {
    ...
    for (auto port : chunkserverPorts) {
      ...
      pid_t pid = cluster->StartSingleChunkServer(id, param.addr, param.conf);
  ...
  static void OpenAndWrite(const std::string& filename) {
    ...
    for (int i = 0; i < 5; ++i) {
      ...
      LOG(INFO) << "write " << filename << ", thread " << (i + 1) << " started";
    }
...
TEST_F(UnstableCSModuleException, HangOneZone) {
  ...
  for (int i = 0; i < 2; ++i) {
    openAndWriteThreads.emplace_back(&UnstableCSModuleException::OpenAndWrite,
