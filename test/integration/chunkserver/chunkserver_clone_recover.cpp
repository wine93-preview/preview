 ...
 public:
  ...
  void SetUp() {
    ...
    cluster_->PrepareConfig<curve::ClientConfigGenerator>(clientConfPath,
                                                          clientConf);
    cluster_->PrepareConfig<curve::S3ConfigGenerator>(kS3ConfigPath, s3Conf);
    ...
    pid_t pid =
        cluster_->StartSingleEtcd(1, ETCD_CLIENT_IP_PORT, ETCD_PEER_IP_PORT,
                                  std::vector<string>{" --name " + ETCD_NAME});
    LOG(INFO) << "etcd 1 started on " + ETCD_CLIENT_IP_PORT + ", pid = " << pid;
    ASSERT_GT(pid, 0);
    ASSERT_TRUE(cluster_->WaitForEtcdClusterAvalible(5));

    // 2. 先启动一个mds，让其成为leader，然后再启动另外两个mds节点
    pid = cluster_->StartSingleMDS(0, MDS0_IP_PORT, MDS0_DUMMY_PORT, mdsConf0,
                                   true);
    ...
    Json::Value topo;
    Json::Value servers;
    std::string chunkServerIpPort[] = {
        CHUNK_SERVER0_IP_PORT, CHUNK_SERVER1_IP_PORT, CHUNK_SERVER2_IP_PORT};
    for (int i = 0; i < 3; ++i) {
      Json::Value server;
    ...
    struct ChunkServiceOpConf conf0 = {&leaderPeer_, logicPoolId_, copysetId_,
                                       5000};
    opConf_ = conf0;
   ...
   * @return: IO是否成功完成
   */
  bool HandleAioWriteRequest(uint64_t offset, uint64_t size, const char* data) {
    gCond.Reset(1);
    gIoRet = 0;
...
TEST_F(CSCloneRecoverTest, CloneFromCurveByReadChunk) {
  ...
  location.assign(CURVEFS_FILENAME + ":" + std::to_string(kChunkSize) + "@cs");
  ...
  ASSERT_EQ(
      0, verify.VerifyReadChunk(cloneChunk1, sn1, 0, 8 * KB, cloneData1.get()));
  ...
  for (int offset = 0; offset < kChunkSize; offset += kChunkServerMaxIoSize) {
    ASSERT_EQ(0,
              verify.VerifyReadChunk(cloneChunk1, sn1, offset,
...
TEST_F(CSCloneRecoverTest, CloneFromCurveByReadChunkWhenLazyAlloc) {
  ...
  temp.assign(8 * KB, 'b');
  ASSERT_EQ(
      0, verify.VerifyWriteChunk(cloneChunk1, sn1, 4 * KB, 8 * KB, temp.c_str(),
                                 cloneData1.get(), CURVEFS_FILENAME, 0));
  ASSERT_EQ(0,
            verify.VerifyGetChunkInfo(cloneChunk1, sn1, NULL_SN, string("")));
  ASSERT_EQ(0, verify.VerifyReadChunk(cloneChunk1, sn1, 0, 12 * KB,
...
TEST_F(CSCloneRecoverTest, RecoverFromS3ByReadChunk) {
  ...
  std::shared_ptr<string> cloneData1(new string(chunkData1_));
  ASSERT_EQ(
      0, verify.VerifyReadChunk(cloneChunk1, sn3, 0, 8 * KB, cloneData1.get()));
  ...
  for (int offset = 0; offset < kChunkSize; offset += kChunkServerMaxIoSize) {
    ASSERT_EQ(0,
              verify.VerifyReadChunk(cloneChunk1, sn3, offset,
                                     kChunkServerMaxIoSize, cloneData1.get()));
  }
