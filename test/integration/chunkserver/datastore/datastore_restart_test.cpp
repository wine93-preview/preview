    ...
    if (err == CSErrorCode::Success) {
      ...
      if (info.isClone) {
        ...
        for (auto& range : setRanges) {
          ...
          size_t length = (range.endIndex - range.beginIndex + 1) * PAGE_SIZE;
          (*datastore_)
              ->ReadChunk(id_, info.curSn, (chunkData + offset), offset,
                          length);
        }
      } else {
        (*datastore_)->ReadChunk(id_, info.curSn, chunkData, 0, kMaxSize);
      }
      ...
      if (info.snapSn > 0) {
        ...
        (*datastore_)
            ->ReadSnapshotChunk(id_, info.snapSn, snapData, 0, kMaxSize);
...
class ExecWrite : public ExecStep {
 public:
  ExecWrite(std::shared_ptr<CSDataStore>* datastore, ChunkID id, SequenceNum sn,
            RangeData data)
...
class ExecPaste : public ExecStep {
 public:
  ExecPaste(std::shared_ptr<CSDataStore>* datastore, ChunkID id, RangeData data)
...
class ExecCreateClone : public ExecStep {
 public:
  ...
  void Exec() override {
    (*datastore_)->CreateCloneChunk(id_, sn_, correctedSn_, size_, location_);
  }
 ...
 private:
  ...
  bool ReplayLog(int lastStep) {
    ...
    for (int beginStep = 0; beginStep <= lastStep; ++beginStep) {
      ...
      if (!CheckStatus(lastStep)) {
        LOG(ERROR) << "Check chunk status failed."
                   << "begin step: step" << beginStep + 1 << ", last step: step"
                   << lastStep + 1;
        return false;
      }
    }
  ...
  bool CheckChunkData(std::shared_ptr<ExecStep> step) {
    ...
    if (info.isClone) {
      ...
      for (auto& range : setRanges) {
        off_t offset = range.beginIndex * PAGE_SIZE;
        size_t length = (range.endIndex - range.beginIndex + 1) * PAGE_SIZE;
        datastore->ReadChunk(id, info.curSn, (actualData + offset), offset,
                             length);
      }
    } else {
  ...
  bool CheckSnapData(std::shared_ptr<ExecStep> step) {
    ...
    CSErrorCode err;
    err =
        datastore->ReadSnapshotChunk(id, info.snapSn, actualData, 0, kMaxSize);
    if (err != CSErrorCode::Success) {
