...
TEST_F(DatastoreIntegrationTest, BasicTest) {
  ...
  errorCode = dataStore_->WriteChunk(id, sn, buf1_1_1, offset, length, nullptr);
  ASSERT_EQ(errorCode, CSErrorCode::Success);
  ...
  errorCode = dataStore_->WriteChunk(id, sn, buf1_1_2, offset, length, nullptr);
  ASSERT_EQ(errorCode, CSErrorCode::Success);
  ...
  offset = PAGE_SIZE;
  length = PAGE_SIZE;
  errorCode = dataStore_->WriteChunk(id, sn, buf1_1_3, offset, length, nullptr);
  ASSERT_EQ(errorCode, CSErrorCode::Success);
  ...
  offset = PAGE_SIZE;
  length = 2 * PAGE_SIZE;
  errorCode = dataStore_->WriteChunk(id, sn, buf1_1_4, offset, length, nullptr);
  ASSERT_EQ(errorCode, CSErrorCode::Success);
