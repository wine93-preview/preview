 ...
 protected:
  virtual void SetUp() {
    ...
    metaDir_ = "./" + std::to_string(PeerCluster::PeerToId(peer1_)) +
               "/chunkfilepool.meta";
    FilePoolHelper::PersistEnCodeMetaInfo(lfs_, kChunkSize, kPageSize, poolDir_,
                                          metaDir_);
  ...
  void TestBasicIO(std::shared_ptr<ChunkServiceVerify> verify) {
    ...
    ASSERT_EQ(0, verify->VerifyWriteChunk(chunkId, sn1, 0, 4 * KB, data.c_str(),
                                          &chunkData));
    ...
    ASSERT_EQ(0, verify->VerifyWriteChunk(chunkId, sn1, 0, 4 * KB, data.c_str(),
                                          &chunkData));
    ASSERT_EQ(0, verify->VerifyReadChunk(chunkId, sn1, 0, 12 * KB, &chunkData));
  ...
  void TestSnapshotIO(std::shared_ptr<ChunkServiceVerify> verify) {
    ...
    data.assign(length, 'a');
    ASSERT_EQ(0, verify->VerifyWriteChunk(chunk1, sn1, 0, 12 * KB, data.c_str(),
                                          &chunkData1a));
...
TEST_F(ChunkServerIoTest, BasicIO_from_external_ip) {
  ...
  struct ChunkServiceOpConf opConf = {&exPeer, logicPoolId_, copysetId_, 2000};
  auto verify = std::make_shared<ChunkServiceVerify>(&opConf);
  TestBasicIO(verify);
}
