...
TEST_F(TestSnapshotCloneClient, TestGetCloneRefStatusFalseNotInit) {
  ...
  auto ret =
      client_->GetCloneRefStatus(filename, user, &status, &fileCheckList);
...
TEST_F(TestSnapshotCloneClient, TestGetCloneRefStatusFalseParseFail) {
  ...
  EXPECT_CALL(*mocksnapshotcleonservice_, default_method(_, _, _, _))
      .WillOnce(DoAll(
          ...
          Invoke([](RpcController* controller, const HttpRequest* request,
                    HttpResponse* response,
      ...
      .WillOnce(
          DoAll(SetArgPointee<2>(response),
                Invoke([](RpcController* controller, const HttpRequest* request,
                  ...
                  Json::Value mainObj;
                  mainObj[kCodeStr] = std::to_string(kErrCodeInternalError);
