...
class TestCopysetClient : public ::testing::Test {
 protected:
  ...
  void SetUp() {
    ...
    mockCsClient_ =
        std::make_shared<MockChunkServerClient>(topo_, option, channelPool);
...
TEST_F(TestCopysetClient, TestDeleteChunkSnapshotOrCorrectSnSuccess) {
  ...
  EXPECT_CALL(*topo_, GetCopySet(_, _))
      .WillOnce(DoAll(SetArgPointee<1>(copyset), Return(true)));

  EXPECT_CALL(*mockCsClient_,
              DeleteChunkSnapshotOrCorrectSn(leader, logicalPoolId, copysetId,
                                             chunkId, sn))
      .WillOnce(Return(kMdsSuccess));

  int ret = client_->DeleteChunkSnapshotOrCorrectSn(logicalPoolId, copysetId,
                                                    chunkId, sn);
...
TEST_F(TestCopysetClient, TestDeleteChunkSnapshotOrCorrectSnRedirectSuccess) {
  ...
  EXPECT_CALL(*mockCsClient_, GetLeader(_, logicalPoolId, copysetId, _))
      .WillOnce(DoAll(SetArgPointee<3>(newLeader), Return(kMdsSuccess)));
...
TEST_F(TestCopysetClient, TestDeleteChunkSnapshotOrCorrectSnGetCopysetFail) {
  ...
  copyset.SetCopySetMembers({0x01, 0x02, 0x03});
  EXPECT_CALL(*topo_, GetCopySet(_, _))
      .WillOnce(DoAll(SetArgPointee<1>(copyset), Return(false)));
...
TEST_F(TestCopysetClient, TestDeleteChunkSnapshotOrCorrectSnRedirectFail) {
  ...
  ChunkServerIdType newLeader = 0x02;
  EXPECT_CALL(*mockCsClient_, GetLeader(_, logicalPoolId, copysetId, _))
      .WillRepeatedly(DoAll(SetArgPointee<3>(newLeader), Return(kMdsFail)));
...
TEST_F(TestCopysetClient, TestDeleteChunkSuccess) {
  ...
  EXPECT_CALL(*mockCsClient_,
              DeleteChunk(leader, logicalPoolId, copysetId, chunkId, sn))
      .WillOnce(Return(kMdsSuccess));

  int ret = client_->DeleteChunk(logicalPoolId, copysetId, chunkId, sn);
  ASSERT_EQ(kMdsSuccess, ret);
}
...
TEST_F(TestCopysetClient, TestDeleteChunkRedirectSuccess) {
  ...
  EXPECT_CALL(*mockCsClient_,
              DeleteChunk(_, logicalPoolId, copysetId, chunkId, sn))
      .WillOnce(Return(kCsClientNotLeader))
      .WillOnce(Return(kMdsSuccess));
