...
TEST(TestCopysetManager, GenCopysetByCopysetNumSuccess) {
  ...
  ASSERT_TRUE(manager.GenCopyset(cluster, 3, &scatterWidth, &out));
...
TEST(TestCopysetManager, GenCopysetByScatterWidthSuccess) {
  ...
  ASSERT_TRUE(manager.GenCopyset(cluster, 0, &scatterWidth, &out));
}
...
TEST(TestCopysetManager, GenCopysetByCopysetNumAndValidSuccess) {
...

  {
    ...
    std::vector<Copyset> out;
    uint32_t scatterWidth = 0;
    ASSERT_TRUE(manager.GenCopyset(cluster, 6000, &scatterWidth, &out));
...

  {
    ...
    std::vector<Copyset> out;
    uint32_t scatterWidth = 0;
    ASSERT_TRUE(manager.GenCopyset(cluster, 9000, &scatterWidth, &out));
...

  {
    ...
    std::vector<Copyset> out;
    uint32_t scatterWidth = 0;
    ASSERT_TRUE(manager.GenCopyset(cluster, 12000, &scatterWidth, &out));
...

  {
    ...
    ASSERT_TRUE(manager.GenCopyset(cluster, 100, &scatterWidth, &out));
...

  {
    ...
    ASSERT_TRUE(manager2.GenCopyset(cluster, 100, &scatterWidth, &out));
    ASSERT_EQ(100, out.size());
  }
}
