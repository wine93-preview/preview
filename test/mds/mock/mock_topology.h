...
class MockTopology : public Topology {
 public:
  ...
  MOCK_METHOD1(AllocateCopySetId, CopySetIdType(PoolIdType logicalPoolId));
  ...
  MOCK_METHOD1(
      AddCopySetList,
      int(const std::vector<::curve::mds::topology::CopySetInfo>& copysets));
  ...
  MOCK_METHOD2(UpdateLogicalPoolAllocateStatus,
               int(const AllocateStatus& status, PoolIdType id));
  ...
  MOCK_METHOD2(UpdateChunkServerRwState,
               int(const ChunkServerStatus& rwState, ChunkServerIdType id));

  MOCK_METHOD2(UpdateChunkServerOnlineState,
               int(const OnlineState& onlineState, ChunkServerIdType id));

  MOCK_METHOD2(UpdateChunkServerDiskStatus,
               int(const ChunkServerState& state, ChunkServerIdType id));

  MOCK_METHOD2(UpdateChunkServerStartUpTime,
               int(uint64_t time, ChunkServerIdType id));
  ...
  MOCK_CONST_METHOD2(FindChunkServerNotRetired,
                     ChunkServerIdType(const std::string& hostIp,
                                       uint32_t port));
  // get
  MOCK_CONST_METHOD2(GetLogicalPool, bool(PoolIdType poolId, LogicalPool* out));
  ...
  MOCK_CONST_METHOD3(GetLogicalPool, bool(const std::string& logicalPoolName,
                                          const std::string& physicalPoolName,
                                          LogicalPool* out));

  MOCK_CONST_METHOD2(GetPhysicalPool, bool(const std::string& physicalPoolName,
                                           PhysicalPool* out));
  ...
  MOCK_CONST_METHOD3(GetChunkServerNotRetired,
                     bool(const std::string& hostIp, uint32_t port,
                          ChunkServer* out));
  MOCK_METHOD2(GetBelongPhysicalPoolId,
               int(ChunkServerIdType csId, PoolIdType* physicalPoolIdOut));
  ...
  MOCK_CONST_METHOD2(GetCopySetInfosInLogicalPool,
                     std::vector<CopySetInfo>(PoolIdType logicalPoolId,
                                              CopySetFilter filter));
  ...
  MOCK_METHOD1(GetHostNameAndPortById, std::string(ChunkServerIdType csId));
...
class MockTopologyStat : public TopologyStat {
 public:
  ...
  MOCK_METHOD2(UpdateChunkServerStat,
               void(ChunkServerIdType csId, const ChunkServerStat& stat));

  MOCK_METHOD2(GetChunkServerStat,
               bool(ChunkServerIdType csId, ChunkServerStat* stat));
  MOCK_METHOD2(GetChunkPoolSize, bool(PoolIdType pId, uint64_t* chunkPoolSize));
  MOCK_METHOD2(updateChunkFilepoolConfig,
               void(bool useChunkFilepool_, bool useChunkFilePoolAsWalPool_));
};
