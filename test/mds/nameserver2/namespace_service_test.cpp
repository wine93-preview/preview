...
class NameSpaceServiceTest : public ::testing::Test {
 ...
 protected:
  void SetUp() override {
    ...
    cleanCore_ = std::make_shared<CleanCore>(storage_, client, allocStatistic_);
    ...
    cleanManager_ =
        std::make_shared<CleanManager>(cleanCore_, cleanTaskManager_, storage_);
    ...
    ASSERT_EQ(curveFSOptions.defaultChunkSize, kCurveFS.GetDefaultChunkSize());
  ...
  bool CheckFilename(const std::string& filename, uint64_t dtime) {
    ...
    if (n <= 2 || !::curve::common::StringToUll(items[n - 1], &time) ||
      ...
      LOG(INFO) << "unexpected filename: " << filename << ", dtime: " << dtime
                << ", time in file: " << time;
      return false;
    }
    return true;
...
TEST_F(NameSpaceServiceTest, test1) {
  ...
  ASSERT_EQ(
      server.AddService(&namespaceService, brpc::SERVER_DOESNT_OWN_SERVICE), 0);
  ...
  // test get allocated size
  {
    ...
    ASSERT_NE(response.allocsizemap().end(), response.allocsizemap().find(0));
...
TEST_F(NameSpaceServiceTest, deletefiletests) {
  ...
  while (attempts < 100) {
    ...
    stub.CheckSnapShotStatus(&cntl, &checkRequest, &checkResponse, NULL);
    if (!cntl.Failed()) {
      if (checkResponse.statuscode() == StatusCode::kSnapshotFileNotExists) {
        break;
      } else {
        ASSERT_EQ(checkResponse.statuscode(), StatusCode::kOK);
        sleep(1);
  ...
  EXPECT_CALL(*chunkService, DeleteChunk(_, _, _, _))
      .WillRepeatedly(DoAll(
          ...
          Invoke([](RpcController* controller, const ChunkRequest* chunkRequest,
                    ChunkResponse* chunkResponse,
  ...
  std::vector<std::string> dfiles;
  {
    ...
    for (int i = 0; i < 2; i++) {
      ...
      ASSERT_EQ(response.fileinfo(i).filestatus(), FileStatus::kFileCreated);
...
TEST_F(NameSpaceServiceTest, testRecoverFile) {
  ...
  date = TimeUtility::GetTimeofDayUs();
  str2sig = Authenticator::GetString2Signature(date, authOptions.rootOwner);
  sig = Authenticator::CalcString2Signature(str2sig, authOptions.rootPassword);
  listRequest.set_signature(sig);
  listRequest.set_filename(RECYCLEBINDIR);
  listRequest.set_owner(authOptions.rootOwner);
  listRequest.set_date(date);
  ...
  cntl.Reset();
  stub.CreateCloneFile(&cntl, &createCloneRequest, &createCloneResponse, NULL);
  if (!cntl.Failed()) {
...
TEST_F(NameSpaceServiceTest, TestDeAllocateSegment) {
  ...
  ASSERT_EQ(
      0, server.AddService(&nameSpaceSerivce, brpc::SERVER_DOESNT_OWN_SERVICE));
