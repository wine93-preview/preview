...
class FackTopologyChunkAllocator : public TopologyChunkAllocator {
 public:
  ...
  bool AllocateChunkRandomInSingleLogicalPool(
      FileType fileType, uint32_t chunkNumer, ChunkSizeType chunkSize,
...
class FakeNameServerStorage : public NameServerStorage {
 public:
  StoreStatus PutFile(const FileInfo& fileInfo) override {
    ...
    memKvMap_.insert(std::move(
        std::pair<std::string, std::string>(storeKey, std::move(value))));
    return StoreStatus::OK;
  }

  StoreStatus GetFile(InodeID id, const std::string& filename,
  ...
  StoreStatus DeleteFile(InodeID id, const std::string& filename) override {
    std::lock_guard<std::mutex> guard(lock_);
    std::string storeKey =
        NameSpaceStorageCodec::EncodeFileStoreKey(id, filename);
    auto iter = memKvMap_.find(storeKey);
  ...
  StoreStatus RenameFile(const FileInfo& oldfileInfo,
    ...
    memKvMap_.insert(std::move(
        std::pair<std::string, std::string>(newStoreKey, std::move(value))));

    return StoreStatus::OK;
  }

  StoreStatus ReplaceFileAndRecycleOldFile(
      const FileInfo& oldFInfo, const FileInfo& newFInfo,
      const FileInfo& conflictFInfo, const FileInfo& recycleFInfo) override {
    std::lock_guard<std::mutex> guard(lock_);

    std::string oldStoreKey = NameSpaceStorageCodec::EncodeFileStoreKey(
        oldFInfo.parentid(), oldFInfo.filename());
    std::string newStoreKey = NameSpaceStorageCodec::EncodeFileStoreKey(
        newFInfo.parentid(), newFInfo.filename());
    std::string conflictStoreKey = NameSpaceStorageCodec::EncodeFileStoreKey(
        conflictFInfo.parentid(), conflictFInfo.filename());
    std::string recyclestoreKey = NameSpaceStorageCodec::EncodeFileStoreKey(
        recycleFInfo.parentid(), recycleFInfo.filename());
  ...
  StoreStatus MoveFileToRecycle(const FileInfo& originFileInfo,
    ...
    std::string recycleFileInfoKey = NameSpaceStorageCodec::EncodeFileStoreKey(
  ...
  StoreStatus ListFile(InodeID startid, InodeID endid,
  ...
  StoreStatus ListSegment(InodeID id, std::vector<PageFileSegment>* segments) {
    std::lock_guard<std::mutex> guard(lock_);
    std::string startStoreKey =
  ...
  StoreStatus ListSnapshotFile(InodeID startid, InodeID endid,
                               std::vector<FileInfo>* files) override {
    std::lock_guard<std::mutex> guard(lock_);
    std::string startStoreKey =
  ...
  StoreStatus GetSegment(InodeID id, uint64_t off,
  ...
  StoreStatus PutSegment(InodeID id, uint64_t off,
  ...
  StoreStatus DeleteSegment(InodeID id, uint64_t off,
                            int64_t* revision) override {
    std::lock_guard<std::mutex> guard(lock_);
    std::string storeKey =
        NameSpaceStorageCodec::EncodeSegmentStoreKey(id, off);
  ...
  StoreStatus SnapShotFile(const FileInfo* originalFileInfo,
    ...
    auto originalFileKey = NameSpaceStorageCodec::EncodeFileStoreKey(
        originalFileInfo->parentid(), originalFileInfo->filename());
    auto snapshotFileKey =
        NameSpaceStorageCodec::EncodeSnapShotFileStoreKey(  // NOLINT
            snapshotFileInfo->parentid(), snapshotFileInfo->filename());
