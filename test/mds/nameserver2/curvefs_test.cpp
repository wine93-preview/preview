...
TEST_F(CurveFSTest, testCreateFile1) {
...

  {
    ...
    auto statusCode = curvefs_->CreateFile(
        "/file1", "owner1", FileType::INODE_PAGEFILE, kMiniFileLength, 0, 0);
  ...
  // test only directory doesn't have throttle parameters
  {
    ...
    auto statusCode = curvefs_->CreateFile("/dir1", "owner1",
...
TEST_F(CurveFSTest, testGetFileInfo) {
  ...
  }
  {
    ...
    EXPECT_CALL(*storage_, GetFile(_, _, _))
        .Times(1)
        .WillOnce(DoAll(SetArgPointee<2>(fileInfo), Return(StoreStatus::OK)));
...
TEST_F(CurveFSTest, testDeleteFile) {
  ...
  // test delete recyclebin pagefile，cleanManager fail
  {
    ...
    ASSERT_EQ(curvefs_->DeleteFile(RECYCLEBINDIR + "/file1", kUnitializedFileID,
                                   true),
...
TEST_F(CurveFSTest, testGetAllocatedSize) {
  ...
  // test page file normal
  {
    ...
    EXPECT_CALL(*storage_, ListSegment(_, _))
        .Times(1)
        .WillOnce(DoAll(SetArgPointee<1>(segments), Return(StoreStatus::OK)));
    ASSERT_EQ(StatusCode::kOK,
              curvefs_->GetAllocatedSize("/tests", &allocSize));
    ASSERT_EQ(3 * segmentSize, allocSize.total);
    std::unordered_map<PoolIdType, uint64_t> expected = {{1, 2 * segmentSize},
                                                         {2, segmentSize}};
  ...
  // test directory normal
  {
    ...
    std::unordered_map<PoolIdType, uint64_t> expected = {{1, 6 * segmentSize},
                                                         {2, 3 * segmentSize}};
  }
  // test GetFile fail
  {
  ...
  // test file type not supported
  {
    ...
    EXPECT_CALL(*storage_, GetFile(_, _, _))
        .Times(1)
        .WillOnce(
            DoAll(SetArgPointee<2>(appendFileInfo), Return(StoreStatus::OK)));
...
TEST_F(CurveFSTest, testReadDir) {
  ...
  // test not directory
  {
    ...
    ASSERT_EQ(curvefs_->ReadDir("/file1", &items), StatusCode::kNotDirectory);
    items.clear();
  }
  ...
  // test getFile Not exist
  {
    ...
    ASSERT_EQ(curvefs_->ReadDir("/file1", &items), StatusCode::kDirNotExist);
  }
  ...
  // test listFile ok
  {
    ...
    EXPECT_CALL(*storage_, ListFile(_, _, _))
        .Times(1)
        .WillOnce(
            DoAll(SetArgPointee<2>(sideEffectArgs), Return(StoreStatus::OK)));
...
TEST_F(CurveFSTest, testRecoverFile) {
  // recover ok
  {
    ...
    EXPECT_CALL(*storage_, GetFile(_, _, _))
        .Times(3)
        .WillOnce(Return(StoreStatus::KeyNotExist))
        .WillOnce(DoAll(SetArgPointee<2>(fileInfo2), Return(StoreStatus::OK)))
        .WillOnce(DoAll(SetArgPointee<2>(fileInfo1), Return(StoreStatus::OK)));
...
TEST_F(CurveFSTest, testRenameFile) {
  ...
  // new file exist, rename success
  {
    ...
    EXPECT_CALL(*storage_, GetFile(_, _, _))
        ...
        // 查找/trash/file2
        .WillOnce(DoAll(SetArgPointee<2>(fileInfo2), Return(StoreStatus::OK)))
        .WillOnce(DoAll(SetArgPointee<2>(fileInfo3), Return(StoreStatus::OK)))
...
TEST_F(CurveFSTest, testExtendFile) {
  ...
  // test enlarge size ok
  {
    ...
    EXPECT_CALL(*storage_, PutFile(_))
        .Times(1)
        .WillOnce(DoAll(SaveArg<0>(&modifiedInfo), Return(StoreStatus::OK)));
...
TEST_F(CurveFSTest, testGetOrAllocateSegment) {
  ...
  // test normal get & allocate not exist segment
  {
    ...
    ASSERT_EQ(curvefs_->GetOrAllocateSegment("/user1/file2", 0, true, &segment),
              StatusCode::kOK);
  }
  ...
  // file length < segment offset + segmentsize
  {
    ...
    ASSERT_EQ(curvefs_->GetOrAllocateSegment("/user1/file2", kMiniFileLength,
                                             false, &segment),
              StatusCode::kParaError);
  }
...
TEST_F(CurveFSTest, TestDeAllocateSegment) {
  ...
  // segment offset not equal to argument
  {
    ...
    EXPECT_CALL(*storage_, GetFile(_, _, _))
        .WillOnce(DoAll(SetArgPointee<2>(fileInfo), Return(StoreStatus::OK)));
    EXPECT_CALL(*storage_, GetSegment(_, _, _))
        .WillOnce(DoAll(SetArgPointee<2>(segment), Return(StoreStatus::OK)));
  ...
  // storage discard segment success
  {
    ...
    ASSERT_EQ(StatusCode::kOK, curvefs_->DeAllocateSegment(filename, offset));
  }
}
...
TEST_F(CurveFSTest, testCreateSnapshotFile) {
  ...
  }
  {
    ...
    FileInfo info;
    ASSERT_EQ(StatusCode::kOK,
              curvefs_->RefreshSession(fileName, "", 0, "", "127.0.0.1", 1234,
                                       "0.0.5", &info));

    FileInfo snapShotFileInfoRet;
    ASSERT_EQ(
        curvefs_->CreateSnapShotFile("/snapshotFile1WithInvalidClientVersion2",
                                     &snapShotFileInfoRet),
  ...
  }
  {
    ...
    FileInfo snapShotFileInfoRet;
    ASSERT_EQ(
        curvefs_->CreateSnapShotFile("/snapshotFile1", &snapShotFileInfoRet),
        StatusCode::kFileUnderSnapShot);
  }
  {
  ...
  }
  {
    ...
    ASSERT_EQ(
        ...
        originalFile.filename() + "-" + std::to_string(originalFile.seqnum()));
  ...
  }
  {
    ...
    FileInfo snapShotFileInfoRet;
    ASSERT_EQ(
        curvefs_->CreateSnapShotFile("/originalFile2", &snapShotFileInfoRet),
        StatusCode::kOK);
    ASSERT_EQ(snapShotFileInfoRet.parentid(), originalFile.id());
    ASSERT_EQ(
        snapShotFileInfoRet.filename(),
...
TEST_F(CurveFSTest, testListSnapShotFile) {
  ...
  }
  {
    ...
    EXPECT_CALL(*storage_, ListSnapshotFile(_, _, _))
        .Times(1)
        .WillOnce(
            DoAll(SetArgPointee<2>(snapFileInfos), Return(StoreStatus::OK)));
...
TEST_F(CurveFSTest, testGetSnapShotFileInfo) {
  ...
  }
  {
    ...
    FileInfo snapshotFileInfo;
    ASSERT_EQ(
        curvefs_->GetSnapShotFileInfo("/originalFile", 1, &snapshotFileInfo),
...
TEST_F(CurveFSTest, GetSnapShotFileSegment) {
  ...
  }
  {
    ...
    PageFileSegment segment;
    ASSERT_EQ(curvefs_->GetSnapShotFileSegment("/originalFile", 1, 1, &segment),
              StatusCode::kParaError);
  }
  {
    ...
    PageFileSegment segment;
    ASSERT_EQ(curvefs_->GetSnapShotFileSegment("/originalFile", 1, 0, &segment),
  ...
  }
  {
    ...
    EXPECT_CALL(*storage_, GetSegment(_, _, _))
        .Times(1)
        .WillOnce(
            DoAll(SetArgPointee<2>(expectSegment), Return(StoreStatus::OK)));

    PageFileSegment segment;
    ASSERT_EQ(curvefs_->GetSnapShotFileSegment("/originalFile", 1, 0, &segment),
              StatusCode::kOK);
    ASSERT_EQ(expectSegment.SerializeAsString(), segment.SerializeAsString());
  }
}
...
TEST_F(CurveFSTest, CheckSnapShotFileStatus) {
  ...
  // snapshot file is not deleting
  {
    ...
    uint32_t progress;
    ASSERT_EQ(curvefs_->CheckSnapShotFileStatus("/originalFile", 1, &fileStatus,
                                                &progress),
              StatusCode::kOK);
  ...
  // snapshot file is deleting, task is not exist, delete success
  {
    ...
    EXPECT_CALL(*storage_, ListSnapshotFile(_, _, _))
        ...
        .WillOnce(
            DoAll(SetArgPointee<2>(snapShotFiles), Return(StoreStatus::OK)))
        .WillOnce(
            DoAll(SetArgPointee<2>(snapShotFiles2), Return(StoreStatus::OK)));
  ...
  // snapshot file is deleting, task is PROGRESSING
  {
    ...
    auto task = std::make_shared<SnapShotCleanTask>(1, nullptr, originalFile);
    TaskProgress taskProgress;
    taskProgress.SetProgress(50);
    taskProgress.SetStatus(TaskStatus::PROGRESSING);
    task->SetTaskProgress(taskProgress);
    EXPECT_CALL(*mockcleanManager_, GetTask(_)).Times(1).WillOnce(Return(task));
...
TEST_F(CurveFSTest, testOpenFile) {
  ...
  // open clone file, clone source is not a valid curve file
  {
    ...
    EXPECT_CALL(*storage_, GetFile(_, _, _)).WillOnce(Return(StoreStatus::OK));
...
TEST_F(CurveFSTest, testCloseFile) {
  ...
  ASSERT_EQ(curvefs_->OpenFile("/file1", "127.0.0.1", &protoSession, &fileInfo),
            StatusCode::kOK);
...
TEST_F(CurveFSTest, testRefreshSession) {
  ...
  // 执行成功
  {
    ...
    ASSERT_EQ(curvefs_->RefreshSession("/file1", protoSession.sessionid(), date,
                                       "signaturexxxx", "127.0.0.1", 1234, "",
                                       &fileInfo1),
              StatusCode::kOK);
...
TEST_F(CurveFSTest, testCheckRenameNewfilePathOwner) {
  ...
  // root用户，签名匹配，date超时
  {
    ...
    std::string sig =
        Authenticator::CalcString2Signature(str2sig, authOptions_.rootPassword);
    ASSERT_EQ(curvefs_->CheckDestinationOwner(filename, authOptions_.rootOwner,
                                              sig, date),
              StatusCode::kOK);
  ...
  // root用户，签名不匹配
  {
    ASSERT_EQ(curvefs_->CheckDestinationOwner("/file1", authOptions_.rootOwner,
                                              "wrongpass", date),
              StatusCode::kOwnerAuthFail);
  }
...
TEST_F(CurveFSTest, testCheckPathOwner) {
  ...
  // root用户，签名匹配, 并检测date过期
  {
    ...
    ASSERT_EQ(
        curvefs_->CheckPathOwner(filename, authOptions_.rootOwner, sig, date),
        StatusCode::kOK);

    ASSERT_EQ(curvefs_->CheckPathOwner(filename, authOptions_.rootOwner, sig,
                                       date + 15 * 2000 * 2000),
              StatusCode::kOwnerAuthFail);
  }
...
TEST_F(CurveFSTest, testCheckFileOwner) {
  ...
  // root用户，签名匹配
  {
    ...
    ASSERT_EQ(
        curvefs_->CheckFileOwner(filename, authOptions_.rootOwner, sig, date),
        StatusCode::kOK);
    ASSERT_EQ(curvefs_->CheckFileOwner(filename, authOptions_.rootOwner, sig,
                                       date + 15 * 2000 * 2000),
              StatusCode::kOwnerAuthFail);
  }
...
TEST_F(CurveFSTest, testCreateCloneFile) {
  ...
  ASSERT_EQ(curvefs_->CreateCloneFile(
                "/file1", "owner1", FileType::INODE_DIRECTORY, kMiniFileLength,
                kStartSeqNum, curvefs_->GetDefaultChunkSize(), 0, 0, nullptr),
            StatusCode::kParaError);

  ASSERT_EQ(
      curvefs_->CreateCloneFile("/file1", "owner1", FileType::INODE_PAGEFILE,
  ...
  }
  {
    ...
    auto statusCode = curvefs_->CreateCloneFile(
        "/file1", "owner1", FileType::INODE_PAGEFILE, kMiniFileLength,
        kStartSeqNum, curvefs_->GetDefaultChunkSize(), 0, 0, &fileInfo);
    ASSERT_EQ(statusCode, StatusCode::kOK);
...
TEST_F(CurveFSTest, testSetCloneFileStatus) {
  {
    ...
    ASSERT_EQ(curvefs_->SetCloneFileStatus("/dir1/file2", kUnitializedFileID,
    ...
    const struct {
      ...
      int putFileTime;
        ...
        {FileStatus::kFileCloning, FileStatus::kFileCloneMetaInstalled,
         StatusCode::kOK, 1},
        {FileStatus::kFileCloning, FileStatus::kFileCloning, StatusCode::kOK,
         1},
        ...
        {FileStatus::kFileCloned, FileStatus::kFileCloned, StatusCode::kOK, 1},
        ...
        {FileStatus::kFileBeingCloned, FileStatus::kFileBeingCloned,
         StatusCode::kOK, 1},
        {FileStatus::kFileCloned, FileStatus::kFileBeingCloned, StatusCode::kOK,
         1},
        {FileStatus::kFileBeingCloned, FileStatus::kFileCloned, StatusCode::kOK,
         1},
        {FileStatus::kFileCreated, FileStatus::kFileCreated, StatusCode::kOK,
         1},
    ...
    for (int i = 0; i < sizeof(testCases) / sizeof(testCases[0]); i++) {
      {
        ...
        ASSERT_EQ(curvefs_->SetCloneFileStatus("/dir1", kUnitializedFileID,
...
TEST_F(CurveFSTest, Init) {
  // test getFile ok
  {
    ...
    for (int i = 0; i < sizeof(testCases) / sizeof(testCases[0]); i++) {
      ...
      ASSERT_EQ(
          testCases[i].ret,
          kCurveFS.Init(storage_, inodeIdGenerator_, mockChunkAllocator_,
                        mockcleanManager_, fileRecordManager_, allocStatistic_,
...
TEST_F(CurveFSTest, ListAllVolumesOnCopyset) {
  ...
  std::vector<std::string> fileNames;
  {
    ...
    EXPECT_CALL(*storage_, ListFile(_, _, _))
        .Times(2)
        .WillOnce(DoAll(SetArgPointee<2>(fileVec1), Return(StoreStatus::OK)))
        .WillOnce(DoAll(SetArgPointee<2>(fileVec2), Return(StoreStatus::OK)));
    ...
    EXPECT_CALL(*storage_, ListSegment(_, _))
        ...
        .WillOnce(DoAll(SetArgPointee<1>(segVec3), Return(StoreStatus::OK)));
    ASSERT_EQ(StatusCode::kOK,
...
TEST_F(CurveFSTest, TestUpdateFileThrottleParams) {
  ...
  // test add new one throttle
  {
    ...
    EXPECT_CALL(*storage_, GetFile(_, _, _))
        .WillOnce(DoAll(SetArgPointee<2>(fileInfo), Return(StoreStatus::OK)));
    EXPECT_CALL(*storage_, PutFile(_))
        .WillOnce(DoAll(SaveArg<0>(&updatedFileInfo), Return(StoreStatus::OK)));
  ...
  // test update existing throttle
  {
    ...
    *throttleParams->add_throttleparams() = existParams;
    ...
    ASSERT_EQ(ThrottleType::IOPS_TOTAL,
              updatedFileInfo.throttleparams().throttleparams(0).type());
    ASSERT_EQ(2000, updatedFileInfo.throttleparams().throttleparams(0).limit());
    ASSERT_EQ(10000,
...
TEST(StripeParamTest, Test) {
  ...
  rc = CheckStripeParam(segmentSize, chunkSize, 4096, segmentSize / chunkSize);
