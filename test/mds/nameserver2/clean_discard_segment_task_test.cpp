...
class CleanDiscardSegmentTaskTest : public ::testing::Test {
 public:
  void SetUp() override {
    ...
    client_ = std::make_shared<CopysetClient>(topology_, option_, channelPool_);
...
TEST_F(CleanDiscardSegmentTaskTest, CommonTest) {
  ...
  EXPECT_CALL(*storage_, ListDiscardSegment(_))
      .WillRepeatedly(
          DoAll(SetArgPointee<0>(discardSegments), Return(StoreStatus::OK)));
...
TEST_F(CleanDiscardSegmentTaskTest, TestListDiscardSegmentFailed) {
  ...
  EXPECT_CALL(*storage_, ListDiscardSegment(_))
      .WillRepeatedly(Return(StoreStatus::InternalError));

  EXPECT_CALL(*cleanManager_, SubmitCleanDiscardSegmentJob(_, _, _)).Times(0);
