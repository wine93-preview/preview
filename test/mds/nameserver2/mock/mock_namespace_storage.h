...
class MockNameServerStorage : public NameServerStorage {
 public:
  ...
  MOCK_METHOD3(GetFile, StoreStatus(InodeID, const std::string&, FileInfo*));

  MOCK_METHOD2(DeleteFile, StoreStatus(InodeID, const std::string&));

  MOCK_METHOD2(DeleteSnapshotFile, StoreStatus(InodeID, const std::string&));

  MOCK_METHOD2(RenameFile, StoreStatus(const FileInfo&, const FileInfo&));

  MOCK_METHOD4(ReplaceFileAndRecycleOldFile,
               StoreStatus(const FileInfo&, const FileInfo&, const FileInfo&,
                           const FileInfo&));

  MOCK_METHOD2(MoveFileToRecycle,
               StoreStatus(const FileInfo&, const FileInfo&));

  MOCK_METHOD3(ListFile,
               StoreStatus(InodeID, InodeID, std::vector<FileInfo>* files));

  MOCK_METHOD3(ListSnapshotFile,
               StoreStatus(InodeID, InodeID, std::vector<FileInfo>* files));

  MOCK_METHOD3(GetSegment,
               StoreStatus(InodeID, uint64_t, PageFileSegment* segment));

  MOCK_METHOD4(PutSegment, StoreStatus(InodeID, uint64_t,
                                       const PageFileSegment*, int64_t*));

  MOCK_METHOD3(DeleteSegment, StoreStatus(InodeID, uint64_t, int64_t*));

  MOCK_METHOD2(SnapShotFile, StoreStatus(const FileInfo*, const FileInfo*));
