...
TEST(TestAllocStatisticHelper, test_CalculateSegmentAlloc) {
  auto mockEtcdClient = std::make_shared<MockEtcdClient>();
  {
    ...
    std::map<PoolIdType, int64_t> out;
    ASSERT_EQ(-1, AllocStatisticHelper::CalculateSegmentAlloc(2, mockEtcdClient,
                                                              &out));
  }
  {
    ...
    EXPECT_CALL(*mockEtcdClient,
                ListWithLimitAndRevision(SEGMENTINFOKEYPREFIX,
                                         SEGMENTINFOKEYEND, GETBUNDLE, 2, _, _))
        .WillOnce(DoAll(SetArgPointee<4>(values), Return(EtcdErrCode::EtcdOK)));
    std::map<PoolIdType, int64_t> out;
    ASSERT_EQ(-1, AllocStatisticHelper::CalculateSegmentAlloc(2, mockEtcdClient,
                                                              &out));
  }
  {
    ...
    std::string lastKey = NameSpaceStorageCodec::EncodeSegmentStoreKey(1, 0);
    EXPECT_CALL(*mockEtcdClient,
                ListWithLimitAndRevision(SEGMENTINFOKEYPREFIX,
                                         SEGMENTINFOKEYEND, GETBUNDLE, 2, _, _))
    ...
    std::map<PoolIdType, int64_t> out;
    ASSERT_EQ(0, AllocStatisticHelper::CalculateSegmentAlloc(2, mockEtcdClient,
                                                             &out));
    ASSERT_EQ(1, out.size());
