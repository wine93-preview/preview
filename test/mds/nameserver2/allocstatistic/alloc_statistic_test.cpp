...
TEST_F(AllocStatisticTest, test_PeriodicPersist_CalculateSegmentAlloc) {
  ...
  EXPECT_CALL(*mockEtcdClient_,
              ListWithLimitAndRevision(SEGMENTINFOKEYPREFIX, SEGMENTINFOKEYEND,
                                       GETBUNDLE, 2, _, _))
      .Times(2)
      .WillOnce(Return(EtcdErrCode::EtcdCanceled))
  ...
  EXPECT_CALL(
      *mockEtcdClient_,
      ListWithLimitAndRevision(lastKey1, SEGMENTINFOKEYEND, GETBUNDLE, 2, _, _))
      .WillOnce(DoAll(SetArgPointee<4>(std::vector<std::string>{encodeSegment,
                                                                encodeSegment}),
                      SetArgPointee<5>(lastKey2), Return(EtcdErrCode::EtcdOK)));
  EXPECT_CALL(*mockEtcdClient_, GetCurrentRevision(_))
      .Times(2)
      .WillOnce(Return(EtcdErrCode::EtcdCanceled))
      .WillOnce(DoAll(SetArgPointee<0>(2), Return(EtcdErrCode::EtcdOK)));
