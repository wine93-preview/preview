 ...
 protected:
  ...
  virtual void SetUp() {
    ...
    testObj_ = std::make_shared<TopologyChunkAllocatorImpl>(
        topology_, allocStatistic_, topoStat_, chunkFilePoolAllocHelp_, option);
  }
...
TEST_F(TestTopologyChunkAllocator,
  ...
  bool ret = testObj_->AllocateChunkRandomInSingleLogicalPool(INODE_PAGEFILE, 2,
                                                              1024, &infos);
...
TEST_F(TestTopologyChunkAllocator,
  ...
  std::vector<CopysetIdInfo> infos;
  bool ret = testObj_->AllocateChunkRandomInSingleLogicalPool(INODE_PAGEFILE, 1,
                                                              1024, &infos);
...
TEST_F(TestTopologyChunkAllocator,
  ...
  ret = testObj_->AllocateChunkRandomInSingleLogicalPool(INODE_PAGEFILE, 2,
                                                         1024, &infos);
...
TEST_F(TestTopologyChunkAllocator,
  ...
  ret = testObj_->AllocateChunkRoundRobinInSingleLogicalPool(INODE_PAGEFILE, 3,
                                                             1024, &infos2);
...
TEST(TestAllocateChunkPolicy, TestChooseSingleLogicalPoolByWeightPoc) {
  ...
  for (int i = 0; i < 100000; i++) {
    PoolIdType pid;
    AllocateChunkPolicy::ChooseSingleLogicalPoolByWeight(poolWeightMap, &pid);
    poolMap[pid]++;
  }
