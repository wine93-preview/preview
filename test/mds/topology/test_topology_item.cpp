...
TEST_F(TestTopologyItem,
  ...
  ASSERT_EQ(true, LogicalPool::TransRedundanceAndPlaceMentPolicyFromJsonStr(
                      jsonstr, type, &rap));
...
TEST_F(TestTopologyItem,
  ...
  LogicalPoolType type = PAGEFILE;
  LogicalPool::RedundanceAndPlaceMentPolicy rap;
  ASSERT_EQ(false, LogicalPool::TransRedundanceAndPlaceMentPolicyFromJsonStr(
                       jsonstr, type, &rap));
}

TEST_F(TestTopologyItem, Test_GetRedundanceAndPlaceMentPolicyJsonStr_success) {
  LogicalPool::RedundanceAndPlaceMentPolicy rap;
  rap.pageFileRAP.replicaNum = 1;
  rap.pageFileRAP.copysetNum = 2;
  rap.pageFileRAP.zoneNum = 3;
  LogicalPool lpool(0x01, "pool1", 0x11, PAGEFILE, rap,
