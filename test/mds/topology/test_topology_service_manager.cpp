...
class TestTopologyServiceManager : public ::testing::Test {
 protected:
  ...
  virtual void SetUp() {
    ...
    topologyChunkAllocator_ = std::make_shared<TopologyChunkAllocatorImpl>(
        topology_, nullptr, topologystat_, chunkFilePoolAllocHelp_,
        topologyOption);
    CopysetOption copysetOption;
    copysetManager_ =
        std::make_shared<curve::mds::copyset::CopysetManager>(copysetOption);
    serviceManager_ = std::make_shared<TopologyServiceManager>(
        topology_, topologystat_, topologyChunkAllocator_, copysetManager_,
        regInfoBuilder_);
    serviceManager_->Init(topologyOption);

    mockCopySetService = new MockCopysetServiceImpl();
 ...
 protected:
  void PrepareAddLogicalPool(
    ...
    LogicalPool pool(id, name, phyPoolId, type, rap, policy, createTime, true,
                     true);
  ...
  void PrepareAddServer(ServerIdType id = 0x31,
    ...
    Server server(id, hostName, internalHostIp, 0, externalHostIp, 0, zoneId,
                  physicalPoolId, desc);
  ...
  void PrepareAddChunkServer(
      ...
      const std::string& externalHostIp = "testExternalIp", uint32_t port = 0,
      const std::string& diskPath = "/") {
    ChunkServer cs(id, token, diskType, serverId, hostIp, port, diskPath,
                   READWRITE, OnlineState::OFFLINE, externalHostIp);
  ...
  void PrepareAddCopySet(CopySetIdType copysetId, PoolIdType logicalPoolId,
                         ...
                         bool scaning = false, LastScanSecType lastScanSec = 0,
...
TEST_F(TestTopologyServiceManager, test_RegistChunkServer_SuccessWithExIp) {
  ...
  EXPECT_CALL(*idGenerator_, GenChunkServerId()).WillOnce(Return(csId));

  EXPECT_CALL(*storage_, StorageChunkServer(_)).WillOnce(Return(true));

  EXPECT_CALL(*regInfoBuilder_, BuildEpochMap(_)).WillOnce(Return(0));
...
TEST_F(TestTopologyServiceManager, test_GetServer_ByIdSuccess) {
  ...
  PrepareAddServer(serverId, "hostname1", "ip1", "ip2", zoneId, physicalPoolId,
                   "desc1");
...
TEST_F(TestTopologyServiceManager, test_ListZoneServer_ByIdSuccess) {
  ...
  PrepareAddServer(serverId, "hostname1", "ip1", "ip2", zoneId, physicalPoolId,
                   "desc1");
  PrepareAddServer(serverId2, "hostname2", "ip3", "ip4", zoneId, physicalPoolId,
                   "desc2");
...
TEST_F(TestTopologyServiceManager, test_GetZone_Success) {
  ...
  PoolIdType poolId = 0x11;
  PrepareAddPhysicalPool(poolId);
  PrepareAddZone(zoneId, "testZone", poolId, "desc1");
...
TEST_F(TestTopologyServiceManager, test_ListPoolZone_ByIdSuccess) {
  ...
  PrepareAddZone(zoneId2, "testZone2", poolId, "desc2");
...
static void CreateCopysetNodeFunc(
...
TEST_F(TestTopologyServiceManager, test_CreateLogicalPool_Success) {
  ...
  PrepareAddChunkServer(0x41, "token1", "nvme", 0x31, "127.0.0.1", "127.0.0.1",
                        port);
  PrepareAddChunkServer(0x42, "token2", "nvme", 0x32, "127.0.0.1", "127.0.0.1",
                        port);
  PrepareAddChunkServer(0x43, "token3", "nvme", 0x33, "127.0.0.1", "127.0.0.1",
                        port);
  ...
  EXPECT_CALL(*idGenerator_, GenCopySetId(_)).WillRepeatedly(Return(copysetId));

  EXPECT_CALL(*storage_, StorageCopySet(_)).WillRepeatedly(Return(true));

  CopysetResponse2 chunkserverResponse;
  chunkserverResponse.set_status(COPYSET_OP_STATUS::COPYSET_OP_STATUS_SUCCESS);
...
TEST_F(TestTopologyServiceManager, test_CreateLogicalPool_InvalidParam) {
  PoolIdType physicalPoolId = 0x11;
  PrepareAddPhysicalPool(physicalPoolId);
...
TEST_F(TestTopologyServiceManager, test_ListLogicalPool_ByIdSuccess) {
  ...
  ASSERT_THAT(response.logicalpoolinfos(0).type(), AnyOf(PAGEFILE, APPENDFILE));
  ...
  ASSERT_THAT(response.logicalpoolinfos(1).type(), AnyOf(PAGEFILE, APPENDFILE));
}
...
TEST_F(TestTopologyServiceManager, test_GetChunkServerListInCopySets_success) {
  PoolIdType logicalPoolId = 0x01;
  PoolIdType physicalPoolId = 0x11;
  CopySetIdType copysetId = 0x51;
...
TEST_F(TestTopologyServiceManager, test_GetCopySetsInChunkServer_ByIdSuccess) {
  PoolIdType logicalPoolId = 0x01;
  PoolIdType physicalPoolId = 0x11;
  ...
  PrepareAddChunkServer(0x41, "token1", "nvme", 0x31, "10.187.0.1",
                        "10.187.26.1", 8200);
  PrepareAddChunkServer(0x42, "token2", "nvme", 0x32, "10.187.0.2",
                        "10.187.26.2", 8200);
  PrepareAddChunkServer(0x43, "token3", "nvme", 0x33, "10.187.0.3",
                        "10.187.26.3", 8200);
  PrepareAddLogicalPool(logicalPoolId, "logicalPool1", physicalPoolId);
...
TEST_F(TestTopologyServiceManager, test_GetCopySetsInChunkServer_ByIpSuccess) {
  PoolIdType logicalPoolId = 0x01;
  PoolIdType physicalPoolId = 0x11;
...
TEST_F(TestTopologyServiceManager, test_GetCopySetsInChunkServer_InvalidParam) {
  GetCopySetsInChunkServerRequest request;
...
TEST_F(TestTopologyServiceManager, test_SetCopysetsAvailFlag) {
  ...
  // storage fail!
  {
    ...
    EXPECT_CALL(*storage_, UpdateCopySet(_)).WillOnce(Return(false));
    serviceManager_->SetCopysetsAvailFlag(&request, &response);
    ASSERT_EQ(kTopoErrCodeStorgeFail, response.statuscode());
  }
}
