 ...
 protected:
  ...
  void PrepareAddServer(ServerIdType id = 0x31,
    ...
    Server server(id, hostName, internalHostIp, internalPort, externalHostIp,
                  externalPort, zoneId, physicalPoolId, desc);
...
TEST_F(TestTopology, test_AddPhysicalPool_success) {
  PhysicalPool pool(0x11, "test1", "desc");
  EXPECT_CALL(*storage_, StoragePhysicalPool(_)).WillOnce(Return(true));
...
TEST_F(TestTopology, test_AddPhysicalPool_IdDuplicated) {
  ...
  PhysicalPool pool(id, "test1", "desc");
...
TEST_F(TestTopology, UpdatePhysicalPool_success) {
  PoolIdType physicalPoolId = 0x11;
  PrepareAddPhysicalPool(physicalPoolId, "name1", "desc1");

  PhysicalPool newPool(physicalPoolId, "name1", "desc2");
...
TEST_F(TestTopology, GetChunkServerInZone_success) {
  ...
  std::list<ChunkServerIdType> csList = topology_->GetChunkServerInZone(zoneId);
  ASSERT_THAT(csList, Contains(csId));
  ASSERT_THAT(csList, Contains(csId2));
}
