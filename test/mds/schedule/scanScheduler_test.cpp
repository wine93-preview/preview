...
class TestScanSchedule : public ::testing::Test {
 protected:
  ...
  void SetUp() override {
    ...
    scanScheduler_ =
        std::make_shared<ScanScheduler>(opt_, topoAdapter_, opController_);
  }
  ...
  void InsertOperator(CopySetInfo copysetInfo, bool start) {
    ...
    auto priority =
        start ? OperatorPriority::LowPriority : OperatorPriority::HighPriority;
...
TEST_F(TestScanSchedule, TestScanInterval) {
  auto lastScanTime =
      TimeUtility::GetTimeofDaySec() - opt_.scanIntervalSec + 10;
  auto copysetInfos1 = GetCopySetInfosInLogicalPool(1);
...
TEST_F(TestScanSchedule, TestScanSelectCancelWithCandidate) {
  ...
  copysetInfos[0].configChangeInfo.set_type(ConfigChangeType::START_SCAN_PEER);
  copysetInfos[1].scaning = true;
