...
::curve::mds::schedule::CopySetInfo GetCopySetInfoForTest() {
  ...
  return ::curve::mds::schedule::CopySetInfo(copySetKey, epoch, leader, peers,
                                             ConfigChangeInfo{},
                                             CopysetStatistics{});
}
...
std::vector<::curve::mds::topology::ChunkServer> GetTopoChunkServerForTest() {
  ...
  for (int i = 1; i <= 4; i++) {
    ...
    ::curve::mds::topology::ChunkServer chunkserver(
        i, "hello", "sata", i, ip, 9000, "", ChunkServerStatus::READWRITE);
    chunkserver.SetChunkServerState(::curve::mds::topology::ChunkServerState{});
    chunkserver.SetStartUpTime(::curve::common::TimeUtility::GetTimeofDaySec());
...
std::vector<::curve::mds::topology::Server> GetServerForTest() {
  ...
  for (int i = 1; i <= 4; i++) {
    std::string ip = "192.168.10." + std::to_string(i);
    ::curve::mds::topology::Server server(i, "server", ip, 0, "", 0, i, 1, "");
...
::curve::mds::topology::LogicalPool GetAppendECFileLogicalPoolForTest() {
  ...
  ::curve::mds::topology::LogicalPool::UserPolicy userPolicy;
  ::curve::mds::topology::LogicalPool logicalPool(
      1, "", 1, LogicalPoolType::APPENDECFILE, policy, userPolicy, 1000, true,
      true);
  return logicalPool;
}
}  // namespace schedule
