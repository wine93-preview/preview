...
class TestSchedulerHelper : public ::testing::Test {
 protected:
  ...
  void SetUp() override { topoAdapter_ = std::make_shared<MockTopoAdapter>(); }
...
TEST_F(TestSchedulerHelper, test_SatisfyScatterWidth_target) {
  ...
  bool target = true;
  {
    ...
    ASSERT_TRUE(SchedulerHelper::SatisfyScatterWidth(
        target, oldValue, newValue, minScatterWidth, scatterWidthRangePerent));
  }
  {
...
TEST_F(TestSchedulerHelper,
  ...
  EXPECT_CALL(*topoAdapter_, GetChunkServerScatterMap(3, _))
      .WillOnce(SetArgPointee<1>(replica3Map));
  int affected = 0;
  bool res = SchedulerHelper::InvovledReplicasSatisfyScatterWidthAfterMigration(
