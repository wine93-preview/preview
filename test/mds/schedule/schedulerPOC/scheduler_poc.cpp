 ...
 public:
  ...
  void BuildMassiveTopo() {
    ...
    for (int i = 1; i <= serverNum; i++) {
      ...
      serverMap_[i] = Server(static_cast<ServerIdType>(i), "", internalHostIP,
                             0, "", 0, i % zoneNum + 1, 1, "");
    }
    ...
    for (int i = 1; i <= serverNum; i++) {
      for (int j = 1; j <= diskNumPerServer; j++) {
        ...
        ChunkServer chunkserver(static_cast<ChunkServerIdType>(id), "", "sata",
                                i, serverMap_[i].GetInternalHostIp(), 9000 + j,
  ...
  std::vector<::curve::mds::topology::CopySetInfo> GetCopySetInfosInLogicalPool(
...
class CopysetSchedulerPOC : public testing::Test {
 protected:
  ...
  void PrintCopySetNumInOnlineChunkServer(PoolIdType lid = 0) {
    ...
    // 打印方差
    float avg =
        static_cast<float>(sumNumber) / static_cast<float>(numberMap.size());
    float variance = 0;
...
TEST_F(CopysetSchedulerPOC,
  ...
  do {
    ...
    ApplyOperatorsInOpController(
        std::set<ChunkServerIdType>{static_cast<ChunkServerIdType>(removeOne)});
  } while (removeOne > 0);
  PrintScatterWithInLogicalPool();
  PrintCopySetNumInLogicalPool();
  // ============================结果====================================
  ...
  do {
    ...
    for (int i = 1; i < 6; i++) {
      if (origin[i] == 0) {
        origin[i] = RandomOfflineOneChunkServer();
        idlist.emplace(origin[i]);
        ApplyOperatorsInOpController(std::set<ChunkServerIdType>{idlist});
        break;
      }
    }
