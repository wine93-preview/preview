 ...
 protected:
  ...
  void SetUp() override {
    ...
    topoAdapter_ = std::make_shared<TopoAdapterImpl>(
        mockTopo_, mockTopoManager_, mockTopoStat_);
  }
...
TEST_F(TestTopoAdapterImpl, test_copysetInfo) {
  ...
  }
  {
    ...
    EXPECT_CALL(*mockTopo_, GetChunkServer(1, _))
        .WillOnce(
            DoAll(SetArgPointee<1>(testTopoChunkServer[0]), Return(true)));
    EXPECT_CALL(*mockTopo_, GetChunkServer(2, _))
        .WillOnce(
            DoAll(SetArgPointee<1>(testTopoChunkServer[1]), Return(true)));
    EXPECT_CALL(*mockTopo_, GetChunkServer(3, _))
        .WillOnce(
            DoAll(SetArgPointee<1>(testTopoChunkServer[2]), Return(true)));
    EXPECT_CALL(*mockTopo_, GetChunkServer(4, _))
        .WillOnce(
            DoAll(SetArgPointee<1>(testTopoChunkServer[3]), Return(true)));
  ...
  }
  {
    ...
    EXPECT_CALL(*mockTopo_, GetLogicalPool(1, _))
        .WillOnce(DoAll(SetArgPointee<1>(lpool), Return(true)));
    EXPECT_CALL(*mockTopo_, GetChunkServer(1, _)).WillOnce(Return(false));
    ASSERT_FALSE(topoAdapter_->GetCopySetInfo(key, &info));
  }
  {
  ...
  }
  {
    ...
    EXPECT_CALL(*mockTopo_, GetCopySetsInCluster(_)).WillOnce(Return(infos));
    EXPECT_CALL(*mockTopo_, GetCopySet(_, _)).WillOnce(Return(false));
...
TEST_F(TestTopoAdapterImpl, test_other_functions) {
  ...
  auto testAppendECFileLogicalPool = GetAppendECFileLogicalPoolForTest();
  {
    ...
    EXPECT_CALL(*mockTopo_, GetLogicalPool(_, _))
        .WillOnce(
            DoAll(SetArgPointee<1>(testPageFileLogicalPool), Return(true)));
    ...
    EXPECT_CALL(*mockTopo_, GetLogicalPool(_, _))
        .WillOnce(
            DoAll(SetArgPointee<1>(testAppendFileLogicalPool), Return(true)));
    ...
    EXPECT_CALL(*mockTopo_, GetLogicalPool(_, _))
        .WillOnce(
            DoAll(SetArgPointee<1>(testAppendECFileLogicalPool), Return(true)));
    ASSERT_EQ(0, topoAdapter_->GetStandardZoneNumInLogicalPool(1));
  }
  {
...
TEST(TestChunkServerInfo, test_onlineState) {
  ChunkServerInfo cs(PeerInfo(1, 1, 1, "", 9000), OnlineState::ONLINE,
                     DiskState::DISKNORMAL, ChunkServerStatus::READWRITE, 2, 1,
                     2, ChunkServerStatisticInfo());
