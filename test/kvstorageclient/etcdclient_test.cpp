...
class TestEtcdClinetImp : public ::testing::Test {
 protected:
  ...
  void SetUp() override {
    ...
    ASSERT_EQ(EtcdErrCode::EtcdDeadlineExceeded, client_->Init(conf, 200, 3));
...
TEST_F(TestEtcdClinetImp, test_EtcdClientInterface) {
  ...
  for (int i = 0; i < 11; i++) {
    ...
    if (i <= 9) {
      ASSERT_EQ(EtcdErrCode::EtcdOK, client_->Put(encodeKey, encodeFileInfo));
      keyMap[i] = encodeKey;
      fileName[i] = filename;
    }
  ...
  std::string encodeNewFileInfo7Key = NameSpaceStorageCodec::EncodeFileStoreKey(
  ...
  Operation op9{OpType::OpPut, const_cast<char*>(encodeNewFileInfo7Key.c_str()),
...
TEST_F(TestEtcdClinetImp, test_CampaignLeader) {
...

  {
    ...
    // leader2启动线程observe
    common::Thread thread3(&EtcdClientImp::LeaderObserve, client1, targetOid,
                           leaderName2);
...
TEST_F(TestEtcdClinetImp, test_ListSegment) {
  ...
  std::string endKey = NameSpaceStorageCodec::EncodeSegmentStoreKey(id1 + 1, 0);
  std::vector<std::string> out;
