...
class CSDataStore_test : public testing::Test {
 public:
  ...
  void FakeEnv() {
    ...
    // fake Write
    ON_CALL(*lfs_, Write(Ge(1), Matcher<const char*>(NotNull()), Ge(0), Gt(0)))
        .WillByDefault(ReturnArg<3>());
    ...
    EXPECT_CALL(*lfs_, Read(1, NotNull(), 0, PAGE_SIZE))
        .WillRepeatedly(DoAll(
            SetArrayArgument<1>(chunk1MetaPage, chunk1MetaPage + PAGE_SIZE),
            Return(PAGE_SIZE)));
    ...
    EXPECT_CALL(*lfs_, Read(3, NotNull(), 0, PAGE_SIZE))
        .WillRepeatedly(DoAll(
            SetArrayArgument<1>(chunk2MetaPage, chunk2MetaPage + PAGE_SIZE),
            Return(PAGE_SIZE)));
  }
...
TEST_F(CSDataStore_test, InitializeErrorTest3) {
  ...
  EXPECT_CALL(*lfs_, Read(1, NotNull(), 0, PAGE_SIZE))
      .WillOnce(
          DoAll(SetArrayArgument<1>(chunk1MetaPage, chunk1MetaPage + PAGE_SIZE),
                Return(PAGE_SIZE)));
  EXPECT_FALSE(dataStore->Initialize());
...
TEST_F(CSDataStore_test, WriteChunkTest1) {
  ...
  EXPECT_CALL(*fpool_, GetFileImpl(chunk3Path, NotNull())).WillOnce(Return(0));
  ...
  EXPECT_CALL(*lfs_, Read(4, NotNull(), 0, PAGE_SIZE))
      .WillOnce(
          DoAll(SetArrayArgument<1>(chunk3MetaPage, chunk3MetaPage + PAGE_SIZE),
                Return(PAGE_SIZE)));
  // will write data
  EXPECT_CALL(*lfs_,
...
TEST_F(CSDataStore_test, WriteChunkTest3) {
  ...
  EXPECT_CALL(*lfs_, Read(3, NotNull(), 0, PAGE_SIZE))
      .WillRepeatedly(
          DoAll(SetArrayArgument<1>(chunk2MetaPage, chunk2MetaPage + PAGE_SIZE),
                Return(PAGE_SIZE)));
  EXPECT_TRUE(dataStore->Initialize());
...
TEST_F(CSDataStore_test, WriteChunkTest4) {
  ...
  offset = PAGE_SIZE;
  length = PAGE_SIZE - 1;
  EXPECT_CALL(*lfs_, Write(3, Matcher<const char*>(NotNull()), _, _)).Times(0);
  EXPECT_EQ(CSErrorCode::InvalidArgError,
            dataStore->WriteChunk(id, sn, buf, offset, length, nullptr));
...
TEST_F(CSDataStore_test, WriteChunkTest7) {
  ...
  EXPECT_CALL(*lfs_, Read(4, NotNull(), 0, PAGE_SIZE))
      .WillOnce(DoAll(SetArrayArgument<1>(metapage, metapage + PAGE_SIZE),
                      Return(PAGE_SIZE)));
  // will update metapage
  EXPECT_CALL(*lfs_, Write(3, Matcher<const char*>(NotNull()), 0, PAGE_SIZE))
      .Times(1);
  // will copy on write
  EXPECT_CALL(*lfs_, Read(3, NotNull(), PAGE_SIZE + offset, length)).Times(1);
...
TEST_F(CSDataStore_test, WriteChunkTest9) {
  ...
  // will copy on write
  EXPECT_CALL(*lfs_, Read(1, NotNull(), PAGE_SIZE + offset, length)).Times(1);
...
TEST_F(CSDataStore_test, WriteChunkTest13) {
  ...
  // 创建 clone chunk
  {
    ...
    EXPECT_CALL(*lfs_, Read(4, NotNull(), 0, PAGE_SIZE))
        .WillOnce(DoAll(
            SetArrayArgument<1>(chunk3MetaPage, chunk3MetaPage + PAGE_SIZE),
            Return(PAGE_SIZE)));
    EXPECT_EQ(
        CSErrorCode::Success,
        dataStore->CreateCloneChunk(id, sn, correctedSn, CHUNK_SIZE, location));
  }
  ...
  // case3:chunk存在，且是clone chunk，部分区域已写过，部分未写过
  {
    ...
    EXPECT_CALL(*lfs_,
                Write(4, Matcher<butil::IOBuf>(_), offset + PAGE_SIZE, length))
        .Times(1);
    EXPECT_CALL(*lfs_, Write(4, Matcher<const char*>(NotNull()), 0, PAGE_SIZE))
        .Times(1);
...
TEST_F(CSDataStore_test, WriteChunkTest15) {
  ...
  EXPECT_CALL(*lfs_, Read(2, NotNull(), 0, PAGE_SIZE))
      .WillRepeatedly(DoAll(SetArrayArgument<1>(chunk1SnapMetaPage,
                                                chunk1SnapMetaPage + PAGE_SIZE),
                            Return(PAGE_SIZE)));
  EXPECT_TRUE(dataStore->Initialize());
  ...
  EXPECT_CALL(*lfs_, Write(2, Matcher<const char*>(NotNull()), _, _)).Times(0);
  // will write data
  EXPECT_CALL(*lfs_,
              Write(1, Matcher<butil::IOBuf>(_), PAGE_SIZE + offset, length))
      .Times(1);
...
TEST_F(CSDataStore_test, WriteChunkErrorTest6) {
  ...
  // 创建 clone chunk
  {
    ...
    EXPECT_EQ(CSErrorCode::InvalidArgError,
              dataStore->CreateCloneChunk(id, sn, correctedSn, CHUNK_SIZE,
                                          longLocation));
  }
  // 创建 clone chunk
  {
...
TEST_F(CSDataStore_test, ReadChunkTest3) {
  ...
  memset(buf, 0, length);
  // test chunk exists
  EXPECT_CALL(*lfs_, Read(1, NotNull(), offset + PAGE_SIZE, length)).Times(1);
  EXPECT_EQ(CSErrorCode::Success,
            dataStore->ReadChunk(id, sn, buf, offset, length));
...
TEST_F(CSDataStore_test, ReadChunkTest4) {
  ...
  offset = 0;
  length = PAGE_SIZE;
  EXPECT_CALL(*lfs_, Read(4, NotNull(), offset + PAGE_SIZE, length)).Times(1);
  EXPECT_EQ(CSErrorCode::Success,
            dataStore->ReadChunk(id, sn, buf, offset, length));
...
TEST_F(CSDataStore_test, ReadSnapshotChunkTest3) {
  ...
  EXPECT_CALL(*lfs_, Read(1, NotNull(), PAGE_SIZE, PAGE_SIZE)).Times(1);
  EXPECT_CALL(*lfs_, Read(1, NotNull(), 4 * PAGE_SIZE, PAGE_SIZE)).Times(1);
  // read snapshot in[PAGE_SIZE, 3*PAGE_SIZE)
  EXPECT_CALL(*lfs_, Read(2, NotNull(), 2 * PAGE_SIZE, 2 * PAGE_SIZE)).Times(1);
  EXPECT_EQ(CSErrorCode::Success,
            dataStore->ReadSnapshotChunk(id, sn, readBuf, offset, length));
...
TEST_F(CSDataStore_test, ReadChunkMetaDataTest2) {
  ...
  memset(buf, 0, PAGE_SIZE);
  // test chunk exists
  EXPECT_CALL(*lfs_, Read(1, NotNull(), 0, PAGE_SIZE)).Times(1);
...
TEST_F(CSDataStore_test, DeleteChunkTest4) {
  ...
  // case1
  {
    ...
    EXPECT_EQ(CSErrorCode::BackwardRequestError, dataStore->DeleteChunk(id, 1));
  }
...
TEST_F(CSDataStore_test, CreateCloneChunkTest) {
  ...
  // case1:输入错误的参数
  {
    ...
    EXPECT_EQ(
        ...
        dataStore->CreateCloneChunk(id, sn, correctedSn, PAGE_SIZE, location));

    // sn == 0
    EXPECT_EQ(
        CSErrorCode::InvalidArgError,
        dataStore->CreateCloneChunk(id, 0, correctedSn, CHUNK_SIZE, location));
  ...
  // 返回ChunkConflictError，但是不会改变原chunk信息
  {
    ...
    EXPECT_EQ(CSErrorCode::ChunkConflictError,
              dataStore->CreateCloneChunk(id, sn + 1, correctedSn, CHUNK_SIZE,
                                          location));
    // correctedSn不一致
    EXPECT_EQ(CSErrorCode::ChunkConflictError,
              dataStore->CreateCloneChunk(id, sn, correctedSn + 1, CHUNK_SIZE,
                                          location));
    // location不一致
    EXPECT_EQ(
        CSErrorCode::ChunkConflictError,
        dataStore->CreateCloneChunk(id, sn, correctedSn, CHUNK_SIZE, "temp"));
    // 检查生成的clone chunk信息
    ASSERT_EQ(CSErrorCode::Success, dataStore->GetChunkInfo(id, &info));
    ASSERT_EQ(id, info.chunkId);
    ASSERT_EQ(sn, info.curSn);
...
TEST_F(CSDataStore_test, PasteChunkTest1) {
  ...
  // case6:chunk存在，且是clone chunk，部分区域已写过，部分未写过
  {
    ...
    // [2 * PAGE_SIZE, 4 * PAGE_SIZE)区域已写过，[0, PAGE_SIZE)为metapage
    EXPECT_CALL(*lfs_,
                Write(4, Matcher<const char*>(NotNull()), PAGE_SIZE, PAGE_SIZE))
        .Times(1);
    EXPECT_CALL(*lfs_, Write(4, Matcher<const char*>(NotNull()), 4 * PAGE_SIZE,
                             PAGE_SIZE))
        .Times(1);
    EXPECT_CALL(*lfs_, Write(4, Matcher<const char*>(NotNull()), 0, PAGE_SIZE))
        .Times(1);
  ...
  // case7:遍写整个chunk
  {
    ...
    length = CHUNK_SIZE;
    // [PAGE_SIZE, 4 * PAGE_SIZE)区域已写过，[0, PAGE_SIZE)为metapage
    EXPECT_CALL(*lfs_, Write(4, Matcher<const char*>(NotNull()), 5 * PAGE_SIZE,
...
TEST_F(CSDataStore_test, NormalChunkAlignmentTest) {
  ...
  {
    EXPECT_CALL(*lfs_, Write(0, Matcher<butil::IOBuf>(_), _, _)).Times(0);
  ...
  // read unaligned test
  {
    EXPECT_CALL(*lfs_, Read(0, NotNull(), _, _)).Times(0);
  ...
  // read aligned test
  {
    ...
    EXPECT_EQ(CSErrorCode::Success, dataStore->ReadChunk(id, sn, buf, 0, 512));
    EXPECT_EQ(CSErrorCode::Success, dataStore->ReadChunk(id, sn, buf, 0, 1024));
    EXPECT_EQ(CSErrorCode::Success, dataStore->ReadChunk(id, sn, buf, 0, 2048));
    EXPECT_EQ(CSErrorCode::Success, dataStore->ReadChunk(id, sn, buf, 0, 3072));
    EXPECT_EQ(CSErrorCode::Success, dataStore->ReadChunk(id, sn, buf, 0, 4096));
  }
