...
class CSChunkfilePoolMockTest : public testing::Test {
 public:
  ...
  Json::Value GenerateMetaJson() {
    // 正常的meta文件的json格式
    uint32_t crcsize = sizeof(kFilePoolMaigic) + sizeof(CHUNK_SIZE) +
                       sizeof(PAGE_SIZE) + poolDir.size();
    char* crcbuf = new char[crcsize];
    ::memcpy(crcbuf, kFilePoolMaigic, sizeof(kFilePoolMaigic));
    ::memcpy(crcbuf + sizeof(kFilePoolMaigic), &CHUNK_SIZE, sizeof(uint32_t));
    ::memcpy(crcbuf + sizeof(uint32_t) + sizeof(kFilePoolMaigic), &PAGE_SIZE,
             sizeof(uint32_t));
  ...
  void FakeMetaFile() {
    ...
    memcpy(buf, root.toStyledString().c_str(), root.toStyledString().size());
...
TEST_F(CSChunkfilePoolMockTest, PersistEnCodeMetaInfoTest) {
  // open失败
  {
    ...
    ASSERT_EQ(-1, FilePoolHelper::PersistEnCodeMetaInfo(
                      lfs_, CHUNK_SIZE, PAGE_SIZE, poolDir, poolMetaPath));
  }
  // open成功，write失败
  {
    EXPECT_CALL(*lfs_, Open(poolMetaPath, _)).WillOnce(Return(1));
  ...
  // open成功，write成功
  {
    ...
    EXPECT_CALL(*lfs_, Close(1)).Times(1);
    ASSERT_EQ(0, FilePoolHelper::PersistEnCodeMetaInfo(
                     lfs_, CHUNK_SIZE, PAGE_SIZE, poolDir, poolMetaPath));
  }
}
...
TEST_F(CSChunkfilePoolMockTest, DecodeMetaInfoFromMetaFileTest) {
  ...
  // open失败
  {
    ...
    ASSERT_EQ(-1, FilePoolHelper::DecodeMetaInfoFromMetaFile(
                      lfs_, poolMetaPath, metaFileSize, &chunksize,
                      &metapagesize, &chunkfilePath));
  }
  // read失败
  {
    EXPECT_CALL(*lfs_, Open(poolMetaPath, _)).WillOnce(Return(1));
...
TEST_F(CSChunkfilePoolMockTest, GetFileTest) {
  ...
  // 存在chunk，open时失败
  {
    ...
    EXPECT_CALL(*lfs_, Open(_, _)).Times(retryTimes).WillRepeatedly(Return(-1));
    EXPECT_CALL(*lfs_, Close(_)).Times(0);
    ASSERT_EQ(-1, pool.GetFile(targetPath, metapage));
  ...
  // 存在chunk，fsync时失败
  {
    ...
    EXPECT_CALL(*lfs_, Fsync(1)).Times(retryTimes).WillRepeatedly(Return(-1));
    EXPECT_CALL(*lfs_, Close(1)).Times(retryTimes);
    ASSERT_EQ(-1, pool.GetFile(targetPath, metapage));
    ASSERT_EQ(10 - retryTimes, pool.Size());
  }
  // 存在chunk，close时失败
  {
    ...
    EXPECT_CALL(*lfs_, Write(1, metapage, 0, PAGE_SIZE))
        .Times(retryTimes)
        .WillRepeatedly(Return(PAGE_SIZE));
    EXPECT_CALL(*lfs_, Fsync(1)).Times(retryTimes).WillRepeatedly(Return(0));
    EXPECT_CALL(*lfs_, Close(1)).Times(retryTimes).WillRepeatedly(Return(-1));
    ASSERT_EQ(-1, pool.GetFile(targetPath, metapage));
    ASSERT_EQ(10 - retryTimes, pool.Size());
  }
  ...
  // 存在chunk，rename时返回非EEXIST错误
  {
    ...
    EXPECT_CALL(*lfs_, Write(1, metapage, 0, PAGE_SIZE))
        ...
        .WillRepeatedly(Return(PAGE_SIZE));
    EXPECT_CALL(*lfs_, Fsync(1)).Times(retryTimes).WillRepeatedly(Return(0));
    EXPECT_CALL(*lfs_, Close(1)).Times(retryTimes).WillRepeatedly(Return(0));
  ...
  // write 时失败
  {
    ...
    EXPECT_CALL(*lfs_, Write(1, Matcher<const char*>(NotNull()), 0, fileSize))
        .Times(retryTimes)
        .WillRepeatedly(Return(-1));
    EXPECT_CALL(*lfs_, Close(1)).Times(retryTimes);
    ASSERT_EQ(-1, pool.GetFile(targetPath, metapage));
