...
TEST_F(Cli2Test, basic) {
  ...
  if (0 > pid1) {
    ...
    StartChunkserver(ip, port + 0, copysetdir.c_str(), confs, snapshotInterval,
                     electionTimeoutMs);
    return;
  }
  ...
  if (0 > pid2) {
    ...
    StartChunkserver(ip, port + 1, copysetdir.c_str(), confs, snapshotInterval,
                     electionTimeoutMs);
    return;
  }
  ...
  if (0 > pid3) {
    ...
    StartChunkserver(ip, port + 2, copysetdir.c_str(), confs, snapshotInterval,
                     electionTimeoutMs);
    return;
  }
  ...
  if (0 > pid4) {
    ...
    StartChunkserver(ip, port + 3, copysetdir.c_str(), confs, snapshotInterval,
                     electionTimeoutMs);
    return;
  }
  ...
  /* remove peer */
  {
    ...
    butil::Status st =
        curve::chunkserver::RemovePeer(logicPoolId, copysetId, conf, peer, opt);
    LOG(INFO) << "remove peer: " << st.error_code() << ", " << st.error_str();
    ASSERT_TRUE(st.ok());
  ...
  /*  add peer */
  {
    ...
    peer.set_address("127.0.0.1:9035:0");
    butil::Status st =
        curve::chunkserver::AddPeer(logicPoolId, copysetId, conf, peer, opt);
  ...
  /* change peers */
  {
    ...
    butil::Status st = curve::chunkserver::ChangePeers(logicPoolId, copysetId,
                                                       conf, newConf, opt);
    LOG(INFO) << "change peers: " << st.error_code() << ", " << st.error_str();
    ASSERT_TRUE(st.ok());
  }
  /* reset peer */
  {
    ...
    butil::Status status =
        curve::chunkserver::ResetPeer(logicPoolId, copysetId, conf, peer, opt);
