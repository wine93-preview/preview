...
class CurveFilesystemAdaptorTest : public testing::Test {
 public:
  void SetUp() {
    fsptr = curve::fs::LocalFsFactory::CreateFs(curve::fs::FileSystemType::EXT4,
                                                "/dev/sda");
    ...
    while (count < 4) {
      std::string filename =
          "./raftsnap/chunkfilepool/" + std::to_string(count);
    ...
    int ret = FilePoolHelper::PersistEnCodeMetaInfo(
        fsptr, chunksize, metapagesize, dirname,
...
TEST_F(CurveFilesystemAdaptorTest, open_file_test) {
  ...
  ClearFilePool();
  fa = fsadaptor->open("./raftsnap/11", O_RDONLY | O_CLOEXEC | O_CREAT, nullptr,
                       &e);
  ASSERT_EQ(nullptr, fa);
}
