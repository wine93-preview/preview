...
TEST_F(CurveFileServiceTest, success_normal_file) {
  ...
  EXPECT_CALL(*reader_, path()).WillRepeatedly(ReturnRef(path));
...
TEST_F(CurveFileServiceTest, error_normal_file_read_fail) {
  ...
  std::string path = "/test";
  EXPECT_CALL(*reader_, path()).WillRepeatedly(ReturnRef(path));
  EXPECT_CALL(*reader_, read_file(_, _, _, _, _, _, _)).WillOnce(Return(EPERM));
  brpc::Channel channel;
  brpc::Controller cntl;
  ASSERT_EQ(channel.Init(serverAddr, nullptr), 0);
  braft::FileService_Stub stub(&channel);
...
TEST(getCurveRaftBaseDir, test) {
  const struct {
    ...
    std::string second;
    std::string expect;
  } testCases[] = {{"/data/copyset/3121/raft_snapshot/snapshot_sds",
                    "raft_snapshot", "/data/copyset/3121/"},
                   {
                       "/raft_snapshot",
                       "raft_snapshot",
                       "/",
  ...
  for (int i = 0; i < sizeof(testCases) / sizeof(testCases[0]); i++) {
    auto ret = getCurveRaftBaseDir(testCases[i].first, testCases[i].second);
