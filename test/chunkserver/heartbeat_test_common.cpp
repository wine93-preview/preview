...
void HeartbeatTestCommon::CleanPeer(LogicPoolID poolId, CopysetID copysetId,
  ...
  while (t1 - t0 < 30 * 1000) {
    ...
    std::string sender = req->ip() + ":" + std::to_string(req->port()) + ":0";
    if (sender != peer) {
      continue;
    }
    if (req->copysetinfos_size() >= 1) {
      ...
      const curve::mds::heartbeat::CopySetInfo& info = req->copysetinfos(i);
...
void HeartbeatTestCommon::CreateCopysetPeers(
  ...
  for (auto it = cslist.begin(); it != cslist.end(); it++) {
    ...
    while (retry > 0) {
      ...
      for (auto peer = confPeers.begin(); peer != confPeers.end(); peer++) {
...
bool HeartbeatTestCommon::SameCopySetInfo(
  ...
  if (expect.has_configchangeinfo()) {
    ...
    if (orig.configchangeinfo().type() != expect.configchangeinfo().type()) {
      return false;
    }
  ...
  while (butil::monotonic_time_ms() - startTime < timeLimit) {
    ...
    if (leader == sender) {
      ...
      if (req->copysetinfos_size() == 1) {
        ...
        LOG(INFO) << "req->copysetinfos:" << req->copysetinfos(0).DebugString()
