...
DEFINE_int32(io_count, 1024 * 1024,
...
DEFINE_string(raftconf, "172.17.0.2:8200:0,172.17.0.2:8201:0,172.17.0.2:8202:0",
...
void prepare_write_request(ThreadInfo* tinfo, CHUNK_OP_TYPE* op, size_t* offset,
                           size_t* len);
void prepare_randwrite_request(ThreadInfo* tinfo, CHUNK_OP_TYPE* op,
...
void (*request_generator[IO_PATTERN_MAX])(ThreadInfo*, CHUNK_OP_TYPE*, size_t*,
                                          size_t*) = {
    NULL,
...
void wait_for_queue(ThreadInfo* tinfo) { thread_wait[tinfo->wait_mode](tinfo); }
...
int update_leader(CopysetInfo* copyset) {
  ...
  butil::Status status = curve::chunkserver::GetLeader(
      copyset->poolId, copyset->copysetId, copyset->conf, &peerId);
  if (status.ok()) {
    ...
    if (copyset->leader == -1) {
      LOG(ERROR) << "Failed to enumerate leader of copyset<" << copyset->poolId
                 << ", " << copyset->copysetId << ">";
      return -1;
    }
  }
...
void chunk_io_complete(IoContext* ioCxt) {
  ...
  LOG_IF(INFO, FLAGS_verbose)
      << "Response from <" << copyset->poolId << ", " << copyset->copysetId
      << ">:" << copyset->leader << ":" << ioCxt->req->chunkid() << ":"
      << ioCxt->req->offset() << ":" << ioCxt->req->size()
      << ", infligth I/Os: "
      << tinfo->ioinflight.load(std::memory_order_acquire)  // NOLINT
      << ", resp data size: "
  ...
  if (cntl->Failed()) {
    LOG_IF(WARNING, FLAGS_verbose)
        << "Thread " << tinfo->id << " failed to complete " << tinfo->io_count
        << "th I/O request: " << cntl->ErrorText();
    tinfo->errors++;
    ...
    LOG_IF(ERROR, FLAGS_verbose || !status_retryable(resp->status()))
        << "Failed I/O response from <" << copyset->poolId << ", "
        << copyset->copysetId << ">:" << copyset->leader << ":"
        << ioCxt->req->chunkid() << ":" << ioCxt->req->offset() << ":"
        << ioCxt->req->size() << ", status: " << resp->status()
        << ", infligth I/Os: "
...
static void* async_io_client(void* arg) {
  ...
  if (init_thread_info(tinfo) != 0) {
    LOG(ERROR) << "Failed to initialize thread infomation for thread "
               << tinfo->id;
    toStop = true;
  }
  ...
  while (!toStop) {
    ...
    curve::chunkserver::ChunkService_Stub* stub =
        ioCxt->copyset->chunk_stubs[ioCxt->copyset->leader];
    // TODO(wenyu): to support more data pattern;
    ...
    if (tinfo->io_time >= tinfo->run_duration) {
      LOG_IF(INFO, FLAGS_verbose)
          << "Thread " << tinfo->id
          << "consumed time(us): " << tinfo->io_time / 1000;
      toStop = true;
...
int create_io_threads(int nr_threads) {
  ...
  if (!FLAGS_use_bthread) {
    for (int i = 0; i < FLAGS_thread_num; ++i) {
      ...
      if (pthread_create(&io_threads[i], NULL, ioroutine, &thread_infos[i]) !=
          0) {
    ...
    for (int i = 0; i < FLAGS_thread_num; ++i) {
      if (bthread_start_background(&io_threads[i], NULL, ioroutine,
                                   &thread_infos[i]) != 0) {
...
void threads_stats(int thread_num, ThreadInfo* total_info) {
  for (int i = 0; i < thread_num; ++i) {
    ...
    LOG(INFO) << "Thread " << thread_infos[i].id << " I/O stats: "
              ...
              << "iops: "
              << thread_infos[i].io_count * 1000000000 / thread_infos[i].io_time
              << ", "
              << "bandwidth(KB/s): "
...
int init_channels() {
  ...
  for (unsigned int i = 0; i < peers.size(); i++) {
    ...
    curve::chunkserver::ChunkService_Stub* chunk_stub =
        new curve::chunkserver::ChunkService_Stub(channel);
    if (!chunk_stub) {
...
int create_copyset(CopysetInfo* info) {
  for (unsigned int i = 0; i < info->conf.size(); i++) {
    ...
    braft::Configuration conf;
    if (conf.parse_from(FLAGS_raftconf) != 0) {
      LOG(ERROR) << "Failed to parse raft configuration from "
                 << FLAGS_raftconf;
    }
    ...
    info->copyset_stubs[i]->CreateCopysetNode(&cntl, &request, &response,
                                              nullptr);
    if (COPYSET_OP_STATUS::COPYSET_OP_STATUS_SUCCESS != response.status() &&
        COPYSET_OP_STATUS::COPYSET_OP_STATUS_EXIST != response.status()) {
      LOG(ERROR) << "Failed to create copyset <" << info->poolId << ", "
                 << info->copysetId << "> on peer " << i
...
int init_copysets() {
  ...
  getLeaderRetry:
    if (update_leader(copysets[i]) >= 0) {
      ...
      LOG(ERROR) << "Failed to get leader of copyset<" << copysets[i]->poolId
