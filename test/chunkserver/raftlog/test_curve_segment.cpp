...
class CurveSegmentTest : public testing::Test {
 protected:
  ...
  void append_entries_curve_segment(
      CurveSegment* segment, const char* data_pattern = "hello, world: %d",
      int start_index = 0, int end_index = 10) {
  ...
  void append_entries_braft_segment(
      braft::Segment* segment, const char* data_pattern = "hello, world: %d",
      int start_index = 0, int end_index = 10) {
    for (int i = start_index; i < end_index; i++) {
      braft::LogEntry* entry = new braft::LogEntry();
      entry->AddRef();
...
TEST_F(CurveSegmentTest, open_segment) {
  ...
  EXPECT_CALL(*file_pool, GetFileImpl(_, _)).WillOnce(Return(0));
