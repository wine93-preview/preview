...
class CurveSegmentLogStorageTest : public testing::Test {
 protected:
  ...
  void SetUp() {
    ...
    EXPECT_CALL(*file_pool, GetFilePoolOpt()).WillRepeatedly(Return(fp_option));
    EXPECT_CALL(*file_pool, GetFileImpl(_, _)).WillRepeatedly(Return(0));
    EXPECT_CALL(*file_pool, RecycleFile(_)).WillRepeatedly(Invoke(recycleFile));
  }
  void TearDown() {
    std::string cmd = std::string("rm -rf ") + kRaftLogDataDir;
    ::system(cmd.c_str());
  }
  void append_entries(std::shared_ptr<braft::LogStorage> storage, int m,
                      int n) {
  ...
  void read_entries(std::shared_ptr<CurveSegmentLogStorage> storage, int start,
                    int end) {
...
TEST_F(CurveSegmentLogStorageTest, basic_test) {
  auto storage = std::make_shared<CurveSegmentLogStorage>(kRaftLogDataDir, true,
                                                          file_pool);
...
TEST_F(CurveSegmentLogStorageTest, append_close_load_append) {
  ...
  // reinit
  storage = std::make_shared<CurveSegmentLogStorage>(kRaftLogDataDir, true,
                                                     file_pool);
