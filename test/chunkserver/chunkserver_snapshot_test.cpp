...
TEST_F(ChunkServerSnapshotTest, OneNode) {
  ...
  WriteThenReadVerify(leaderId, logicPoolId, copysetId, chunkId, length, ch + 1,
                      loop);
...
TEST_F(ChunkServerSnapshotTest, OneNodeShutdown) {
  ...
  // 测试发现集群不可用
  ReadVerifyNotAvailable(leaderId, logicPoolId, copysetId, chunkId, length, ch,
                         1);
...
TEST_F(ChunkServerSnapshotTest, TwoNodesShutdownOnePeer) {
  ...
  if (0 == ::strcmp(leaderId.to_string().c_str(), peer1.to_string().c_str())) {
...
TEST_F(ChunkServerSnapshotTest, ShutdownOnePeerRestartFromInstallSnapshot) {
  ...
  ::sleep(1.5 * snapshotTimeoutS);
  // 再次发起 read/write
  WriteThenReadVerify(leaderId, logicPoolId, copysetId, chunkId, length, ch + 2,
                      loop);
  ...
  // 再次发起 read/write
  WriteThenReadVerify(leaderId, logicPoolId, copysetId, chunkId, length, ch + 3,
                      loop);
  ...
  for (int i = 0; i < kMaxLoop; ++i) {
    status =
        TransferLeader(logicPoolId, copysetId, conf, shutdownPeerid, options);
  ...
  // 再次发起 read/write
  WriteThenReadVerify(leaderId, logicPoolId, copysetId, chunkId, length, ch + 4,
                      loop);
...
TEST_F(ChunkServerSnapshotTest, ShutdownOnePeerAndRemoveData) {
  ...
  ASSERT_EQ(
      0,
      ::system(
          TestCluster::RemoveCopysetDirCmd(shutdownPeerid).c_str()));  // NOLINT
...
TEST_F(ChunkServerSnapshotTest, AddPeerAndRecoverFromInstallSnapshot) {
  ...
  for (int i = 0; i < kMaxChunkId; ++i) {
    WriteThenReadVerify(leaderId, logicPoolId, copysetId, i, length, ch, loop);
  }
  ...
  // add 一个 peer
  {
    ...
    butil::Status status =
        AddPeer(logicPoolId, copysetId, cluster.CopysetConf(), peer4, options);
  ...
  ASSERT_EQ(0,
            ::strcmp(leaderId.to_string().c_str(), peer4.to_string().c_str()));
...
/**
 *  * 验证3个节点的 remove 一个节点，然后再 add 回来，并控制让其从 install
 * snapshot 恢复
 * 1. 创建3个副本的复制组
 * 2. 等待 leader 产生，write 数据，然后 read 出来验证一遍
...
TEST_F(ChunkServerSnapshotTest, RemovePeerAndRecoverFromInstallSnapshot) {
  ...
  // remove 一个 peer
  {
    ...
    butil::Status status = RemovePeer(
        logicPoolId, copysetId, cluster.CopysetConf(), removePeerid, options);
    ASSERT_EQ(0, status.error_code());
  }
