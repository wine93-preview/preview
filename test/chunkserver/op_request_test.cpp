...
TEST(ChunkOpRequestTest, encode) {
  ...
  {
    ChunkOpRequest* opReq = new PasteChunkInternalRequest(
        nodePtr, &request, nullptr, nullptr, nullptr);
  ...
  request.set_size(size);
  {
    ChunkOpRequest* opReq = new ReadChunkRequest(nodePtr, nullptr, cntl,
                                                 &request, nullptr, nullptr);

    butil::IOBuf log;
    ASSERT_EQ(0, opReq->Encode(&request, nullptr, &log));
  ...
  {
    ChunkOpRequest* opReq =
        new DeleteSnapshotRequest(nodePtr, cntl, &request, nullptr, nullptr);
  ...
  {
    ChunkOpRequest* opReq =
        new CreateCloneChunkRequest(nodePtr, cntl, &request, nullptr, nullptr);
  ...
  {
    ChunkOpRequest* opReq =
        new ScanChunkRequest(nodePtr, nullptr, &request, nullptr, nullptr);
...
TEST(ChunkOpRequestTest, OnApplyErrorTest) {
  ...
  // read snapshot: data store error
  {
    ...
    brpc::Controller* cntl = new brpc::Controller();
    ChunkOpRequest* opReq =
        new ReadSnapshotRequest(nodePtr, cntl, &request, &response, nullptr);
    dataStore->InjectError();
    OpFakeClosure done;
  ...
  // delete : data store error
  {
    ...
    brpc::Controller* cntl = new brpc::Controller();
    ChunkOpRequest* opReq =
        new DeleteChunkRequest(nodePtr, cntl, &request, &response, nullptr);
    dataStore->InjectError();
    OpFakeClosure done;
    ASSERT_DEATH(opReq->OnApply(appliedIndex, &done), "");
    delete opReq;
  ...
  // delete snapshot: data store error
  {
    ...
    brpc::Controller* cntl = new brpc::Controller();
    ChunkOpRequest* opReq =
        new DeleteSnapshotRequest(nodePtr, cntl, &request, &response, nullptr);
    dataStore->InjectError();
    OpFakeClosure done;
    ASSERT_DEATH(opReq->OnApply(appliedIndex, &done), "");
    delete opReq;
  ...
  // delete snapshot: backward request error
  {
    ...
    ASSERT_FALSE(cntl->Failed());
    ASSERT_EQ(0, cntl->ErrorCode());
    ASSERT_EQ(CHUNK_OP_STATUS::CHUNK_OP_STATUS_BACKWARD, response.status());
    delete opReq;
    delete cntl;
  }
  // write: data store error
  {
    ...
    brpc::Controller* cntl = new brpc::Controller();
    ChunkOpRequest* opReq =
        new WriteChunkRequest(nodePtr, cntl, &request, &response, nullptr);
    dataStore->InjectError();
    OpFakeClosure done;
    ASSERT_DEATH(opReq->OnApply(appliedIndex, &done), "");
    delete opReq;
  ...
  // scan: data store internal error
  {
    ...
    ChunkOpRequest* opReq = new ScanChunkRequest(nodePtr, scanManager, &request,
                                                 &response, nullptr);
    dataStore->InjectError();
    OpFakeClosure done;
    ASSERT_DEATH(opReq->OnApply(appliedIndex, &done), "");
