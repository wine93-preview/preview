...
class CSMetricTest : public ::testing::Test {
 public:
  ...
  void InitFilePool(std::shared_ptr<FilePool> filePool,
    ...
    FilePoolHelper::PersistEnCodeMetaInfo(lfs_, CHUNK_SIZE, PAGE_SIZE, poolDir,
                                          poolMetaPath);
...
TEST_F(CSMetricTest, OnResponseTest) {
  ...
  // 统计下载成功的情况
  metric_->OnResponse(logicId, copysetId, CSIOMetricType::DOWNLOAD, size, latUs,
                      hasError);
...
TEST_F(CSMetricTest, CountTest) {
  ...
  CopysetNodeOptions copysetNodeOptions = copysetMgr_->GetCopysetNodeOptions();
