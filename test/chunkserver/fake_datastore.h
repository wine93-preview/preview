...
class FakeCSDataStore : public CSDataStore {
 public:
  FakeCSDataStore(DataStoreOptions options, std::shared_ptr<LocalFileSystem> fs)
  ...
  CSErrorCode DeleteSnapshotChunkOrCorrectSn(ChunkID id,
                                             SequenceNum correctedSn) override {
    CSErrorCode errorCode = HasInjectError();
    if (errorCode != CSErrorCode::Success) {
      return errorCode;
    }
...
class FakeFilePool : public FilePool {
 public:
  explicit FakeFilePool(std::shared_ptr<LocalFileSystem> lfs) : FilePool(lfs) {}
  ~FakeFilePool() {}
