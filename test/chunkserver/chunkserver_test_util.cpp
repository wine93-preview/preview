...
std::shared_ptr<FilePool> InitFilePool(
  ...
  int ret = curve::chunkserver::FilePoolHelper::PersistEnCodeMetaInfo(
      fsptr, chunkfileSize, metaPageSize, dirname, metaPath);
...
int StartChunkserver(const char* ip, int port, const char* copysetdir,
  ...
  if (server.Start(port, NULL) != 0) {
    LOG(ERROR) << "Fail to start Server, port: " << port << ", errno: " << errno
               << ", " << strerror(errno);
    return -1;
  }
  ...
  CHECK(CopysetNodeManager::GetInstance().CreateCopysetNode(logicPoolId,
                                                            copysetId, peers));
  auto copysetNode =
      CopysetNodeManager::GetInstance().GetCopysetNode(logicPoolId, copysetId);
...
int TestCluster::WaitLeader(PeerId* leaderId) {
  ...
  for (int i = 0; i < kMaxLoop; ++i) {
    ...
    if (status.ok()) {
      ...
      LOG(INFO) << "Wait leader success, leader is: " << leaderId->to_string();
      return 0;
    } else {
