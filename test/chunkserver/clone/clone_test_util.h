...
class FakeChunkClosure : public ::google::protobuf::Closure {
 ...
 public:
  FakeChunkClosure()
      : isDone_(false), cntl_(nullptr), request_(nullptr), response_(nullptr) {}
  ~FakeChunkClosure() { std::unique_ptr<FakeChunkClosure> selfGuard(this); }
  ...
  void Run() {
    ...
    resContent_.attachment.append(cntl_->response_attachment().to_string());
  }
...
class UnitTestClosure : public ::google::protobuf::Closure {
 public:
  ...
  void Run() { isDone_ = true; }

  void Reset() {
    if (response_ != nullptr) response_->Clear();
    if (cntl_ != nullptr) cntl_->response_attachment().clear();
    isDone_ = false;
  }
