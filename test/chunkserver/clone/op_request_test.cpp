...
class OpRequestTest : public testing::Test {
 public:
  ...
  void FakeCopysetNode() {
    ...
    EXPECT_CALL(*node_, GetAppliedIndex()).WillRepeatedly(Return(LAST_INDEX));
...
TEST_F(OpRequestTest, CreateCloneTest) {
  ...
  std::shared_ptr<CreateCloneChunkRequest> opReq =
      std::make_shared<CreateCloneChunkRequest>(node_, cntl, request, response,
                                                closure);
  /**
   * 测试Encode/Decode
   */
...
TEST_F(OpRequestTest, ReadChunkTest) {
  ...
  closure->SetRequest(request);
  closure->SetResponse(response);
  std::shared_ptr<ReadChunkRequest> opReq = std::make_shared<ReadChunkRequest>(
      node_, cloneMgr_.get(), cntl, request, response, closure);
  /**
   * 测试Encode/Decode
   */
  {
