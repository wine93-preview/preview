...
class TrashTest : public ::testing::Test {
 ...
 public:
  void RecycleCopyset50Times() {
    ...
    for (int i = 0; i < 50; i++) {
      ASSERT_EQ(
          0, trash->RecycleCopySet("./runlog/trash_test0/copysets/12345678"));
    }
  }
...
TEST_F(TrashTest, test_cleanCopySet_list_empty_delete_err) {
  ...
  EXPECT_CALL(*lfs, List(_, _))
      .WillOnce(DoAll(SetArgPointee<1>(files), Return(0)))
      .WillOnce(DoAll(SetArgPointee<1>(std::vector<std::string>{}), Return(0)));
  EXPECT_CALL(*lfs, Open("./runlog/trash_test0/trash/4294967493.55555", _))
      .WillOnce(Return(10));
  struct stat info;
  time(&info.st_ctime);
...
TEST_F(TrashTest, test_cleanCopySet_list_noEmpty_recycleChunks_list_err) {
  std::vector<std::string> files{"4294967493.55555"};
  std::vector<std::string> raftfiles{RAFT_LOG_DIR, RAFT_SNAP_DIR, RAFT_META_DIR,
                                     RAFT_DATA_DIR, "hello"};
  ...
  EXPECT_CALL(*lfs, List("./runlog/trash_test0/trash/4294967493.55555/log", _))
      .WillOnce(DoAll(SetArgPointee<1>(empty), Return(0)));
  EXPECT_CALL(*lfs, List("./runlog/trash_test0/trash/4294967493.55555/data", _))
      .WillOnce(DoAll(SetArgPointee<1>(empty), Return(0)));
  EXPECT_CALL(
      *lfs,
      ...
      .WillOnce(DoAll(SetArgPointee<1>(empty), Return(0)));
  EXPECT_CALL(*lfs,
              List("./runlog/trash_test0/trash/4294967493.55555/raft_meta", _))
      .WillOnce(DoAll(SetArgPointee<1>(empty), Return(0)));
...
TEST_F(TrashTest,
  ...
  EXPECT_CALL(
      *pool,
      RecycleFile("./runlog/trash_test0/trash/4294967493.55555/data/chunk_123"))
      .WillOnce(Return(0));
  EXPECT_CALL(
      *pool,
      RecycleFile("./runlog/trash_test0/trash/4294967493.55555/data/chunk_345"))
      .WillOnce(Return(0));
...
TEST_F(TrashTest, test_chunk_num_statistic) {
  ...
  std::vector<std::string> logfiles1{"curve_log_10086_10087",
                                     "curve_log_inprogress_10088",
                                     "log_10083_10084", "log_inprogress_10085"};
