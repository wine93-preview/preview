...
TEST(ChunkServerMeta, test_encode_and_decode) {
  ...
  ASSERT_TRUE(ChunkServerMetaHelper::EncodeChunkServerMeta(metadata, &strOut));
  ...
  strOut.clear();
  ASSERT_FALSE(ChunkServerMetaHelper::EncodeChunkServerMeta(metadata, &strOut));

  // 3. 解码异常
  metadata.set_token("hello");
  metadata.set_checksum(9999);
  ASSERT_TRUE(ChunkServerMetaHelper::EncodeChunkServerMeta(metadata, &strOut));
  ASSERT_FALSE(ChunkServerMetaHelper::DecodeChunkServerMeta(strOut, &metaOut));
}
}  // namespace chunkserver
}  // namespace curve
