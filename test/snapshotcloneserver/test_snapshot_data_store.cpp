...
TEST_F(TestS3SnapshotDataStore, testDataChunkTransferAddPart) {
  ...
  ASSERT_EQ(0,
            store_->DataChunkTranferAddPart(cdName, task, 1, 1024 * 1024, buf));
  ASSERT_EQ(-1,
            store_->DataChunkTranferAddPart(cdName, task, 2, 1024 * 1024, buf));
  delete[] buf;
}
...
TEST(TestChunkDataName, TestToChunkDataNameSuccess) {
  std::vector<ChunkDataName> testcases = {{"file1", 10, 100},
                                          {"file-1", 10, 100},
                                          {"file-", 10, 100},
                                          {"file1", 0, 0}};
...
TEST(TestChunkDataName, TestToChunkDataNameFail) {
  std::vector<std::string> testcases = {"", "-10-100"};
...
TEST(TestChunkIndexData, TestGetAllChunkIndex) {
  ...
  ChunkDataName name("file1", 10, 100);
  indexData.PutChunkDataName(name);
  std::vector<ChunkIndexType> ret = indexData.GetAllChunkIndex();
