  ...
  static void SetUpTestCase() {
    ...
    std::vector<std::string> options = {
        {"mds.listen.addr=127.0.0.1:8888", "mds.registerToMDS=false",
         ...
         "chunkserver.rpcTimeoutMS=1", "chunkserver.maxRetrySleepIntervalUS=1",
...
TEST_F(TestCurveFsClientImpl, TestClientInterfaceFail) {
  ...
  ret = client_->CreateCloneFile("source1", "file1", "user1", 1024, 1, 1024, 0,
                                 0, &fInfo);
  ASSERT_LT(ret, 0);
  ret = client_->CreateCloneFile("source1", "file1", clientOption_.mdsRootUser,
                                 1024, 1, 1024, 0, 0, &fInfo);
  ASSERT_LT(ret, 0);
