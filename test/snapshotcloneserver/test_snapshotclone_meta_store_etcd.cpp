...
class TestSnapshotCloneMetaStoreEtcd : public ::testing::Test {
 public:
  ...
  void SetUp() {
    ...
    metaStore_ =
        std::make_shared<SnapshotCloneMetaStoreEtcd>(kvStorageClient_, codec_);
  }

  void TearDown() { metaStore_ = nullptr; }
...
bool JudgeCloneInfoEqual(const CloneInfo& left, const CloneInfo& right) {
  if (left.GetTaskId() == right.GetTaskId() &&
      ...
      left.GetSrc() == right.GetSrc() && left.GetDest() == right.GetDest() &&
...
bool JudgeSnapshotInfoEqual(const SnapshotInfo& left,
  ...
  if (left.GetUuid() == right.GetUuid() && left.GetUser() == right.GetUser() &&
...
TEST_F(TestSnapshotCloneMetaStoreEtcd, TestAddSnapInfoAndGetSuccess) {
  SnapshotInfo snapInfo("snapuuid", "snapuser", "file1", "snapxxx", 100, 1024,
                        2048, 4096, 0, 0, 0, Status::pending);
...
TEST_F(TestSnapshotCloneMetaStoreEtcd, TestAddSnapshotInfoPutInfoEtcdFail) {
  SnapshotInfo snapInfo("snapuuid", "snapuser", "file1", "snapxxx", 100, 1024,
                        2048, 4096, 0, 0, 0, Status::pending);
...
TEST_F(TestSnapshotCloneMetaStoreEtcd, TestDeleteSnapshotSuccess) {
  SnapshotInfo snapInfo("snapuuid", "snapuser", "file1", "snapxxx", 100, 1024,
                        2048, 4096, 0, 0, 0, Status::pending);
...
TEST_F(TestSnapshotCloneMetaStoreEtcd, TestDeleteSnapshotDeleteFromEtcdFail) {
  SnapshotInfo snapInfo("snapuuid", "snapuser", "file1", "snapxxx", 100, 1024,
                        2048, 4096, 0, 0, 0, Status::pending);
...
TEST_F(TestSnapshotCloneMetaStoreEtcd, TestUpdateSnapshotAndGetSuccess) {
  SnapshotInfo snapInfo("snapuuid", "snapuser", "file1", "snapxxx", 100, 1024,
                        2048, 4096, 0, 0, 0, Status::pending);
  ...
  SnapshotInfo snapInfo2("snapuuid", "snapuser2", "file2", "snapxxx2", 101,
                         1025, 2049, 4097, 1, 0, 0, Status::done);
...
TEST_F(TestSnapshotCloneMetaStoreEtcd, TestUpdateSnapshotPutInfoEtcdFail) {
  SnapshotInfo snapInfo("snapuuid", "snapuser", "file1", "snapxxx", 100, 1024,
                        2048, 4096, 0, 0, 0, Status::pending);
  ...
  SnapshotInfo snapInfo2("snapuuid", "snapuser2", "file2", "snapxxx2", 101,
                         1025, 2049, 4097, 0, 0, 1, Status::done);
...
TEST_F(TestSnapshotCloneMetaStoreEtcd, TestCASSnapshot) {
  SnapshotInfo snapInfo("uuid", "user", "", "", 0, 0, 0, 0, 0, 0, 0,
                        Status::pending);
  ...
  auto tearDown = [&]() { ASSERT_EQ(metaStore_->DeleteSnapshot("uuid"), 0); };

  // CASE 1: Etcd put failed -> CASSnapshot failed
  {
    setUp();

    auto cas = [](SnapshotInfo* snapinfo) -> SnapshotInfo* { return snapinfo; };
  ...
  // CASE 2: Set snapshot success
  {
    ...
    SnapshotInfo setInfo("uuid", "user1", "", "", 1, 1, 1, 1, 1, 1, 1,
                         Status::done);
  ...
  // CASE 4: Not needed to set snapshot
  {
    ...
    auto cas = [](SnapshotInfo* snapinfo) -> SnapshotInfo* { return nullptr; };

    EXPECT_CALL(*kvStorageClient_, Put(_, _)).Times(0);
    ASSERT_EQ(metaStore_->CASSnapshot("uuid", cas), 0);
...
TEST_F(TestSnapshotCloneMetaStoreEtcd, TestGetSnapshotInfoFail) {
  SnapshotInfo outInfo;
  int ret = metaStore_->GetSnapshotInfo("snapuuid", &outInfo);
  ASSERT_EQ(-1, ret);
}

TEST_F(TestSnapshotCloneMetaStoreEtcd, TestGetSnapshotList1Success) {
  SnapshotInfo snapInfo("snapuuid", "snapuser", "file1", "snapxxx", 100, 1024,
                        2048, 4096, 0, 0, 0, Status::pending);
...
TEST_F(TestSnapshotCloneMetaStoreEtcd, TestGetSnapshotList1Fail) {
  std::vector<SnapshotInfo> list;
  int ret = metaStore_->GetSnapshotList("file1", &list);
  ASSERT_EQ(-1, ret);
}

TEST_F(TestSnapshotCloneMetaStoreEtcd, TestGetSnapshotList2Success) {
  SnapshotInfo snapInfo("snapuuid", "snapuser", "file1", "snapxxx", 100, 1024,
                        2048, 4096, 0, 0, 0, Status::pending);
...
TEST_F(TestSnapshotCloneMetaStoreEtcd, TestGetSnapshotList2Fail) {
  std::vector<SnapshotInfo> list;
...
TEST_F(TestSnapshotCloneMetaStoreEtcd, TestAddCloneInfoAndGetSuccess) {
  CloneInfo cloneInfo("uuid1", "user1", CloneTaskType::kClone, "src1", "dst1",
                      1, 2, 3, CloneFileType::kFile, false,
...
TEST_F(TestSnapshotCloneMetaStoreEtcd, TestAddCloneInfoPutInfoEtcdFail) {
  CloneInfo cloneInfo("uuid1", "user1", CloneTaskType::kClone, "src1", "dst1",
                      1, 2, 3, CloneFileType::kFile, false,
                      CloneStep::kCompleteCloneFile, CloneStatus::cloning);
...
TEST_F(TestSnapshotCloneMetaStoreEtcd, TestDeleteCloneInfoSuccess) {
  CloneInfo cloneInfo("uuid1", "user1", CloneTaskType::kClone, "src1", "dst1",
                      1, 2, 3, CloneFileType::kFile, false,
                      CloneStep::kCompleteCloneFile, CloneStatus::cloning);
...
TEST_F(TestSnapshotCloneMetaStoreEtcd, TestDeleteCloneInfoDeleteFromEtcdFail) {
  CloneInfo cloneInfo("uuid1", "user1", CloneTaskType::kClone, "src1", "dst1",
                      1, 2, 3, CloneFileType::kFile, false,
                      CloneStep::kCompleteCloneFile, CloneStatus::cloning);
...
TEST_F(TestSnapshotCloneMetaStoreEtcd, TestUpdateCloneInfoAndGetSuccess) {
  CloneInfo cloneInfo("uuid1", "user1", CloneTaskType::kClone, "src1", "dst1",
                      1, 2, 3, CloneFileType::kFile, false,
                      CloneStep::kCompleteCloneFile, CloneStatus::cloning);
  ...
  CloneInfo cloneInfo2("uuid1", "user2", CloneTaskType::kClone, "src2", "dst2",
                       2, 3, 4, CloneFileType::kFile, false, CloneStep::kEnd,
                       CloneStatus::done);
...
TEST_F(TestSnapshotCloneMetaStoreEtcd, TestUpdateCloneInfoPutInfoEtcdFail) {
  CloneInfo cloneInfo("uuid1", "user1", CloneTaskType::kClone, "src1", "dst1",
                      1, 2, 3, CloneFileType::kFile, false,
                      CloneStep::kCompleteCloneFile, CloneStatus::cloning);
...
TEST_F(TestSnapshotCloneMetaStoreEtcd, TestUpdateCloneInfoNotExist) {
  CloneInfo cloneInfo("uuid1", "user1", CloneTaskType::kClone, "src1", "dst1",
                      1, 2, 3, CloneFileType::kFile, false,
                      CloneStep::kCompleteCloneFile, CloneStatus::cloning);
...
TEST_F(TestSnapshotCloneMetaStoreEtcd, TestGetCloneInfoFail) {
  CloneInfo outInfo;
  int ret = metaStore_->GetCloneInfo("uuid1", &outInfo);
  ASSERT_EQ(-1, ret);
}

TEST_F(TestSnapshotCloneMetaStoreEtcd, TestGetCloneInfoByFileNameSuccess) {
  CloneInfo cloneInfo("uuid1", "user1", CloneTaskType::kClone, "src1", "dst1",
                      1, 2, 3, CloneFileType::kFile, false,
                      CloneStep::kCompleteCloneFile, CloneStatus::cloning);
...
TEST_F(TestSnapshotCloneMetaStoreEtcd, TestGetCloneInfoByFileNameFail) {
  std::vector<CloneInfo> list;
  int ret = metaStore_->GetCloneInfoByFileName("dst1", &list);
  ASSERT_EQ(-1, ret);
}

TEST_F(TestSnapshotCloneMetaStoreEtcd, TestGetCloneInfoListSuccess) {
  CloneInfo cloneInfo("uuid1", "user1", CloneTaskType::kClone, "src1", "dst1",
                      1, 2, 3, CloneFileType::kFile, false,
                      CloneStep::kCompleteCloneFile, CloneStatus::cloning);
...
TEST_F(TestSnapshotCloneMetaStoreEtcd, TestGetCloneInfoListFail) {
  std::vector<CloneInfo> list;
  int ret = metaStore_->GetCloneInfoList(&list);
  ASSERT_EQ(-1, ret);
}

TEST_F(TestSnapshotCloneMetaStoreEtcd, TestInitSuccess) {
  SnapshotInfo snapInfo("snapuuid", "snapuser", "file1", "snapxxx", 100, 1024,
                        2048, 4096, 0, 0, 0, Status::pending);
  ...
  EXPECT_CALL(*kvStorageClient_,
              List(_, _, Matcher<std::vector<std::string>*>(_)))  // NOLINT
      .WillOnce(DoAll(SetArgPointee<2>(out), Return(EtcdErrCode::EtcdOK)))
      .WillOnce(DoAll(SetArgPointee<2>(cloneOut), Return(EtcdErrCode::EtcdOK)));

  int ret = metaStore_->Init();
  ASSERT_EQ(0, ret);
}

TEST_F(TestSnapshotCloneMetaStoreEtcd, TestInitListSnapshotFail) {
  EXPECT_CALL(*kvStorageClient_,
              List(_, _, Matcher<std::vector<std::string>*>(_)))  // NOLINT
      .WillOnce(Return(EtcdErrCode::EtcdUnknown));
...
TEST_F(TestSnapshotCloneMetaStoreEtcd, TestInitListCloneInfoFail) {
  SnapshotInfo snapInfo("snapuuid", "snapuser", "file1", "snapxxx", 100, 1024,
                        2048, 4096, 0, 0, 0, Status::pending);
  SnapshotCloneCodec codec;
  std::string value;
...
TEST_F(TestSnapshotCloneMetaStoreEtcd, TestInitDecodeSnapshotFail) {
  std::vector<std::string> out;
  out.push_back("xxx");
...
TEST_F(TestSnapshotCloneMetaStoreEtcd, TestInitDecodeCloneInfoFail) {
  SnapshotInfo snapInfo("snapuuid", "snapuser", "file1", "snapxxx", 100, 1024,
                        2048, 4096, 0, 0, 0, Status::pending);
  SnapshotCloneCodec codec;
  std::string value;
  ...
  EXPECT_CALL(*kvStorageClient_,
              List(_, _, Matcher<std::vector<std::string>*>(_)))  // NOLINT
      .WillOnce(DoAll(SetArgPointee<2>(out), Return(EtcdErrCode::EtcdOK)))
      .WillOnce(DoAll(SetArgPointee<2>(out2), Return(EtcdErrCode::EtcdOK)));
