...
class TestSnapshotCoreImpl : public ::testing::Test {
 public:
  ...
  virtual void SetUp() {
    ...
    core_ = std::make_shared<SnapshotCoreImpl>(client_, metaStore_, dataStore_,
                                               snapshotRef_, option);
...
TEST_F(TestSnapshotCoreImpl, TestDeleteSnapshotPreSuccess) {
  ...
  EXPECT_CALL(*metaStore_, UpdateSnapshot(_)).WillOnce(Return(kErrCodeSuccess));
...
TEST_F(TestSnapshotCoreImpl, TestHandleCreateSnapshotTaskSuccess) {
  ...
  EXPECT_CALL(*client_, GetSnapshot(fileName, user, seqNum, _))
      .WillOnce(DoAll(SetArgPointee<3>(snapInfo), Return(LIBCURVE_ERROR::OK)));

  EXPECT_CALL(*metaStore_, CASSnapshot(_, _)).WillOnce(Return(kErrCodeSuccess));
  EXPECT_CALL(*metaStore_, UpdateSnapshot(_)).WillOnce(Return(kErrCodeSuccess));
...
TEST_F(TestSnapshotCoreImpl, TestHandleCreateSnapshotTask_GetChunkInfoFail) {
  ...
  EXPECT_CALL(*client_, GetChunkInfo(_, _))
      .WillOnce(
          DoAll(SetArgPointee<1>(chunkInfo), Return(-LIBCURVE_ERROR::FAILED)));
...
TEST_F(TestSnapshotCoreImpl,
  ...
  EXPECT_CALL(*client_, GetSnapshotSegmentInfo(fileName, user, seqNum, _, _))
      .Times(2)
      .WillOnce(DoAll(SetArgPointee<4>(segInfo1), Return(LIBCURVE_ERROR::OK)))
      .WillOnce(DoAll(SetArgPointee<4>(segInfo2), Return(LIBCURVE_ERROR::OK)));
...
TEST_F(TestSnapshotCoreImpl, TestHandleDeleteSnapshotTaskSuccess) {
  ...
  EXPECT_CALL(*dataStore_, ChunkIndexDataExist(_)).WillRepeatedly(Return(true));
