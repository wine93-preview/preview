...
class MockSnapshotCore : public SnapshotCore {
 public:
  ...
  MOCK_METHOD4(CreateSnapshotPre,
               int(const std::string& file, const std::string& user,
                   const std::string& snapshotName, SnapshotInfo* snapInfo));

  MOCK_METHOD1(HandleCreateSnapshotTask,
               void(std::shared_ptr<SnapshotTaskInfo> task));

  MOCK_METHOD4(DeleteSnapshotPre,
               int(UUID uuid, const std::string& user,
                   const std::string& fileName, SnapshotInfo* snapInfo));

  MOCK_METHOD1(HandleDeleteSnapshotTask,
               void(std::shared_ptr<SnapshotTaskInfo> task));

  MOCK_METHOD2(GetFileSnapshotInfo,
               int(const std::string& file, std::vector<SnapshotInfo>* info));

  MOCK_METHOD1(GetSnapshotList, int(std::vector<SnapshotInfo>* list));

  MOCK_METHOD2(GetSnapshotInfo, int(const UUID uuid, SnapshotInfo* info));
...
class MockSnapshotCloneMetaStore : public SnapshotCloneMetaStore {
 public:
  ...
  MOCK_METHOD2(GetSnapshotList,
               int(const std::string& filename, std::vector<SnapshotInfo>* v));
  MOCK_METHOD1(GetSnapshotList, int(std::vector<SnapshotInfo>* list));
  MOCK_METHOD0(GetSnapshotCount, uint32_t());
  ...
  MOCK_METHOD2(GetCloneInfo, int(const std::string& taskID, CloneInfo* info));
  MOCK_METHOD2(GetCloneInfoByFileName,
               int(const std::string& fileName, std::vector<CloneInfo>* list));
  MOCK_METHOD1(GetCloneInfoList, int(std::vector<CloneInfo>* list));
};
...
class MockSnapshotDataStore : public SnapshotDataStore {
 public:
  ...
  MOCK_METHOD2(PutChunkIndexData,
               int(const ChunkIndexDataName& name, const ChunkIndexData& meta));
  MOCK_METHOD2(GetChunkIndexData,
               int(const ChunkIndexDataName& name, ChunkIndexData* meta));
  MOCK_METHOD1(DeleteChunkIndexData, int(const ChunkIndexDataName& name));
  MOCK_METHOD1(ChunkIndexDataExist, bool(const ChunkIndexDataName& name));
  MOCK_METHOD2(GetChunkData, int(const ChunkDataName& name, ChunkData* data));
  MOCK_METHOD1(DeleteChunkData, int(const ChunkDataName& name));
  MOCK_METHOD1(ChunkDataExist, bool(const ChunkDataName& name));
  MOCK_METHOD2(SetSnapshotFlag, int(const ChunkIndexDataName& name, int flag));
  MOCK_METHOD1(GetSnapshotFlag, int(const ChunkIndexDataName& name));
  MOCK_METHOD2(DataChunkTranferInit, int(const ChunkDataName& name,
  ...
  MOCK_METHOD2(DataChunkTranferComplete,
               int(const ChunkDataName& name,
                   std::shared_ptr<TransferTask> task));
  MOCK_METHOD2(DataChunkTranferAbort, int(const ChunkDataName& name,
                                          std::shared_ptr<TransferTask> task));
};
...
class MockCurveFsClient : public CurveFsClient {
 public:
  ...
  MOCK_METHOD3(CreateSnapshot, int(const std::string& filename,
                                   const std::string& user, uint64_t* seq));
  MOCK_METHOD3(DeleteSnapshot, int(const std::string& filename,
                                   const std::string& user, uint64_t seq));
  MOCK_METHOD4(GetSnapshot,
               int(const std::string& filename, const std::string& user,
                   uint64_t seq, FInfo* snapInfo));
  MOCK_METHOD5(GetSnapshotSegmentInfo,
               int(const std::string& filename, const std::string& user,
                   uint64_t seq, uint64_t offset, SegmentInfo* segInfo));
  MOCK_METHOD6(ReadChunkSnapshot,
               int(ChunkIDInfo cidinfo, uint64_t seq, uint64_t offset,
                   uint64_t len, char* buf, SnapCloneClosure* scc));
  MOCK_METHOD2(DeleteChunkSnapshotOrCorrectSn,
               int(const ChunkIDInfo& cidinfo, uint64_t correctedSeq));

  MOCK_METHOD4(CheckSnapShotStatus, int(std::string filename, std::string user,
                                        uint64_t seq, FileStatus* filestatus));

  MOCK_METHOD2(GetChunkInfo,
               int(const ChunkIDInfo& cidinfo, ChunkInfoDetail* chunkInfo));

  MOCK_METHOD9(CreateCloneFile,
               int(const std::string& source, const std::string& filename,
  ...
  MOCK_METHOD6(CreateCloneChunk,
               int(const std::string& location, const ChunkIDInfo& chunkidinfo,
                   uint64_t sn, uint64_t csn, uint64_t chunkSize,
                   SnapCloneClosure* scc));

  MOCK_METHOD4(RecoverChunk,
               int(const ChunkIDInfo& chunkidinfo, uint64_t offset,
  ...
  MOCK_METHOD3(SetCloneFileStatus,
               int(const std::string& filename, const FileStatus& filestatus,
                   const std::string& user));

  MOCK_METHOD3(GetFileInfo, int(const std::string& filename,
                                const std::string& user, FInfo* fileInfo));

  MOCK_METHOD5(GetOrAllocateSegmentInfo,
               int(bool allocate, uint64_t offset, FInfo* fileInfo,
                   const std::string& user, SegmentInfo* segInfo));

  MOCK_METHOD5(RenameCloneFile,
               int(const std::string& user, uint64_t originId,
                   uint64_t destinationId, const std::string& origin,
                   const std::string& destination));

  MOCK_METHOD3(DeleteFile, int(const std::string& fileName,
                               const std::string& user, uint64_t fileId));

  MOCK_METHOD2(Mkdir, int(const std::string& dirpath, const std::string& user));

  MOCK_METHOD2(ChangeOwner,
               int(const std::string& filename, const std::string& newOwner));
};

class MockSnapshotServiceManager : public SnapshotServiceManager {
 public:
  MockSnapshotServiceManager() : SnapshotServiceManager(nullptr, nullptr) {}
  ~MockSnapshotServiceManager() {}
  MOCK_METHOD4(CreateSnapshot,
               int(const std::string& file, const std::string& user,
                   const std::string& desc, UUID* uuid));

  MOCK_METHOD3(DeleteSnapshot, int(const UUID& uuid, const std::string& user,
  ...
  MOCK_METHOD4(GetFileSnapshotInfoById,
               int(const std::string& file, const std::string& user,
                   const UUID& uuid, std::vector<FileSnapshotInfo>* info));

  MOCK_METHOD2(GetSnapshotListByFilter,
               int(const SnapshotFilterCondition& filter,
                   std::vector<FileSnapshotInfo>* info));

  MOCK_METHOD3(CancelSnapshot, int(const UUID& uuid, const std::string& user,
                                   const std::string& file));
};

class MockCloneServiceManager : public CloneServiceManager {
 public:
  MockCloneServiceManager() : CloneServiceManager(nullptr, nullptr, nullptr) {}
  ~MockCloneServiceManager() {}

  MOCK_METHOD6(CloneFile,
               int(const UUID& source, const std::string& user,
                   const std::string& destination, bool lazyFlag,
                   std::shared_ptr<CloneClosure> entity, TaskIdType* taskId));
  ...
  MOCK_METHOD2(Flatten, int(const std::string& user, const TaskIdType& taskId));

  MOCK_METHOD2(GetCloneTaskInfo,
               int(const std::string& user, std::vector<TaskCloneInfo>* info));

  MOCK_METHOD1(GetCloneTaskInfo, int(std::vector<TaskCloneInfo>* info));

  MOCK_METHOD3(GetCloneTaskInfoById,
               int(const std::string& user, const TaskIdType& taskId,
                   std::vector<TaskCloneInfo>* info));

  MOCK_METHOD3(GetCloneTaskInfoByName,
               int(const std::string& user, const std::string& fileName,
                   std::vector<TaskCloneInfo>* info));

  MOCK_METHOD2(GetCloneTaskInfoByFilter, int(const CloneFilterCondition& filter,
                                             std::vector<TaskCloneInfo>* info));
  ...
  MOCK_METHOD3(GetCloneRefStatus,
               int(const std::string& src, CloneRefStatus* refStatus,
...
class MockCloneCore : public CloneCore {
 public:
  MOCK_METHOD6(CloneOrRecoverPre,
               int(const UUID& source, const std::string& user,
                   const std::string& destination, bool lazyFlag,
                   CloneTaskType taskType, CloneInfo* info));
  ...
  MOCK_METHOD3(FlattenPre, int(const std::string& user,
                               const TaskIdType& taskId, CloneInfo* cloneInfo));

  MOCK_METHOD1(GetCloneInfoList, int(std::vector<CloneInfo>* cloneInfos));

  MOCK_METHOD2(GetCloneInfo, int(TaskIdType taskId, CloneInfo* cloneInfo));

  MOCK_METHOD2(GetCloneInfoByFileName,
               int(const std::string& fileName, std::vector<CloneInfo>* list));

  MOCK_METHOD0(GetSnapshotRef, std::shared_ptr<SnapshotReference>());

  MOCK_METHOD0(GetCloneRef, std::shared_ptr<CloneReference>());
  ...
  MOCK_METHOD1(HandleDeleteCloneInfo, int(const CloneInfo& cloneInfo));
};
