...
class TestCloneCoreImpl : public ::testing::Test {
 public:
  ...
  virtual void SetUp() {
    ...
    core_ = std::make_shared<CloneCoreImpl>(client_, metaStore_, dataStore_,
                                            snapshotRef_, cloneRef_, option);
...
TEST_F(TestCloneCoreImpl, TestClonePreForSnapTaskExist) {
  ...
  CloneInfo info1("taskid1", user, CloneTaskType::kClone, source, destination,
                  100, 101, 0, CloneFileType::kSnapshot, lazyFlag,
...
TEST_F(TestCloneCoreImpl, TestClonePreForSnapFailOnFileExist) {
  ...
  std::vector<CloneInfo> cloneInfoList;
  CloneInfo info1("taskid1", user, CloneTaskType::kRecover, source, destination,
                  100, 101, 0, CloneFileType::kSnapshot, lazyFlag,
                  CloneStep::kCreateCloneFile, CloneStatus::cloning);
  cloneInfoList.push_back(info1);
  EXPECT_CALL(*metaStore_, GetCloneInfoByFileName(destination, _))
...
TEST_F(TestCloneCoreImpl, TestClonePreDestinationAndTaskExist) {
  ...
  std::vector<CloneInfo> cloneInfoList;
  CloneInfo info1("taskid1", user, CloneTaskType::kClone, source, destination,
                  100, destId, 0, CloneFileType::kFile, lazyFlag,
...
TEST_F(TestCloneCoreImpl,
  ...
  CloneInfo info("id1", "user1", CloneTaskType::kClone, "snapid1", "file1", 1,
                 2, 100, CloneFileType::kSnapshot, true,
...
TEST_F(TestCloneCoreImpl, HandleCloneOrRecoverTaskFailOnGetSnapshotInfo) {
  ...
  SnapshotInfo info(uuid, user, fileName, desc, seqnum, chunksize, segmentsize,
                    filelength, 0, 0, time, status);
...
TEST_F(TestCloneCoreImpl,
  ...
  CloneInfo info("id1", "user1", CloneTaskType::kRecover, "snapid1", "file1", 1,
                 2, 100, CloneFileType::kSnapshot, true,
...
TEST_F(TestCloneCoreImpl, HandleCloneOrRecoverTaskStage2SuccessForCloneByFile) {
  CloneInfo info("id1", "user1", CloneTaskType::kClone, "snapid1", "file1", 1,
                 2, 100, CloneFileType::kFile, true, CloneStep::kRecoverChunk,
                 CloneStatus::cloning);
  info.SetStatus(CloneStatus::cloning);
  auto cloneMetric = std::make_shared<CloneInfoMetric>("id1");
  auto cloneClosure = std::make_shared<CloneClosure>();
...
void TestCloneCoreImpl::MockCreateCloneFileSuccess(
  ...
  EXPECT_CALL(*client_, CreateCloneFile(_, _, _, _, _, _, _, _, _))
      .WillOnce(DoAll(SetArgPointee<8>(fInfoOut), Return(LIBCURVE_ERROR::OK)));
}
...
void TestCloneCoreImpl::MockCreateCloneChunkSuccess(
  ...
  EXPECT_CALL(*client_, CreateCloneChunk(AnyOf(location1, location2), _, _,
      ...
      .WillRepeatedly(DoAll(
          Invoke([](const std::string& location, const ChunkIDInfo& chunkidinfo,
...
void TestCloneCoreImpl::MockChangeOwnerSuccess(
    std::shared_ptr<CloneTaskInfo> task) {
  EXPECT_CALL(*client_, ChangeOwner(_, _)).WillOnce(Return(LIBCURVE_ERROR::OK));
}
...
void TestCloneCoreImpl::MockCreateCloneFileFail(
  ...
  EXPECT_CALL(*client_, CreateCloneFile(_, _, _, _, _, _, _, _, _))
      .WillOnce(
          DoAll(SetArgPointee<8>(fInfoOut), Return(-LIBCURVE_ERROR::FAILED)));
}
