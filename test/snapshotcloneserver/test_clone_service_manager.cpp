...
class TestCloneServiceManager : public ::testing::Test {
 public:
  ...
  virtual void SetUp() {
    ...
    EXPECT_CALL(*cloneServiceManagerBackend_, Init(_, _)).Times(1);

    ASSERT_EQ(0, manager_->Init(option_)) << "manager init fail.";

    EXPECT_CALL(*cloneServiceManagerBackend_, Start()).Times(1);
    ASSERT_EQ(0, manager_->Start()) << "manager start fail.";
    manager_->SetDLock(dlock_);
  }

  virtual void TearDown() {
    EXPECT_CALL(*cloneServiceManagerBackend_, Stop()).Times(1);
    manager_->Stop();
...
TEST_F(TestCloneServiceManager, TestCloneFileSuccess) {
  const UUID source = "uuid1";
  const std::string user = "user1";
  const std::string destination = "file1";
  bool lazyFlag = true;

  CloneInfo cloneInfo("uuid1", user, CloneTaskType::kClone, source, destination,
                      CloneFileType::kSnapshot, lazyFlag);

  EXPECT_CALL(*cloneCore_,
              CloneOrRecoverPre(source, user, destination, lazyFlag,
                                CloneTaskType::kClone, _))
      .WillOnce(DoAll(SetArgPointee<5>(cloneInfo), Return(kErrCodeSuccess)));
  ...
  int ret = manager_->CloneFile(source, user, destination, lazyFlag, closure,
                                &taskId);
...
TEST_F(TestCloneServiceManager, TestCloneFilePreFail) {
  const UUID source = "uuid1";
  const std::string user = "user1";
  const std::string destination = "file1";
  bool lazyFlag = true;
  ...
  EXPECT_CALL(*cloneCore_,
              CloneOrRecoverPre(source, user, destination, lazyFlag,
                                CloneTaskType::kClone, _))
      .WillOnce(
          DoAll(SetArgPointee<5>(cloneInfo), Return(kErrCodeInternalError)));
...
struct FakeCloneClosure : public CloneClosure {
  FakeCloneClosure() : taskId_(""), errorCode_(-1), callNum_(0) {}
  void Run() {
    callNum_++;
    taskId_ = GetTaskId();
...
TEST_F(TestCloneServiceManager, TestCloneFileSuccessByTaskExist) {
  ...
  EXPECT_CALL(*cloneCore_,
              CloneOrRecoverPre(source, user, destination, lazyFlag,
                                CloneTaskType::kClone, _))
      .WillOnce(DoAll(SetArgPointee<5>(cloneInfo), Return(kErrCodeTaskExist)));
...
TEST_F(TestCloneServiceManager, TestCloneFilePushTaskFail) {
  const UUID source = "uuid1";
  const std::string user = "user1";
  const std::string destination = "file1";
  bool lazyFlag = true;
  ...
  EXPECT_CALL(*cloneCore_,
              CloneOrRecoverPre(source, user, destination, lazyFlag,
                                CloneTaskType::kClone, _))
      .WillRepeatedly(
          DoAll(SetArgPointee<5>(cloneInfo), Return(kErrCodeSuccess)));
  ...
  ret = manager_->CloneFile(source, user, destination, lazyFlag, closure,
                            &taskId);
...
TEST_F(TestCloneServiceManager, TestRecoverFileSuccess) {
  const UUID source = "uuid1";
  const std::string user = "user1";
  const std::string destination = "file1";
  bool lazyFlag = true;

  CloneInfo cloneInfo("uuid1", user, CloneTaskType::kRecover, source,
                      destination, CloneFileType::kSnapshot, lazyFlag);

  EXPECT_CALL(*cloneCore_,
              CloneOrRecoverPre(source, user, destination, lazyFlag,
                                CloneTaskType::kRecover, _))
      .WillOnce(DoAll(SetArgPointee<5>(cloneInfo), Return(kErrCodeSuccess)));
  ...
  TaskIdType taskId;
  auto closure = std::make_shared<CloneClosure>();
  int ret = manager_->RecoverFile(source, user, destination, lazyFlag, closure,
                                  &taskId);
  ASSERT_EQ(kErrCodeSuccess, ret);
...
TEST_F(TestCloneServiceManager, TestRecoverFilePreFail) {
  const UUID source = "uuid1";
  const std::string user = "user1";
  const std::string destination = "file1";
  bool lazyFlag = true;
...
TEST_F(TestCloneServiceManager, TestRecoverFileSuccessByTaskExist) {
  const UUID source = "uuid1";
  const std::string user = "user1";
  const std::string destination = "file1";
  bool lazyFlag = true;
...
TEST_F(TestCloneServiceManager, TestRecoverFilePushTaskFail) {
  const UUID source = "uuid1";
  const std::string user = "user1";
  const std::string destination = "file1";
  bool lazyFlag = true;
  ...
  ret = manager_->RecoverFile(source, user, destination, lazyFlag, closure,
                              &taskId);
  ASSERT_EQ(kErrCodeTaskExist, ret);
...
TEST_F(TestCloneServiceManager, TestGetCloneTaskInfoSuccess) {
  ...
  EXPECT_CALL(*cloneCore_, HandleCloneOrRecoverTask(_))
      .WillOnce(Invoke([&cond1, &cond2](std::shared_ptr<CloneTaskInfo> task) {
        cond2.Wait();
        task->Finish();
        cond1.Signal();
      }));
  ...
  EXPECT_CALL(*cloneCore_, GetCloneInfoList(_))
      .WillOnce(DoAll(SetArgPointee<0>(cloneInfos), Return(kErrCodeSuccess)));
...
TEST_F(TestCloneServiceManager, GetCloneTaskInfoByFilterFail) {
  ...
  EXPECT_CALL(*cloneCore_, GetCloneInfoList(_))
      .WillRepeatedly(
          DoAll(SetArgPointee<0>(cloneInfos), Return(kErrCodeSuccess)));
...
TEST_F(TestCloneServiceManager, TestGetCloneTaskInfoByUUIDSuccess) {
  ...
  CloneInfo cloneInfo(uuid, user, CloneTaskType::kClone, source, destination,
                      CloneFileType::kSnapshot, lazyFlag);
  ...
  EXPECT_CALL(*cloneCore_, GetCloneInfo(_, _))
      .WillOnce(DoAll(SetArgPointee<1>(cloneInfo), Return(kErrCodeSuccess)));
...
TEST_F(TestCloneServiceManager, TestGetCloneTaskInfoFailNotExist) {
  std::vector<CloneInfo> cloneInfos;
  EXPECT_CALL(*cloneCore_, GetCloneInfoList(_))
      .WillOnce(DoAll(SetArgPointee<0>(cloneInfos), Return(-1)));
...
TEST_F(TestCloneServiceManager, TestGetCloneTaskInfoByUUIDFailNotExist) {
  ...
  CloneInfo cloneInfo;
  EXPECT_CALL(*cloneCore_, GetCloneInfo(_, _))
      .WillOnce(DoAll(SetArgPointee<1>(cloneInfo), Return(-1)));
...
TEST_F(TestCloneServiceManager, TestGetCloneTaskInfoByFileNameFailNotExist) {
  ...
  std::vector<CloneInfo> list;
  EXPECT_CALL(*cloneCore_, GetCloneInfoByFileName(_, _))
      .WillOnce(DoAll(SetArgPointee<1>(list), Return(-1)));

  std::vector<TaskCloneInfo> infos;
  int ret = manager_->GetCloneTaskInfoByName("user1", destination, &infos);
  ASSERT_EQ(kErrCodeFileNotExist, ret);
}
...
TEST_F(TestCloneServiceManager, TestRecoverCloneTaskSuccess) {
  ...
  // for Flatten
  CloneInfo cloneInfo2("uuid2", user, CloneTaskType::kClone, source,
                       destination, 1, 1, 1, CloneFileType::kSnapshot, lazyFlag,
...
TEST_F(TestCloneServiceManager, TestGetCloneTaskInfoFailOnGetCloneInfo) {
  ...
  EXPECT_CALL(*cloneCore_, GetCloneInfo(_, _))
      .WillOnce(
          DoAll(SetArgPointee<1>(cloneInfo), Return(kErrCodeInternalError)));
...
TEST_F(TestCloneServiceManager, TestRecoverCloneTaskGetCloneInfoListFail) {
  ...
  EXPECT_CALL(*cloneCore_, GetCloneInfoList(_))
      .WillOnce(
          DoAll(SetArgPointee<0>(cloneInfos), Return(kErrCodeInternalError)));
...
TEST_F(TestCloneServiceManager, TestRecoverCloneTaskPushTaskFail) {
  ...
  std::shared_ptr<SnapshotReference> snapRef =
      std::make_shared<SnapshotReference>();
  EXPECT_CALL(*cloneCore_, GetSnapshotRef()).WillOnce(Return(snapRef));
...
TEST_F(TestCloneServiceManager, TestGetCloneRefStatusSuccessNoRef) {
  ...
  CloneInfo cloneInfo1("uuid1", user, CloneTaskType::kClone, source,
                       destination, CloneFileType::kFile, lazyFlag);
  cloneInfo1.SetStatus(CloneStatus::done);
  CloneInfo cloneInfo2("uuid2", user, CloneTaskType::kClone, source2,
                       destination, CloneFileType::kFile, lazyFlag);
  ...
  // no record found
  EXPECT_CALL(*cloneCore_, GetCloneInfoList(_)).WillOnce(Return(-1));
...
TEST_F(TestCloneServiceManager, TestGetCloneRefStatusSuccessHasRef) {
  ...
  CloneInfo cloneInfo3("uuid3", user, CloneTaskType::kClone, source,
                       destination, CloneFileType::kFile, lazyFlag);
...
TEST_F(TestCloneServiceManagerBackend, TestInitStop) {
...
TEST_F(TestCloneServiceManagerBackend, TestEmptyCloneInfoList) {
  std::vector<CloneInfo> cloneInfos;
...
TEST_F(TestCloneServiceManagerBackend, TestNoMetaInstalledClone) {
  CloneInfo cloneInfo1("taskId1", "user1", CloneTaskType::kClone, "source1",
                       "destination1", CloneFileType::kSnapshot, true);
  CloneInfo cloneInfo2("taskId2", "user2", CloneTaskType::kRecover, "source2",
                       "destination2", CloneFileType::kFile, false);
  std::vector<CloneInfo> cloneInfos;
...
TEST_F(TestCloneServiceManagerBackend, TestMetaInstalledCloneNotExist) {
  CloneInfo cloneInfo1("taskId1", "user1", CloneTaskType::kClone, "source1",
                       "destination1", CloneFileType::kSnapshot, true);
  CloneInfo cloneInfo2("taskId2", "user2", CloneTaskType::kRecover, "source2",
                       "destination2", 0, 0, 0, CloneFileType::kFile, false,
  ...
  EXPECT_CALL(*cloneCore_, GetCloneInfoList(_))
      .WillRepeatedly(
          DoAll(SetArgPointee<0>(cloneInfos), Return(kErrCodeSuccess)));
  EXPECT_CALL(*cloneCore_, GetCloneInfo(_, _)).WillRepeatedly(Return(-1));
  cloneServiceManagerBackend_->Start();
  usleep(testCount * backEndReferenceRecordScanIntervalMs * 1000);
  cloneServiceManagerBackend_->Stop();
}

TEST_F(TestCloneServiceManagerBackend, TestMetaInstalledCloneNotMetaInstalled) {
  CloneInfo cloneInfo1("taskId1", "user1", CloneTaskType::kClone, "source1",
                       "destination1", CloneFileType::kSnapshot, true);
  CloneInfo cloneInfo2("taskId2", "user2", CloneTaskType::kRecover, "source2",
                       "destination2", 0, 0, 0, CloneFileType::kFile, false,
  ...
  EXPECT_CALL(*cloneCore_, GetCloneInfo(_, _))
      .WillRepeatedly(
          DoAll(SetArgPointee<1>(cloneInfo1), Return(kErrCodeSuccess)));
  cloneServiceManagerBackend_->Start();
  usleep(testCount * backEndReferenceRecordScanIntervalMs * 1000);
  cloneServiceManagerBackend_->Stop();
}

TEST_F(TestCloneServiceManagerBackend, TestMetaInstalledCloneFileExist) {
  CloneInfo cloneInfo1("taskId1", "user1", CloneTaskType::kClone, "source1",
                       "destination1", CloneFileType::kSnapshot, true);
  CloneInfo cloneInfo2("taskId2", "user2", CloneTaskType::kRecover, "source2",
                       "destination2", 0, 0, 0, CloneFileType::kFile, false,
  ...
  EXPECT_CALL(*cloneCore_, GetCloneInfo(_, _))
      .WillRepeatedly(
          DoAll(SetArgPointee<1>(cloneInfo2), Return(kErrCodeSuccess)));
...
TEST_F(TestCloneServiceManagerBackend, TestMetaInstalledCloneDeleteFail) {
  CloneInfo cloneInfo1("taskId1", "user1", CloneTaskType::kClone, "source1",
                       "destination1", CloneFileType::kSnapshot, true);
  CloneInfo cloneInfo2("taskId2", "user2", CloneTaskType::kRecover, "source2",
                       "destination2", 0, 0, 0, CloneFileType::kFile, false,
...
TEST_F(TestCloneServiceManagerBackend, TestMetaInstalledCloneDeleteSuccess) {
  CloneInfo cloneInfo1("taskId1", "user1", CloneTaskType::kClone, "source1",
                       "destination1", CloneFileType::kSnapshot, true);
  CloneInfo cloneInfo2("taskId2", "user2", CloneTaskType::kRecover, "source2",
                       "destination2", 0, 0, 0, CloneFileType::kFile, false,
