...
class FakeChunkClosure : public ClientClosure {
 public:
  explicit FakeChunkClosure(CountDownEvent* event)
      : ClientClosure(nullptr, nullptr), reqeustClosure(nullptr), event(event) {
...
TEST_F(RequestSenderTest, TestReadChunkAppliedIndex) {
...

  {
    ...
    appliedIndex = 100;
    requestSender.ReadChunk(ChunkIDInfo(), 0, 0, 0, appliedIndex, {}, &closure);
...
TEST_F(RequestSenderTest, TestReadChunkSourceInfo) {
...

  {
    ...
    sourceInfo.cloneFileSource.clear();
    requestSender.ReadChunk(ChunkIDInfo(), 0, 0, 0, appliedIndex, sourceInfo,
                            &closure);
