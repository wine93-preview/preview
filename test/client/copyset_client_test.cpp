...
TEST_F(CopysetClientTest, normal_test) {
  ...
  for (int i = 0; i < 10; ++i) {
    ...
    copysetClient.WriteChunk(reqCtx->idinfo_, fileId, epoch, 0, iobuf, offset,
                             len, {}, reqDone);
    cond.Wait();
  ...
  for (int i = 0; i < 10; ++i) {
    ...
    copysetClient.ReadChunk(reqCtx->idinfo_, sn, offset, len, 0, {}, reqDone);
    cond.Wait();
    ASSERT_EQ(CHUNK_OP_STATUS::CHUNK_OP_STATUS_SUCCESS,
              reqDone->GetErrorCode());
  }
...
TEST_F(CopysetClientTest, write_error_test) {
  ...
  /* 不是 leader，返回正确的 leader */
  {
    ...
    EXPECT_CALL(mockChunkService, WriteChunk(_, _, _, _))
        .Times(2)
        .WillOnce(DoAll(SetArgPointee<2>(response1), Invoke(WriteChunkFunc)))
  ...
  /* 不是 leader，但返回的是错误 leader */
  {
    ...
    copysetClient.WriteChunk(reqCtx->idinfo_, fileId, epoch, 0, iobuf, offset,
                             len, {}, reqDone);
    cond.Wait();
    auto elpased = curve::common::TimeUtility::GetTimeofDayUs() - startTimeUs;
...
TEST_F(CopysetClientTest, read_error_test) {
  ...
  /* 不是 leader，返回正确的 leader */
  {
    ...
    EXPECT_CALL(mockChunkService, ReadChunk(_, _, _, _))
        ...
        .WillOnce(DoAll(SetArgPointee<2>(response2), Invoke(ReadChunkFunc)));
    copysetClient.ReadChunk(reqCtx->idinfo_, sn, offset, len, 0, {}, reqDone);
    cond.Wait();
    ASSERT_EQ(CHUNK_OP_STATUS::CHUNK_OP_STATUS_SUCCESS,
              reqDone->GetErrorCode());
...
TEST_F(CopysetClientTest, read_snapshot_error_test) {
  ...
  /* 非法参数 */
  {
    ...
    EXPECT_CALL(mockChunkService, ReadChunkSnapshot(_, _, _, _))
        .Times(1)
        .WillOnce(
            DoAll(SetArgPointee<2>(response), Invoke(ReadChunkSnapshotFunc)));
    copysetClient.ReadChunkSnapshot(reqCtx->idinfo_, sn, offset, len, reqDone);
    cond.Wait();
    ASSERT_EQ(CHUNK_OP_STATUS::CHUNK_OP_STATUS_INVALID_REQUEST,
              reqDone->GetErrorCode());
  }
  ...
  /* 不是 leader，返回正确的 leader */
  {
    ...
    EXPECT_CALL(mockChunkService, ReadChunkSnapshot(_, _, _, _))
        .Times(2)
        .WillOnce(
            DoAll(SetArgPointee<2>(response1), Invoke(ReadChunkSnapshotFunc)))
        .WillOnce(
            DoAll(SetArgPointee<2>(response2), Invoke(ReadChunkSnapshotFunc)));
    copysetClient.ReadChunkSnapshot(reqCtx->idinfo_, sn, offset, len, reqDone);
    cond.Wait();
    ASSERT_EQ(CHUNK_OP_STATUS::CHUNK_OP_STATUS_SUCCESS,
              reqDone->GetErrorCode());
...
TEST_F(CopysetClientTest, delete_snapshot_error_test) {
  ...
  /* 非法参数 */
  {
    ...
    EXPECT_CALL(mockChunkService,
        ...
        .Times(1)
        .WillOnce(
            DoAll(SetArgPointee<2>(response), Invoke(DeleteChunkSnapshotFunc)));
    copysetClient.DeleteChunkSnapshotOrCorrectSn(reqCtx->idinfo_, sn, reqDone);
    cond.Wait();
    ASSERT_EQ(CHUNK_OP_STATUS::CHUNK_OP_STATUS_INVALID_REQUEST,
              reqDone->GetErrorCode());
  }
  ...
  /* 不是 leader，但是没有返回 leader，刷新 meta cache 成功 */
  {
    ...
    EXPECT_CALL(mockChunkService,
        ...
        .Times(2)
        .WillOnce(
            DoAll(SetArgPointee<2>(response1), Invoke(DeleteChunkSnapshotFunc)))
        .WillOnce(DoAll(SetArgPointee<2>(response2),
                        Invoke(DeleteChunkSnapshotFunc)));
    copysetClient.DeleteChunkSnapshotOrCorrectSn(reqCtx->idinfo_, sn, reqDone);
    cond.Wait();
...
TEST_F(CopysetClientTest, create_s3_clone_error_test) {
  ...
  /* 非法参数 */
  {
    ...
    EXPECT_CALL(mockChunkService, CreateCloneChunk(_, _, _, _))
        .Times(1)  // NOLINT
        .WillOnce(
            DoAll(SetArgPointee<2>(response), Invoke(CreateCloneChunkFunc)));
    copysetClient.CreateCloneChunk(reqCtx->idinfo_, "destination", sn, 1, 1024,
                                   reqDone);
    cond.Wait();
    ASSERT_EQ(CHUNK_OP_STATUS::CHUNK_OP_STATUS_INVALID_REQUEST,
              reqDone->GetErrorCode());
  ...
  /* op success */
  {
    ...
    EXPECT_CALL(mockChunkService, CreateCloneChunk(_, _, _, _))
        .Times(1)
        .WillOnce(
            DoAll(SetArgPointee<2>(response2), Invoke(CreateCloneChunkFunc)));
    copysetClient.CreateCloneChunk(reqCtx->idinfo_, "destination", sn, 1, 1024,
                                   reqDone);
    cond.Wait();
    ASSERT_EQ(CHUNK_OP_STATUS::CHUNK_OP_STATUS_SUCCESS,
  ...
  /* 不是 leader，但是没有返回 leader，刷新 meta cache 成功 */
  {
    ...
    EXPECT_CALL(mockChunkService, CreateCloneChunk(_, _, _, _))
        .Times(2)  // NOLINT
        .WillOnce(
            DoAll(SetArgPointee<2>(response1), Invoke(CreateCloneChunkFunc)))
        .WillOnce(
            DoAll(SetArgPointee<2>(response2), Invoke(CreateCloneChunkFunc)));
    copysetClient.CreateCloneChunk(reqCtx->idinfo_, "destination", sn, 1, 1024,
                                   reqDone);
...
TEST_F(CopysetClientTest, recover_chunk_error_test) {
  ...
  /* 非法参数 */
  {
    ...
    EXPECT_CALL(mockChunkService, RecoverChunk(_, _, _, _))
        .Times(1)
        .WillOnce(DoAll(SetArgPointee<2>(response), Invoke(RecoverChunkFunc)));
  ...
  /* 不是 leader，但是没有返回 leader，刷新 meta cache 成功 */
  {
    ...
    EXPECT_CALL(mockChunkService, RecoverChunk(_, _, _, _))
        .Times(2)
        .WillOnce(DoAll(SetArgPointee<2>(response1), Invoke(RecoverChunkFunc)))
        .WillOnce(DoAll(SetArgPointee<2>(response2), Invoke(RecoverChunkFunc)));
    copysetClient.RecoverChunk(reqCtx->idinfo_, 0, 4096, reqDone);
    cond.Wait();
    ASSERT_EQ(CHUNK_OP_STATUS::CHUNK_OP_STATUS_SUCCESS,
...
TEST_F(CopysetClientTest, get_chunk_info_test) {
  ...
  /* 非法参数 */
  {
    ...
    EXPECT_CALL(mockChunkService, GetChunkInfo(_, _, _, _))
        .Times(1)
        .WillOnce(DoAll(SetArgPointee<2>(response), Invoke(GetChunkInfoFunc)));
  ...
  /* 不是 leader，返回正确的 leader */
  {
    ...
    EXPECT_CALL(mockChunkService, GetChunkInfo(_, _, _, _))
        .Times(2)
        .WillOnce(DoAll(SetArgPointee<2>(response1), Invoke(GetChunkInfoFunc)))
        .WillOnce(DoAll(SetArgPointee<2>(response2), Invoke(GetChunkInfoFunc)));
    copysetClient.GetChunkInfo(reqCtx->idinfo_, reqDone);
    cond.Wait();
    ASSERT_EQ(CHUNK_OP_STATUS::CHUNK_OP_STATUS_SUCCESS,
              reqDone->GetErrorCode());
...
TEST(ChunkServerBackwardTest, ChunkServerBackwardTest) {
  ...
  ASSERT_TRUE(fileinstance.Initialize("/test", mdsclient, userinfo, OpenFlags{},
  ...
  brpc::Server server;
  ASSERT_EQ(
      0, server.AddService(&fakechunkservice, brpc::SERVER_DOESNT_OWN_SERVICE))
      << "Fail to add fakechunkservice";
  ASSERT_EQ(0,
            server.AddService(&fakeCliservice, brpc::SERVER_DOESNT_OWN_SERVICE))
...
TEST_F(CopysetClientTest, retry_rpc_sleep_test) {
  ...
  ioSenderOpt.failRequestOpt.chunkserverRPCTimeoutMS = 1000;
  ioSenderOpt.failRequestOpt.chunkserverOPMaxRetry = 3;
  ioSenderOpt.failRequestOpt.chunkserverOPRetryIntervalUS = sleepUsBeforeRetry;
  ioSenderOpt.failRequestOpt.chunkserverMaxRPCTimeoutMS = 3500;
  ioSenderOpt.failRequestOpt.chunkserverMaxRetrySleepIntervalUS = 3500000;
  ioSenderOpt.chunkserverEnableAppliedIndexRead = 1;
...
TEST(CopysetClientBasicTest, TestReScheduleWhenSessionNotValid) {
  ...
  ASSERT_EQ(0, copysetClient.Init(&metaCache, ioSenderOption, &requestScheduler,
                                  nullptr));
