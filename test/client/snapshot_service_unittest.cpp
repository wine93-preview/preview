...
TEST(SnapInstance, SnapShotTest) {
  ...
  if (server.AddService(&curvefsservice, brpc::SERVER_DOESNT_OWN_SERVICE) !=
      0) {
    LOG(FATAL) << "Fail to add service";
  }
  if (server.AddService(&topologyservice, brpc::SERVER_DOESNT_OWN_SERVICE) !=
      0) {
    LOG(FATAL) << "Fail to add service";
  }
  ...
  FakeReturn* fakeret = new FakeReturn(nullptr, static_cast<void*>(&response));
  ...
  FakeReturn* fakeret1 = new FakeReturn(nullptr, static_cast<void*>(&response));
  ...
  ASSERT_EQ(-LIBCURVE_ERROR::UNDER_SNAPSHOT,
            cl.CreateSnapShot(filename, userinfo, &seq));
  ...
  ASSERT_EQ(LIBCURVE_ERROR::OK, cl.RenameCloneFile(userinfo, 1, 2, "1", "2"));
  ...
  ASSERT_EQ(-LIBCURVE_ERROR::AUTHFAIL,
            cl.RenameCloneFile(userinfo, 1, 2, "1", "2"));
  ASSERT_EQ(2, seq);
  ...
  noexist_resp.set_statuscode(
      ::curve::mds::StatusCode::kSnapshotFileNotExists);  // NOLINT
  ...
  ASSERT_EQ(-LIBCURVE_ERROR::FAILED,
            cl.DeleteSnapShot(filename, userinfo, seq));
  ...
  getresponse1->mutable_pagefilesegment()->set_segmentsize(1 * 1024 * 1024 *
                                                           1024ul);  // NOLINT
  ...
  FakeReturn* getfakeret1 =
      new FakeReturn(nullptr, static_cast<void*>(getresponse1));
  ...
  FakeReturn* getfake = new FakeReturn(nullptr, static_cast<void*>(getresp));
  ...
  FakeReturn* getfakeret2 =
      new FakeReturn(&getcntl, static_cast<void*>(getresponse2));
  ...
  listresponse->mutable_fileinfo(0)->set_segmentsize(1 * 1024 * 1024 *
                                                     1024ul);  //  NOLINT
  ...
  listresponse->mutable_fileinfo(1)->set_filetype(
      curve::mds::FileType::INODE_PAGEFILE);  // NOLINT
  ...
  listresponse->mutable_fileinfo(1)->set_segmentsize(1 * 1024 * 1024 *
                                                     1024ul);  //  NOLINT
  ...
  listresponse_seq->mutable_fileinfo(0)->set_filetype(
      curve::mds::FileType::INODE_PAGEFILE);  // NOLINT
  ...
  listresponse_seq->mutable_fileinfo(0)->set_segmentsize(1 * 1024 * 1024 *
                                                         1024ul);  //  NOLINT
  ...
  ASSERT_EQ(-LIBCURVE_ERROR::NOTEXIST,
            cl.GetSnapShot(filename, emptyuserinfo, seq, &sinfo_seq));
...
TEST(SnapInstance, ReadChunkSnapshotTest) {
  ...
  ioctxmana->ReadSnapChunk(ChunkIDInfo(cid, 2, 3), 0, 0, len, buf, &scc);
  ...
  ASSERT_EQ(0, ioctxmana->ReadSnapChunk(ChunkIDInfo(cid, 2, 3), 0, 0, len, buf,
                                        &scc2));
...
TEST(SnapInstance, DeleteChunkSnapshotTest) {
  ...
  ASSERT_EQ(0, ioctxmana->DeleteSnapChunkOrCorrectSn(ChunkIDInfo(cid, 2, 3),
                                                     0));  // NOLINT
...
TEST(SnapInstance, RecoverChunkTest) {
  ...
  mocksch->EnableScheduleFailed();
  ASSERT_EQ(0, ioctxmana->RecoverChunk(ChunkIDInfo(cid, 2, 3), 1,
                                       4 * 1024 * 1024, &scc2));
...
TEST(SnapInstance, CreateCloneChunkTest) {
  ...
  IOManager4Chunk* ioctxmana = cl.GetIOManager4Chunk();
  ioctxmana->SetRequestScheduler(mocksch);
  ioctxmana->CreateCloneChunk("destination", ChunkIDInfo(cid, 2, 3), 1, 2, 1024,
                              &scc);
  ...
  mocksch->EnableScheduleFailed();
  ASSERT_EQ(0, ioctxmana->CreateCloneChunk(
                   "destination", ChunkIDInfo(cid, 2, 3), 1, 2, 1024, &scc2));
