...
TEST_F(MDSClientTest, MkDir) {
  ...
  curvefsservice.SetCreateFileFakeReturn(fakeret1);
  ASSERT_EQ(LIBCURVE_ERROR::OK, globalclient->Mkdir(dirpath.c_str(), userinfo));
...
TEST_F(MDSClientTest, GetOrAllocateSegment) {
  ...
  mdsclient_.GetServerList(segInfo.lpcpIDInfo.lpid, segInfo.lpcpIDInfo.cpidVec,
                           &cpinfoVec);
...
TEST_F(MDSClientTest, GetLeaderTest) {
  ...
  if (chunkserver1.AddService(&cliservice1, brpc::SERVER_DOESNT_OWN_SERVICE) !=
      0) {
    ASSERT_TRUE(false) << "Fail to add service";
  }

  if (chunkserver2.AddService(&cliservice2, brpc::SERVER_DOESNT_OWN_SERVICE) !=
      0) {
    ASSERT_TRUE(false) << "Fail to add service";
  }

  if (chunkserver3.AddService(&cliservice3, brpc::SERVER_DOESNT_OWN_SERVICE) !=
      0) {
    ASSERT_TRUE(false) << "Fail to add service";
  }

  if (chunkserver4.AddService(&cliservice4, brpc::SERVER_DOESNT_OWN_SERVICE) !=
      0) {
    ASSERT_TRUE(false) << "Fail to add service";
  }
...
TEST_F(MDSClientTest, ListDir) {
  ...
  for (int i = 0; i < 5; i++) {
    ...
    ASSERT_EQ(dir->fileStat[i].filetype, curve::mds::FileType::INODE_PAGEFILE);
  ...
  for (int i = 0; i < 5; i++) {
    ...
    ASSERT_EQ(filestatVec[i].filetype, curve::mds::FileType::INODE_PAGEFILE);
 ...
 public:
  ...
  void SetUp() override {
    ...
    for (int i = 0; i < kChunkServerNum; ++i) {
      ...
      const auto& ipPort = "127.0.0.1:" + std::to_string(chunkserverPorts[i]);
...
TEST_F(ServiceHelperGetLeaderTest, EmptyCopysetPeerInfoTest) {
  ...
  ASSERT_EQ(-1, ServiceHelper::GetLeader(getLeaderInfo, &leaderAddr, &leaderId,
                                         nullptr));
}
...
TEST_F(MDSClientTest, StatFileStatusTest) {
  ...
  for (auto status : fileStatus) {
    ...
    std::unique_ptr<curve::client::FInfo_t> finfo(new curve::client::FInfo_t());
    FileStatInfo fstat;
    globalclient->StatFile(filename, userinfo, &fstat);
