...
TEST_F(MDSClientTest, TestRenameFile) {
  ...
  // mds first return not support, then success
  {
    ...
    responseNotSupport.set_statuscode(curve::mds::StatusCode::kNotSupported);
...
TEST_F(MDSClientTest, TestDeleteFile) {
  ...
  // mds first return not support, then success
  {
    ...
    ASSERT_EQ(LIBCURVE_ERROR::OK, mdsClient_.DeleteFile(fileName, userInfo));
  }
}
...
TEST_F(MDSClientTest, TestChangeOwner) {
  ...
  // mds return not support
  {
    ...
    EXPECT_CALL(mockNameService_, ChangeOwner(_, _, _, _))
        .WillRepeatedly(DoAll(
            SetArgPointee<2>(response),
            Invoke(FakeRpcService<ChangeOwnerRequest, ChangeOwnerResponse>)));
...
TEST_F(MDSClientTest, TestOpenFile) {
  ...
  // rpc always failed
  {
    EXPECT_CALL(mockNameService_, OpenFile(_, _, _, _))
        .WillRepeatedly(
            Invoke(FakeRpcService<OpenFileRequest, OpenFileResponse, true>));

    auto startMs = TimeUtility::GetTimeofDayMs();
    ASSERT_EQ(
        LIBCURVE_ERROR::FAILED,
        mdsClient_.OpenFile(fileName, userInfo, &fileInfo, &fEpoch, &session));
    auto endMs = TimeUtility::GetTimeofDayMs();
    ASSERT_LE(option_.mdsMaxRetryMS, endMs - startMs);
  }
...
TEST_F(MDSClientTest, TestIncreaseEpoch) {
  ...
  // rpc always failed
  {
    ...
    ASSERT_EQ(LIBCURVE_ERROR::FAILED,
              mdsClient_.IncreaseEpoch(fileName, userInfo, &fileInfo, &fEpoch,
                                       &csLocs));
  }
  // rpc response failed
  {
  ...
  // success 1, not have externalAddr
  {
    ...
    for (int i = 0; i < 10; i++) {
      curve::common::ChunkServerLocation* lc = response.mutable_cslocs()->Add();
