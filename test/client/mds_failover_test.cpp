...
TEST(MDSChangeTest, MDSFailoverTest) {
  ...
  auto task1 = [&](int mdsindex, uint64_t rpctimeoutMS, brpc::Channel* channel,
  ...
  mds1RetryTimes = 0;
  mds2RetryTimes = 0;
  auto task2 = [&](int mdsindex, uint64_t rpctimeoutMS, brpc::Channel* channel,
                   brpc::Controller* cntl) -> int {
    if (mdsindex == 0) {
      mds0RetryTimes++;
  ...
  auto task3 = [&](int mdsindex, uint64_t rpctimeoutMS, brpc::Channel* channel,
                   brpc::Controller* cntl) -> int {
    if (mdsindex == 0) {
      mds0RetryTimes++;
      return -ECONNRESET;
  ...
  auto task4 = [&](int mdsindex, uint64_t rpctimeoutMS, brpc::Channel* channel,
                   brpc::Controller* cntl) -> int {
    if (mdsindex == 0) {
      mds0RetryTimes++;
  ...
  auto task5 = [&](int mdsindex, uint64_t rpctimeoutMs, brpc::Channel* channel,
  ...
  rpcexcutor.SetCurrentWorkIndex(0);
  int calledTimes = 0;
  auto task6 = [&](int mdsindex, uint64_t rpctimeoutMs, brpc::Channel* channel,
                   brpc::Controller* cntl) {
    ++calledTimes;
    return -EHOSTDOWN;
  };
