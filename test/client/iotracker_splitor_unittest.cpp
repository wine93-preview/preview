...
class IOTrackerSplitorTest : public ::testing::Test {
 public:
  void SetUp() {
    ...
    fileinstance_->Initialize("/test", mdsclient_, userinfo, OpenFlags{}, fopt);
    InsertMetaCache();
  ...
  void InsertMetaCache() {
    ...
    getsegmentfakeret = new FakeReturn(nullptr, static_cast<void*>(response));
    ...
    for (int i = 256; i < 512; i++) {
      auto chunk = cloneSourceResponse->mutable_pagefilesegment()->add_chunks();
      chunk->set_copysetid(i);
      chunk->set_chunkid(i);
    }
    ...
    mdsclient_->GetServerList(lpcsIDInfo.lpid, lpcsIDInfo.cpidVec, &cpinfoVec);
    ...
    closeResp->set_statuscode(::curve::mds::StatusCode::kOK);
    auto* closeFakeRet = new FakeReturn(nullptr, static_cast<void*>(closeResp));
...
TEST_F(IOTrackerSplitorTest, ManagerStartRead) {
  ...
  auto threadfunc = [&]() {
    ASSERT_EQ(length, ioctxmana->Read(data, offset, length, mdsclient_.get()));
  };
  std::thread process(threadfunc);
...
TEST_F(IOTrackerSplitorTest, RequestSourceInfoTest) {
  ...
  // 第一个chunk
  sourceInfo = Splitor::CalcRequestSourceInfo(&ioTracker, &metaCache, chunkIdx);
  ...
  for (int i = 1; i < 9; ++i) {
    ChunkIndex chunkIdx = i * cloneSourceInfo.segmentSize / fileInfo.chunksize;
    RequestSourceInfo sourceInfo =
...
TEST_F(IOTrackerSplitorTest, TestDisableStripeForStripeFile) {
  ...
  std::vector<RequestContext*> reqlist;
  ASSERT_EQ(0, Splitor::IO2ChunkRequests(&ioTracker, cache, &reqlist, &dataCopy,
...
TEST_F(IOTrackerSplitorTest, TimedCloseFd) {
  ...
  fakeHandlers.emplace(
      ...
      std::forward_as_tuple(
          instance, ::time(nullptr) - fopt.ioOpt.closeFdThreadOption.fdTimeout,
          false));
