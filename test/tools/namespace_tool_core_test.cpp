...
TEST_F(NameSpaceToolCoreTest, CreateFile) {
  ...
  // 1、正常情况
  EXPECT_CALL(*client_, CreateFile(_, _, _, _, _)).Times(1).WillOnce(Return(0));
  ASSERT_EQ(0, namespaceTool.CreateFile(fileName, length, true, stripeUnit,
                                        stripeCount));
  ...
  EXPECT_CALL(*client_, CreateFile(_, _, _, _, _))
      .Times(1)
      .WillOnce(Return(-1));
  ASSERT_EQ(-1, namespaceTool.CreateFile(fileName, length, true, stripeUnit,
                                         stripeCount));
}
...
TEST_F(NameSpaceToolCoreTest, GetAllocatedSize) {
  ...
  // 1、正常情况
  uint64_t allocSize;
  EXPECT_CALL(*client_, GetAllocatedSize(_, _, _)).Times(1).WillOnce(Return(0));
...
TEST_F(NameSpaceToolCoreTest, QueryChunkCopyset) {
  ...
  ASSERT_EQ(
      0, namespaceTool.QueryChunkCopyset(fileName, offset, &chunkId, &copyset));
...
TEST_F(NameSpaceToolCoreTest, TestUpdateThrottle) {
  ...
  // 1. throttle type is invalid
  {
    ...
    ASSERT_EQ(-1,
              namespaceTool.UpdateFileThrottle("/test", "hello", 10000, 0, 0));
  }
  ...
  // 2. burst and burstLength is not specified
  {
    ...
    ASSERT_EQ(0, namespaceTool.UpdateFileThrottle("/test", "BPS_TOTAL", 10000,
                                                  -1, -1));
  ...
  // 3. burst lower than limit
  {
    ...
    ASSERT_EQ(-1, namespaceTool.UpdateFileThrottle("/test", "BPS_TOTAL", 10000,
                                                   5000, -1));
  }
