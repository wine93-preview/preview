...
TEST_F(VersionToolTest, GetAndCheckMdsVersion) {
  ...
  EXPECT_CALL(*metricClient_, GetMetric(_, _, _))
      .Times(3)
      .WillRepeatedly(DoAll(SetArgPointee<2>("0.0.1"), Return(MetricRet::kOK)));
  std::string version;
...
TEST_F(VersionToolTest, GetClientVersion) {
  ...
  EXPECT_CALL(*metricClient_, GetMetric(_, kProcessCmdLineMetricName, _))
      ...
      .WillOnce(DoAll(SetArgPointee<2>(kProcessPython), Return(MetricRet::kOK)))
      .WillOnce(DoAll(SetArgPointee<2>(kProcessOther), Return(MetricRet::kOK)))
      .WillRepeatedly(
          DoAll(SetArgPointee<2>(kProcessNebdServer), Return(MetricRet::kOK)));
...
TEST_F(VersionToolTest, GetAndCheckSnapshotCloneVersion) {
  ...
  std::vector<std::string> failedList;
  ASSERT_EQ(0,
            versionTool.GetAndCheckSnapshotCloneVersion(&version, &failedList));
  ASSERT_EQ("0.0.1", version);
  ASSERT_TRUE(failedList.empty());
