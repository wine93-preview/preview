...
class MockMDSClient : public MDSClient {
 public:
  ...
  MOCK_METHOD3(GetAllocatedSize, int(const std::string&, uint64_t*, AllocMap*));
  MOCK_METHOD2(GetFileSize, int(const std::string&, uint64_t*));
  MOCK_METHOD2(ListDir, int(const std::string&, std::vector<FileInfo>*));
  MOCK_METHOD3(GetSegmentInfo,
               GetSegmentRes(const std::string&, uint64_t, PageFileSegment*));
  MOCK_METHOD2(DeleteFile, int(const std::string&, bool));
  MOCK_METHOD5(CreateFile,
               int(const std::string&, uint64_t, bool, uint64_t, uint64_t));
  MOCK_METHOD2(ExtendVolume, int(const std::string&, uint64_t));
  MOCK_METHOD3(GetChunkServerListInCopySet,
               int(const PoolIdType&, const CopySetIdType&,
                   std::vector<ChunkServerLocation>*));
  MOCK_METHOD3(GetChunkServerListInCopySets,
               int(const PoolIdType&, const std::vector<CopySetIdType>&,
                   std::vector<CopySetServerInfo>*));
  MOCK_METHOD1(ListPhysicalPoolsInCluster, int(std::vector<PhysicalPoolInfo>*));
  MOCK_METHOD2(ListLogicalPoolsInPhysicalPool,
               int(const PoolIdType&, std::vector<LogicalPoolInfo>*));
  MOCK_METHOD2(ListZoneInPhysicalPool,
               int(const PoolIdType&, std::vector<ZoneInfo>*));
  MOCK_METHOD2(ListServersInZone,
               int(const ZoneIdType&, std::vector<ServerInfo>*));
  MOCK_METHOD2(ListChunkServersOnServer,
               int(const ServerIdType&, std::vector<ChunkServerInfo>*));
  MOCK_METHOD2(ListChunkServersOnServer,
               int(const std::string&, std::vector<ChunkServerInfo>*));
  MOCK_METHOD2(GetChunkServerInfo,
               int(const ChunkServerIdType&, ChunkServerInfo*));
  MOCK_METHOD2(GetChunkServerInfo, int(const std::string&, ChunkServerInfo*));
  MOCK_METHOD2(GetCopySetsInChunkServer,
               int(const ChunkServerIdType&, std::vector<CopysetInfo>*));
  MOCK_METHOD2(GetCopySetsInChunkServer,
               int(const std::string&, std::vector<CopysetInfo>*));
  ...
  MOCK_METHOD1(ListChunkServersInCluster, int(std::vector<ChunkServerInfo>*));
  MOCK_METHOD1(ListChunkServersInCluster,
               int(std::map<PoolIdType, std::vector<ChunkServerInfo>>*));
  ...
  MOCK_METHOD2(ListVolumesOnCopyset,
               int(const std::vector<common::CopysetInfo>&,
                   std::vector<std::string>*));
  MOCK_METHOD2(SetCopysetsAvailFlag, int(const std::vector<CopysetInfo>, bool));
