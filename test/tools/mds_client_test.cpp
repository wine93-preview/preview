...
class ToolMDSClientTest : public ::testing::Test {
 protected:
  ...
  void SetUp() {
    ...
    ASSERT_EQ(0,
              server->AddService(nameService, brpc::SERVER_DOESNT_OWN_SERVICE));
    ASSERT_EQ(0,
              server->AddService(topoService, brpc::SERVER_DOESNT_OWN_SERVICE));
  ...
  void GetChunkServerInfoForTest(ChunkServerIdType id, ChunkServerInfo* csInfo,
                                 bool retired = false) {
  ...
  EXPECT_CALL(*nameService, CreateFile(_, _, _, _))
      .WillOnce(DoAll(
          ...
          Invoke([](RpcController* controller,
                    ...
                    curve::mds::CreateFileResponse* response,
                    Closure* done) { brpc::ClosureGuard doneGuard(done); })));
  ASSERT_EQ(0, mdsClient.CreateFile(fileName, length, stripeUnit, stripeCount));
}
...
TEST_F(ToolMDSClientTest, GetChunkServerListInCopySets) {
  ...
  ASSERT_EQ(-1, mdsClient.GetChunkServerListInCopySet(logicalPoolId, copysetId,
                                                      &csLocs));
...
TEST_F(ToolMDSClientTest, ListPhysicalPoolsInCluster) {
  ...
  EXPECT_CALL(*topoService, ListPhysicalPool(_, _, _, _))
      .Times(6)
      .WillRepeatedly(Invoke(
          [](RpcController* controller, const ListPhysicalPoolRequest* request,
...
TEST_F(ToolMDSClientTest, SetLogicalPoolScanState) {
  auto succCallback =
      callback<SetLogicalPoolScanStateRequest, SetLogicalPoolScanStateResponse>;
