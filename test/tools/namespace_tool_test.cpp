...
TEST_F(NameSpaceToolTest, GetFile) {
  ...
  EXPECT_CALL(*core_, GetFileInfo(_, _))
      .Times(1)
      .WillOnce(DoAll(SetArgPointee<1>(fileInfo), Return(0)));
  EXPECT_CALL(*core_, GetAllocatedSize(_, _, _)).Times(1).WillOnce(Return(-1));
  ASSERT_EQ(-1, namespaceTool.RunCommand("get"));
...
TEST_F(NameSpaceToolTest, CreateFile) {
  ...
  // 1、正常情况
  EXPECT_CALL(*core_, CreateFile(_, _, _, _, _)).Times(1).WillOnce(Return(0));
  ASSERT_EQ(0, namespaceTool.RunCommand("create"));

  // 2、创建失败
  EXPECT_CALL(*core_, CreateFile(_, _, _, _, _)).Times(1).WillOnce(Return(-1));
