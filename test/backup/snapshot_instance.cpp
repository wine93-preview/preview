...
bool SnapInstance::Initialize() {
  ...
  do {
    ...
    if (mdsclient_.Initialize(ClientConfig::GetMetaServerOption().metaaddr) !=
        0) {  // NOLINT
    ...
    ioctxManager_ =
        new (std::nothrow) IOManager4Chunk(mc_, scheduler_);  // NOLINT
    ...
    if (-1 ==
        scheduler_->Init(
            ClientConfig::GetRequestSchedulerOption().queueCapacity,   // NOLINT
            ClientConfig::GetRequestSchedulerOption().threadpoolSize,  // NOLINT
            reqsenderManager_, mc_)) {
...
int SnapInstance::GetSnapShot(std::string filename, uint64_t seq, FInfo* fi) {
...
int SnapInstance::GetSnapshotSegmentInfo(std::string filename, uint64_t seq,
                                         uint64_t offset, SegmentInfo* segif) {
  return mdsclient_.GetSnapshotSegmentInfo(filename, seq, offset, segif, mc_);
}

int SnapInstance::ReadChunkSnapshot(LogicPoolID lpid, CopysetID cpid,
                                    ChunkID chunkid, uint64_t seq,
                                    uint64_t offset, uint64_t len, void* buf) {
  return ioctxManager_->ReadSnapChunk(lpid, cpid, chunkid, seq, offset, len,
                                      buf);  // NOLINT
}
...
int SnapInstance::DeleteChunkSnapshotOrCorrectSn(LogicPoolID lpid,
  ...
  return ioctxManager_->DeleteSnapChunkOrCorrectSn(lpid, cpid, chunkid,
                                                   correctedSeq);
}

int SnapInstance::GetChunkInfo(LogicPoolID lpid, CopysetID cpid,
                               ChunkID chunkid, ChunkInfoDetail* chunkInfo) {
