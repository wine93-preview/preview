...
std::shared_ptr<NebdFileInstance> CurveRequestExecutor::Reopen(
    const std::string& filename, const ExtendAttribute& xattr) {
  std::string curveFileName = FileNameParser::Parse(filename);
  if (curveFileName.empty()) {
    return nullptr;
  }
  ...
  if (fd >= 0) {
    ...
    if (xattr.count(kOpenFlagsAttrKey)) {
      curveFileInstance->xattr[kOpenFlagsAttrKey] = xattr.at(kOpenFlagsAttrKey);
    }
    return curveFileInstance;
  }
...
int CurveRequestExecutor::GetInfo(NebdFileInstance* fd,
...
int CurveRequestExecutor::AioRead(NebdFileInstance* fd,
                                  NebdServerAioContext* aioctx) {
  int curveFd = GetCurveFdFromNebdFileInstance(fd);
  if (curveFd < 0) {
    return -1;
...
int CurveRequestExecutor::AioWrite(NebdFileInstance* fd,
                                   NebdServerAioContext* aioctx) {
  int curveFd = GetCurveFdFromNebdFileInstance(fd);
  if (curveFd < 0) {
    return -1;
...
int CurveRequestExecutor::Flush(NebdFileInstance* fd,
                                NebdServerAioContext* aioctx) {
  (void)fd;
...
int CurveRequestExecutor::InvalidCache(NebdFileInstance* fd) {
  ...
  if (curveFileInstance == nullptr || curveFileInstance->fd < 0 ||
...
int CurveRequestExecutor::FromNebdCtxToCurveCtx(NebdServerAioContext* nebdCtx,
