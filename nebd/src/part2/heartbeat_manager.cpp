...
int HeartbeatManager::Run() {
  ...
  checkTimeoutThread_ = std::thread(&HeartbeatManager::CheckTimeoutFunc, this);
...
void HeartbeatManager::CheckTimeoutFunc() {
  while (
    ...
    for (const auto& entityPair : fileEntityMap) {
      ...
      TimeUtility::TimeStampToStandard(curEntity->GetFileTimeStamp() / 1000,
                                       &standardTime);
      LOG(INFO) << "Close file which has timed out. "
                << "Last time received heartbeat or request: " << standardTime;
