...
void SetResponse(NebdServerAioContext* context, RetCode retCode) {
  switch (context->op) {
    case LIBAIO_OP::LIBAIO_OP_READ: {
      ...
      if (context->ret >= 0) {
        brpc::Controller* cntl = dynamic_cast<brpc::Controller*>(context->cntl);
...
void NebdFileServiceImpl::OpenFile(google::protobuf::RpcController* cntl_base,
  ...
  int fd = fileManager_->Open(
      request->filename(), request->has_flags() ? &request->flags() : nullptr);

  if (fd > 0) {
    response->set_retcode(RetCode::kOK);
    response->set_fd(fd);
    LOG(INFO) << "Open file success. "
              << "filename: " << request->filename() << ", fd: " << fd;
  } else {
...
void NebdFileServiceImpl::Write(google::protobuf::RpcController* cntl_base,
  ...
  NebdServerAioContext* aioContext = new (std::nothrow) NebdServerAioContext();
  ...
  *buf = cntl->request_attachment();
  ...
  if (copySize != aioContext->size) {
    LOG(ERROR) << "Copy attachment failed. "
               << "fd: " << request->fd() << ", offset: " << request->offset()
               << ", size: " << request->size() << ", copy size: " << copySize;
    return;
  }
  ...
  if (rc < 0) {
    LOG(ERROR) << "Write file failed. "
               << "fd: " << request->fd() << ", offset: " << request->offset()
               << ", size: " << request->size() << ", return code: " << rc;
  } else {
    buf.release();
    doneGuard.release();
  }
...
void NebdFileServiceImpl::ResizeFile(google::protobuf::RpcController* cntl_base,
  ...
  if (rc < 0) {
    LOG(ERROR) << "Resize file failed. "
               << "fd: " << request->fd() << ", newsize: " << request->newsize()
               << ", return code: " << rc;
  } else {
    response->set_retcode(RetCode::kOK);
  }
