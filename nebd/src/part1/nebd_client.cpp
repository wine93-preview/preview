...
// // NOLINT | Name                      | Value | Description | Defined At | //
// NOLINT | ------------------------- | ----- |
// ---------------------------------------- | ----------------------- | //
...
int NebdClient::Close(int fd) {
  auto task = [&](brpc::Controller* cntl, brpc::Channel* channel,
    ...
    *rpcFailed = cntl->Failed();
    if (*rpcFailed) {
      LOG(WARNING) << "CloseFile rpc failed, error = " << cntl->ErrorText()
                   << ", log id = " << cntl->log_id();
      return -1;
    } else {
      if (response.retcode() != RetCode::kOK) {
...
int NebdClient::Discard(int fd, NebdClientAioContext* aioctx) {
  auto task = [this, fd, aioctx]() {
    ...
    AioDiscardClosure* done =
        new (std::nothrow) AioDiscardClosure(fd, aioctx, option_.requestOption);
...
int NebdClient::AioRead(int fd, NebdClientAioContext* aioctx) {
  auto task = [this, fd, aioctx]() {
    ...
    AioReadClosure* done =
        new (std::nothrow) AioReadClosure(fd, aioctx, option_.requestOption);
    done->cntl.set_timeout_ms(-1);
    done->cntl.set_log_id(logId_.fetch_add(1, std::memory_order_relaxed));
...
int NebdClient::AioWrite(int fd, NebdClientAioContext* aioctx) {
  auto task = [this, fd, aioctx]() {
    ...
    AioWriteClosure* done =
        new (std::nothrow) AioWriteClosure(fd, aioctx, option_.requestOption);
...
int NebdClient::Flush(int fd, NebdClientAioContext* aioctx) {
  auto task = [this, fd, aioctx]() {
    ...
    AioFlushClosure* done =
        new (std::nothrow) AioFlushClosure(fd, aioctx, option_.requestOption);
    done->cntl.set_timeout_ms(-1);
    done->cntl.set_log_id(logId_.fetch_add(1, std::memory_order_relaxed));
...
int NebdClient::InvalidCache(int fd) {
  auto task = [&](brpc::Controller* cntl, brpc::Channel* channel,
    ...
    *rpcFailed = cntl->Failed();
    if (*rpcFailed) {
      LOG(WARNING) << "InvalidCache rpc failed, error = " << cntl->ErrorText()
                   << ", log id = " << cntl->log_id();
      return -1;
    } else {
      if (response.retcode() != nebd::client::RetCode::kOK) {
...
int NebdClient::InitNebdClientOption(Configuration* conf) {
  bool ret = false;
  ret =
      conf->GetStringValue("nebdserver.serverAddress", &option_.serverAddress);
...
int NebdClient::InitChannel() {
  ...
  int ret = channel_.Init(option_.serverAddress.c_str(), nullptr);
  if (ret != 0) {
