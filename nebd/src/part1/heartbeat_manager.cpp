...
HeartbeatManager::HeartbeatManager(
    ...
    : metaCache_(metaCache), running_(false), logId_(1) {}

int HeartbeatManager::Init(const HeartbeatOption& option) {
  heartbeatOption_ = option;

  int ret = channel_.Init(option.serverAddress.c_str(), nullptr);
  if (ret != 0) {
...
void HeartbeatManager::HeartBetaThreadFunc() {
  ...
  while (running_) {
    SendHeartBeat();
    sleeper_.wait_for(std::chrono::seconds(heartbeatOption_.intervalS));
  }
}

void HeartbeatManager::Run() {
  running_ = true;
  heartbeatThread_ = std::thread(&HeartbeatManager::HeartBetaThreadFunc, this);
}
...
void HeartbeatManager::SendHeartBeat() {
  ...
  LOG(INFO) << "Send Heartbeat request, log id = " << cntl.log_id()
            << ", pid = " << request.pid()
            << ", nebd version = " << request.nebdversion() << ", files ["
            << oss.str() << ']';
  ...
  if (isCntlFailed) {
    LOG(WARNING) << "Heartbeat request failed, error = " << cntl.ErrorText()
                 << ", log id = " << cntl.log_id();
  }
}
