...
TEST_F(TestReuqestExecutorCurve, test_Open) {
  ...
  // 1. 传入的fileName解析失败
  {
    ...
    std::shared_ptr<NebdFileInstance> ret = executor.Open(errFileName, nullptr);
  ...
  // 2. curveclient open失败
  {
    ...
    std::shared_ptr<NebdFileInstance> ret = executor.Open(fileName, nullptr);
    ASSERT_TRUE(nullptr == ret);
  }
...
TEST_F(TestReuqestExecutorCurve, test_ReOpen) {
  ...
  // 1. 传入的fileName解析失败
  {
    ...
    std::shared_ptr<NebdFileInstance> ret = executor.Reopen(errFileName, xattr);
    ASSERT_TRUE(nullptr == ret);
  }

  // 2. repoen失败
  {
    EXPECT_CALL(*curveClient_, ReOpen(curveFileName, _)).WillOnce(Return(-1));
    std::shared_ptr<NebdFileInstance> ret = executor.Reopen(fileName, xattr);
    ASSERT_TRUE(nullptr == ret);
  }
...
TEST_F(TestReuqestExecutorCurve, test_Close) {
  ...
  // 4. close成功
  {
    auto curveFileIns = new CurveFileInstance();
    curveFileIns->fd = 1;
    EXPECT_CALL(*curveClient_, Close(1)).WillOnce(Return(LIBCURVE_ERROR::OK));
