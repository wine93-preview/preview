...
class FileServiceTest : public ::testing::Test {
 public:
  void SetUp() {
    ...
    fileService_ = std::make_shared<NebdFileServiceImpl>(fileManager_, false);
    g_test_executor = new MockRequestExecutor();
  }
  void TearDown() { delete g_test_executor; }
...
TEST_F(FileServiceTest, OpenTest) {
  ...
  // open success
  EXPECT_CALL(*fileManager_, Open(testFile1, _)).WillOnce(Return(1));
...
TEST_F(FileServiceTest, WriteTest) {
  ...
  ASSERT_EQ(*reinterpret_cast<butil::IOBuf*>(aioCtx->buf), data);

  ASSERT_FALSE(done.IsRunned());

  // write failed
  done.Reset();
  EXPECT_CALL(*fileManager_, AioWrite(fd, NotNull())).WillOnce(Return(-1));
  fileService_->Write(&cntl, &request, &response, &done);
  ASSERT_EQ(response.retcode(), RetCode::kNoOK);
  ASSERT_TRUE(done.IsRunned());
  ...
  EXPECT_CALL(*fileManager_, AioWrite(_, _)).Times(0);
  fileService_->Write(&cntl, &request, &response, &done);
  ASSERT_EQ(response.retcode(), RetCode::kNoOK);
  ASSERT_TRUE(done.IsRunned());
}
...
TEST_F(FileServiceTest, ReadTest) {
  ...
  // read success
  EXPECT_CALL(*fileManager_, AioRead(fd, NotNull())).WillOnce(Return(0));
  fileService_->Read(&cntl, &request, &response, &done);
  ASSERT_FALSE(done.IsRunned());

  // read failed
  done.Reset();
  EXPECT_CALL(*fileManager_, AioRead(fd, NotNull())).WillOnce(Return(-1));
  fileService_->Read(&cntl, &request, &response, &done);
  ASSERT_EQ(response.retcode(), RetCode::kNoOK);
  ASSERT_TRUE(done.IsRunned());
}
...
TEST_F(FileServiceTest, FlushTest) {
  ...
  // flush success
  EXPECT_CALL(*fileManager_, Flush(fd, NotNull())).WillOnce(Return(0));
  fileService_->Flush(&cntl, &request, &response, &done);
  ASSERT_FALSE(done.IsRunned());

  // flush failed
  done.Reset();
  EXPECT_CALL(*fileManager_, Flush(fd, NotNull())).WillOnce(Return(-1));
  fileService_->Flush(&cntl, &request, &response, &done);
  ASSERT_EQ(response.retcode(), RetCode::kNoOK);
  ASSERT_TRUE(done.IsRunned());
}
...
TEST_F(FileServiceTest, DiscardTest) {
  ...
  // discard success
  EXPECT_CALL(*fileManager_, Discard(fd, NotNull())).WillOnce(Return(0));
  fileService_->Discard(&cntl, &request, &response, &done);
  ASSERT_FALSE(done.IsRunned());

  // discard failed
  done.Reset();
  EXPECT_CALL(*fileManager_, Discard(fd, NotNull())).WillOnce(Return(-1));
  fileService_->Discard(&cntl, &request, &response, &done);
  ASSERT_EQ(response.retcode(), RetCode::kNoOK);
  ASSERT_TRUE(done.IsRunned());
}
...
TEST_F(FileServiceTest, GetInfoTest) {
  ...
  EXPECT_CALL(*fileManager_, GetInfo(fd, NotNull())).WillOnce(Return(-1));
  fileService_->GetInfo(&cntl, &request, &response, &done);
  ASSERT_EQ(response.retcode(), RetCode::kNoOK);
  ASSERT_TRUE(done.IsRunned());
}
...
TEST_F(FileServiceTest, CloseTest) {
  ...
  // close success
  EXPECT_CALL(*fileManager_, Close(fd, true)).WillOnce(Return(0));
  ...
  EXPECT_CALL(*fileManager_, Close(fd, true)).WillOnce(Return(-1));
  fileService_->CloseFile(&cntl, &request, &response, &done);
  ASSERT_EQ(response.retcode(), RetCode::kNoOK);
  ASSERT_TRUE(done.IsRunned());
}
...
TEST_F(FileServiceTest, ResizeTest) {
  ...
  EXPECT_CALL(*fileManager_, Extend(fd, size)).WillOnce(Return(-1));
  fileService_->ResizeFile(&cntl, &request, &response, &done);
  ASSERT_EQ(response.retcode(), RetCode::kNoOK);
  ASSERT_TRUE(done.IsRunned());
}
...
TEST_F(FileServiceTest, InvalidCacheTest) {
  ...
  EXPECT_CALL(*fileManager_, InvalidCache(fd)).WillOnce(Return(0));
  ...
  EXPECT_CALL(*fileManager_, InvalidCache(fd)).WillOnce(Return(-1));
  fileService_->InvalidateCache(&cntl, &request, &response, &done);
  ASSERT_EQ(response.retcode(), RetCode::kNoOK);
  ASSERT_TRUE(done.IsRunned());
}
