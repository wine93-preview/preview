...
class NebdFileClientTest : public ::testing::Test {
 public:
  ...
  void AddMockService() {
    ASSERT_EQ(0,
              server.AddService(&mockService, brpc::SERVER_DOESNT_OWN_SERVICE))
        << "Add service failed";
  }
...

  {
    ...
    EXPECT_CALL(mockService, OpenFile(_, _, _, _))
        ...
        .WillOnce(DoAll(
            ...
            Invoke(
                MockClientFunc<OpenFileRequest, OpenFileResponse>)));  // NOLINT
    ASSERT_EQ(-1, Open4Nebd(kFileName, nullptr));
  }
...

  {
    ...
    EXPECT_CALL(mockService, GetInfo(_, _, _, _))
        ...
        .WillOnce(DoAll(
            SetArgPointee<2>(response),
            Invoke(
                MockClientFunc<GetInfoRequest, GetInfoResponse>)));  // NOLINT
    ASSERT_EQ(-1, GetFileSize4Nebd(1));
  }
...

  {
    ...
    EXPECT_CALL(mockService, Discard(_, _, _, _))
        ...
        .WillOnce(DoAll(
            SetArgPointee<2>(response),
            Invoke(
                MockClientFunc<DiscardRequest, DiscardResponse>)));  // NOLINT
    aioOpReturn = false;
    ASSERT_EQ(0, Discard4Nebd(1, ctx));
    std::unique_lock<std::mutex> ulk(mtx);
    cond.wait(ulk, []() { return aioOpReturn.load(); });
